...
def load_csv_files():
    """Belirtilen CSV dosyalarını yükle (simülasyon modunda dec* ile)"""
    csv_files = [
    ...
    for csv_file in csv_files:
        sim_file = get_simulation_filename(csv_file)
        try:
            if os.path.exists(sim_file):
                df = pd.read_csv(sim_file)
                print(f"OK {sim_file} yüklendi: {len(df)} satır")
                loaded_files[sim_file] = df
            else:
                print(f"! {sim_file} dosyası bulunamadı")
        except Exception as e:
            print(f"! {sim_file} yüklenirken hata: {e}")
...
def update_price_data(df, ib):
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            try:
                # Simülasyon modunda 15 Aralık 2024'ten önceki verileri çek
                from simulation_helper import get_historical_end_date
                end_date = get_historical_end_date()
                end_date_str = end_date.strftime('%Y%m%d %H:%M:%S')
                
                bars = ib.reqHistoricalData(
                    contract,
                    endDateTime=end_date_str,  # 15 Aralık 2024'ten önceki veriler
                ...
                print(f"  Historical data çekildi (end: {end_date_str}): {len(bars) if bars else 0} bar")
            ...
            if bars and len(bars) > 0:
                ...
                if len(bars_df) >= 246:  # En az 246 gün veri gerekli
                    try:
                        ...
                        sma20_chg = ((last_price - sma20) / sma20) * 100
                        sma63_chg = ((last_price - sma63) / sma63) * 100
                        sma246_chg = ((last_price - sma246) / sma246) * 100
                        
                        df.at[idx, 'SMA20 chg'] = f"{sma20_chg:.2f}"
                        df.at[idx, 'SMA63 chg'] = f"{sma63_chg:.2f}"
                        df.at[idx, 'SMA246 chg'] = f"{sma246_chg:.2f}"
                        
                        print(f"  OK SMA değerleri: SMA20={sma20:.2f}, SMA63={sma63:.2f}, SMA246={sma246:.2f}")
                    except Exception as e:
...
def process_csv_files(loaded_files, ib):
    ...
    for original_filename, df in loaded_files.items():
        ...
        try:
            ...
            new_filename = get_simulation_filename(original_filename)
            ...
            print(f"OK {new_filename} dosyasına kaydedildi ({len(df)} hisse)")
...
def main():
    ...
    try:
        ...
        print(f"\nOK Tüm dosyalar işlendi ve yeni dosyalar oluşturuldu.")
