...
class PreferredStockMonitor(tk.Tk):
    ...
    def opt50_mal_topla(self):
        ...
        try:
            ...
            # Selectable treeview için sütunları oluştur
            columns = [
                "symbol", "last", "bid", "ask", "spread_pct", "target_price", 
                "cheapness_score", "final_thg", "avg_adv", "final_shares"
            ]
            
            # Selectable treeview oluştur (tb_modules'dan)
            tree_frame, opportunity_tree, scrollbar, selected_positions = create_selectable_treeview(
                opportunity_window, 
                columns, 
            ...
            for col in columns:
                opportunity_tree.heading(col, text=column_headings.get(col, col))
            ...
            refresh_btn = ttk.Button(
                button_frame,
                text="Yenile",
                command=lambda: (opportunity_window.destroy(), self.opt50_mal_topla())
            )
            refresh_btn.pack(side=tk.LEFT, padx=5)
    ...
    def take_profit_from_longs(self):
        ...
        try:
            ...
            # Selectable treeview için sütunları oluştur
            columns = [
                "symbol", "group", "quantity", "avg_cost", "last", "bid", "ask", 
                "target_price", "profit_percent", "outperform_score", "benchmark"
            ]
            
            # Selectable treeview oluştur (tb_modules'dan)
            tree_frame, profit_tree, scrollbar, selected_positions = create_selectable_treeview(
                profit_window, 
                columns, 
                default_lot_size=200
            )
            ...
            # Sütun başlıklarını güncelle
            for col in columns:
                profit_tree.heading(col, text=column_headings.get(col, col))
            ...
            for data in profit_opportunities:
                ...
                # Değerleri dönüştür
                # Değerleri oluştur (Select sütunu otomatik olarak eklenir)
                values = (
                    data['symbol'],
                    data['group'],
                    f"{int(data['quantity']):,}",
                ...
                # add_selectable_row modül fonksiyonunu kullan
                profit_tree.add_selectable_row(values, tag)
    ...
    def take_profit_from_shorts(self):
        ...
        try:
            ...
            # TAMAMEN YENİ TIKLAMA FONKSİYONU
            def on_item_selection(event):
                # Doğrudan treeview'ın seçimini al
                selected_items = profit_tree.selection()
                
                # Tüm itemleri önce temizle
                for item_id in profit_tree.get_children():
                    profit_tree.set(item_id, "Select", "□")
                
                # selected_positions setini temizle
                selected_positions.clear()
                
                # Seçili itemleri işaretle
                for item_id in selected_items:
                    item_values = profit_tree.item(item_id, "values")
                    if item_values and len(item_values) > 1:
                        symbol = item_values[1]
            ...
            # Treeview'ı çoklu seçim modunda yapılandır ve olayları bağla
            profit_tree.configure(selectmode="extended")  # Çoklu seçime izin ver
            
            # CTRL tuşu ile çoklu seçim yapabilmek için özel işlev
            def on_tree_click(event):
                # Seçimi korumak için CTRL tuşu basılı mı kontrol et
                is_ctrl_pressed = event.state & 0x4  # CTRL tuşunun state değeri
                
                if not is_ctrl_pressed:
                    # CTRL basılı değilse, treeview'ın normal davranışını kullan
                    return
                ...
                # Öğe zaten seçili mi kontrol et
                is_selected = item_id in profit_tree.selection()
                
                if is_selected:
                    # Eğer öğe zaten seçiliyse, seçimden kaldır
                    profit_tree.selection_remove(item_id)
                else:
                    # Değilse, mevcut seçime ekle
                    profit_tree.selection_add(item_id)
                
                # Event'i işlenmiş olarak belirt, böylece treeview'ın normal seçim davranışı çalışmaz
                return "break"
            
            # Öğe seçimlerini izle
            profit_tree.bind("<<TreeviewSelect>>", on_item_selection)
            # Özel çoklu seçim davranışı için tıklama olayını bağla
            profit_tree.bind("<ButtonPress-1>", on_tree_click)
            ...
            # Select All butonu - basitleştirilmiş versiyon
            def select_all_items():
                # Tüm itemleri seç
                for item_id in profit_tree.get_children():
                    profit_tree.selection_add(item_id)
                
                # Seçim işlemi otomatik olarak on_item_selection event fonksiyonu tarafından halledilecek
            ...
            # Deselect All butonu - basitleştirilmiş versiyon
            def deselect_all_items():
                # Tüm seçimleri kaldır
                profit_tree.selection_remove(profit_tree.get_children())
                
                # Seçim temizleme işlemi otomatik olarak on_item_selection event fonksiyonu tarafından halledilecek
            ...
            # Tamamen yeniden yazılmış Hidden Buy butonu - Çoklu seçim destekli
            def hidden_buy_selected():
                print("Hidden Buy fonksiyonu çalıştırılıyor...")
                
                # Doğrudan treeview seçiminden al
                selected_items = profit_tree.selection()
                ...
                for item_id in selected_items:
                    ...
                    try:
                        symbol = item_values[1]
                        current_quantity = float(item_values[3].replace(",", ""))
                        ...
                        quantity = min(200, int(current_quantity))
            ...
            def test_button_clicked():
                ...
                # Seçili pozisyonları kontrol et
                selected_items = []
                for item_id in profit_tree.get_children():
                    item_values = profit_tree.item(item_id, "values")
                    if item_values and item_values[0] == "✓":
                        selected_items.append(item_values[1])  # Symbol
                
                # Sonucu göster
                if selected_items:
                    msg = f"Seçili pozisyonlar: {', '.join(selected_items)}"
                    messagebox.showinfo("Test", msg)
                else:
                    messagebox.showinfo("Test", "Hiç pozisyon seçilmemiş!")
            ...
            hidden_buy_btn.pack(side=tk.LEFT, padx=10, pady=10)
            ...
            # Seçili pozisyon için emir - yeniden yazılmış versiyon
            def place_order_for_selected():
                print("Seçili pozisyon için emir fonksiyonu çalıştırılıyor...")
                
                # Treeview'dan doğrudan seçimi al
                selected_items = profit_tree.selection()
                if not selected_items:
                    messagebox.showinfo("Uyarı", "Lütfen en az bir pozisyon seçin.")
                    return
                
                print(f"Seçilen pozisyon sayısı: {len(selected_items)}")
                
                # İlk seçili pozisyonu kullan
                selected_item_id = selected_items[0]
                
                try:
                    # Seçili hissenin verilerini al
                    item_values = profit_tree.item(selected_item_id, "values")
                    ...
                    symbol = item_values[1]
                    target_price = float(item_values[8])
                    current_quantity_str = item_values[3].replace(",", "")
                    current_quantity = int(float(current_quantity_str))
                    
                    print(f"Seçilen sembol: {symbol}, Hedef fiyat: {target_price}, Mevcut miktar: {current_quantity}")
                    ...
                    if not shares_prompt:
                        print("Kullanıcı miktar girişinden vazgeçti")
                        return
                    ...
                    if confirm:
                        print(f"Emir onaylandı: {symbol} x {quantity} @ {target_price}")
                        try:
                            # Kontrat oluştur
                            contract = Stock(symbol, 'SMART', 'USD')
                            
                            # Emir oluştur ve gönder
                            limit_order = LimitOrder('BUY', quantity, round(target_price, 2))
                            limit_order.hidden = True
                            self.ib.placeOrder(contract, limit_order)
                            print(f"Emir gönderildi: {symbol} BUY @ {target_price:.2f} x {quantity}")
                            
                            messagebox.showinfo("Başarılı", f"{symbol} için alış emri başarıyla gönderildi!")
                        except Exception as e:
                            print(f"Emir gönderirken hata: {e}")
                            import traceback
                            traceback.print_exc()
                            messagebox.showerror("Hata", f"Emir gönderilirken hata oluştu: {str(e)}")
                    else:
                        print("Emir iptal edildi")
                except Exception as e:
                    print(f"Pozisyon verilerini işlerken hata: {e}")
                    import traceback
                    traceback.print_exc()
    ...
    def cashpark35_mal_topla(self):
        ...
        try:
            ...
            def on_tree_click(event):
                ...
                # Tıklanan öğeyi belirle
                item_id = opportunity_tree.identify_row(event.y)
                if not item_id:
                    return
                
                # Öğe zaten seçili mi kontrol et
                is_selected = item_id in opportunity_tree.selection()
                
                if is_selected:
                    # Eğer öğe zaten seçiliyse, seçimden kaldır
                    opportunity_tree.selection_remove(item_id)
                else:
                    # Değilse, mevcut seçime ekle
                    opportunity_tree.selection_add(item_id)
                
                # Event'i işlenmiş olarak belirt, böylece treeview'ın normal seçim davranışı çalışmaz
                return "break"
            
            # Özel çoklu seçim davranışı için tıklama olayını bağla
            opportunity_tree.bind("<ButtonPress-1>", on_tree_click)
