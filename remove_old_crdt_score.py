...
for filename in ek_files:
    try:
        ...
        # Remove old CRDT SCORE_ column (with space)
        if 'CRDT SCORE_' in df.columns:
            print(f"  Removing 'CRDT SCORE_' column")
            df = df.drop('CRDT SCORE_', axis=1)
        
        # Also remove CRDT_SCORE_ column if it exists
