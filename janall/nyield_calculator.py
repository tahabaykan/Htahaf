...
def calculate_yield(settlement_date, maturity_date, coupon_rate, price, redemption=25, frequency=2, basis=0):
    ...
    try:
        ...
        # Coupon rate kontrolü
        if pd.isna(coupon_rate) or coupon_rate == '' or coupon_rate == 0:
            return np.nan
            
        if isinstance(coupon_rate, str):
            coupon_rate = float(coupon_rate.replace('%', '')) / 100
        else:
            coupon_rate = float(coupon_rate) / 100  # Yüzde formatına çevir
...
def load_treasury_yields(csv_file="../treyield.csv"):
    """
    Treasury yield'larını CSV dosyasından yükler (ana StockTracker dizininden)
    """
    try:
...
def process_yek_files():
    """
    NEK dosyalarını işleyerek YEK dosyalarını oluşturur
    """
    try:
        ...
        # NEK dosyalarını bul (ana StockTracker dizininden)
        nek_files = glob.glob('../nek*.csv')
        if not nek_files:
            print("nek*.csv dosyaları bulunamadı!")
            return
        
        print(f"Bulunan NEK dosyaları: {len(nek_files)}")
        
        for nek_file in nek_files:
            try:
                print(f"\nİşleniyor: {nek_file}")
                
                # NEK dosyasını oku (ana dizinden)
                df = pd.read_csv(nek_file)
                ...
                if not coupon_col:
                    print(f"  Coupon kolonu bulunamadı: {nek_file}")
                    continue
                
                # Cally kolonlarını ekle
                cally_columns = ['2Y Cally', '5Y Cally', '7Y Cally', '10Y Cally', '15Y Cally', '20Y Cally', '30Y Cally']
                for col in cally_columns:
                    if col not in df.columns:
                        df[col] = ''
                
                # YEK dosya adını oluştur (ana dizinde kaydet)
                yek_file = nek_file.replace('nek', 'yek')
                
                # CSV'ye kaydet (ana StockTracker dizininde)
                df.to_csv(yek_file, index=False)
                print(f"  {yek_file} dosyası oluşturuldu (ana dizinde)!")
                
            except Exception as e:
                print(f"  {nek_file} işleme hatası: {e}")
...
def collect_csv_files():
    ...
    try:
        ...
        # Toplanacak CSV dosyaları (ana StockTracker dizininden)
        csv_files = [
        ...
        if all_data:
            ...
            # ncollected.csv'ye kaydet (ana StockTracker dizininde)
            collected_df.to_csv('../ncollected.csv', index=False, encoding='utf-8')
            
            print(f"\nSUCCESS: ncollected.csv oluşturuldu (ana dizinde)!")
...
def create_competition_priority():
    ...
    try:
        ...
        # ncollected.csv'yi oku (ana dizinden - orada oluşturuldu)
        ...
        if competition_data:
            ...
            # ncomppriority.csv'ye kaydet (ana StockTracker dizininde)
            comp_df.to_csv('../ncomppriority.csv', index=False, encoding='utf-8')
            
            print(f"\nSUCCESS: ncomppriority.csv oluşturuldu (ana dizinde)!")
...
def add_ncomp_to_yek_files():
    ...
    try:
        ...
        # ncomppriority.csv'yi oku (ana dizinden - orada oluşturuldu)
        ...
        # YEK dosyalarını bul (ana dizinden - orada oluşturuldu)
        ...
        for yek_file in yek_files:
            try:
                ...
                print(f"  SUCCESS: {yek_file} güncellendi (ana dizinde)")
    ...
    try:
        ...
        for yek_file in yek_files:
            try:
                ...
                print(f"[OK] {yek_file} yüklendi: {len(df)} satır")
                ...
                if coupon_col and div_adj_price_col:
                    ...
                    for idx, row in df.iterrows():
                        try:
                            ...
                            print(f"    Satır {idx}: COUPON={coupon_rate}, Div adj.price={div_adj_price}")
                    ...
                    print(f"[OK] {yek_file} güncellendi (ana dizinde)")
