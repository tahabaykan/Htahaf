...
class MainWindow(tk.Tk):
    ...
    def toggle_live_data(self):
        ...
        if not self.live_data_running:
            ...
            print("\n[ETF] 📸 ETF'ler için snapshot sistemi başlatılıyor...")
    ...
    def update_scores_with_market_data(self):
        """Market data ile skorları güncelle - Sadece streaming veri kullanır"""
        try:
            ...
            for idx, row in self.df.iterrows():
                ...
                # Prev close'u streaming'den al (cache kaldırıldı)
                prev_close = market_data.get('prevClose', 0)
                ...
                if idx < 3:  # İlk 3 ticker için debug
                    ...
                    # print(f"[DEBUG] {ticker}: FINAL_THG={final_thg:.2f}, spread={spread:.4f}")
                    
                    # Passive fiyatları hesapla ve göster
                    ...
                    # print(f"[DEBUG] {ticker}: pf_bid_buy={pf_bid_buy:.2f}, pf_front_buy={pf_front_buy:.2f}, pf_ask_buy={pf_ask_buy:.2f}")
                    
                    # Değişimleri hesapla ve göster
                    ...
                    # print(f"[DEBUG] {ticker}: pf_bid_buy_chg={pf_bid_buy_chg:.4f}, pf_front_buy_chg={pf_front_buy_chg:.4f}, pf_ask_buy_chg={pf_ask_buy_chg:.4f}")
                    
                    # Ucuzluk skorlarını hesapla ve göster
                    ...
                    # print(f"[DEBUG] {ticker}: bid_buy_ucuzluk={bid_buy_ucuzluk:.4f}, front_buy_ucuzluk={front_buy_ucuzluk:.4f}, ask_buy_ucuzluk={ask_buy_ucuzluk:.4f}")
                    
                    # Final skorları hesapla ve göster
                    final_bb = final_thg - 400 * bid_buy_ucuzluk
                    final_fb = final_thg - 400 * front_buy_ucuzluk
                    final_ab = final_thg - 400 * ask_buy_ucuzluk
                    
                    # print(f"[DEBUG] {ticker}: final_bb={final_bb:.2f}, final_fb={final_fb:.2f}, final_ab={final_ab:.2f}")
                    # print("---")
    ...
    def calculate_scores_for_stock(self, ticker, row, bid, ask, last_price, prev_close, benchmark_chg=0):
        """Tek hisse için skorları hesapla ve dictionary döndür"""
        try:
            # Debug: Market data durumunu kontrol et
            print(f"[DEBUG] {ticker}: bid={bid}, ask={ask}, last={last_price}")
            
            # Market data eksikse bile devam et, varsayılan değerler kullan
            if bid == 0 and ask == 0:
                print(f"[DEBUG] {ticker}: Market data eksik, varsayılan değerler kullanılıyor")
                bid = ask = last_price = 1.0  # Varsayılan değerler
            ...
            print(f"[DEBUG] {ticker}: SHORT_FINAL={short_final}")
            
            # Fiyat hesaplamaları
            pf_bid_buy = bid
            pf_front_buy = last_price
            pf_ask_buy = ask
            
            # Change hesaplamaları
            pf_bid_buy_chg = bid - prev_close
            pf_front_buy_chg = last_price - prev_close
            pf_ask_buy_chg = ask - prev_close
            
            # Ucuzluk skorları (1000x ağırlık)
            bid_buy_ucuzluk = (pf_bid_buy_chg / prev_close) * 1000 if prev_close > 0 else 0
            front_buy_ucuzluk = (pf_front_buy_chg / prev_close) * 1000 if prev_close > 0 else 0
            ask_buy_ucuzluk = (pf_ask_buy_chg / prev_close) * 1000 if prev_close > 0 else 0
            
            # Pahalılık skorları (1000x ağırlık)
            ask_sell_pahalilik = (pf_ask_buy_chg / prev_close) * 1000 if prev_close > 0 else 0
            front_sell_pahalilik = (pf_front_buy_chg / prev_close) * 1000 if prev_close > 0 else 0
            bid_sell_pahalilik = (pf_bid_buy_chg / prev_close) * 1000 if prev_close > 0 else 0
            
            # Final skorlar (SHORT_FINAL kullanarak)
            final_bb = (short_final * bid_buy_ucuzluk) if short_final > 0 else 0
            final_fb = (short_final * front_buy_ucuzluk) if short_final > 0 else 0
            final_ab = (short_final * ask_buy_ucuzluk) if short_final > 0 else 0
            final_as = (short_final * ask_sell_pahalilik) if short_final > 0 else 0
            final_fs = (short_final * front_sell_pahalilik) if short_final > 0 else 0
            final_bs = (short_final * bid_sell_pahalilik) if short_final > 0 else 0
            
            # Yeni Final SAS, Final SFS, Final SBS skorları (SHORT_FINAL kullanarak)
            final_sas = (short_final * ask_sell_pahalilik) if short_final > 0 else 0
            final_sfs = (short_final * front_sell_pahalilik) if short_final > 0 else 0
            final_sbs = (short_final * bid_sell_pahalilik) if short_final > 0 else 0
            
            print(f"[DEBUG] {ticker}: final_bb={final_bb}, final_fb={final_fb}, final_ab={final_ab}")
            
            print(f"[DEBUG] {ticker}: Final_BB_skor={final_bb}")
            
            return {
                'Final_BB_skor': final_bb,
                'Final_FB_skor': final_fb,
                'Final_AB_skor': final_ab,
                'Final_AS_skor': final_as,
                'Final_FS_skor': final_fs,
                'Final_BS_skor': final_bs,
                'Final_SAS_skor': final_sas,
                'Final_SFS_skor': final_sfs,
                'Final_SBS_skor': final_sbs,
                'Bid_buy_ucuzluk_skoru': bid_buy_ucuzluk,
                'Front_buy_ucuzluk_skoru': front_buy_ucuzluk,
                'Ask_buy_ucuzluk_skoru': ask_buy_ucuzluk,
                'Ask_sell_pahalilik_skoru': ask_sell_pahalilik,
                'Front_sell_pahalilik_skoru': front_sell_pahalilik,
                'Bid_sell_pahalilik_skoru': bid_sell_pahalilik
            }
            
        except Exception as e:
            # print(f"[SKOR] ❌ {ticker} skor hesaplama hatası: {e}")
            return None
    ...
    def calculate_scores(self, ticker, row, bid, ask, last_price, prev_close, benchmark_chg=0):
        ...
        try:
            ...
            final_sas = (short_final * ask_sell_pahali) if short_final > 0 else 0
            final_sfs = (short_final * front_sell_pahali) if short_final > 0 else 0
            final_sbs = (short_final * bid_sell_pahali) if short_final > 0 else 0
    ...
    def get_prev_close_for_symbol(self, symbol: str) -> float:
        """Symbol için prev close değerini döndür"""
        try:
            # CSV'lerden prev close al
            if hasattr(self, 'prev_close_data') and symbol in self.prev_close_data:
                return float(self.prev_close_data[symbol])
            
            # ETF Panel'den al
            if hasattr(self, 'etf_panel') and self.etf_panel:
                if symbol in self.etf_panel.etf_prev_close_data:
                    return float(self.etf_panel.etf_prev_close_data[symbol])
        ...
        try:
            ...
            if hasattr(self, 'etf_panel') and self.etf_panel:
                if symbol in self.etf_panel.etf_data:
                    return float(self.etf_panel.etf_data[symbol].get('last', 0))
        ...
        try:
            ...
            for group in groups:
                try:
                    ...
                    csv_file = f"ssfinek{group}.csv"
                    
                    # Dosya var mı kontrol et
                    if not os.path.exists(csv_file):
                        print(f"[TOP TEN] ⚠️ {csv_file} bulunamadı")
                        continue
                    
                    # Dosyayı doğrudan aç
                    print(f"[TOP TEN] 📋 {group} dosyası açılıyor...")
                    self.show_file_data(csv_file)
                    
                    # DataFrame'i doğrudan kullan
                    df = self.df
                    print(f"[TOP TEN] 📋 {group}: {len(df)} hisse bulundu")
                    
                    # Tüm sayfaları tarayarak hisseleri topla
                    group_stocks = []
                    
                    # Mevcut sayfa
                    current_df = df
                    
                    # Tüm sayfaları tara (sayfalama varsa)
                    page_count = 0
                    while len(current_df) > 0:
                        page_count += 1
                        print(f"[TOP TEN] 📄 {group} sayfa {page_count}: {len(current_df)} hisse")
                        
                        # Bu sayfadaki hisseleri al
                        for index, row in current_df.iterrows():
                            ticker = row['PREF IBKR']
                            
                            # DataFrame'de Final_BB_skor var mı kontrol et
                            if 'Final_BB_skor' in current_df.columns:
                                final_bb_skor = row.get('Final_BB_skor', 0)
                                # Tüm skorları al (pozitif ve negatif)
                                group_stocks.append({
                                    'symbol': ticker,
                                    'Final_BB_skor': final_bb_skor
                                })
                                print(f"[DEBUG] {ticker}: Final_BB_skor={final_bb_skor}")
                            else:
                                # Skor yoksa hesapla
                                market_data = self.hammer.get_market_data(ticker) if hasattr(self, 'hammer') and self.hammer else None
                            ...
                            # Skorları hesapla
                            scores = self.calculate_scores_for_stock(ticker, row, bid, ask, last_price, prev_close, benchmark_chg)
                            ...
                            if scores and 'Final_BB_skor' in scores:
                                ...
                                print(f"[DEBUG] {ticker}: Skor hesaplanamadı veya eksik")
                                final_bb_skor = 0  # Varsayılan değer
                        
                        # Sonraki sayfa var mı kontrol et (sayfalama sistemi varsa)
                        if hasattr(self, 'next_page') and hasattr(self, 'total_pages'):
                            if self.current_page < self.total_pages - 1:
                                self.next_page()
                                current_df = self.df
                            else:
                            ...
                            # Sayfalama yoksa sadece mevcut sayfayı kullan
                            break
                    
                    print(f"[TOP TEN] 📊 {group}: Toplam {len(group_stocks)} hisse bulundu ({page_count} sayfa)")
                    ...
                    print(f"[TOP TEN] 📊 {group}: Unique {len(unique_stocks)} hisse (duplicate'ler kaldırıldı)")
                    
                    if len(unique_stocks) == 0:
                        print(f"[TOP TEN] ⚠️ {group}: Unique hisse bulunamadı")
                        continue
                    
                    # En yüksek %10'u seç (min 1, max 8) - unique_stocks üzerinden
                    ...
                    unique_stocks.sort(key=lambda x: x['Final_BB_skor'], reverse=True)
                    top_stocks = unique_stocks[:select_count]
    ...
    def find_group_window(self, group):
        """Grup penceresini bul"""
        try:
            # Toplevel pencereleri tara
            for widget in self.winfo_children():
                if hasattr(widget, 'title') and hasattr(widget, 'df'):
                    title = widget.title()
    ...
    def show_order_confirmation(self, title, symbols, scores, order_type, group):
        ...
        cols = ['select', 'symbol', 'order_info', 'bid', 'ask', 'spread', 'last', 'score']
        if order_type == "ask_sell":
            headers = ['Seç', 'Symbol', 'Emir Bilgisi', 'Bid', 'Ask', 'Spread', 'Last', 'Final SAS Skor']
        else:
            headers = ['Seç', 'Symbol', 'Emir Bilgisi', 'Bid', 'Ask', 'Spread', 'Last', 'Final BB Skor']
        ...
        lot_size = int(getattr(self, 'lot_entry', None).get() or 200) if hasattr(self, 'lot_entry') else 200
        ...
        for symbol, score in zip(symbols, scores):
            ...
            # Emir bilgisi
            order_info = f"{lot_size} lot {order_direction} @ ${order_price:.2f} (HIDDEN)"
        ...
        def confirm_order():
            ...
            try:
                ...
                for symbol in selected_symbols:
                    ...
                    print(f"[ORDER] ✅ {symbol}: {order_type} emri gönderildi - {lot_size} lot @ ${price:.2f}")
