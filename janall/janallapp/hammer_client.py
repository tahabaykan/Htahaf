...
class HammerClient:
    ...
    def _on_message(self, ws, message):
        ...
        try:
            ...
            if cmd == "connect":
                ...
                # ETF'ler için snapshot işlenecek, preferred stock'lar için kaldırıldı
                symbol = result.get('sym')
                ...
                if success == "OK":
                    if symbol in etf_list:
                        ...
                        # PREFERRED STOCK SNAPSHOT KALDIRILDI
                        pass
                else:
                    print(f"[HAMMER] ❌ Snapshot hatası: {symbol}")
    ...
    def get_symbol_snapshot(self, symbol):
        """Bir sembol için snapshot verilerini al - ETF'ler için aktif, preferred stock'lar için kaldırıldı"""
        if not self.connected or not self.authenticated:
            return False
            
        # PREFERRED STOCK'LAR İÇİN SNAPSHOT KALDIRILDI!
        if " PR" in symbol:
            # Snapshot mesajı kaldırıldı
            return False
            
        # ETF'ler için snapshot aktif
        etf_list = ["SPY", "TLT", "IEF", "IEI", "PFF", "KRE", "IWM", "SHY", "PGF"]
        
        if symbol in etf_list:
            # ETF Snapshot Request mesajı kaldırıldı
            ...
            print(f"[HAMMER] ⚠️ Unknown symbol: {symbol}")
            return False
    ...
    def subscribe_symbol(self, symbol, include_l2=False):
        ...
        if symbol in etf_list:
            ...
            # Preferred stocks: "AHL PRF" -> "AHL-F", "VNO PRN" -> "VNO-N", "TRTX PRC" -> "TRTX-C"
            # Sadece " PR" içeren hisseleri "-" ile değiştir
