...
class PsfAlgo:
    ...
    def start_pisdongu_cycle(self):
        """PISDoNGU döngüsünü başlat"""
        if not self.pisdongu_active:
            return
            
        self.pisdongu_cycle_count += 1
        print(f"[PISDoNGU] 🔄 Döngü #{self.pisdongu_cycle_count} başlatılıyor...")
    ...
    def schedule_next_pisdongu_cycle(self):
        ...
        print("[PISDoNGU] ⏰ Timer kuruldu - 180 saniye sonra yeni döngü başlayacak")
    ...
    def on_fill(self, ticker, side, price, size, **kwargs):
        ...
        if ticker in self.exclude_list:
            print(f"[PSFAlgo EXCLUDE] ❌ {ticker} exclude listesinde - fill işlenmedi")
            return
    ...
    def open_reverse_order(self, ticker, side, size, fill_price):
        ...
        # ✅ EXCLUDE LIST kontrolü - reverse order da ignore edilmeli
        if ticker in self.exclude_list:
            print(f"[PSFAlgo EXCLUDE] ❌ {ticker} exclude listesinde - reverse order açılmadı")
            return False
        ...
        try:
            ...
            # ✅ YENİ 2 AŞAMALI KAR GARANTİSİ MANTIGI
            ...
            if side == 'SHORT':  # LONG arttırma fill'i sonrası SHORT reverse
                ...
                # 1. AŞAMA: Minimum 5 cent kar garantisi
                min_profit_price = fill_price + 0.05
                print(f"[REVERSE] {ticker} LONG→SHORT: Fill={fill_price:.3f} → Min kar fiyatı={min_profit_price:.3f} (+5 cent)")
                
                # 2. AŞAMA: Ask-spread*0.15 kontrolü (sadece min kar sağlanıyorsa)
                if ask > 0 and spread > 0:
                    spread_price = ask - spread * 0.15
                    print(f"[REVERSE] {ticker} Spread kontrolü: Ask={ask:.3f}, Spread={spread:.3f} → Ask-spread*0.15={spread_price:.3f}")
                    
                    # Hangi fiyat daha yüksekse onu kullan (en yüksek kar garantisi)
                    if min_profit_price >= spread_price:
                        price = min_profit_price
                        logic = f"Min kar fiyatı kullanıldı: {price:.3f} (5 cent kar = en yüksek kar garantisi)"
                    else:
                        price = spread_price
                        logic = f"Spread kontrolü kullanıldı: {price:.3f} (ask-spread*0.15 = en yüksek kar garantisi)"
                else:
                    # Ask/spread verisi yoksa sadece min kar kullan
                    price = min_profit_price
                    logic = f"Ask/spread verisi yok, min kar kullanıldı: {price:.3f}"
                
                print(f"[REVERSE] {ticker} LONG→SHORT reverse: {logic}")
                ...
                # 1. AŞAMA: Minimum 5 cent kar garantisi
                min_profit_price = fill_price - 0.05
                print(f"[REVERSE] {ticker} SHORT→LONG: Fill={fill_price:.3f} → Min kar fiyatı={min_profit_price:.3f} (-5 cent)")
                
                # 2. AŞAMA: Bid+spread*0.15 kontrolü (sadece min kar sağlanıyorsa)
                if bid > 0 and spread > 0:
                    spread_price = bid + spread * 0.15
                    print(f"[REVERSE] {ticker} Spread kontrolü: Bid={bid:.3f}, Spread={spread:.3f} → Bid+spread*0.15={spread_price:.3f}")
                    
                    # Hangi fiyat daha düşükse onu kullan (en yüksek kar garantisi)
                    if min_profit_price <= spread_price:
                        price = min_profit_price
                        logic = f"Min kar fiyatı kullanıldı: {price:.3f} (5 cent kar = en yüksek kar garantisi)"
                    else:
                        price = spread_price
                        logic = f"Spread kontrolü kullanıldı: {price:.3f} (bid+spread*0.15 = en yüksek kar garantisi)"
                else:
                    # Bid/spread verisi yoksa sadece min kar kullan
                    price = min_profit_price
                    logic = f"Bid/spread verisi yok, min kar kullanıldı: {price:.3f}"
                
                print(f"[REVERSE] {ticker} SHORT→LONG reverse: {logic}")
    ...
    def send_order(self, ticker, price, final_thg, side, size=200):
        ...
        # ✅ EXCLUDE LIST kontrolü - önce bu kontrol edilmeli
        if ticker in self.exclude_list:
            print(f"[PSFAlgo EXCLUDE] ❌ {ticker} exclude listesinde - emir gönderilmedi")
            log_reasoning(f"{ticker} exclude listesinde olduğu için emir gönderilmedi")
            return False
    ...
    def run_new_long_tp_fs(self):
        ...
        # FINAL FS en düşük 5 hisse seç (sadece long pozisyonlar)
        ...
        for ticker, score in selected_stocks:
            ...
            spread_ok, spread_msg = self.check_front_spread_condition(ticker, 'FRONT_SELL')
        ...
        # Yeterli hisse yoksa sıradaki hisseleri dene
        if len(valid_tickers) < 5:
            print(f"[PSF CHAIN 6] ⚠️ Sadece {len(valid_tickers)}/5 hisse koşulları sağlıyor, sıradaki hisseler deneniyor...")
            ...
            for ticker, score in extended_stocks:
        ...
        # ✅ 6. adım tamamlandı, otomatik olarak 7. adıma geç
        print("[PSF CHAIN 6] ✅ Long TP FS tamamlandı, Short TP BB'ye geçiliyor...")
        self.advance_chain()
    ...
    def run_new_short_tp_fb(self):
        ...
        # FINAL FB en yüksek 5 hisse seç (sadece short pozisyonlar)
        selected_stocks = self.get_top_stocks_by_score(
            self.current_window, 
            'Final FB skor', 
            count=5, 
        ...
        for ticker, score in selected_stocks:
            ...
            # Spread koşulu kontrolü
            spread_ok, spread_msg = self.check_front_spread_condition(ticker, 'FRONT_BUY')
        ...
        # Yeterli hisse yoksa sıradaki hisseleri dene
        if len(valid_tickers) < 5:
            print(f"[PSF CHAIN 8] ⚠️ Sadece {len(valid_tickers)}/5 hisse spread koşulunu sağlıyor, sıradaki hisseler deneniyor...")
        ...
        # ✅ 8. adım tamamlandı, otomatik olarak 9. adıma geç (eski sistem)
        print("[PSF CHAIN 8] ✅ Short TP FB tamamlandı, eski sisteme (T-Losers Old) geçiliyor...")
        self.advance_chain()
    ...
    def run_new_t_losers_bb_data_ready(self):
        ...
        for ticker, score in filtered_stocks[:5]:  # İlk 5'i al
        ...
        if not valid_tickers:
            print("[PSF CHAIN 1] ❌ Hiçbir hisse pozisyon güvenliği sağlamıyor")
            self.advance_chain()
            return
    ...
    def run_new_t_gainers_as_data_ready(self):
        ...
        if not valid_tickers:
            print("[PSF CHAIN 3] ❌ Hiçbir hisse koşulları sağlamıyor")
            self.advance_chain()
            return
    ...
    def run_new_t_losers_fb(self):
        ...
        # FINAL FB en yüksek 5 hisse seç
        selected_stocks = self.get_top_stocks_by_score(
            self.current_window, 
            'Final FB skor', 
            count=5, 
        ...
        # Yeterli hisse yoksa sıradaki hisseleri dene
        if len(valid_tickers) < 5:
            print(f"[PSF CHAIN 2] ⚠️ Sadece {len(valid_tickers)}/5 hisse spread koşulunu sağlıyor, sıradaki hisseler deneniyor...")
        ...
        # ✅ 2. adım tamamlandı, otomatik olarak 3. adıma geç
        print("[PSF CHAIN 2] ✅ T-Losers FB tamamlandı, T-Gainers AS'a geçiliyor...")
        self.advance_chain()
    ...
    def run_new_t_gainers_fs(self):
        ...
        # Yeterli hisse yoksa sıradaki hisseleri dene
        if len(valid_tickers) < 5:
            print(f"[PSF CHAIN 4] ⚠️ Sadece {len(valid_tickers)}/5 hisse koşulları sağlıyor, sıradaki hisseler deneniyor...")
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    if ticker in self.exclude_list:
                        excluded_count += 1
                        print(f"[SCORE SELECTION EXCLUDE] ❌ {ticker} exclude listesinde - atlandı")
                        continue
    ...
    def check_front_spread_condition(self, ticker, order_type):
        """
        Front order için spread koşulunu kontrol eder
        ...
        Returns:
            (bool, str): (koşul_sağlandı, açıklama_mesajı)
        """
        try:
            # Market data'dan bid/ask al
            bid = 0
            ...
            if hasattr(self.market_data, 'last_data') and ticker in self.market_data.last_data:
                ...
                bid = float(md.get('bid', 0)) if md.get('bid') not in [None, 'N/A', ''] else 0
                ask = float(md.get('ask', 0)) if md.get('ask') not in [None, 'N/A', ''] else 0
                last = float(md.get('last', 0)) if md.get('last') not in [None, 'N/A', ''] else 0
                
            # Market data yoksa varsayılan olarak başarılı say (test için)
            if bid <= 0 or ask <= 0 or last <= 0:
                print(f"[SPREAD CHECK] ❌ {ticker} market data eksik - bid:{bid}, ask:{ask}, last:{last}")
                return False, f"Market data eksik (bid:{bid}, ask:{ask}, last:{last}) - güvenlik nedeniyle red"
            
            spread = ask - bid
            
            if spread <= 0:
                return False, f"Geçersiz spread: {spread:.4f}"
            
            if order_type == 'FRONT_BUY':
                # Front Buy koşulu: Last fiyat bid'e yakın (≤ spread*0.35) ve ask'den uzak (≥ spread*0.65)
                distance_from_bid = last - bid
                distance_from_ask = ask - last
                
                close_to_bid = distance_from_bid <= spread * 0.35
                far_from_ask = distance_from_ask >= spread * 0.65
                
                if close_to_bid and far_from_ask:
                    return True, f"Front Buy OK - Bid yakın: {distance_from_bid:.4f}≤{spread*0.35:.4f}, Ask uzak: {distance_from_ask:.4f}≥{spread*0.65:.4f}"
                else:
                    return False, f"Front Buy KO - Bid: {distance_from_bid:.4f}/{spread*0.35:.4f}, Ask: {distance_from_ask:.4f}/{spread*0.65:.4f}"
            elif order_type == 'FRONT_SELL':
                # Front Sell koşulu: Last fiyat ask'e yakın (≤ spread*0.35) ve bid'den uzak (≥ spread*0.65)
                distance_from_ask = ask - last
                distance_from_bid = last - bid
                
                close_to_ask = distance_from_ask <= spread * 0.35
                far_from_bid = distance_from_bid >= spread * 0.65
                
                if close_to_ask and far_from_bid:
                    return True, f"Front Sell OK - Ask yakın: {distance_from_ask:.4f}≤{spread*0.35:.4f}, Bid uzak: {distance_from_bid:.4f}≥{spread*0.65:.4f}"
                else:
                    return False, f"Front Sell KO - Ask: {distance_from_ask:.4f}/{spread*0.35:.4f}, Bid: {distance_from_bid:.4f}/{spread*0.65:.4f}"
            else:
                return False, f"Bilinmeyen order type: {order_type}"
                
        except Exception as e:
            print(f"[SPREAD CHECK] ❌ {ticker} spread kontrolü hatası: {e}")
            return False, f"Spread kontrolü hatası: {str(e)}"
    ...
    def polygonize_ticker(self, ticker):
        ...
        return ticker.replace(' PR', 'p').replace(' ', '')
