...
import logging
from Htahaf.ib_api.conid_manager import load_conids
import threading
import time
...
def load_mastermind_data():
    data = {}
    for csv_path in ["Htahaf/mastermind_histport.csv", "Htahaf/mastermind_extltport.csv"]:
        try:
...
class MaltoplaWindow(tk.Toplevel):
    def __init__(self, parent, title, csv_path, market_data_dict, etf_data, benchmark_type='T', ibkr_client=None, exdiv_data=None, c_ticker_to_csv=None, c_csv_data=None):
        ...
        for _, row in df.iterrows():
            ...
            def get_val(col):
                val = row.get(col, '')
                # Eğer C-pref mapping mevcutsa ve ticker mappingdeyse, ilgili CSV'den çek
                if self.c_ticker_to_csv and self.c_csv_data and ticker in self.c_ticker_to_csv:
                    csv_name = self.c_ticker_to_csv[ticker]
                    try:
                        csv_row = self.c_csv_data[csv_name].loc[ticker]
                        # FINAL_THG için özel: sadece C-pref ve istenen kolon FINAL_THG ise
                        if col == 'FINAL_THG':
                            val_csv = csv_row.get('FINAL_THG', '') if hasattr(csv_row, 'get') else csv_row['FINAL_THG'] if 'FINAL_THG' in csv_row else ''
                            if val_csv != '' and not pd.isna(val_csv):
                                return val_csv
                        # Diğer kolonlar için eski davranış
                        val_csv = csv_row.get(col, '') if hasattr(csv_row, 'get') else csv_row[col] if col in csv_row else ''
                        if val_csv != '' and not pd.isna(val_csv):
                            return val_csv
                    except Exception:
                        pass
                # Eski davranış: mastermind_data'dan tamamla
                if (val == '' or pd.isna(val)) and col in mm_row:
                    return mm_row[col]
                return val
            ...
            def final_skor(final_thg, skor):
                try:
                    return float(final_thg) - 160 * float(skor)
                except:
                    return 'N/A'
