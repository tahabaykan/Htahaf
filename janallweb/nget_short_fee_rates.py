...
def main():
    """Ana program"""
    print("Short fee rate verisi çekme işlemi başlatılıyor... (EKHELD dosyaları için)")
    
    # İşlenecek dosyalar - EKHELD dosyaları
    input_files = [
    ...
    for input_file in input_files:
        if os.path.exists(input_file):
            ...
            print(f"✓ {input_file} bulundu")
        else:
            print(f"⚠️ {input_file} bulunamadı")
    
    if not existing_files:
        print("HATA: Hiçbir EKHELD dosyası bulunamadı!")
        sys.exit(1)
    
    print(f"\nToplam {len(existing_files)} dosya işlenecek")
    ...
    try:
        # Tüm sembolleri işle
        result_df = process_all_symbols(ib, existing_files)
        
        # Sonuçları nsmiall.csv'ye kaydet
        output_file = "nsmiall.csv"
