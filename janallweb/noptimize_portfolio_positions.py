...
def select_top_stocks(data, num_stocks=35, max_stocks_per_company=2, group_limits=None, cgrup_limits=None):
    """
    ...
    Ayrıca CGRUP bazlı özel limitler uygular (sadece Historical için)
    """
    ...
    if cgrup_limits:
        print("CGRUP bazlı özel limitler:")
        for k, v in cgrup_limits.items():
            print(f"  {k}: {v}")
    ...
    cgrup_counts = {}    # CGRUP bazlı sayım
    # Toplu limitler için sayaçlar
    cgrup_14a = 0  # C400+C425+C450
    cgrup_14b = 0  # C475+C500+C525
    cgrup_10 = 0   # C550+C575
    
    for index, row in sorted_data.iterrows():
        ticker = row['PREF IBKR']
        company = row['CMON']
        cgrup = row['CGRUP'] if 'CGRUP' in row else None
        group = row['Group'] if 'Group' in row else None
        
        include_stock = True
        
        # CGRUP bazlı limitler
        if cgrup_limits and cgrup:
            current_cgrup_count = cgrup_counts.get(cgrup, 0)
            if current_cgrup_count >= cgrup_limits.get(cgrup, 5):
                include_stock = False
        
        # Toplu CGRUP limitleri
        if cgrup in ['C400', 'C425', 'C450']:
            if cgrup_14a >= 14:
                include_stock = False
        if cgrup in ['C475', 'C500', 'C525']:
            if cgrup_14b >= 14:
                include_stock = False
        if cgrup in ['C550', 'C575']:
            if cgrup_10 >= 10:
                include_stock = False
        
        # Grup limiti kontrolü
        if group_limits and group is not None:
            group_val = group if not pd.isna(group) else -1
            current_group_count = group_counts.get(group_val, 0)
            if group_val in group_limits and current_group_count >= group_limits[group_val]:
                include_stock = False
        
        # Şirket limiti kontrolü
        current_company_count = company_counts.get(company, 0)
        if current_company_count >= max_stocks_per_company:
            include_stock = False
        
        # Portföy dolduysa dur
        ...
        if include_stock:
            ...
            if cgrup_limits and cgrup:
                cgrup_counts[cgrup] = cgrup_counts.get(cgrup, 0) + 1
            if cgrup in ['C400', 'C425', 'C450']:
                cgrup_14a += 1
            if cgrup in ['C475', 'C500', 'C525']:
                cgrup_14b += 1
...
def process_file(input_file, output_file, num_stocks=50, max_stocks_per_company=2, target_shares=25000, target_dollars=None, group_limits=None, cgrup_limits=None):
    """
    ...
    cgrup_limits : dict, optional
        CGRUP bazlı özel limitler (default: None)
    """
    try:
        ...
        portfolio = select_top_stocks(data, num_stocks, max_stocks_per_company, group_limits, cgrup_limits)
...
def main():
    ...
    for file_info in files_to_process:
        ...
        # Sadece Historical için CGRUP limitleri uygula
        cgrup_limits = None
        if file_info["type"] == "Historical":
            cgrup_limits = {
                'C400': 5, 'C425': 5, 'C450': 5, 'C475': 5, 'C500': 5, 'C525': 5,
                'C550': 5, 'C575': 5, 'C600': 5, 'C625': 5, 'C650': 5, 'C675': 5,
                'C700': 5, 'C725': 5, 'C750': 5, 'C775': 5
            }
