...
class HammerClient:
    ...
    def connect(self):
        ...
        try:
            ...
            # Bağlantı ve kimlik doğrulama için bekle
            timeout = 10
            ...
            print("[HAMMER] ❌ Timeout: Bağlantı veya kimlik doğrulama başarısız")
            self.disconnect()
            return False
        ...
        try:
            ...
            if cmd == "connect":
                if success == "OK":
                    ...
                    print(f"[HAMMER] ❌ Kimlik doğrulama başarısız: {result}")
                    self.authenticated = False
                    self.disconnect()
                    ...
                    for tr in tx.get('transactions', []):
                        try:
                            ...
                            if status == 'Filled' and is_new and symbol and filled_qty > 0:
                                ...
                                # nfilled dosyasına kaydet (main_window varsa)
                                if self.main_window and hasattr(self.main_window, 'save_fill_to_nfilled'):
                                    fill_data = {
                                        'order_id': f"hammer_{int(datetime.now().timestamp())}",
                                        'symbol': display_symbol,
                                        'action': action,
                                        'side': action,
                                        'qty': filled_qty,
                                    ...
                                    try:
                                        self.main_window.save_fill_to_nfilled(fill_data)
                                    except Exception as e:
                                        print(f"[HAMMER] ❌ nfilled kaydetme hatası: {e}")
                        except Exception:
                            continue
                except Exception:
                    pass
    ...
    def _handle_market_data(self, data):
        ...
        try:
            ...
            # Mevcut high/low değerlerini al (yoksa 0)
            existing_data = self.market_data.get(display_symbol, {})
            current_high = existing_data.get("high", 0)
            current_low = existing_data.get("low", 0)
            
            # High/Low güncelle (L1Update'ten gelen değer veya trade'den takip)
            new_high = safe_float(data.get("high")) or current_high
            new_low = safe_float(data.get("low")) or current_low
            
            # Eğer L1'den high/low gelmiyorsa, trade fiyatından takip et
            if last_price > 0:
                if new_high == 0 or last_price > new_high:
                    new_high = last_price
                if new_low == 0 or last_price < new_low:
                    new_low = last_price
            ...
            market_data = {
                ...
                "prev_close": safe_float(data.get("prevClose")) or safe_float(data.get("previClose")) or safe_float(data.get("close")),
    ...
    def subscribe_symbol(self, symbol, include_l2=True):
        """Bir sembole abone ol"""
        try:
            # PR format düzeltmesi
            ...
            if include_l2:
                ...
                self._send_command(cmd_l2)
                
            return True
        except Exception as e:
            print(f"[HAMMER CLIENT] ❌ Subscribe hatası: {e}")
            return False
    ...
    def get_market_data(self, symbol):
        ...
        # Market data display_symbol formatında saklanıyor (örn: "GS PRA")
        # Önce orijinal symbol ile ara (display_symbol formatında)
        if symbol in self.market_data:
            return self.market_data.get(symbol, {})
        
        # Bulamazsa formatted symbol ile ara (Hammer formatında: "GS-A")
        formatted_symbol = symbol
        if " PR" in symbol:
        ...
        # Son olarak ters dönüşüm dene (örn: "GS-A" -> "GS PRA")
        if "-" in symbol and " PR" not in symbol:
            base, suffix = symbol.split("-", 1)
            display_symbol = f"{base} PR{suffix}"
            if display_symbol in self.market_data:
                return self.market_data.get(display_symbol, {})
    ...
    def subscribe_alerts(self):
        ...
        try:
            ...
            self._send_command(cmd)
            print("[HAMMER] ✅ Alerts'e subscribe olundu")
            return True
        except Exception as e:
            print(f"[HAMMER] ❌ subscribe_alerts hatası: {e}")
            return False
    
    def enum_alerts(self, alert_type=None):
        ...
        try:
            ...
            if resp and resp.get('success') == 'OK':
                ...
                print(f"[HAMMER] ✅ {len(alerts)} alert bulundu")
                return alerts
            else:
                print(f"[HAMMER] ❌ enumAlerts hatası: {resp.get('result', 'Bilinmeyen hata')}")
                return []
        except Exception as e:
            print(f"[HAMMER] ❌ enum_alerts hatası: {e}")
            return []
    
    def add_alert(self, symbol, alert_type, main_price=None):
        """Yeni alert ekle
        
        alert_type: 'NewHigh', 'NewLow', 'NewHighLow', 'PriceUp', 'PriceDown', etc.
        """
        try:
            ...
            alert = {
                ...
                "NotifyColorValue": "#90EE90" if "High" in alert_type else "#FFC0CB"
            }
            
            if main_price is not None:
                alert["MainPrice"] = main_price
                alert["MainValue"] = main_price
            ...
            if resp and resp.get('success') == 'OK':
                alert_id = resp.get('result', {}).get('alertID', 'N/A')
                print(f"[HAMMER] ✅ Alert eklendi: {symbol} ({alert_type}) - ID: {alert_id}")
                return alert_id
            else:
                print(f"[HAMMER] ❌ addAlert hatası: {resp.get('result', 'Bilinmeyen hata')}")
                return None
        except Exception as e:
            print(f"[HAMMER] ❌ add_alert hatası: {e}")
            return None
    ...
    def delete_alert(self, alert_id):
        ...
        try:
            ...
            if resp and resp.get('success') == 'OK':
                print(f"[HAMMER] ✅ Alert silindi: {alert_id}")
                return True
            else:
                print(f"[HAMMER] ❌ deleteAlert hatası: {resp.get('result', 'Bilinmeyen hata')}")
                return False
        except Exception as e:
            print(f"[HAMMER] ❌ delete_alert hatası: {e}")
            return False
    
    def enable_disable_alert(self, alert_id, enabled=True):
        """Alert'i etkinleştir veya devre dışı bırak"""
        try:
            cmd = {
                "cmd": "enableDisableAlert",
                "alertID": alert_id,
                "enabled": enabled
            }
            ...
            if resp and resp.get('success') == 'OK':
                ...
                print(f"[HAMMER] ✅ Alert {status}: {alert_id}")
                return True
            else:
                print(f"[HAMMER] ❌ enableDisableAlert hatası: {resp.get('result', 'Bilinmeyen hata')}")
                return False
        except Exception as e:
            print(f"[HAMMER] ❌ enable_disable_alert hatası: {e}")
            return False
    
    def get_symbol_snapshot(self, symbol):
        """Sembol için anlık veri al (high, low, last, vb.)"""
        try:
            # Sembol formatını düzelt
            formatted_symbol = symbol
            if " PR" in symbol:
            ...
            print(f"[HAMMER] ❌ get_symbol_snapshot hatası: {e}")
            return None
    
    def enum_ports(self):
        """Tüm portfolio'ları listele"""
        try:
            cmd = {"cmd": "enumPorts"}
            resp = self._send_and_wait(cmd)
            ...
            if resp and resp.get('success') == 'OK':
                ...
                print(f"[HAMMER] ✅ {len(ports)} portfolio bulundu")
                return ports
            else:
                print(f"[HAMMER] ❌ enumPorts hatası: {resp.get('result', 'Bilinmeyen hata')}")
                return []
        except Exception as e:
            print(f"[HAMMER] ❌ enum_ports hatası: {e}")
            return []
    
    def get_port_symbols(self, port_id, detailed=False):
        """Portfolio'daki sembolleri al"""
        try:
            cmd = {
                "cmd": "enumPortSymbols",
                "portID": port_id,
            ...
            if resp and resp.get('success') == 'OK':
                ...
                print(f"[HAMMER] ❌ enumPortSymbols hatası: {resp.get('result', 'Bilinmeyen hata')}")
                return []
        except Exception as e:
            print(f"[HAMMER] ❌ get_port_symbols hatası: {e}")
            return []