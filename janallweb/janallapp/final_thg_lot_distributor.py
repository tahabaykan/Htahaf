...
❌ YANLIŞ: "janall/ssfinekheldff.csv"
=================================
"""
...
class FinalThgLotDistributor:
    ...
    def setup_ui(self):
        ...
        # V15TUMCSV butonu (%12 seçim, minimum 2, heldkuponlu 10 pair)
        v15_tumcsv_btn = ttk.Button(button_frame, text="V15TUMCSV", 
                                    command=lambda: self.apply_tumcsv_rules(selection_percent=0.12, min_selection=2, heldkuponlu_pair_count=10),
                                    style='Accent.TButton')
        v15_tumcsv_btn.pack(side='left', padx=5)
        
        # V20TUMCSV butonu (%15 seçim, minimum 3, heldkuponlu 12 pair)
        v20_tumcsv_btn = ttk.Button(button_frame, text="V20TUMCSV", 
                                    command=lambda: self.apply_tumcsv_rules(selection_percent=0.15, min_selection=3, heldkuponlu_pair_count=12),
                                    style='Accent.TButton')
        v20_tumcsv_btn.pack(side='left', padx=5)
        ...
        # BB Long Gort filtresi
        bb_gort_filter_frame = ttk.Frame(bb_long_frame)
        bb_gort_filter_frame.pack(fill='x', padx=5, pady=2)

        ttk.Label(bb_gort_filter_frame, text="Gort Filtresi:", font=("Arial", 9, "bold")).pack(side='left')
        self.bb_gort_filter_var = tk.StringVar()
        self.bb_gort_filter_entry = ttk.Entry(bb_gort_filter_frame, textvariable=self.bb_gort_filter_var, width=8)
        self.bb_gort_filter_entry.pack(side='left', padx=2)

        self.bb_gort_filter_type = tk.StringVar(value="below")
        ttk.Radiobutton(bb_gort_filter_frame, text="Below", variable=self.bb_gort_filter_type, value="below",
                        command=self.apply_bb_filter).pack(side='left', padx=2)
        ttk.Radiobutton(bb_gort_filter_frame, text="Above", variable=self.bb_gort_filter_type, value="above",
                        command=self.apply_bb_filter).pack(side='left', padx=2)
        ...
        bb_long_columns = ('Grup', 'Sembol', 'CGRUP', 'Final_BB_skor', 'Final_SFS_skor', 'FBtot', 'SMI', 'SMA63 chg', 'GORT', 'MAXALW', 'Mevcut Pozisyon', 'Hesaplanan Lot', 'Eklenebilir Lot')
        ...
        # FB Long Gort filtresi
        fb_gort_filter_frame = ttk.Frame(fb_long_frame)
        fb_gort_filter_frame.pack(fill='x', padx=5, pady=2)

        ttk.Label(fb_gort_filter_frame, text="Gort Filtresi:", font=("Arial", 9, "bold")).pack(side='left')
        self.fb_gort_filter_var = tk.StringVar()
        self.fb_gort_filter_entry = ttk.Entry(fb_gort_filter_frame, textvariable=self.fb_gort_filter_var, width=8)
        self.fb_gort_filter_entry.pack(side='left', padx=2)

        self.fb_gort_filter_type = tk.StringVar(value="below")
        ttk.Radiobutton(fb_gort_filter_frame, text="Below", variable=self.fb_gort_filter_type, value="below",
                        command=self.apply_fb_filter).pack(side='left', padx=2)
        ttk.Radiobutton(fb_gort_filter_frame, text="Above", variable=self.fb_gort_filter_type, value="above",
                        command=self.apply_fb_filter).pack(side='left', padx=2)
        ...
        fb_long_columns = ('Grup', 'Sembol', 'CGRUP', 'Final_FB_skor', 'Final_SFS_skor', 'FBtot', 'SMI', 'SMA63 chg', 'GORT', 'MAXALW', 'Mevcut Pozisyon', 'Hesaplanan Lot', 'Eklenebilir Lot')
        ...
        # SAS Short Gort filtresi
        sas_gort_filter_frame = ttk.Frame(sas_short_frame)
        sas_gort_filter_frame.pack(fill='x', padx=5, pady=2)

        ttk.Label(sas_gort_filter_frame, text="Gort Filtresi:", font=("Arial", 9, "bold")).pack(side='left')
        self.sas_gort_filter_var = tk.StringVar()
        self.sas_gort_filter_entry = ttk.Entry(sas_gort_filter_frame, textvariable=self.sas_gort_filter_var, width=8)
        self.sas_gort_filter_entry.pack(side='left', padx=2)

        self.sas_gort_filter_type = tk.StringVar(value="below")
        ttk.Radiobutton(sas_gort_filter_frame, text="Below", variable=self.sas_gort_filter_type, value="below",
                        command=self.apply_sas_filter).pack(side='left', padx=2)
        ttk.Radiobutton(sas_gort_filter_frame, text="Above", variable=self.sas_gort_filter_type, value="above",
                        command=self.apply_sas_filter).pack(side='left', padx=2)
        ...
        sas_short_columns = ('Grup', 'Sembol', 'CGRUP', 'Final_SAS_skor', 'Final_FB_skor', 'SFStot', 'SMI', 'SMA63 chg', 'GORT', 'MAXALW', 'Mevcut Pozisyon', 'Hesaplanan Lot', 'Eklenebilir Lot')
        ...
        # SFS Short Gort filtresi
        sfs_gort_filter_frame = ttk.Frame(sfs_short_frame)
        sfs_gort_filter_frame.pack(fill='x', padx=5, pady=2)

        ttk.Label(sfs_gort_filter_frame, text="Gort Filtresi:", font=("Arial", 9, "bold")).pack(side='left')
        self.sfs_gort_filter_var = tk.StringVar()
        self.sfs_gort_filter_entry = ttk.Entry(sfs_gort_filter_frame, textvariable=self.sfs_gort_filter_var, width=8)
        self.sfs_gort_filter_entry.pack(side='left', padx=2)

        self.sfs_gort_filter_type = tk.StringVar(value="below")
        ttk.Radiobutton(sfs_gort_filter_frame, text="Below", variable=self.sfs_gort_filter_type, value="below",
                        command=self.apply_sfs_filter).pack(side='left', padx=2)
        ttk.Radiobutton(sfs_gort_filter_frame, text="Above", variable=self.sfs_gort_filter_type, value="above",
                        command=self.apply_sfs_filter).pack(side='left', padx=2)
        ...
        sfs_short_columns = ('Grup', 'Sembol', 'CGRUP', 'Final_SFS_skor', 'Final_FB_skor', 'SFStot', 'SMI', 'SMA63 chg', 'GORT', 'MAXALW', 'Mevcut Pozisyon', 'Hesaplanan Lot', 'Eklenebilir Lot')
    ...
    def apply_tumcsv_rules(self, selection_percent=0.10, min_selection=2, heldkuponlu_pair_count=8):
        """
        YENİ SİSTEM: Mini450'den grup bazlı hisse seçimi
        Her gruptan belirli yüzde hisse seç, lot dağılımını alfa katsayısı ile yap
        
        Args:
            selection_percent: Her gruptan seçilecek hisse yüzdesi (varsayılan 0.10 = %10)
            min_selection: Minimum seçilecek hisse sayısı (varsayılan 2)
            heldkuponlu_pair_count: HELDKUPONLU grubu için seçilecek pair sayısı (varsayılan 8)
        """
        try:
            version_name = f"V{int(selection_percent * 100)}TUMCSV" if selection_percent != 0.10 else "TUMCSV"
            print(f"🚀 {version_name} AYARLAMASI BAŞLIYOR...")
            print("=" * 80)
            print("📊 Mini450 bazlı grup seçim sistemi")
            print(f"📈 Her gruptan %{int(selection_percent * 100)} hisse seçilecek (minimum {min_selection}, en yüksek FINAL BB skorlu)")
            if heldkuponlu_pair_count != 8:
                print(f"🎯 HELDKUPONLU için {heldkuponlu_pair_count} pair ({heldkuponlu_pair_count * 2} hisse) seçilecek")
            ...
            for group in long_groups_only:
                ...
                # HELDKUPONLU grubu için özel CGRUP bazlı seçim (sadece 'heldkuponlu', 'heldkuponlukreorta' ve 'heldkuponlukreciliz' ayrı değerlendirilir)
                if group.lower() == 'heldkuponlu':
                    print(f"   🎯 HELDKUPONLU grubu için özel CGRUP bazlı seçim uygulanıyor...")
                    print(f"   📊 Her sekme için {heldkuponlu_pair_count} adet HELDKUPONLU seçilecek:")
                    print(f"      🔵 BB Long: {heldkuponlu_pair_count} adet")
                    print(f"      🟢 FB Long: {heldkuponlu_pair_count} adet (BB ile aynı olabilir)")
                    print(f"      🟠 SAS Short: {heldkuponlu_pair_count} adet (LONG tarafındaki hisseler hariç)")
                    print(f"      🔴 SFS Short: {heldkuponlu_pair_count} adet (LONG tarafındaki hisseler hariç, SAS ile aynı olabilir)")
                    print(f"   📋 Kurallar: C600, C625, C650 serbest (0 da olabilir), diğer CGRUP'lardan en az 1'er")
                    
                    # BB Long için HELDKUPONLU seçimi (10/12 adet)
                    bb_long_stocks = self.select_heldkuponlu_by_score_type(
                        group_stocks, 'Final_BB_skor', 'LONG', heldkuponlu_pair_count, excluded_symbols=set()
                    )
                    
                    # FB Long için HELDKUPONLU seçimi (10/12 adet, BB ile aynı hisseler olabilir)
                    fb_long_stocks = self.select_heldkuponlu_by_score_type(
                        group_stocks, 'Final_FB_skor', 'LONG', heldkuponlu_pair_count, excluded_symbols=set()
                    )
                    
                    # SAS Short için HELDKUPONLU seçimi (10/12 adet, LONG tarafındaki hisseler hariç)
                    long_selected_symbols = {s.get('PREF IBKR', '') for s in bb_long_stocks + fb_long_stocks}
                    sas_short_stocks = self.select_heldkuponlu_by_score_type(
                        group_stocks, 'Final_SAS_skor', 'SHORT', heldkuponlu_pair_count, excluded_symbols=long_selected_symbols
                    )
                    
                    # SFS Short için HELDKUPONLU seçimi (10/12 adet, LONG tarafındaki hisseler hariç, SAS ile aynı olabilir)
                    sfs_short_stocks = self.select_heldkuponlu_by_score_type(
                        group_stocks, 'Final_SFS_skor', 'SHORT', heldkuponlu_pair_count, excluded_symbols=long_selected_symbols
                    )
                    
                    # LONG hisseleri listeye ekle
                    for stock in bb_long_stocks:
                        stock['GRUP'] = group
                        stock['SKOR_TURU'] = 'BB_LONG'
                        all_long_stocks.append(stock)
                    
                    for stock in fb_long_stocks:
                        stock['GRUP'] = group
                        stock['SKOR_TURU'] = 'FB_LONG'
                        all_long_stocks.append(stock)
                    
                    # SHORT hisseleri listeye ekle (heldkuponlu LONG grubunda ama SHORT hisseleri de var)
                    for stock in sas_short_stocks:
                        stock['GRUP'] = group
                        stock['SKOR_TURU'] = 'SAS_SHORT'
                        all_short_stocks.append(stock)
                    
                    for stock in sfs_short_stocks:
                        stock['GRUP'] = group
                        stock['SKOR_TURU'] = 'SFS_SHORT'
                        all_short_stocks.append(stock)
                    
                    print(f"   📊 HELDKUPONLU Özel Seçim Sonuçları:")
                    ...
                    # Diğer gruplar için normal seçim
                    # Her skor türü için belirli yüzde hisse seç
                    selected_count = max(min_selection, round(len(group_stocks) * selection_percent))
                    print(f"   🎯 %{int(selection_percent * 100)} kural: {len(group_stocks)} hisse → {selected_count} hisse seçilecek (minimum {min_selection})")
            ...
            for group in short_groups_only:
                ...
                # HELDKUPONLU grubu için özel işlem yapılmaz (zaten LONG kısmında işlendi)
                if group.lower() == 'heldkuponlu':
                    print(f"   ⚠️ HELDKUPONLU grubu SHORT kısmında işlenmiyor (LONG kısmında zaten işlendi)")
                    continue
            ...
            if all_long_stocks or all_short_stocks:
                version_name = f"V{int(selection_percent * 100)}TUMCSV" if selection_percent != 0.10 else "TUMCSV"
                print(f"\n📊 {version_name} AYARLAMASI TAMAMLANDI!")
                ...
                version_name = f"V{int(selection_percent * 100)}TUMCSV" if selection_percent != 0.10 else "TUMCSV"
                if not (hasattr(self.main_window, 'runall_allowed_mode') and self.main_window.runall_allowed_mode):
                    messagebox.showinfo("Başarılı", f"{version_name} ayarlaması tamamlandı!\nLONG: {len(all_long_stocks)} hisse\nSHORT: {len(all_short_stocks)} hisse")
                else:
                    version_name = f"V{int(selection_percent * 100)}TUMCSV" if selection_percent != 0.10 else "TUMCSV"
                    print(f"[{version_name}] ℹ️ Allowed modu aktif - '{version_name} ayarlaması tamamlandı' penceresi gösterilmedi")
            ...
            version_name = f"V{int(selection_percent * 100)}TUMCSV" if selection_percent != 0.10 else "TUMCSV"
            print(f"❌ {version_name} ayarlaması hatası: {e}")
            # RUNALL Allowed modunda hata mesajını gösterme
            if not (hasattr(self.main_window, 'runall_allowed_mode') and self.main_window.runall_allowed_mode):
                messagebox.showerror("Hata", f"{version_name} ayarlaması hatası: {e}")
            else:
    ...
    def get_befday_qty_for_symbol(self, symbol):
        """Hisse için gün başı pozisyonu al"""
        try:
            if self.main_window and hasattr(self.main_window, 'load_bef_position'):
                return self.main_window.load_bef_position(symbol)
            return 0
        except Exception as e:
            print(f"[JFIN] ⚠️ {symbol} BefDay Qty alınamadı: {e}")
            return 0
    
    def calculate_addable_lot_jfin(self, maxalw, current_position, calculated_lot=0, symbol=None):
        """Eklenebilir lot hesapla - Long için (JFIN için) - YENİ KURALLARLA"""
        try:
            maxalw_num = float(maxalw) if maxalw != 'N/A' else 0
            current_num = float(current_position) if current_position != 'N/A' else 0
            
            # Eski limit: MAXALW - Mevcut Pozisyon
            old_limit = max(0, maxalw_num - current_num)
            
            # YENİ KURALLAR: ADDNEWPOS limitleri
            new_limit = old_limit
            if self.main_window and hasattr(self.main_window, 'get_addnewpos_limit') and symbol:
                try:
                    befday_qty = self.get_befday_qty_for_symbol(symbol)
                    daily_limit, reason = self.main_window.get_addnewpos_limit(symbol, maxalw_num, befday_qty)
                    
                    # Mevcut günlük değişim
                    current_change = abs(current_num - befday_qty)
                    remaining = daily_limit - current_change
                    
                    if remaining > 0:
                        new_limit = min(old_limit, remaining)
                        print(f"[JFIN] 📊 {symbol} ADDNEWPOS Limit: Eski={old_limit:.0f}, GünlükLimit={daily_limit:.0f}, Değişim={current_change:.0f}, Kalan={remaining:.0f}, Yeni={new_limit:.0f}")
                    else:
                        new_limit = 0
                        print(f"[JFIN] ⚠️ {symbol} ADDNEWPOS Limit doldu: {current_change:.0f} >= {daily_limit:.0f}")
                except Exception as e:
                    print(f"[JFIN] ⚠️ {symbol} ADDNEWPOS limit hesaplama hatası: {e}")
            
            return max(0, new_limit)
        except Exception as e:
            print(f"[JFIN] ❌ Long eklenebilir lot hesaplama hatası: {e}")
            return 0

    def calculate_addable_lot_short_jfin(self, maxalw, current_position, calculated_lot=0, symbol=None):
        """Eklenebilir lot hesapla - Short için (mutlak değerler) (JFIN için) - YENİ KURALLARLA"""
        try:
            maxalw_num = float(maxalw) if maxalw != 'N/A' else 0
            current_num = abs(float(current_position)) if current_position != 'N/A' else 0
            ...
            if self.main_window and hasattr(self.main_window, 'get_addnewpos_limit') and symbol:
                try:
                    ...
                    # Mevcut günlük değişim (short için)
                    current_change = abs(current_num - abs(befday_qty))
                    remaining = daily_limit - current_change
                    
                    if remaining > 0:
                        new_limit = min(old_limit, remaining)
                        print(f"[JFIN] 📊 {symbol} SHORT ADDNEWPOS Limit: Eski={old_limit:.0f}, GünlükLimit={daily_limit:.0f}, Değişim={current_change:.0f}, Kalan={remaining:.0f}, Yeni={new_limit:.0f}")
                    else:
                        new_limit = 0
                        print(f"[JFIN] ⚠️ {symbol} SHORT ADDNEWPOS Limit doldu: {current_change:.0f} >= {daily_limit:.0f}")
                except Exception as e:
                    print(f"[JFIN] ⚠️ {symbol} SHORT ADDNEWPOS limit hesaplama hatası: {e}")
    ...
    def calculate_fbtot(self, fbplagr, fbratgr):
        """FBtot hesapla - Yeni formül: FBPlagr * 0.5 + FBRatgr * 1.5"""
        try:
            if fbplagr != "N/A" and fbratgr != "N/A":
                plagr_num = float(fbplagr)
                ratgr_num = float(fbratgr)
                fbtot = (plagr_num * 0.5) + (ratgr_num * 1.5)  # Yeni formül
                return f"{fbtot:.4f}"
            return "N/A"
        except Exception as e:
    ...
    def calculate_sfstot(self, sfsplagr, sfsratgr):
        """SFStot hesapla - Yeni formül: SFSPlagr * 0.5 + SFSRatgr * 1.5"""
        try:
            if sfsplagr != "N/A" and sfsratgr != "N/A":
                plagr_num = float(sfsplagr)
                ratgr_num = float(sfsratgr)
                sfstot = (plagr_num * 0.5) + (ratgr_num * 1.5)  # Yeni formül
                return f"{sfstot:.4f}"
            return "N/A"
        except Exception as e:
    ...
    def calculate_fbtot_for_symbol(self, symbol, group):
        """Hisse için FBtot değerini hesapla - Mini450'deki değeri kullan (tutarlılık için)"""
        try:
            # Önce mini450'deki değeri kontrol et (varsa direkt kullan)
            if hasattr(self.main_window, 'df') and self.main_window.df is not None:
                if 'Fbtot' in self.main_window.df.columns:
                    symbol_row = self.main_window.df[self.main_window.df['PREF IBKR'] == symbol]
                    if not symbol_row.empty:
                        fbtot_value = symbol_row['Fbtot'].iloc[0]
                        if pd.notna(fbtot_value) and fbtot_value != 'N/A' and fbtot_value != '':
                            return str(fbtot_value)
            
            # Mini450'de yoksa eski yöntemle hesapla (fallback)
            grup_value = group
            avg_final_fb = 0
            fbplagr = "N/A"
            fbratgr = "N/A"
    ...
    def calculate_sfstot_for_symbol(self, symbol, group):
        """Hisse için SFStot değerini hesapla - Mini450'deki değeri kullan (tutarlılık için)"""
        try:
            # Önce mini450'deki değeri kontrol et (varsa direkt kullan)
            if hasattr(self.main_window, 'df') and self.main_window.df is not None:
                if 'SFStot' in self.main_window.df.columns:
                    symbol_row = self.main_window.df[self.main_window.df['PREF IBKR'] == symbol]
                    if not symbol_row.empty:
                        sfstot_value = symbol_row['SFStot'].iloc[0]
                        if pd.notna(sfstot_value) and sfstot_value != 'N/A' and sfstot_value != '':
                            return str(sfstot_value)
    ...
    def show_jfin_orders(self, order_type, percentage):
        ...
        try:
            ...
            for item in visible_items:
                ...
                if len(values) >= 12:  # En az 12 kolon olmalı (CGRUP eklendi, Mevcut Pozisyon var)
                    try:
                        grup = values[0]
                        symbol = values[1]
                        
                        # CGRUP bilgisini Mini450'den al
                        cgrup = 'N/A'
                        if hasattr(self.main_window, 'df') and self.main_window.df is not None:
                            symbol_row = self.main_window.df[self.main_window.df['PREF IBKR'] == symbol]
                            if not symbol_row.empty and 'CGRUP' in self.main_window.df.columns:
                                cgrup_val = symbol_row['CGRUP'].iloc[0]
                                cgrup = str(cgrup_val) if pd.notna(cgrup_val) else 'N/A'

                        # Final score güvenli dönüştürme (CGRUP eklendi, index 2->3)
                        try:
                            final_score = float(values[3]) if values[3] not in ['N/A', '', None] else 0
                        except (ValueError, TypeError):
                            final_score = 0

                        # SMI değerini al (CGRUP eklendi, index 5->6)
                        try:
                            smi = values[6] if values[6] not in ['N/A', '', None] else 'N/A'
                        except (IndexError, ValueError, TypeError):
                            smi = 'N/A'
                        
                        # SMA63 chg değerini al (CGRUP eklendi, index 6->7)
                        try:
                            sma63_chg = values[7] if values[7] not in ['N/A', '', None] else 'N/A'
                        except (IndexError, ValueError, TypeError):
                            sma63_chg = 'N/A'
                        
                        # GORT değerini al (CGRUP eklendi, index 8)
                        try:
                            gort = values[8] if len(values) > 8 and values[8] not in ['N/A', '', None] else 'N/A'
                        except (IndexError, ValueError, TypeError):
                            gort = 'N/A'
                        
                        # FBtot/SFStot değerini al (CGRUP eklendi, index 5)
                        try:
                            fbtot_sfstot = values[5] if len(values) > 5 and values[5] not in ['N/A', '', None] else 'N/A'
                        except (IndexError, ValueError, TypeError):
                            fbtot_sfstot = 'N/A'
                        
                        # MAXALW değerini al (CGRUP eklendi, index 8->9)
                        try:
                            maxalw_str = values[9] if len(values) > 9 and values[9] not in ['N/A', '', None] else '0'
                        ...
                        # Mevcut Pozisyon değerini al (CGRUP eklendi, index 9->10)
                        try:
                            current_pos_str = values[10] if len(values) > 10 and values[10] not in ['N/A', '', None] else '0'
                        ...
                        # Hesaplanan Lot değerini al (CGRUP eklendi, index 10->11)
                        try:
                            calculated_lot_str = values[11] if len(values) > 11 and values[11] not in ['N/A', '', None] else '0'
                        ...
                        # Eklenebilir lot hesapla (symbol parametresi ile yeni kurallar uygulanır)
                        eklenebilir_lot = self.calculate_addable_lot_jfin(maxalw, current_position, calculated_lot, symbol)
                        ...
                        if final_lot >= 100:
                            orders.append({
                                'group': grup,
                                'symbol': symbol,
                                'cgrup': cgrup,  # CGRUP bilgisi eklendi
                                'score': final_score,
                                'smi': smi,
                                'sma63_chg': sma63_chg,
                                'gort': gort,  # GORT bilgisi eklendi
                                'fbtot_sfstot': fbtot_sfstot,  # FBtot/SFStot bilgisi eklendi
            ...
            if max_addable_lot is not None and max_addable_lot > 0:
                ...
                # Exposure percentage ayarını uygula (default: %60)
                exposure_percentage = 60  # Varsayılan
                if hasattr(self.main_window, 'addnewpos_exposure_percentage'):
                    exposure_percentage = self.main_window.addnewpos_exposure_percentage
                
                # Exposure percentage'i 0-100 arası kontrol et
                if not (0 <= exposure_percentage <= 100):
                    exposure_percentage = 60
                
                # Adjusted max_addable_lot hesapla (exposure percentage uygulanmış)
                adjusted_max_addable_lot = max_addable_lot * (exposure_percentage / 100.0)
                
                if current_total_lot != adjusted_max_addable_lot:
                    # Lotları orantılı olarak güncelle (weighted)
                    # Eğer current_total_lot < adjusted_max_addable_lot ise artır, > ise azalt
                    ratio = adjusted_max_addable_lot / current_total_lot
                    print(f"[ADDNEWPOS] 🔄 Exposure limitine göre lot güncelleme: {current_total_lot:,} → {adjusted_max_addable_lot:,.0f} (Oran: {ratio:.3f}, Exposure %: {exposure_percentage:.0f}%)")
                    print(f"[ADDNEWPOS] 📊 Orijinal max_addable_lot: {max_addable_lot:,}, Adjusted: {adjusted_max_addable_lot:,.0f} (%{exposure_percentage:.0f})")
                    ...
                    self.main_window.log_message(f"🔄 ADDNEWPOS: Exposure limitine göre lotlar güncellendi: {current_total_lot:,} → {final_total:,} lot (Hedef: {adjusted_max_addable_lot:,.0f}, Exposure %: {exposure_percentage:.0f}%)")
    ...
    def get_order_condition_reason(self, symbol, order_type, group, gort, fbtot_sfstot, sma63_chg, 
                                   lot=None, eklenebilir_lot=None, maxalw=None, befday_qty=None, 
                                   current_qty=None, open_orders_qty=None, max_change_limit=None, 
                                   potential_daily_change=None, lot_reduced=False, original_lot=None):
        """Emir için koşul kontrollerini topla ve formatlanmış sebep string'i döndür"""
        try:
            conditions = []
                ...
                if market_data:
                    try:
                        ...
                        if bid > 0 and ask > 0:
                            ...
                            if spread > 0.07:
                                conditions.append(f"Spread {spread:.4f} (> 0.07)")
                            else:
                                conditions.append(f"Spread {spread:.4f} (≤ 0.07)")
                    except (ValueError, TypeError):
                        pass
            ...
            # Gort kontrolü
            if gort and gort != 'N/A' and gort != '':
                try:
                    gort_num = float(gort)
                    # Filtre değerlerini al
                    if order_type in ['BB', 'FB', 'SoftFB']:
                        gort_filter_value = self.bb_gort_filter_var.get().strip() if order_type == 'BB' else self.fb_gort_filter_var.get().strip()
                        gort_filter_type = self.bb_gort_filter_type.get() if order_type == 'BB' else self.fb_gort_filter_type.get()
                    else:  # SAS, SFS, SoftFS
                        gort_filter_value = self.sas_gort_filter_var.get().strip() if order_type == 'SAS' else self.sfs_gort_filter_var.get().strip()
                        gort_filter_type = self.sas_gort_filter_type.get() if order_type == 'SAS' else self.sfs_gort_filter_type.get()
                    
                    if gort_filter_value and gort_filter_value.strip():
                        exempted_groups = self.get_exempted_groups()
                        if group.lower() not in exempted_groups:
                            filter_num = float(gort_filter_value)
                            if gort_filter_type == 'above':
                                if gort_num >= filter_num:
                                    conditions.append(f"Gort {gort_num:.2f} (≥ {filter_num})")
                                else:
                                    conditions.append(f"Gort {gort_num:.2f} (< {filter_num})")
                            else:  # below
                                if gort_num <= filter_num:
                            ...
                            conditions.append(f"Gort {gort_num:.2f} (exempted)")
                    else:
                        conditions.append(f"Gort {gort_num:.2f}")
                except (ValueError, TypeError):
                    conditions.append(f"Gort {gort}")
            
            # FBtot/SFStot kontrolü
            if fbtot_sfstot and fbtot_sfstot != 'N/A' and fbtot_sfstot != '':
                try:
                    tot_num = float(fbtot_sfstot)
                    # Filtre değerlerini al
                    if order_type in ['BB', 'FB', 'SoftFB']:
                        fbtot_filter_value = self.bb_fbtot_filter_var.get().strip() if order_type == 'BB' else self.fb_fbtot_filter_var.get().strip()
                        fbtot_filter_type = self.bb_fbtot_filter_type.get() if order_type == 'BB' else self.fb_fbtot_filter_type.get()
                        tot_name = "FBtot"
                    else:  # SAS, SFS, SoftFS
                        sfstot_filter_value = self.sas_sfstot_filter_var.get().strip() if order_type == 'SAS' else self.sfs_sfstot_filter_var.get().strip()
                        sfstot_filter_type = self.sas_sfstot_filter_type.get() if order_type == 'SAS' else self.sfs_sfstot_filter_type.get()
                        tot_name = "SFStot"
                        fbtot_filter_value = sfstot_filter_value
                        fbtot_filter_type = sfstot_filter_type
                    
                    if fbtot_filter_value and fbtot_filter_value.strip():
                        exempted_groups = self.get_exempted_groups()
                        if group.lower() not in exempted_groups:
                            filter_num = float(fbtot_filter_value)
                            if fbtot_filter_type == 'above':
                                if tot_num >= filter_num:
                                    conditions.append(f"{tot_name} {tot_num:.2f} (≥ {filter_num})")
                                else:
                                    conditions.append(f"{tot_name} {tot_num:.2f} (< {filter_num})")
                            else:  # below
                                if tot_num <= filter_num:
                            ...
                            conditions.append(f"{tot_name} {tot_num:.2f} (exempted)")
                    else:
                        conditions.append(f"{tot_name} {tot_num:.2f}")
                except (ValueError, TypeError):
                    conditions.append(f"{'FBtot' if order_type in ['BB', 'FB', 'SoftFB'] else 'SFStot'} {fbtot_sfstot}")
            
            # SMA63 chg kontrolü
            if sma63_chg and sma63_chg != 'N/A' and sma63_chg != '':
                try:
                    sma_num = float(sma63_chg)
                    # Filtre değerlerini al
                    if order_type in ['BB', 'FB', 'SoftFB']:
                        sma_filter_value = self.bb_sma_filter_var.get().strip() if order_type == 'BB' else self.fb_sma_filter_var.get().strip()
                        sma_filter_type = self.bb_filter_type.get() if order_type == 'BB' else self.fb_filter_type.get()
                    else:  # SAS, SFS, SoftFS
                        sma_filter_value = self.sas_sma_filter_var.get().strip() if order_type == 'SAS' else self.sfs_sma_filter_var.get().strip()
                        sma_filter_type = self.sas_filter_type.get() if order_type == 'SAS' else self.sfs_filter_type.get()
                    
                    if sma_filter_value and sma_filter_value.strip():
                        exempted_groups = self.get_exempted_groups()
                        if group.lower() not in exempted_groups:
                            filter_num = float(sma_filter_value)
                            if sma_filter_type == 'above':
                                if sma_num >= filter_num:
                                    conditions.append(f"SMA63 chg {sma_num:.2f} (≥ {filter_num})")
                                else:
                                    conditions.append(f"SMA63 chg {sma_num:.2f} (< {filter_num})")
                            else:  # below
                                if sma_num <= filter_num:
                            ...
                            conditions.append(f"SMA63 chg {sma_num:.2f} (exempted)")
                    else:
                        conditions.append(f"SMA63 chg {sma_num:.2f}")
                except (ValueError, TypeError):
                    conditions.append(f"SMA63 chg {sma63_chg}")
            
            # Lot koşulları
            if lot is not None:
                conditions.append(f"Lot: {lot:,.0f}")
                if eklenebilir_lot is not None:
                    if lot <= eklenebilir_lot:
                        conditions.append(f"Eklenebilir: {eklenebilir_lot:,.0f} (OK)")
                    else:
                        conditions.append(f"Eklenebilir: {eklenebilir_lot:,.0f} (AŞILDI)")
                if lot_reduced and original_lot is not None:
                    conditions.append(f"Lot Azaltıldı: {original_lot:,.0f} → {lot:,.0f}")
            
            # Exposure koşulları
            if maxalw is not None and maxalw > 0:
                conditions.append(f"MAXALW: {maxalw:,.0f}")
                if max_change_limit is not None:
                    conditions.append(f"Max Change Limit: {max_change_limit:,.0f} (MAXALW*3/4)")
                    if potential_daily_change is not None:
                        if potential_daily_change <= max_change_limit:
                            conditions.append(f"Potansiyel Günlük Değişim: {potential_daily_change:,.0f} (≤ Limit)")
                        else:
                            conditions.append(f"Potansiyel Günlük Değişim: {potential_daily_change:,.0f} (> Limit)")
            
            if befday_qty is not None and current_qty is not None:
                daily_change = abs(current_qty - befday_qty)
                conditions.append(f"Günlük Değişim: {daily_change:,.0f} (BefDay: {befday_qty:,.0f}, Mevcut: {current_qty:,.0f})")
                if open_orders_qty is not None and open_orders_qty > 0:
                    conditions.append(f"Açık Emirler: {open_orders_qty:,.0f}")
            
            # Minimum lot kontrolü
            if lot is not None:
                if lot >= 100:
                    conditions.append(f"Min Lot (≥100): OK")
                else:
                    conditions.append(f"Min Lot (≥100): BAŞARISIZ ({lot:,.0f})")
            
            # Koşulları birleştir
            if conditions:
                return ", ".join(conditions)
            else:
                return "Koşul bilgisi yok"
                
        except Exception as e:
            print(f"[ORDER CONDITION] ❌ {symbol} koşul toplama hatası: {e}")
            return f"Hata: {str(e)}"
    ...
    def show_jfin_order_confirmation(self, order_type, percentage, orders):
        ...
        try:
            ...
            # JFIN penceresini main_window'a bildir (ADDNEWPOS için) - HER ZAMAN SET ET
            self.main_window.addnewpos_jfin_window = win
            print(f"[JFIN] ✅ JFIN penceresi kaydedildi: {win.title()}")
            ...
            # Emirler tablosu - SMA63 chg kolonu eklendi + Mevcut Lot kolonu + CGRUP kolonu
            columns = ('Seç', 'Grup', 'Sembol', 'CGRUP', 'Skor', 'SMI', 'SMA63 chg', 'Emir Fiyatı', 'Mevcut Lot', 'Eklenebilir Lot', 'Hesaplanan Lot', 'Emir Türü')
            ...
            for col in columns:
                ...
                if col == 'Seç':
                    ...
                    order_tree.column(col, width=80)  # CGRUP kolonu genişliği
                elif col == 'SMI':
            ...
            # Order dict'lerini sakla (symbol -> order dict mapping)
            order_dict_map = {}
            
            # Emirleri doldur
            for i, order in enumerate(orders):
                symbol = order['symbol']
                # Order dict'ini sakla
                order_dict_map[symbol] = order
                ...
                values = (
                    ...
                    order['group'],
                    order['symbol'],
                    order.get('cgrup', 'N/A'),  # CGRUP kolonu - Sembol'den hemen sonra
                    score_text,
            ...
            def send_orders():
                ...
                for item in order_tree.get_children():
                    ...
                    if values[0] == '☑':  # Seçili ise (checkbox dolu)
                        symbol = values[2]  # CGRUP eklendi, index değişmedi (hala 2)
                        ...
                        # Order dict'inden bilgileri al
                        order_info = order_dict_map.get(symbol, {})
                        
                        selected_orders.append({
                            'symbol': symbol,
                            'price': values[7],  # Emir fiyatı (CGRUP eklendi, 6->7)
                            'lot': int(str(values[10]).replace(',', '')),  # Hesaplanan lot (CGRUP eklendi, 9->10)
                            'order_type': values[11],  # Emir türü (CGRUP eklendi, 10->11)
                            'group': order_info.get('group', 'N/A'),
                            'gort': order_info.get('gort', 'N/A'),
                            'fbtot_sfstot': order_info.get('fbtot_sfstot', 'N/A'),
                            'sma63_chg': order_info.get('sma63_chg', 'N/A')
                        })
                ...
                if selected_orders:
                    ...
                    for order in selected_orders:
                        try:
                            ...
                            if price_str in ['N/A', 'KOŞUL YOK', '', 'None', 'nan']:
                                ...
                                if hasattr(self.main_window, 'log_psfalgo_activity'):
                                    # Koşul bilgilerini topla
                                    condition_reason = self.get_order_condition_reason(
                                        symbol, order_type, order.get('group', 'N/A'),
                                        order.get('gort', 'N/A'), order.get('fbtot_sfstot', 'N/A'),
                                        order.get('sma63_chg', 'N/A'), lot=lot
                                    )
                                    reason = f"Fiyat N/A ({price_str}), {condition_reason}" if condition_reason else f"Fiyat N/A ({price_str})"
                                    self.main_window.log_psfalgo_activity("EMIR_BLOKE", f"{symbol} Fiyat N/A ({price_str})", "BLOCKED", reason, "ADDNEWPOS")
                                error_count += 1
                                continue
                            ...
                            try:
                                ...
                                if price <= 0:
                                    ...
                                    if hasattr(self.main_window, 'log_psfalgo_activity'):
                                        ...
                                        reason = f"Geçersiz Fiyat ({price}), {condition_reason}" if condition_reason else f"Geçersiz Fiyat ({price})"
                                        self.main_window.log_psfalgo_activity("EMIR_BLOKE", f"{symbol} Geçersiz Fiyat ({price})", "BLOCKED", reason, "ADDNEWPOS")
                                    error_count += 1
                                    continue
                            except (ValueError, TypeError):
                                ...
                                if hasattr(self.main_window, 'log_psfalgo_activity'):
                                    ...
                                    reason = f"Fiyat Parse Hatası, {condition_reason}" if condition_reason else "Fiyat Parse Hatası"
                                    self.main_window.log_psfalgo_activity("EMIR_BLOKE", f"{symbol} Fiyat Parse Hatası", "BLOCKED", reason, "ADDNEWPOS")
                                error_count += 1
                                continue
                            
                            # MAXALW*3/4 limit kontrolü - Lot'u gerekirse azalt
                            maxalw = None
                            befday_qty = None
                            current_qty = None
                            open_orders_qty = None
                            max_change_limit = None
                            potential_daily_change = None
                            lot_reduced = False
                            original_lot = None
                            eklenebilir_lot = None
                                ...
                                try:
                                    ...
                                    if maxalw > 0:
                                        ...
                                        if hasattr(self.main_window, 'load_bef_position'):
                                            befday_qty = self.main_window.load_bef_position(symbol) or 0
                                        ...
                                        if hasattr(self.main_window, 'get_open_orders_sum'):
                                            open_orders_qty = self.main_window.get_open_orders_sum(symbol, use_cache=True) or 0
                                        
                                        current_potential = current_qty + open_orders_qty
                                        
                                        # Eklenebilir lot bilgisini al
                                        order_info = order_dict_map.get(symbol, {})
                                        eklenebilir_lot = order_info.get('eklenebilir_lot', None)
                                        ...
                                        # MAXALW*3/4 limitini aşacaksa lot'u azalt
                                        if potential_daily_change > max_change_limit:
                                            # İzin verilen maksimum potansiyel pozisyonu hesapla
                                            if new_potential > befday_qty:
                                                # Pozisyon artıyor (BUY)
                                                max_allowed_potential = befday_qty + max_change_limit
                                                max_allowed_lot = max_allowed_potential - current_potential
                                            else:
                                                # Pozisyon azalıyor (SELL)
                                                max_allowed_potential = befday_qty - max_change_limit
                                                max_allowed_lot = current_potential - max_allowed_potential
                                            
                                            # Negatif lot kontrolü
                                            if max_allowed_lot <= 0:
                                                print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacak ({potential_daily_change:.0f} > {max_change_limit:.0f}), emir atlanıyor (izin verilen lot: {max_allowed_lot:.0f})")
                                                if hasattr(self.main_window, 'log_psfalgo_activity'):
                                                    # Koşul bilgilerini topla
                                                    condition_reason = self.get_order_condition_reason(
                                                        symbol, order_type, order.get('group', 'N/A'),
                                                        order.get('gort', 'N/A'), order.get('fbtot_sfstot', 'N/A'),
                                                        order.get('sma63_chg', 'N/A'), lot=lot, eklenebilir_lot=eklenebilir_lot,
                                                        maxalw=maxalw, befday_qty=befday_qty, current_qty=current_qty,
                                                        open_orders_qty=open_orders_qty, max_change_limit=max_change_limit,
                                                        potential_daily_change=potential_daily_change
                                                    )
                                                    reason = f"MAXALW*3/4 Limit Aşıldı (PotChg:{potential_daily_change:.0f} > Limit:{max_change_limit:.0f}, AllowedLot:{max_allowed_lot:.0f}), {condition_reason}"
                                                    details_msg = f"{symbol} MAXALW*3/4 Limit | PotChg:{potential_daily_change:.0f} > Limit:{max_change_limit:.0f} AllowedLot:{max_allowed_lot:.0f}"
                                                    self.main_window.log_psfalgo_activity("EMIR_BLOKE", details_msg, "BLOCKED", reason, "ADDNEWPOS")
                                                error_count += 1
                                                continue
                                            
                                            # Lot'u 100'ün katına yuvarla (aşağı yuvarla)
                                            max_allowed_lot = (int(max_allowed_lot) // 100) * 100
                                            
                                            # Minimum 100 lot kontrolü
                                            if max_allowed_lot < 100:
                                                print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacak ({potential_daily_change:.0f} > {max_change_limit:.0f}), emir atlanıyor (izin verilen lot çok küçük: {max_allowed_lot})")
                                                if hasattr(self.main_window, 'log_psfalgo_activity'):
                                                    # Koşul bilgilerini topla
                                                    condition_reason = self.get_order_condition_reason(
                                                        symbol, order_type, order.get('group', 'N/A'),
                                                    ...
                                                    reason = f"MAXALW*3/4 Limit Aşıldı (Small Lot: {max_allowed_lot} < 100), {condition_reason}"
                                                    details_msg = f"{symbol} Small Lot Block | PotChg:{potential_daily_change:.0f} > Limit:{max_change_limit:.0f} CalcLot:{max_allowed_lot}"
                                                    self.main_window.log_psfalgo_activity("EMIR_BLOKE", details_msg, "BLOCKED", reason, "ADDNEWPOS")
                                                error_count += 1
                                                continue
                                            ...
                                            lot = max_allowed_lot
                                            lot_reduced = True
                                            print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacaktı ({potential_daily_change:.0f} > {max_change_limit:.0f}), lot azaltıldı: {original_lot:,} → {lot:,} lot")
                                except Exception as e:
                                    print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 kontrolü hatası: {e}, orijinal lot ile devam ediliyor")
                                        ...
                                        if success:
                                            ...
                                            if hasattr(self.main_window, 'log_psfalgo_activity'):
                                                ...
                                                condition_reason = self.get_order_condition_reason(
                                                    symbol, order_type, order.get('group', 'N/A'),
                                                    order.get('gort', 'N/A'), order.get('fbtot_sfstot', 'N/A'),
                                                    order.get('sma63_chg', 'N/A'), lot=part_lot, eklenebilir_lot=eklenebilir_lot,
                                                    maxalw=maxalw, befday_qty=befday_qty, current_qty=current_qty,
                                                    open_orders_qty=open_orders_qty, max_change_limit=max_change_limit,
                                                    potential_daily_change=potential_daily_change, lot_reduced=lot_reduced,
                                                    original_lot=original_lot
                                                )
                                                details_msg = f"{symbol} {side} {part_lot} @ {price:.2f} (Part {i}) | Type:{order_type}"
                                                self.main_window.log_psfalgo_activity("EMIR_ILETILDI", details_msg, "SUCCESS", condition_reason, "ADDNEWPOS")
                                        else:
                                            error_count += 1
                                            print(f"[JFIN] ❌ {symbol}: {part_lot} lot {order_type} @ ${price:.2f} - Parça {i}/{len(lot_parts)} - Başarısız")
                                            if hasattr(self.main_window, 'log_psfalgo_activity'):
                                                self.main_window.log_psfalgo_activity("EMIR_HATASI", f"{symbol} {part_lot} @ {price:.2f} (Parça {i})", "ERROR", "Failed to place order", "ADDNEWPOS")
                                    else:
                                        error_count += 1
                                        print(f"[JFIN] ❌ Mode manager bulunamadı!")
                                        if hasattr(self.main_window, 'log_psfalgo_activity'):
                                            self.main_window.log_psfalgo_activity("EMIR_HATASI", f"{symbol} Mode Manager Not Found", "ERROR", "Internal Error", "ADDNEWPOS")
                            else:
                                ...
                                if hasattr(self.main_window, 'mode_manager'):
                                    ...
                                    if success:
                                        ...
                                        if hasattr(self.main_window, 'log_psfalgo_activity'):
                                            ...
                                            self.main_window.log_psfalgo_activity("EMIR_ILETILDI", f"{symbol} {side} {lot} @ {price:.2f}", "SUCCESS", condition_reason, "ADDNEWPOS")
                                    else:
                                        error_count += 1
                                        print(f"[JFIN] ❌ {symbol}: {lot} lot {order_type} @ ${price:.2f} - Başarısız")
                                        if hasattr(self.main_window, 'log_psfalgo_activity'):
                                            self.main_window.log_psfalgo_activity("EMIR_HATASI", f"{symbol} {lot} @ {price:.2f}", "ERROR", "Failed to place order", "ADDNEWPOS")
                                else:
                                    error_count += 1
                                    print(f"[JFIN] ❌ Mode manager bulunamadı!")
                                    if hasattr(self.main_window, 'log_psfalgo_activity'):
                            ...
                            if hasattr(self.main_window, 'log_psfalgo_activity'):
                                self.main_window.log_psfalgo_activity("EMIR_HATASI", f"{order['symbol']} {e}", "ERROR", str(e), "ADDNEWPOS")
                    ...
                    if success_count > 0:
                        ...
                        # RUNALL döngüsünde ise 2 dakika bekleyip emirleri iptal et ve tekrar başla
                        # runall_allowed_mode aktifse RUNALL döngüsü çalışıyor demektir
                        runall_running = hasattr(self.main_window, 'runall_loop_running') and self.main_window.runall_loop_running
                        runall_allowed = hasattr(self.main_window, 'runall_allowed_mode') and self.main_window.runall_allowed_mode
                        should_schedule_callback = runall_running or runall_allowed
                        print(f"[JFIN] 🔍 RUNALL kontrolü: runall_loop_running={runall_running}, runall_allowed_mode={runall_allowed}, should_schedule_callback={should_schedule_callback}")
                        if should_schedule_callback:
                            self.main_window.log_message("✅ ADDNEWPOS emirleri gönderildi, Qpcal işlemi başlatılıyor...")
                            print(f"[RUNALL] ✅ ADDNEWPOS emirleri gönderildi ({success_count} başarılı), Qpcal işlemi başlatılıyor...")
                            ...
                            # Qpcal işlemini başlat (2 dakika sayacı Qpcal callback'inde başlatılacak)
                            print(f"[RUNALL] 🔄 Qpcal işlemi başlatılıyor...")
                            self.main_window.runall_execute_qpcal()
                        ...
                        print(f"[JFIN] ⚠️ Hiç emir gönderilemedi! success_count=0, error_count={error_count}")
                        # RUNALL döngüsünde bile olsa hiç emir gönderilemediyse callback çağırma
    ...
    def auto_send_jfin_orders(self, win, order_tree, send_orders_func):
        ...
        try:
            ...
            if self.main_window:
                self.main_window.addnewpos_jfin_window = win
                print(f"[JFIN] ✅ JFIN penceresi auto_send_jfin_orders içinde kaydedildi: {win.title()}")
            ...
            for item in order_tree.get_children():
                ...
                if values[0] == '☑':
                    ...
                    if hasattr(self.main_window, 'get_maxalw_for_symbol'):
                        ...
                        # MAXALW*3/4 limitini aşacaksa emir lotunu azalt
                        if potential_daily_change > max_change_limit:
                            # İzin verilen maksimum potansiyel pozisyonu hesapla
                            if new_potential > befday_qty:
                                # Pozisyon artıyor (BUY)
                            ...
                            # Negatif lot kontrolü
                            if max_allowed_lot <= 0:
                                print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacak ({potential_daily_change:.0f} > {max_change_limit:.0f}), emir seçimi kaldırıldı (izin verilen lot: {max_allowed_lot:.0f})")
                            ...
                            # Minimum 100 lot kontrolü
                            if max_allowed_lot < 100:
                                print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacak ({potential_daily_change:.0f} > {max_change_limit:.0f}), emir seçimi kaldırıldı (izin verilen lot çok küçük: {max_allowed_lot})")
                                current_values = list(values)
                                current_values[0] = '☐'  # Seçimi kaldır
                                order_tree.item(item, values=current_values, tags=('unselected',))
                                continue
                            
                            # Lot miktarını güncelle (treeview'deki değeri değiştir)
                            current_values = list(values)
                            # Lot değeri values[9] index'inde (hesaplanan lot)
                            try:
                                original_lot = int(str(values[9]).replace(',', ''))
                                current_values[9] = f"{max_allowed_lot:,}"  # Yeni lot değeri
                                order_tree.item(item, values=current_values)
                                print(f"[JFIN] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacaktı ({potential_daily_change:.0f} > {max_change_limit:.0f}), lot azaltıldı: {original_lot:,} → {max_allowed_lot:,} lot")
                            except Exception as e:
                                print(f"[JFIN] ⚠️ {symbol}: Lot güncelleme hatası: {e}, emir seçimi kaldırıldı")
                                current_values[0] = '☐'  # Seçimi kaldır
                                order_tree.item(item, values=current_values, tags=('unselected',))
                                continue
    ...
    def get_group_stocks_from_mini450(self, group):
        ...
        try:
            ...
            # Exclude edilmiş hisseleri kontrol et
            excluded_count = 0
            
            for _, row in mini450_df.iterrows():
                symbol = row.get('PREF IBKR', '')
                if symbol in group_symbols:
                    # Exclude edilmiş hisse kontrolü
                    if hasattr(self.main_window, 'is_ticker_excluded') and self.main_window.is_ticker_excluded(symbol):
                        excluded_count += 1
                        print(f"   🚫 {symbol} exclude edilmiş, atlanıyor")
                        continue
                    
                    stock_dict = row.to_dict()
                    group_stocks.append(stock_dict)
            
            print(f"   📊 {group}: {len(group_symbols)} CSV hisse → {len(group_stocks)} Mini450'de bulundu (Exclude edilmiş: {excluded_count})")
            return group_stocks
    ...
    def select_top_stocks_by_score(self, stocks, score_column, count, direction):
        ...
        try:
            ...
            # Exclude edilmiş hisseleri filtrele
            excluded_count = 0
            if hasattr(self.main_window, 'is_ticker_excluded'):
                original_count = len(df)
                df = df[~df['PREF IBKR'].apply(lambda x: self.main_window.is_ticker_excluded(x))]
                excluded_count = original_count - len(df)
                if excluded_count > 0:
                    print(f"   🚫 {excluded_count} exclude edilmiş hisse filtrelendi")
            ...
            # Şirket kısıtı uygula - Grup bazlı %40 kuralı
            print(f"   🏢 Şirket kısıtı uygulanıyor (Grup bazlı %40 kuralı)...")
            selected_stocks = []
            company_counts = {}  # Her şirketin kaç hisse seçildiğini takip et
            
            # Grup içindeki toplam hisse sayısı (exclude edilmemiş)
            total_stocks_in_group = len(df)
            ...
            for _, row in df_sorted.iterrows():
                ...
                if company not in company_counts:
                    ...
                    # Şirket kısıtı: Grup içindeki toplam hisse sayısının %40'ı kadar aynı CMON'dan seçilebilir
                    max_per_company = round(company_total * 0.4)
                    # Minimum 1 hisse seçilebilir (eğer 0.5'ten küçükse bile)
                    if max_per_company < 1:
                        max_per_company = 1
                    # Toplam sayıdan fazla olamaz
                    max_per_company = min(max_per_company, company_total)
                    company_counts[company] = {'selected': 0, 'max': max_per_company, 'total': company_total}
                    print(f"      🏢 {company}: {company_total} hisse → maksimum {max_per_company} seçilebilir (%40 kuralı: {company_total} * 0.4 = {company_total * 0.4:.1f} → {max_per_company})")
    ...
    def select_heldkuponlu_cgrup_combinations(self, group_stocks, long_score_col, short_score_col, min_cgrup_count=5, target_pair_count=8):
        """
        heldkuponlu grubu için özel CGRUP bazlı long-short kombinasyon seçimi
        YENİ MANTIK:
        - C600, C625, C650 gibi özel CGRUP'lardan en az 1'er pair seç (serbest, daha fazla da olabilir)
        - Diğer CGRUP'lardan en az 1'er pair seç
        - Toplam target_pair_count kadar pair seç (skor bazlı devam eder)
        - Şirket limiti (%40) uygulanır
        
        Args:
            group_stocks: Grup içindeki tüm hisseler
            long_score_col: Long skor kolonu ('Final_BB_skor' veya 'Final_FB_skor')
            short_score_col: Short skor kolonu ('Final_SAS_skor' veya 'Final_SFS_skor')
            min_cgrup_count: Minimum farklı CGRUP sayısı (kullanılmıyor, geriye uyumluluk için)
            target_pair_count: Toplam seçilecek pair sayısı (V15: 10, V20: 12)
        
        Returns:
            (long_stocks, short_stocks): Seçilen long ve short hisseler
        """
        try:
            print(f"   🎯 HELDKUPONLU ÖZEL CGRUP SEÇİMİ BAŞLIYOR...")
            print(f"   📊 Long skor: {long_score_col}, Short skor: {short_score_col}")
            print(f"   🎯 Toplam {target_pair_count} pair seçilecek ({target_pair_count * 2} hisse: {target_pair_count} long + {target_pair_count} short)")
            
            if len(group_stocks) == 0:
                return [], []
            
            # DataFrame'e çevir
            df = pd.DataFrame(group_stocks)
            
            # CGRUP kolonunu kontrol et
            if 'CGRUP' not in df.columns:
                print(f"   ⚠️ CGRUP kolonu bulunamadı, normal seçime dönülüyor")
                return [], []
            
            # Skor kolonlarını kontrol et
            if long_score_col not in df.columns or short_score_col not in df.columns:
                print(f"   ⚠️ Skor kolonları bulunamadı")
                return [], []
            
            # Numeric olmayan değerleri temizle
            df[long_score_col] = pd.to_numeric(df[long_score_col], errors='coerce')
            df[short_score_col] = pd.to_numeric(df[short_score_col], errors='coerce')
            
            # CGRUP değerlerini string'e çevir (karşılaştırma için)
            df['CGRUP'] = df['CGRUP'].astype(str)
            
            # Özel CGRUP'lar (C600, C625, C650) - bunlardan en az 1'er pair seçilecek
            special_cgrups = ['C600', 'C625', 'C650']
            
            # CGRUP'a göre grupla
            cgrup_groups = df.groupby('CGRUP')
            
            # Her CGRUP için en iyi long ve short kombinasyonunu bul
            cgrup_combinations = []
            special_cgrup_combinations = []  # Özel CGRUP'lar için
            other_cgrup_combinations = []    # Diğer CGRUP'lar için
            
            for cgrup, cgrup_df in cgrup_groups:
                # Bu CGRUP'taki long ve short hisseleri al
                long_candidates = cgrup_df[cgrup_df[long_score_col].notna()].copy()
                short_candidates = cgrup_df[cgrup_df[short_score_col].notna()].copy()
                
                # Exclude edilmiş hisseleri filtrele
                if hasattr(self.main_window, 'is_ticker_excluded'):
                    long_candidates = long_candidates[
                        ~long_candidates['PREF IBKR'].apply(lambda x: self.main_window.is_ticker_excluded(x))
                    ]
                    short_candidates = short_candidates[
                        ~short_candidates['PREF IBKR'].apply(lambda x: self.main_window.is_ticker_excluded(x))
                    ]
                
                if len(long_candidates) == 0 or len(short_candidates) == 0:
                    continue
                
                # En iyi long hisseyi bul (en yüksek skor)
                best_long = long_candidates.loc[long_candidates[long_score_col].idxmax()]
                long_score = best_long[long_score_col]
                
                # En iyi short hisseyi bul (en düşük skor)
                best_short = short_candidates.loc[short_candidates[short_score_col].idxmin()]
                short_score = best_short[short_score_col]
                
                # Long-Short farkını hesapla
                score_diff = long_score - short_score
                
                combo = {
                    'cgrup': cgrup,
                    'long_stock': best_long.to_dict(),
                    'short_stock': best_short.to_dict(),
                    'long_score': long_score,
                    'short_score': short_score,
                    'score_diff': score_diff
                }
                
                cgrup_combinations.append(combo)
                
                # Özel CGRUP'lara ayır
                if cgrup in special_cgrups:
                    special_cgrup_combinations.append(combo)
                else:
                    other_cgrup_combinations.append(combo)
                
                print(f"      📊 CGRUP {cgrup}: Long={best_long.get('PREF IBKR', 'N/A')} ({long_score:.2f}), "
                      f"Short={best_short.get('PREF IBKR', 'N/A')} ({short_score:.2f}), Fark={score_diff:.2f}")
            
            if len(cgrup_combinations) == 0:
                print(f"   ⚠️ Hiç CGRUP kombinasyonu bulunamadı")
                return [], []
            
            # Özel CGRUP kombinasyonlarını skor farkına göre sırala
            special_cgrup_combinations.sort(key=lambda x: x['score_diff'], reverse=True)
            # Diğer CGRUP kombinasyonlarını skor farkına göre sırala
            other_cgrup_combinations.sort(key=lambda x: x['score_diff'], reverse=True)
            
            print(f"   📊 Özel CGRUP'lar (C600, C625, C650): {len(special_cgrup_combinations)} adet")
            print(f"   📊 Diğer CGRUP'lar: {len(other_cgrup_combinations)} adet")
            
            # Şirket limiti hesaplamaları için hazırlık
            company_max_limits = {}
            for company in df['CMON'].unique():
                ...
                if max_per_company < 1:
                    max_per_company = 1
                max_per_company = min(max_per_company, company_total)
                company_max_limits[company] = max_per_company
            
            # Seçim mantığı: 
            # 1. Diğer CGRUP'lardan (C600, C625, C650 hariç) en az 1'er pair seç
            # 2. Kalan yerleri skor bazlı doldur (C600, C625, C650 dahil tüm CGRUP'lar serbest)
            selected_combinations = []
            selected_cgrups = set()
            company_counts = {'long': {}, 'short': {}}  # Şirket bazlı sayılar
            
            # 1. Diğer CGRUP'lardan (C600, C625, C650 hariç) en az 1'er pair seç
            print(f"   🎯 Adım 1: Diğer CGRUP'lardan (C600, C625, C650 hariç) en az 1'er pair seçiliyor...")
            for combo in other_cgrup_combinations:
                if len(selected_combinations) >= target_pair_count:
                    break
                
                cgrup = combo['cgrup']
                long_stock = combo['long_stock']
                short_stock = combo['short_stock']
                long_company = long_stock.get('CMON', 'N/A')
                short_company = short_stock.get('CMON', 'N/A')
                
                # Şirket limiti kontrolü
                max_long = company_max_limits.get(long_company, 1)
                max_short = company_max_limits.get(short_company, 1)
                
                long_count = company_counts['long'].get(long_company, 0)
                short_count = company_counts['short'].get(short_company, 0)
                
                # En az 1 pair seç (şirket limiti izin veriyorsa)
                if cgrup not in selected_cgrups:
                    if long_count < max_long and short_count < max_short:
                        selected_combinations.append(combo)
                        selected_cgrups.add(cgrup)
                        company_counts['long'][long_company] = long_count + 1
                        company_counts['short'][short_company] = short_count + 1
                        print(f"      ✅ CGRUP {cgrup} eklendi (diğer CGRUP, en az 1 kuralı) - Pair #{len(selected_combinations)}")
            
            # 2. Kalan yerleri skor bazlı doldur (C600, C625, C650 dahil tüm CGRUP'lar serbest, şirket limiti kontrolü ile)
            print(f"   🎯 Adım 2: Kalan yerler skor bazlı dolduruluyor (C600, C625, C650 serbest, hedef: {target_pair_count} pair)...")
            all_remaining_combos = special_cgrup_combinations + other_cgrup_combinations
            all_remaining_combos.sort(key=lambda x: x['score_diff'], reverse=True)
            
            for combo in all_remaining_combos:
                if len(selected_combinations) >= target_pair_count:
                    break
                ...
                # Şirket limiti izin veriyorsa ekle
                if long_count < max_long and short_count < max_short:
                    # Aynı kombinasyonu tekrar eklememek için kontrol
                    combo_already_added = any(
                        c['cgrup'] == cgrup and 
                        c['long_stock'].get('PREF IBKR') == long_stock.get('PREF IBKR') and
                        c['short_stock'].get('PREF IBKR') == short_stock.get('PREF IBKR')
                        for c in selected_combinations
                    )
                    
                    if not combo_already_added:
                        selected_combinations.append(combo)
                        if cgrup not in selected_cgrups:
                            selected_cgrups.add(cgrup)
                        company_counts['long'][long_company] = long_count + 1
                        company_counts['short'][short_company] = short_count + 1
                        cgrup_type = "özel CGRUP (serbest)" if cgrup in special_cgrups else "diğer CGRUP"
                        print(f"      ✅ CGRUP {cgrup} eklendi (skor bazlı, {cgrup_type}) - Pair #{len(selected_combinations)}")
            
            # Seçilen kombinasyonları long ve short listelerine ayır (zaten şirket limiti kontrolü yapıldı)
            long_stocks = []
            short_stocks = []
            final_company_counts = {'long': {}, 'short': {}}  # Final sayılar için
            cgrup_pair_counts = {}  # CGRUP bazlı pair sayıları
            ...
            for combo in selected_combinations:
                ...
                # Long hisseyi ekle
                long_stocks.append(long_stock)
                final_company_counts['long'][long_company] = final_company_counts['long'].get(long_company, 0) + 1
                
                # Short hisseyi ekle
                short_stocks.append(short_stock)
                final_company_counts['short'][short_company] = final_company_counts['short'].get(short_company, 0) + 1
                
                # CGRUP sayısını güncelle
                cgrup = combo['cgrup']
                cgrup_pair_counts[cgrup] = cgrup_pair_counts.get(cgrup, 0) + 1
                
                print(f"      ✅ Pair eklendi: Long={long_stock.get('PREF IBKR', 'N/A')} ({long_company}), "
                      f"Short={short_stock.get('PREF IBKR', 'N/A')} ({short_company}) - CGRUP: {cgrup}")
            
            # Seçilen CGRUP'ları göster
            selected_cgrups_list = sorted(selected_cgrups)
            print(f"   ✅ {len(selected_cgrups_list)} farklı CGRUP seçildi: {selected_cgrups_list}")
            print(f"   📊 Long: {len(long_stocks)} hisse, Short: {len(short_stocks)} hisse")
            print(f"   🎯 Toplam: {len(long_stocks) + len(short_stocks)} hisse ({len(selected_combinations)} pair)")
            
            # CGRUP bazlı pair dağılımını göster
            for cgrup, count in sorted(cgrup_pair_counts.items()):
                cgrup_type = "özel (serbest)" if cgrup in special_cgrups else "diğer"
                print(f"      📊 CGRUP {cgrup} ({cgrup_type}): {count} pair")
            
            # Şirket bazlı özet
            print(f"   🏢 Şirket bazlı özet (Long):")
            for company, count in sorted(final_company_counts['long'].items()):
                max_limit = company_max_limits.get(company, 'N/A')
                print(f"      📊 {company}: {count} hisse (limit: {max_limit})")
            
            print(f"   🏢 Şirket bazlı özet (Short):")
            for company, count in sorted(final_company_counts['short'].items()):
                max_limit = company_max_limits.get(company, 'N/A')
                print(f"      📊 {company}: {count} hisse (limit: {max_limit})")
            ...
            print(f"   ❌ HELDKUPONLU CGRUP seçim hatası: {e}")
            import traceback
            traceback.print_exc()
            return [], []
    
    def select_heldkuponlu_by_score_type(self, group_stocks, score_column, direction, target_count, excluded_symbols=set()):
        """
        HELDKUPONLU grubu için skor türüne göre hisse seçimi (LONG veya SHORT)
        
        Args:
            group_stocks: HELDKUPONLU grubundaki tüm hisseler
            score_column: Skor kolonu ('Final_BB_skor', 'Final_FB_skor', 'Final_SAS_skor', 'Final_SFS_skor')
            direction: 'LONG' veya 'SHORT'
            target_count: Seçilecek hisse sayısı (V15: 10, V20: 12)
            excluded_symbols: Hariç tutulacak symbol'ler (örn: diğer sekmede seçilenler)
        
        Returns:
            selected_stocks: Seçilen hisseler listesi
        """
        try:
            print(f"   🎯 HELDKUPONLU {direction} seçimi başlıyor ({score_column})...")
            print(f"   📊 Hedef: {target_count} adet, Hariç tutulan: {len(excluded_symbols)} hisse")
            ...
            # CGRUP kolonunu kontrol et
            if 'CGRUP' not in df.columns:
                print(f"   ⚠️ CGRUP kolonu bulunamadı")
                return []
            ...
            # Hariç tutulan symbol'leri filtrele
            if excluded_symbols:
                df = df[~df['PREF IBKR'].isin(excluded_symbols)]
            
            # CGRUP değerlerini string'e çevir
            df['CGRUP'] = df['CGRUP'].astype(str)
            
            # Özel CGRUP'lar (C600, C625, C650) - bunlar serbest, minimum 1 kuralı yok
            special_cgrups = ['C600', 'C625', 'C650']
            
            # Tüm hisseleri skor bazlı sırala (CGRUP bazlı değil, tüm hisseler)
            if direction == 'LONG':
                df_sorted = df.sort_values(score_column, ascending=False)
            else:  # SHORT
                df_sorted = df.sort_values(score_column, ascending=True)
            
            # CGRUP'a göre grupla (sadece "en az 1" kontrolü için)
            cgrup_groups = df.groupby('CGRUP')
            all_cgrups = set(cgrup_groups.groups.keys())
            
            print(f"   📊 Toplam {len(df_sorted)} hisse, {len(all_cgrups)} farklı CGRUP bulundu")
            
            # Şirket limiti hesaplamaları
            company_max_limits = {}
            for company in df['CMON'].unique():
                if pd.isna(company) or company == 'N/A':
                    continue
            ...
            # Seçim mantığı
            selected_stocks = []
            selected_cgrups = set()  # Diğer CGRUP'lardan en az 1'er seçildi mi kontrolü için
            company_counts = {}
            
            # 1. Önce diğer CGRUP'lardan (C600, C625, C650 hariç) en az 1'er seç
            print(f"   🎯 Adım 1: Diğer CGRUP'lardan (C600, C625, C650 hariç) en az 1'er seçiliyor...")
            for _, row in df_sorted.iterrows():
                if len(selected_stocks) >= target_count:
                    break
                
                cgrup = str(row.get('CGRUP', 'N/A'))
                company = row.get('CMON', 'N/A')
                symbol = row.get('PREF IBKR', 'N/A')
                
                # Özel CGRUP'ları atla (onlar serbest, sonra seçilecek)
                if cgrup in special_cgrups:
                    continue
                
                # Şirket limiti kontrolü
                max_per_company = company_max_limits.get(company, 1)
                company_count = company_counts.get(company, 0)
                
                # En az 1 seç (şirket limiti izin veriyorsa)
                if cgrup not in selected_cgrups:
                    if company_count < max_per_company:
                        selected_stocks.append(row.to_dict())
                        selected_cgrups.add(cgrup)
                        company_counts[company] = company_count + 1
                        print(f"      ✅ CGRUP {cgrup} eklendi (diğer CGRUP, en az 1 kuralı) - {symbol} - #{len(selected_stocks)}")
            
            # 2. Kalan yerleri skor bazlı doldur (C600, C625, C650 dahil tüm CGRUP'lar serbest, aynı CGRUP'tan birden fazla seçilebilir)
            print(f"   🎯 Adım 2: Kalan yerler skor bazlı dolduruluyor (C600, C625, C650 serbest, hedef: {target_count})...")
            for _, row in df_sorted.iterrows():
                if len(selected_stocks) >= target_count:
                    break
                ...
                # Aynı hisseyi tekrar eklememek için kontrol
                already_selected = any(s.get('PREF IBKR', '') == symbol for s in selected_stocks)
                
                if not already_selected and company_count < max_per_company:
                    selected_stocks.append(row.to_dict())
                    if cgrup not in selected_cgrups:
                        selected_cgrups.add(cgrup)
                    company_counts[company] = company_count + 1
                    cgrup_type = "özel CGRUP (serbest)" if cgrup in special_cgrups else "diğer CGRUP"
                    print(f"      ✅ CGRUP {cgrup} eklendi (skor bazlı, {cgrup_type}) - {symbol} - #{len(selected_stocks)}")
            
            print(f"   ✅ {direction} seçimi tamamlandı: {len(selected_stocks)}/{target_count} hisse")
            print(f"   📊 Seçilen CGRUP'lar: {sorted(selected_cgrups)}")
            
            return selected_stocks
            
        except Exception as e:
            print(f"   ❌ HELDKUPONLU {direction} seçim hatası: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    def get_jfin_calculated_lot(self, symbol, order_side, jfin_tab=None):
        """
        JFIN sayfasından hesaplanan lot bilgisini al
        
        Args:
            symbol: Hisse sembolü
            order_side: 'BUY' (Long) veya 'SELL' (Short)
            jfin_tab: Hangi JFIN sekmesinde işlem yapıldığı ('BB', 'FB', 'SAS', 'SFS')
                     None ise order_side'a göre otomatik belirlenir
        
        Returns:
            Hesaplanan lot miktarı (int), bulunamazsa 0
        """
        try:
            # Eğer jfin_tab belirtilmişse, sadece o sekmedeki değeri kullan
            if jfin_tab:
                jfin_tab = jfin_tab.upper()
                if jfin_tab == 'BB' and hasattr(self, 'bb_lots'):
                    return int(self.bb_lots.get(symbol, 0))
                elif jfin_tab == 'FB' and hasattr(self, 'fb_lots'):
                    return int(self.fb_lots.get(symbol, 0))
                elif jfin_tab == 'SAS' and hasattr(self, 'sas_lots'):
                    return int(self.sas_lots.get(symbol, 0))
                elif jfin_tab == 'SFS' and hasattr(self, 'sfs_lots'):
                    return int(self.sfs_lots.get(symbol, 0))
                return 0
            
            # jfin_tab belirtilmemişse, order_side'a göre otomatik belirle
            # ADDNEWPOS şu an sadece BB ile çalışıyor, bu yüzden varsayılan olarak BB kullan
            if order_side.upper() == 'BUY':
                # Long pozisyonlar için - ADDNEWPOS şu an sadece BB kullanıyor
                if hasattr(self, 'bb_lots') and symbol in self.bb_lots:
                    return int(self.bb_lots.get(symbol, 0))
                # BB'de yoksa FB'ye bakma (sadece ilgili sekmedeki değeri kullan)
                return 0
            else:  # SELL - Short pozisyonlar için
                # Short pozisyonlar için - gelecekte SAS veya SFS kullanılacak
                # Şu an SAS yok, bu yüzden SFS'ye bakma (sadece ilgili sekmedeki değeri kullan)
                return 0
        except Exception as e:
            print(f"[JFIN] ❌ {symbol} hesaplanan lot alma hatası: {e}")
            return 0
    ...
    def display_tumcsv_results_by_score_type(self, all_long_stocks, all_short_stocks):
        ...
        try:
            ...
            # Lot bilgilerini instance'a kaydet (controller için erişilebilir olması için)
            self.bb_lots = bb_lots
            self.fb_lots = fb_lots
            self.sas_lots = sas_lots
            self.sfs_lots = sfs_lots
            ...
            for stock in bb_long_stocks_list:
                try:
                    ...
                    # Eklenebilir lot hesapla (symbol parametresi ile yeni kurallar uygulanır)
                    addable_lot = self.calculate_addable_lot_jfin(maxalw, current_position, calculated_lots, symbol)
                    ...
                    bb_long_values = (group, symbol, cgrup, final_bb_skor, final_sfs_skor, fbtot, smi, sma63_chg, gort_str, maxalw, current_position, calculated_lots, addable_lot)
            ...
            for stock in fb_long_stocks_list:
                try:
                    ...
                    fb_long_values = (group, symbol, cgrup, final_fb_skor, final_sfs_skor, fbtot, smi, sma63_chg, gort_str, maxalw, current_position, calculated_lots, addable_lot)
            ...
            for stock in sas_short_stocks_list:
                try:
                    ...
                    # Eklenebilir lot hesapla (Short için - symbol parametresi ile yeni kurallar uygulanır)
                    addable_lot = self.calculate_addable_lot_short_jfin(maxalw, current_position, calculated_lots, symbol)
                    ...
                    sas_short_values = (group, symbol, cgrup, final_sas_skor, final_fb_skor, sfstot, smi, sma63_chg, gort_str, maxalw, abs(current_position), calculated_lots, addable_lot)
            ...
            for stock in sfs_short_stocks_list:
                try:
                    ...
                    sfs_short_values = (group, symbol, cgrup, final_sfs_skor, final_fb_skor, sfstot, smi, sma63_chg, gort_str, maxalw, abs(current_position), calculated_lots, addable_lot)
    ...
    def should_apply_gort_filter(self, group, gort_value, filter_value, filter_type):
        """Gort filtresi uygulanıp uygulanmayacağını belirler"""
        try:
            # Filtre değeri boş ise filtreleme yapma
            if not filter_value or filter_value.strip() == '':
                return True
            ...
            exempted_groups = self.get_exempted_groups()
            if group.lower() in exempted_groups:
                print(f"[Gort FILTER] ✅ {group} exempted, filtreleme yapılmıyor")
                return True

            # Gort değeri geçerli değilse, filtrelemeye dahil etme
            if gort_value == 'N/A' or gort_value == '' or gort_value is None:
                print(f"[Gort FILTER] ❌ Gort değeri geçersiz: {gort_value}")
                return False

            filter_num = float(filter_value)
            gort_num = float(gort_value)

            if filter_type == 'above':
                result = gort_num >= filter_num
                print(f"[Gort FILTER] 🔍 {group}: Gort={gort_num} >= {filter_num} = {result}")
                return result
            else:  # below
                result = gort_num <= filter_num
                print(f"[Gort FILTER] 🔍 {group}: Gort={gort_num} <= {filter_num} = {result}")
    ...
    def apply_bb_filter(self):
        ...
        try:
            ...
            # Gort filtre değerlerini önceden al
            gort_filter_value = self.bb_gort_filter_var.get().strip()
            gort_filter_type = self.bb_gort_filter_type.get()
            ...
            print(f"[BB FILTER] 🔍 Gort Filtre: {gort_filter_type} {gort_filter_value}")
            ...
            for item in all_items:
                try:
                    ...
                    if len(values) >= 9:  # En az 9 kolon olmalı (CGRUP eklendi, GORT index 8)
                        group = values[0]
                        symbol = values[1]
                        sma_value = values[7]  # SMA63 chg kolonu (CGRUP eklendi, 6->7)
                        fbtot_value = values[5]  # FBtot kolonu (CGRUP eklendi, 4->5)
                        gort_value = values[8]  # GORT kolonu (CGRUP eklendi, 7->8)
                        ...
                        gort_pass = self.should_apply_gort_filter(group, gort_value, gort_filter_value, gort_filter_type)

                        print(f"[BB FILTER] 🔍 {symbol}: SMA={sma_value} (pass={sma_pass}), FBtot={fbtot_value} (pass={fbtot_pass}), Gort={gort_value} (pass={gort_pass})")

                        if sma_pass and fbtot_pass and gort_pass:
    ...
    def apply_fb_filter(self):
        ...
        try:
            ...
            # Gort filtre değerlerini önceden al
            gort_filter_value = self.fb_gort_filter_var.get().strip()
            gort_filter_type = self.fb_gort_filter_type.get()
            ...
            print(f"[FB FILTER] 🔍 Gort Filtre: {gort_filter_type} {gort_filter_value}")
            ...
            for item in all_items:
                try:
                    ...
                    if len(values) >= 9:  # En az 9 kolon olmalı (CGRUP eklendi, GORT index 8)
                        ...
                        print(f"[FB FILTER] 🔍 {symbol}: SMA={sma_value} (pass={sma_pass}), FBtot={fbtot_value} (pass={fbtot_pass}), Gort={gort_value} (pass={gort_pass})")
    ...
    def apply_sas_filter(self):
        ...
        try:
            ...
            # Gort filtre değerlerini önceden al
            gort_filter_value = self.sas_gort_filter_var.get().strip()
            gort_filter_type = self.sas_gort_filter_type.get()
            ...
            print(f"[SAS FILTER] 🔍 Gort Filtre: {gort_filter_type} {gort_filter_value}")
            ...
            for item in all_items:
                try:
                    ...
                    if len(values) >= 9:  # En az 9 kolon olmalı (CGRUP eklendi, GORT index 8)
                        ...
                        symbol = values[1]
                        sma_value = values[7]  # SMA63 chg kolonu (CGRUP eklendi, 6->7)
                        sfstot_value = values[5]  # SFStot kolonu (CGRUP eklendi, 4->5)
                        gort_value = values[8]  # GORT kolonu (CGRUP eklendi, 7->8)
                        ...
                        print(f"[SAS FILTER] 🔍 {symbol}: SMA={sma_value} (pass={sma_pass}), SFStot={sfstot_value} (pass={sfstot_pass}), Gort={gort_value} (pass={gort_pass})")

                        if sma_pass and sfstot_pass and gort_pass:
    ...
    def apply_sfs_filter(self):
        ...
        try:
            ...
            # Gort filtre değerlerini önceden al
            gort_filter_value = self.sfs_gort_filter_var.get().strip()
            gort_filter_type = self.sfs_gort_filter_type.get()
            ...
            print(f"[SFS FILTER] 🔍 Gort Filtre: {gort_filter_type} {gort_filter_value}")
            ...
            for item in all_items:
                try:
                    ...
                    if len(values) >= 9:  # En az 9 kolon olmalı (CGRUP eklendi, GORT index 8)
                        ...
                        print(f"[SFS FILTER] 🔍 {symbol}: SMA={sma_value} (pass={sma_pass}), SFStot={sfstot_value} (pass={sfstot_pass}), Gort={gort_value} (pass={gort_pass})")
