        ...
        try:
            ...
            for order in open_orders:
                try:
                    ...
                    # ib_async'te avgFillPrice bilgisi orderStatus'ta var mı kontrol et
                    avg_fill_price = 0.0
                    last_fill_price = 0.0
                    if hasattr(order.orderStatus, 'avgFillPrice'):
                        avg_fill_price = float(order.orderStatus.avgFillPrice) if order.orderStatus.avgFillPrice and order.orderStatus.avgFillPrice > 0 else 0.0
                    if hasattr(order.orderStatus, 'lastFillPrice'):
                        last_fill_price = float(order.orderStatus.lastFillPrice) if order.orderStatus.lastFillPrice and order.orderStatus.lastFillPrice > 0 else 0.0
                    ...
                    order_data = {
                        ...
                        'price': float(order.order.lmtPrice) if order.order.lmtPrice else 0.0,  # Emir fiyatı (limit_price ile aynı)
            ...
            if "-" in symbol and len(symbol.split("-")) == 2:
                ...
                try:
                    ...
                    if ibkr_symbol != symbol:
                        print(f"[IBKR] 🔄 Ticker conversion: {symbol} -> {ibkr_symbol}")
                except Exception as e:
                    print(f"[IBKR] ⚠️ Ticker conversion hatası: {e}, orijinal symbol kullanılıyor")
                    ibkr_symbol = symbol
            
            print(f"[IBKR] 🔄 Emir gönderiliyor: {ibkr_symbol} {side} {quantity} @ ${price:.2f}")
