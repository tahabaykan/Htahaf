...
class TakeProfitPanel:
    ...
    def analyze_lrpan(self):
        """LRPAN analiz penceresi - Seçili pozisyonlar için"""
        try:
            print(f"[LRPAN] 🔍 Take Profit LRPAN analizi başlatılıyor...")
            
            if not self.selected_positions:
                messagebox.showwarning("Uyarı", "Hiç pozisyon seçilmedi!")
                return
            
            # LRPAN sonuçları penceresi
            lrpan_win = tk.Toplevel(self.win)
            lrpan_win.title("LRPAN - Last Real Print Analyzer")
            lrpan_win.geometry("700x500")
            # transient ve grab_set kaldırıldı - bağımsız pencere, minimize edilebilir
            
            # Açıklama label
            info_label = ttk.Label(lrpan_win, 
                text="LRPAN: Son 25 tick'te 100/200/300 lot olan en yakın print'i bulur.",
                font=('Arial', 9, 'italic'))
            info_label.pack(pady=5)
            
            # LRPAN sonuçları tablosu
            columns = ('Symbol', 'Qty', 'LRPAN', 'Bid', 'Ask', 'Status')
            lrpan_tree = ttk.Treeview(lrpan_win, columns=columns, show='headings', height=18)
            
            col_widths = {'Symbol': 100, 'Qty': 80, 'LRPAN': 90, 'Bid': 80, 'Ask': 80, 'Status': 120}
            for col in columns:
                lrpan_tree.heading(col, text=col)
                lrpan_tree.column(col, width=col_widths.get(col, 80), anchor='center')
            ...
            # Seçili pozisyonlar için analiz
            for symbol in self.selected_positions:
                ...
                values = [symbol, f"{qty:,}", lrpan_str, bid_str, ask_str, status]
            ...
            print(f"[LRPAN] ✅ Analiz tamamlandı: {len(self.selected_positions)} pozisyon")
        ...
        try:
            if hasattr(self.parent, 'hammer') and self.parent.hammer and self.parent.hammer.connected:
                ...
                if tick_data and 'data' in tick_data and tick_data['data']:
                    ...
                    for tick in last_15_ticks:
                        ...
                        if price > 0:
                            ...
                            if size in [100, 200, 300]:
                                weight = 1.00
                                real_lot_count += 1
                                print(f"[GRPAN] 📊 {symbol}: ${price:.2f} @ {size} lot → REAL (1.00)")
                            else:
                                weight = 0.25
                                print(f"[GRPAN] 📊 {symbol}: ${price:.2f} @ {size} lot → other (0.25)")
                    ...
                    print(f"[GRPAN] 🔍 {symbol}: {len(weighted_prices)} tick, {real_lot_count} real lot (100/200/300)")
                    
                    # Ağırlıklı MOD hesapla
                    # Her fiyat için toplam ağırlığı hesapla
                    ...
                    mode_weight = price_weights[mode_price]
                    
                    print(f"[GRPAN] 📊 {symbol}: MOD fiyat = ${mode_price:.2f} (ağırlık: {mode_weight:.2f})")
                    
                    # ±0.04 cent aralığını belirle
                    range_min = mode_price - 0.04
                    range_max = mode_price + 0.04
                    
                    print(f"[GRPAN] 📊 {symbol}: Aralık = ${range_min:.2f} - ${range_max:.2f}")
                    ...
                    # Aralıktaki real lot sayısı (filtrelenmiş tick'lerden)
                    ...
                    print(f"[GRPAN] 📊 {symbol}: Aralıkta {real_in_range} real lot (100/200/300)")
                    ...
                    if concentration >= 50:
                        print(f"[GRPAN] ✅ {symbol}: GRPAN = ${mode_price:.2f} (Yoğunluk: %{concentration:.1f})")
                    else:
                        print(f"[GRPAN] ⚠️ {symbol}: Düşük yoğunluk (%{concentration:.1f} < 50%)")
    ...
    def analyze_grpan(self):
        """GRPAN analiz penceresi - Seçili pozisyonlar için ağırlıklı lot yoğunluğu"""
        try:
            print(f"[GRPAN] 🔍 Take Profit GRPAN analizi başlatılıyor...")
            ...
            columns = ('Symbol', 'Qty', 'GRPAN', 'Range', 'Weight', 'Conc%', 'RealLot', 'Status')
            grpan_tree = ttk.Treeview(grpan_win, columns=columns, show='headings', height=18)
            
            col_widths = {'Symbol': 100, 'Qty': 80, 'GRPAN': 90, 'Range': 140, 'Weight': 100, 'Conc%': 70, 'RealLot': 80, 'Status': 120}
            for col in columns:
            ...
            for symbol in self.selected_positions:
                ...
                if grpan_result:
                    ...
                    values = [symbol, f"{qty:,}", f"${mode_price:.2f}", range_str, weight_str, f"{concentration:.0f}%", real_str, status]
                else:
                    values = [symbol, f"{qty:,}", "N/A", "N/A", "N/A", "N/A", "N/A", "❌ NO DATA"]
                    tag = 'no_data'
            ...
            print(f"[GRPAN] ✅ Analiz tamamlandı: {len(self.selected_positions)} pozisyon")
    ...
    def calculate_qpcal(self, grpan_price, bid, ask):
        """
        Qpcal değerini hesapla - GRPAN fiyatına göre
        ...
        """
        try:
            if grpan_price is None or bid <= 0 or ask <= 0:
                return None, None, False
            
            # Uzaklıkları hesapla (mutlak değer)
            bid_distance = abs(grpan_price - bid)
            ask_distance = abs(grpan_price - ask)
            ...
            print(f"[QPcal] ❌ Qpcal hesaplama hatası: {e}")
            return None, None, False
    
    def show_qpcal_window(self):
        """Qpcal penceresi - Seçili pozisyonlar için GRPAN bazlı profit skorlama"""
        try:
            print(f"[QPcal] 🔍 Take Profit Qpcal analizi başlatılıyor...")
            ...
            qpcal_win.title("Qpcal - Q Profit Calculator (GRPAN Bazlı)")
            qpcal_win.geometry("1000x550")
            # transient ve grab_set kaldırıldı - bağımsız pencere, minimize edilebilir
            
            # Başlık
            title_label = ttk.Label(qpcal_win, text="Qpcal - GRPAN Bazlı Profit Skorlama", 
                                   font=("Arial", 12, "bold"))
            title_label.pack(pady=5)
            
            # Açıklama
            position_text = "LONG pozisyonlar" if self.position_type == "longs" else "SHORT pozisyonlar"
            info_label = ttk.Label(qpcal_win, 
                                  text=f"GRPAN fiyatına en uzak taraf (Bid/Ask) bazında profit hesaplama - {position_text}",
                                  font=("Arial", 9, 'italic'))
            ...
            # Qpcal sonuçları tablosu
            columns = ('Symbol', 'Qty', 'GRPAN', 'GRConf', 'Bid', 'Ask', 'Qpcal', 'Direction', 'Both')
            qpcal_tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=18)
            
            col_widths = {
                'Symbol': 100, 'Qty': 80, 'GRPAN': 90, 'GRConf': 60, 
                'Bid': 80, 'Ask': 80, 'Qpcal': 130, 'Direction': 80, 'Both': 50
            }
            
            for col in columns:
                qpcal_tree.heading(col, text=col)
                qpcal_tree.column(col, width=col_widths.get(col, 80), anchor='center')
            ...
            # Qpcal sonuçlarını hesapla ve topla
            qpcal_results = []
            ...
            for symbol in self.selected_positions:
                ...
                # Qpcal hesapla
                qpcal_value, direction, both_flag = self.calculate_qpcal(grpan_price, bid, ask)
            ...
            for result in qpcal_results:
                ...
                if result['qpcal'] is not None:
                    qpcal_display = f"{result['qpcal']:.2f} {result['direction']}"
                    if result['both']:
                        qpcal_display += " ✓"
                else:
                    qpcal_display = "N/A"
                ...
                # Tag belirleme - yüksek Qpcal değerleri daha parlak
                if result['qpcal'] is not None:
                    if result['qpcal'] >= 0.30:
                        tag = 'high'
                    elif result['qpcal'] >= 0.15:
                        tag = 'medium'
                    else:
                        tag = 'low'
                else:
                ...
                qpcal_tree.insert('', 'end', values=values, tags=(tag,))
            
            # Tag renkleri
            qpcal_tree.tag_configure('high', background='#32CD32')  # Lime yeşil - en iyi fırsat
            qpcal_tree.tag_configure('medium', background='#90EE90')  # Açık yeşil
            qpcal_tree.tag_configure('low', background='#FFFFE0')  # Açık sarı
            qpcal_tree.tag_configure('no_data', background='#D3D3D3')  # Gri
            
            # Sonuç bilgisi
            info_frame = ttk.Frame(qpcal_win)
            info_frame.pack(fill='x', padx=10, pady=5)
            
            high_count = sum(1 for r in qpcal_results if r['qpcal'] and r['qpcal'] >= 0.30)
            medium_count = sum(1 for r in qpcal_results if r['qpcal'] and 0.15 <= r['qpcal'] < 0.30)
            
            summary_label = ttk.Label(info_frame, 
                text=f"🎯 Yüksek fırsat (≥$0.30): {high_count} | Orta fırsat (≥$0.15): {medium_count} | Toplam: {len(qpcal_results)}",
                font=("Arial", 10))
            summary_label.pack(side='left')
            
            print(f"[QPcal] ✅ Analiz tamamlandı: {len(self.selected_positions)} pozisyon")
    ...
    def analyze_trpan(self):
        """TRPAN analiz penceresi - Seçili pozisyonlar için fiyat yoğunluğu"""
        try:
            print(f"[TRPAN] 🔍 Take Profit TRPAN analizi başlatılıyor...")
            ...
            columns = ('Symbol', 'Qty', 'MOD Price', 'Range', 'Count', 'Conc%', 'Status')
            trpan_tree = ttk.Treeview(trpan_win, columns=columns, show='headings', height=18)
            
            col_widths = {'Symbol': 100, 'Qty': 80, 'MOD Price': 90, 'Range': 140, 'Count': 70, 'Conc%': 70, 'Status': 120}
            for col in columns:
            ...
            for symbol in self.selected_positions:
                ...
                if trpan_result:
                    ...
                    range_max = mode_price + 0.04
                    range_str = f"${range_min:.2f} - ${range_max:.2f}"
                    count_str = f"{count_in_range}/{total_count}"
                    ...
                    values = [symbol, f"{qty:,}", f"${mode_price:.2f}", range_str, count_str, f"{concentration:.0f}%", status]
                else:
                    values = [symbol, f"{qty:,}", "N/A", "N/A", "N/A", "N/A", "❌ NO DATA"]
                    tag = 'no_data'
            ...
            print(f"[TRPAN] ✅ Analiz tamamlandı: {len(self.selected_positions)} pozisyon")
    ...
    def setup_ui(self):
        ...
        # Qpcal butonu - GRPAN bazlı profit skorlama
        self.btn_qpcal = ttk.Button(top_frame, text="Qpcal", 
                                   command=self.show_qpcal_window, width=8)
        self.btn_qpcal.pack(side='right', padx=2)
        
        # GRPAN butonu - Ağırlıklı Lot Yoğunluğu Analizi
        self.btn_grpan = ttk.Button(top_frame, text="GRPAN", 
                                   command=self.analyze_grpan, width=8)
        self.btn_grpan.pack(side='right', padx=2)
        
        # TRPAN butonu - Fiyat Yoğunluğu Analizi
        self.btn_trpan = ttk.Button(top_frame, text="TRPAN", 
                                   command=self.analyze_trpan, width=8)
        self.btn_trpan.pack(side='right', padx=2)
        
        # LRPAN butonu - 100/200/300 lot analizi
        self.btn_lrpan = ttk.Button(top_frame, text="LRPAN", 
                                   command=self.analyze_lrpan, width=8)
        self.btn_lrpan.pack(side='right', padx=2)
    ...
    def setup_table(self):
        ...
        if self.position_type == "longs":
            cols = ['select', 'symbol', 'qty', 'avg_cost', 'current_price', 'fbtot', 'gort', 'pnl_vs_cost', 'market_value', 'ask_sell_pahalilik', 'front_sell_pahalilik', 'outperf_chg_pct', 'timebased_bench_chg', 'avg_adv', 'maxalw', 'smi', 'final_fb', 'final_sfs', 'grup', 'avg_final_fb', 'avg_final_sfs', 'fbplagr', 'fbratgr']
            headers = ['Seç', 'Symbol', 'Qty', 'Avg Cost', 'Current', 'FBtot', 'GORT', 'PnL', 'Market Value', 'Ask Sell Pahalılık', 'Front Sell Pahalılık', 'Outperf%', 'Timebased', 'AVG_ADV', 'MAXALW', 'SMI', 'Final FB', 'Final SFS', 'Grup', 'Avg Final FB', 'Avg Final SFS', 'FBPlagr', 'FBRatgr']
        else:  # shorts
            cols = ['select', 'symbol', 'qty', 'avg_cost', 'current_price', 'sfstot', 'gort', 'pnl_vs_cost', 'market_value', 'bid_buy_ucuzluk', 'front_buy_ucuzluk', 'outperf_chg_pct', 'timebased_bench_chg', 'avg_adv', 'maxalw', 'smi', 'final_fb', 'final_sfs', 'grup', 'avg_final_fb', 'avg_final_sfs', 'sfsplagr', 'sfsratgr']
            headers = ['Seç', 'Symbol', 'Qty', 'Avg Cost', 'Current', 'SFStot', 'GORT', 'PnL', 'Market Value', 'Bid Buy Ucuzluk', 'Front Buy Ucuzluk', 'Outperf%', 'Timebased', 'AVG_ADV', 'MAXALW', 'SMI', 'Final FB', 'Final SFS', 'Grup', 'Avg Final FB', 'Avg Final SFS', 'SFSPlagr', 'SFSRatgr']
    ...
    def load_positions(self):
        ...
        try:
            ...
            for pos in filtered_positions:
                ...
                divisor = getattr(self.parent, 'rule_avg_adv_divisor', 10)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            from janallapp.myjdata import get_final_jdata_for_symbol, FINAL_JDATA_RESULTS
            ...
            if grup_value and grup_value != 'N/A':
                ...
                if self.position_type == "shorts":
                    ...
                    if sfstot == "N/A":
                        ...
                        # Mini450'den aldıysak, sfsplagr ve sfsratgr'ı da hesapla (gösterim için)
                        final_sfs_value = self.get_final_sfs_from_csv(symbol)
                        sfsplagr = self.calculate_sfsplagr(symbol, grup_value)
                        sfsratgr = self.calculate_sfsratgr(symbol, final_sfs_value, avg_final_sfs)
                else:
    ...
    def calculate_fbtot(self, fbplagr, fbratgr):
        """FBtot hesapla - Yeni formül: FBPlagr * 0.5 + FBRatgr * 1.5 (mini450 ile aynı)"""
        try:
            # FBPlagr'dan ondalık değeri çıkar (örn: "2/24 (0.08)" -> 0.08 veya direkt sayısal değer)
            fbplagr_value = 0
            if fbplagr != "N/A":
                try:
                    if "(" in str(fbplagr):
                        # Parantez içindeki ondalık değeri al
                        decimal_part = str(fbplagr).split("(")[1].split(")")[0]
                        fbplagr_value = float(decimal_part)
                    else:
                        # Direkt sayısal değer
                        fbplagr_value = float(fbplagr)
                except:
                    fbplagr_value = 0
            ...
            # Yeni formül: FBPlagr * 0.5 + FBRatgr * 1.5
            if fbplagr_value > 0 or fbratgr_value > 0:
                total = (fbplagr_value * 0.5) + (fbratgr_value * 1.5)
                result = f"{total:.4f}"
                print(f"[TAKE PROFIT] 📊 FBtot: {result} ({fbplagr_value:.4f} * 0.5 + {fbratgr_value:.4f} * 1.5)")
                return result
            else:
                return "N/A"
    ...
    def calculate_sfstot(self, sfsplagr, sfsratgr):
        """SFStot hesapla - Yeni formül: SFSPlagr * 0.5 + SFSRatgr * 1.5 (mini450 ile aynı)"""
        try:
            # SFSPlagr'dan ondalık değeri çıkar (örn: "2/24 (0.08)" -> 0.08 veya direkt sayısal değer)
            sfsplagr_value = 0
            if sfsplagr != "N/A":
                try:
                    if "(" in str(sfsplagr):
                        # Parantez içindeki ondalık değeri al
                        decimal_part = str(sfsplagr).split("(")[1].split(")")[0]
                        sfsplagr_value = float(decimal_part)
                    else:
                        # Direkt sayısal değer
                        sfsplagr_value = float(sfsplagr)
                except:
                    sfsplagr_value = 0
            ...
            # Yeni formül: SFSPlagr * 0.5 + SFSRatgr * 1.5
            if sfsplagr_value > 0 or sfsratgr_value > 0:
                total = (sfsplagr_value * 0.5) + (sfsratgr_value * 1.5)
                result = f"{total:.4f}"
                print(f"[TAKE PROFIT] 📊 SFStot: {result} ({sfsplagr_value:.4f} * 0.5 + {sfsratgr_value:.4f} * 1.5)")
                return result
            else:
                return "N/A"
