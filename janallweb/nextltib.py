...
def main():
    ...
    try:
        ...
        # Yeni çıkmış hisseleri tespit et (SMA88 veya SMA268 verisi eksik olanlar)
        sma88_missing = df['SMA88'].isna() | (df['SMA88'] == '') | df['SMA88'].isnull()
        sma268_missing = df['SMA268'].isna() | (df['SMA268'] == '') | df['SMA268'].isnull()
        new_issues_mask = sma88_missing | sma268_missing
        
        if new_issues_mask.any():
            new_issues_df = df[new_issues_mask].copy()
            new_issues_df.to_csv('newis.csv', index=False, encoding='utf-8-sig')
            
            print(f"\n=== YENİ ÇIKMIŞ HİSSELER TESPİTİ ===")
            print(f"Yeni çıkmış hisseler 'newis.csv' dosyasına kaydedildi. ({len(new_issues_df)} hisse)")
            
            # Yeni çıkmış hisseleri listele
            new_stocks = new_issues_df['PREF IBKR'].tolist()
            print("Yeni çıkmış hisseler:")
            for i, stock in enumerate(new_stocks, 1):
                sma88_status = "YOK" if pd.isna(new_issues_df[new_issues_df['PREF IBKR'] == stock]['SMA88'].iloc[0]) or new_issues_df[new_issues_df['PREF IBKR'] == stock]['SMA88'].iloc[0] == '' else "VAR"
                sma268_status = "YOK" if pd.isna(new_issues_df[new_issues_df['PREF IBKR'] == stock]['SMA268'].iloc[0]) or new_issues_df[new_issues_df['PREF IBKR'] == stock]['SMA268'].iloc[0] == '' else "VAR"
                print(f"{i:2d}. {stock:<10} (SMA88: {sma88_status}, SMA268: {sma268_status})")
            print("=" * 50)
        else:
