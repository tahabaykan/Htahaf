...
try:
    df = pd.read_csv('extlt_results.csv', sep=',')  # CSV ayracını belirt
...
try:
    ...
    print(df[['PREF IBKR', price_column, 
              ...
              '1Y Low', '1Y_Low_diff', '1Y_Low_diff_norm']].head(10))
              
    # En yüksek normalize değerlerine sahip hisseleri göster
    ...
    df.to_csv('normalized_extlt.csv', 
             index=False,
             float_format='%.6f',
             sep=',',                
             encoding='utf-8-sig',   
    ...
    print("\nSonuçlar 'normalized_extlt.csv' dosyasına kaydedildi.")
