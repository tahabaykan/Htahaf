...
def calculate_final_thg(df):
    ...
    try:
        ...
        # Yeni çıkmış olabilecek hisseleri tespit et (SMA88 ve SMA268 verisi eksik olanlar)
        sma88_missing = df['SMA88_chg_norm'].isna()
        sma268_missing = df['SMA268_chg_norm'].isna()
        
        print("\n=== YENİ ÇIKMIŞ OLABİLECEK HİSSELER ===")
        print("SMA88_chg_norm verisi eksik olanlar:")
        if sma88_missing.any():
            missing_sma88_stocks = df[sma88_missing]['PREF IBKR'].tolist()
            print(f"Toplam {len(missing_sma88_stocks)} hisse:")
            for i, stock in enumerate(missing_sma88_stocks, 1):
                print(f"{i:2d}. {stock}")
        else:
            print("Yok")
            
        print("\nSMA268_chg_norm verisi eksik olanlar:")
        if sma268_missing.any():
            missing_sma268_stocks = df[sma268_missing]['PREF IBKR'].tolist()
            print(f"Toplam {len(missing_sma268_stocks)} hisse:")
            for i, stock in enumerate(missing_sma268_stocks, 1):
                print(f"{i:2d}. {stock}")
        else:
            print("Yok")
            
        # Hem SMA88 hem SMA268 verisi eksik olanlar (çok yeni hisseler)
        both_missing = sma88_missing & sma268_missing
        print("\nHem SMA88 hem SMA268 verisi eksik olanlar (ÇOK YENİ HİSSELER):")
        if both_missing.any():
            very_new_stocks = df[both_missing]['PREF IBKR'].tolist()
            print(f"Toplam {len(very_new_stocks)} hisse:")
            for i, stock in enumerate(very_new_stocks, 1):
                print(f"{i:2d}. {stock}")
                
            # Bu hisselerin detaylarını göster
            print("\nÇok yeni hisselerin detayları:")
            print("PREF IBKR   FF-FR_   CMON    COUPON   DIV AMOUNT")
            print("-" * 55)
            for stock in very_new_stocks:
                stock_data = df[df['PREF IBKR'] == stock].iloc[0]
                ff_fr = stock_data.get('FF-FR_', 'N/A')
                cmon = stock_data.get('CMON', 'N/A')
                coupon = stock_data.get('COUPON', 'N/A')
                div_amount = stock_data.get('DIV AMOUNT', 'N/A')
                print(f"{stock:<10} {ff_fr:<7} {cmon:<7} {coupon:<8} {div_amount}")
        else:
            print("Yok")
        print("=" * 50)
        ...
        # TIME TO DIV hesaplama ve exponential skorlama
        print("\nTIME TO DIV hesaplanıyor ve exponential skorlama uygulanıyor...")
        
        # EX-DIV DATE'den TIME TO DIV hesapla
        today = datetime.now()
        df['EX_DIV_DATE'] = pd.to_datetime(df['EX-DIV DATE'], errors='coerce')
        df['TIME_TO_DIV'] = (df['EX_DIV_DATE'] - today).dt.days
        
        # TIME TO DIV exponential skorlama (katsayı: 1.5)
        def calculate_div_bonus(time_to_div):
            if pd.isna(time_to_div) or time_to_div < 0:
                return 0
            
            # TIME TO DIV arttıkça bonus azalmalı
            if time_to_div <= 2:
                return 60.0 * 1.5  # 90 puan (maksimum)
            elif time_to_div <= 25:
                # 2-25 gün arasında hızlı azalış
                # Formül: 90 - (time_to_div - 2) * 2.0 * 1.5
                bonus = 90.0 - (time_to_div - 2) * 2.0 * 1.5
                return max(round(bonus, 2), 0)  # Minimum 0
            elif time_to_div <= 89:
                # 25-89 gün arasında yavaş azalış
                # Formül: 50 - (time_to_div - 25) * 0.5 * 1.5
                bonus = 50.0 - (time_to_div - 25) * 0.5 * 1.5
                return max(round(bonus, 2), 0)  # Minimum 0
            else:
                return 0  # Minimum bonus
        
        df['DIV_BONUS'] = df['TIME_TO_DIV'].apply(calculate_div_bonus)
        
        # FINAL_THG'ye DIV_BONUS ekle
        df['FINAL_THG'] = df['FINAL_THG'] + df['DIV_BONUS']
        df['FINAL_THG'] = df['FINAL_THG'].round(2)
        
        # TIME TO DIV skorlama örnekleri yazdır
        print("\n=== TIME TO DIV Yeni Skorlama Örnekleri (Katsayı: 1.5) ===")
        test_times = [0, 1, 2, 5, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 89, 90]
        print("TIME_TO_DIV  DIV_BONUS")
        print("-" * 25)
        for time in test_times:
            bonus = calculate_div_bonus(time)
            print(f"{time:>10}  {bonus:>8.2f}")
...
def main():
    try:
        ...
        result_df.loc[ff_mask, 'LFINAL_THG'] = (
            result_df.loc[ff_mask, 'FINAL_THG'] * 0.4 +  # FINAL_THG'yi 0.4 ile çarp
            result_df.loc[ff_mask, 'Expannr'] * 17  # Expannr'ı 17 ile çarp
        ).round(4)
        
        # FLR hisseleri için hesaplama
        result_df.loc[flr_mask, 'LFINAL_THG'] = (
            result_df.loc[flr_mask, 'FINAL_THG'] * 0.2 +  # FINAL_THG'yi 0.2 ile çarp
            result_df.loc[flr_mask, 'Expannr'] * 19  # Expannr'ı 19 ile çarp
        ).round(4)
        
        # NFF hisseleri için hesaplama
        result_df.loc[nff_mask, 'LFINAL_THG'] = (
            result_df.loc[nff_mask, 'YIELD_TO_CALL'] * 550 +  # Yield to Call'ı 550 ile çarp
            result_df.loc[nff_mask, 'FINAL_THG'] * 0.35 +  # FINAL_THG'yi 0.35 ile çarp
            result_df.loc[nff_mask, 'Expannr'] * 14  # Expannr'ı 14 ile çarp
        ).round(4)
        
        # Matur hisseleri için hesaplama
        result_df.loc[matur_mask, 'LFINAL_THG'] = (
            result_df.loc[matur_mask, 'YIELD_TO_MATUR'] * 550 +  # Yield to Maturity'yi 550 ile çarp
            result_df.loc[matur_mask, 'FINAL_THG'] * 0.5 +  # FINAL_THG'yi 0.5 ile çarp
        ...
        result_df.loc[normal_mask, 'LFINAL_THG'] = (
            result_df.loc[normal_mask, 'FINAL_THG'] * 0.4 +  # FINAL_THG'yi 0.4 ile çarp
            result_df.loc[normal_mask, 'Expannr'] * 10  # Expannr'ı 10 ile çarp
        ).round(4)
        ...
        # Yeni çıkmış hisseler (SMA88 veya SMA268 verisi eksik olanlar)
        sma88_missing = result_df['SMA88_chg_norm'].isna()
        sma268_missing = result_df['SMA268_chg_norm'].isna()
        new_issues_mask = sma88_missing | sma268_missing
        new_issues_df = None
        
        if new_issues_mask.any():
            new_issues_df = result_df[new_issues_mask].copy()
            new_issues_df = new_issues_df.drop_duplicates(subset=['PREF IBKR'], keep='first')
            new_issues_df.to_csv('newis_extlt.csv', index=False, float_format='%.4f', encoding='utf-8-sig')
            print(f"Yeni çıkmış hisseler 'newis_extlt.csv' dosyasına kaydedildi. ({len(new_issues_df)} hisse)")
            
            # Yeni çıkmış hisselerin detaylarını yazdır
            print(f"\n=== YENİ ÇIKMIŞ HİSSELER (newis_extlt.csv) ===")
            print("PREF IBKR   FF-FR_   CMON     COUPON   DIV AMT   SMA88_miss  SMA268_miss")
            print("-" * 75)
            for _, row in new_issues_df.iterrows():
                ff_fr = str(row.get('FF-FR_', 'N/A'))[:6]
                cmon = str(row.get('CMON', 'N/A'))[:7]
                coupon = f"{row.get('COUPON', 0):.2f}" if pd.notna(row.get('COUPON')) else 'N/A'
                div_amount = f"{row.get('DIV AMOUNT', 0):.3f}" if pd.notna(row.get('DIV AMOUNT')) else 'N/A'
                sma88_miss = 'YES' if pd.isna(row['SMA88_chg_norm']) else 'NO'
                sma268_miss = 'YES' if pd.isna(row['SMA268_chg_norm']) else 'NO'
                print(f"{row['PREF IBKR']:<10} {ff_fr:<7} {cmon:<8} {coupon:<8} {div_amount:<9} {sma88_miss:<10} {sma268_miss}")
        else:
            print("Yeni çıkmış hisse bulunamadı.")
        ...
        # Yeni çıkmış hisseler kategorisini de ekle
        if new_issues_df is not None:
            categories['Yeni Çıkmış'] = new_issues_df
        
        for category, df in categories.items():
            if len(df) > 0:
                # En yüksek FINAL_THG skorları (Top 10)
                print(f"\n=== {category} Hisseleri - En Yüksek FINAL_THG Skorları (Top 10) ===")
                if category == 'Matur':
                    print("PREF IBKR  FINAL_THG  LFINAL_THG  Expannr  YIELD_TO_MATUR")
                elif category == 'NFF':
                    print("PREF IBKR  FINAL_THG  LFINAL_THG  Expannr  YIELD_TO_CALL")
                else:  # FF, FLR ve Normal hisseler için
                    print("PREF IBKR  FINAL_THG  LFINAL_THG  Expannr")
                print("-" * 80)
                
                if category == 'Matur':
                    top_10 = df.nlargest(10, 'FINAL_THG')[
                        ['PREF IBKR', 'FINAL_THG', 'LFINAL_THG', 'Expannr', 'YIELD_TO_MATUR']
                    ]
                    for _, row in top_10.iterrows():
                        print(f"{row['PREF IBKR']:<10} {row['FINAL_THG']:>10.4f} {row['LFINAL_THG']:>10.4f} "
                              f"{row['Expannr']:>8.4f} {row['YIELD_TO_MATUR']:>12.4f}")
                elif category == 'NFF':
                    top_10 = df.nlargest(10, 'FINAL_THG')[
                        ['PREF IBKR', 'FINAL_THG', 'LFINAL_THG', 'Expannr', 'YIELD_TO_CALL']
                    ]
                    for _, row in top_10.iterrows():
                        print(f"{row['PREF IBKR']:<10} {row['FINAL_THG']:>10.4f} {row['LFINAL_THG']:>10.4f} "
                    ...
                    top_10 = df.nlargest(10, 'FINAL_THG')[
                        ['PREF IBKR', 'FINAL_THG', 'LFINAL_THG', 'Expannr']
                    ]
                    for _, row in top_10.iterrows():
                        print(f"{row['PREF IBKR']:<10} {row['FINAL_THG']:>10.4f} {row['LFINAL_THG']:>10.4f} "
                              f"{row['Expannr']:>8.4f}")
                
                # En düşük FINAL_THG skorları (Bottom 10)
                print(f"\n=== {category} Hisseleri - En Düşük FINAL_THG Skorları (Bottom 10) ===")
                if category == 'Matur':
                    print("PREF IBKR  FINAL_THG  LFINAL_THG  Expannr  YIELD_TO_MATUR")
                elif category == 'NFF':
                    print("PREF IBKR  FINAL_THG  LFINAL_THG  Expannr  YIELD_TO_CALL")
                ...
                if category == 'Matur':
                    bottom_10 = df.nsmallest(10, 'FINAL_THG')[
                        ['PREF IBKR', 'FINAL_THG', 'LFINAL_THG', 'Expannr', 'YIELD_TO_MATUR']
                    ]
