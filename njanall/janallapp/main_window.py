...
from .path_helper import get_csv_path, NJANALL_BASE_DIR
...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        self.exception_manager = ExceptionListManager(get_csv_path("exception_list.csv"))
    ...
    def load_main_csv_on_startup(self):
        ...
        try:
            csv_file = get_csv_path('janalldata.csv')
    ...
    def setup_ui(self):
        ...
        self.btn_prev_close = ttk.Button(top_frame, text="Prev Close Çek", command=self.fetch_all_prev_close)
        ...
        self.btn_mypos = ttk.Button(top_frame, text="Pozisyonlarım", width=14,
        ...
        btn_main = ttk.Button(main_frame, text="JANALLDATA", 
                            command=lambda: self.show_file_data(get_csv_path('janalldata.csv'), is_main=True))
        btn_main.pack(side='left', padx=2)
        
        # Mini450 butonu - Tüm hisseleri mini görünümde göster
        btn_mini450 = ttk.Button(main_frame, text="🔍 mini450", 
                                command=self.show_mini450_view, 
                                style='Accent.TButton')
        btn_mini450.pack(side='left', padx=5)
        
        # Psfalgo butonu - Pozisyon yönetimi robotu
        btn_psfalgo = ttk.Button(main_frame, text="🤖 Psfalgo", 
        ...
        self.btn_lot_divider = ttk.Button(main_frame, text="📦 Lot Bölücü: OFF", 
                                        command=self.toggle_lot_divider, 
                                        style='Accent.TButton')
        self.btn_lot_divider.pack(side='left', padx=5)
        
        # Exposure bilgisi gösterimi - Lot Bölücü yanında
        self.exposure_label = ttk.Label(main_frame, text="HAMPRO mod açık - Long: 0.00 | Short: 0.00 | Total: 0.00", 
        ...
        for i, file in enumerate(csv_files):
            ...
            btn = ttk.Button(current_frame, text=short_name, command=lambda f=file: self.show_file_data(get_csv_path(f)))
    ...
    def get_benchmark_change_for_ticker(self, ticker):
        ...
        try:
            ...
            # Benchmark shift değerini uygula (General BM Shifter)
    ...
    def toggle_live_data(self):
        ...
        if not self.live_data_running:
            ...
            self.show_file_data(get_csv_path('janalldata.csv'), is_main=True)
    ...
    def fetch_all_prev_close(self):
        """Tüm hisseler için previous close değerlerini çek - KALDIRILDI!"""
    ...
    def fetch_prev_close_for_ticker(self, ticker):
        """Bir hisse için previous close değerini çek - KALDIRILDI!"""
        print(f"[PREV CLOSE] 🚫 PREV CLOSE ÇEKME İŞLEMİ TAMAMEN KALDIRILDI: {ticker}")
        return
    ...
    def load_prev_close_from_csv(self):
        ...
        try:
            ...
            csv_files = []
            for file in os.listdir(NJANALL_BASE_DIR):
                if file.startswith('janek_ss') and file.endswith('.csv'):
                    csv_files.append(get_csv_path(file))
    ...
    def update_live_data(self):
        ...
        # GUI güncellemesini daha az sıklıkta yap (mini450'de)
        if hasattr(self, 'is_mini450_active') and self.is_mini450_active:
            # Mini450 modunda GUI güncellemesini yavaşlat
            self.update_table()
    ...
    def get_final_thg_for_symbol(self, symbol: str) -> float:
        ...
        try:
            ...
            # Tüm janek_ss dosyalarını bul (njanall dizininde)
            janek_pattern = os.path.join(NJANALL_BASE_DIR, 'janek_ss*.csv')
            janek_files = glob.glob(janek_pattern)
    ...
    def top_ten_bid_buy(self):
        """Top Ten Bid Buy - En yüksek Final_BB_skorlu %10 hisse için bid buy emirleri"""
        try:
            print("[TOP TEN] 🔄 Top Ten Bid Buy başlatılıyor...")
            
            # CSV dosyasını sıfırla (yeni işlem başlıyor)
            self.reset_pref_csv()
            
            # Tüm skorları hesapla
            self.calculate_scores_for_all_stocks()
            
            # Grup dosyaları (belirtilen sırayla)
            ...
            for group in groups:
                try:
                    print(f"[TOP TEN] 🔄 {group} grubu işleniyor...")
                    
                    # GRUP BUTONUNA BASTIĞINDA AÇILAN PENCEREDEKİ DATAFRAME'İ BUL
                    group_window = self.find_group_window(group)
                    if group_window and hasattr(group_window, 'df'):
                        group_df = group_window.df  # Açılan penceredeki DataFrame
                        print(f"[TOP TEN] 📋 {group}: Açılan penceredeki DataFrame'de {len(group_df)} hisse var")
                        
                        # Debug: Açılan penceredeki DataFrame'in ilk birkaç hissesini göster
                        if len(group_df) > 0:
                            first_tickers = group_df['PREF IBKR'].head(5).tolist()
                            print(f"[DEBUG] Açılan penceredeki DataFrame'de ilk 5 hisse: {first_tickers}")
                            
                            # Debug: Açılan penceredeki DataFrame'de skor kolonları var mı kontrol et
                            if 'Final_BB_skor' in group_df.columns:
                                print(f"[DEBUG] ✅ Açılan penceredeki DataFrame'de Final_BB_skor kolonu var")
                                # İlk birkaç hissenin skorlarını göster
                                ...
                                print(f"[DEBUG] ❌ Açılan penceredeki DataFrame'de Final_BB_skor kolonu yok!")
                                print(f"[DEBUG] Mevcut kolonlar: {list(group_df.columns)}")
                                
                                # Skorlar yoksa hesapla!
                                print(f"[DEBUG] 🔄 {group} penceresinde skorlar hesaplanıyor...")
                                group_window.calculate_scores_for_all_stocks()
                                print(f"[DEBUG] ✅ {group} penceresinde skorlar hesaplandı")
                                
                                # Şimdi tekrar kontrol et
                                if 'Final_BB_skor' in group_df.columns:
                                    print(f"[DEBUG] ✅ Artık Final_BB_skor kolonu var!")
                                else:
                                    print(f"[DEBUG] ❌ Hala Final_BB_skor kolonu yok!")
                    else:
                        print(f"[TOP TEN] ⚠️ {group}: Açılan pencere bulunamadı, butona tıklanıyor...")
                        
                        # GRUP BUTONUNU BUL VE TIKLA!
                        group_button = self.find_group_button(group)
                        if group_button:
                            print(f"[TOP TEN] 🔘 {group} butonu bulundu, tıklanıyor...")
                            group_button.invoke()  # Butona tıkla
                            
                            # Pencere açılsın
                            import time
                            time.sleep(1)
                            
                            # Şimdi tekrar pencereyi bul
                            print(f"[TOP TEN] 🔍 {group} penceresi tekrar aranıyor...")
                            group_window = self.find_group_window(group)
                            if group_window and hasattr(group_window, 'df'):
                                group_df = group_window.df
                                print(f"[TOP TEN] ✅ {group}: Pencere açıldı ve bulundu! {len(group_df)} hisse")
                            else:
                                print(f"[TOP TEN] ❌ {group}: Pencere hala bulunamadı, ana DataFrame kullanılıyor")
                                group_df = self.df
                        else:
                            print(f"[TOP TEN] ❌ {group} butonu bulunamadı, ana DataFrame kullanılıyor")
                            group_df = self.df
                        
                        # Debug: Ana DataFrame'deki ilk birkaç hisseyi göster
                        if len(group_df) > 0:
                            first_tickers = group_df['PREF IBKR'].head(5).tolist()
                            print(f"[DEBUG] Ana DataFrame'de ilk 5 hisse: {first_tickers}")
                            
                            # Debug: Ana DataFrame'de skor kolonları var mı kontrol et
                            if 'Final_BB_skor' in group_df.columns:
                                print(f"[DEBUG] ✅ Ana DataFrame'de Final_BB_skor kolonu var")
                            else:
                                print(f"[DEBUG] ❌ Ana DataFrame'de Final_BB_skor kolonu yok!")
                        
                        # Debug: Ana DataFrame'de hangi kolonlar var kontrol et
                        print(f"[DEBUG] Ana DataFrame'deki kolonlar: {list(group_df.columns)}")
                    
                    # TÜM SAYFALARI TARA - SADECE İLK SAYFA DEĞİL!
                    print(f"[TOP TEN] 🔍 {group}: Tüm sayfalar taranıyor...")
                    
                    # Toplam sayfa sayısını al
                    total_pages = getattr(group_window, 'total_pages', 1)
                    print(f"[TOP TEN] 📄 {group}: Toplam {total_pages} sayfa bulundu")
                    
                    # Tüm sayfalardaki hisseleri topla
                    all_group_stocks = []
                    
                    for page in range(total_pages):
                        print(f"[TOP TEN] 📄 {group}: Sayfa {page + 1}/{total_pages} taranıyor...")
                        
                        # Sayfayı değiştir
                        if hasattr(group_window, 'go_to_page'):
                            group_window.go_to_page(page)
                            # Sayfa değişince biraz bekle
                            import time
                            time.sleep(1)
                        ...
                        for index, row in current_page_df.iterrows():
                            ...
                            # SKORU KENDİMİZ HESAPLAYALIM - DATAFRAME'DEN OKUMAYALIM!
                            print(f"[DEBUG] {ticker}: Skor hesaplanıyor...")
                            ...
                            if market_data:
                                ...
                                if scores and 'Final_BB_skor' in scores:
                                    ...
                                    print(f"[DEBUG] {ticker}: Skor hesaplanamadı")
                                    final_bb_skor = 0
                            else:
                                print(f"[DEBUG] {ticker}: Market data alınamadı")
                                final_bb_skor = 0
                    ...
                    print(f"[TOP TEN] 📊 {group}: Tüm sayfalardan toplam {len(all_group_stocks)} hisse bulundu")
                    
                    # Duplicate hisseleri kaldır
                    group_stocks = []
                    seen_symbols = set()
                    ...
                    print(f"[TOP TEN] 📊 {group}: Unique {len(group_stocks)} hisse (duplicate'ler kaldırıldı)")
                    
                    if len(group_stocks) == 0:
                        print(f"[TOP TEN] ⚠️ {group}: Uygun hisse bulunamadı")
                        continue
                    ...
                    print(f"[TOP TEN] ✅ {group}: {len(top_stocks)} hisse seçildi (top %10)")
                    
                    # Onay penceresi aç
                    symbols = [stock['symbol'] for stock in top_stocks]
                    scores = [stock['Final_BB_skor'] for stock in top_stocks]
                    
                    # Onay penceresi aç ve kullanıcının yanıtını bekle
                    ...
                    print(f"[TOP TEN] ❌ {group} hatası: {e}")
                    continue
                    
        except Exception as e:
            print(f"[TOP TEN] ❌ Genel hata: {e}")
    ...
    def find_group_button(self, group):
        ...
        try:
            ...
            print(f"[TOP TEN] ❌ Buton arama hatası: {e}")
            return None
    
    def find_group_window(self, group):
        """Grup penceresini bul - Tüm açık pencereleri tara"""
        try:
            print(f"[DEBUG] {group} grubu için pencere aranıyor...")
            
            # TÜM AÇIK PENCERELERİ BUL - FARKLI YÖNTEMLER DENE
            import tkinter as tk
            
            # 1. YÖNTEM: Ana penceredeki tüm child widget'ları tara
            all_windows = []
            
            # Ana pencereyi ekle
            all_windows.append(self)
            
            # Ana penceredeki tüm child widget'ları recursive olarak tara
            ...
            print(f"[DEBUG] Bulunan toplam widget sayısı: {len(all_windows)}")
            print(f"[DEBUG] Widget türleri: {[w.__class__.__name__ for w in all_windows[:10]]}...")
            
            # 2. YÖNTEM: Her widget'ı kontrol et
            for window in all_windows:
                try:
                    # Title var mı kontrol et
                    if hasattr(window, 'title'):
                        title = window.title()
                        print(f"[DEBUG] Widget title: {title}")
                        
                        # DataFrame var mı kontrol et
                        if hasattr(window, 'df'):
                            print(f"[DEBUG] Widget'da df var, uzunluk: {len(window.df)}")
                            
                            # Grup adını title'da ara
                            if group.lower() in title.lower():
                                print(f"[DEBUG] ✅ {group} grubu için pencere bulundu: {title}")
                                return window
                            else:
                                print(f"[DEBUG] ❌ {group} grubu için uygun değil: {title}")
                        else:
                    ...
                    print(f"[DEBUG] Widget kontrol hatası: {e}")
                    continue
            
            # 3. YÖNTEM: Toplevel pencereleri de tara
            print(f"[DEBUG] Toplevel pencereler aranıyor...")
                    ...
                    if isinstance(widget, tk.Toplevel):
                        ...
                        if hasattr(widget, 'title'):
                            ...
                            if hasattr(widget, 'df') and group.lower() in title.lower():
                                print(f"[DEBUG] ✅ {group} grubu için Toplevel pencere bulundu: {title}")
                                return widget
                except:
                    continue
            
            print(f"[DEBUG] ❌ {group} grubu için hiç pencere bulunamadı!")
            return None
            
        except Exception as e:
            print(f"[TOP TEN] ❌ Pencere arama hatası: {e}")
            return None
    
    def bottom_ten_ask_sell(self):
        """Bottom Ten Ask Sell - En düşük Final_SAS_skorlu %10 hisse için ask sell emirleri"""
        try:
            print("[BOTTOM TEN] 🔄 Bottom Ten Ask Sell başlatılıyor...")
            ...
            for group in groups:
                try:
                    print(f"[BOTTOM TEN] 🔄 {group} grubu işleniyor...")
                    
                    # CSV dosyasını yükle
                    csv_file = get_csv_path(f"janek_ssfinek{group}.csv")
                    
                    # Dosya var mı kontrol et
                    if not os.path.exists(csv_file):
                        print(f"[BOTTOM TEN] ⚠️ {csv_file} dosyası bulunamadı")
                        continue
                    
                    # Dosyayı yükle ve DataFrame'e aktar
                    self.show_file_data(csv_file)
                    
                    # Tüm sayfaları tara
                    group_stocks = []
                    current_page = 0
                    max_pages = 50  # Güvenlik için maksimum sayfa sayısı
                    
                    while current_page < max_pages:
                        # Mevcut sayfadaki hisseleri işle
                        for index, row in self.df.iterrows():
                            ticker = row['PREF IBKR']
                        ...
                        # Eğer sayfa değişmediyse (son sayfa) döngüyü kır
                        if len(self.df) == 0:
                            break
                    
                    print(f"[BOTTOM TEN] 📊 {group}: Toplam {len(group_stocks)} hisse bulundu")
                    
                    if len(group_stocks) == 0:
                        print(f"[BOTTOM TEN] ⚠️ {group}: Grupta hisse bulunamadı")
                        continue
                    
                    # Duplicate hisseleri kaldır (aynı symbol'den sadece birini al)
                    unique_stocks = []
                    seen_symbols = set()
                    ...
                    print(f"[BOTTOM TEN] 📊 {group}: Unique {len(unique_stocks)} hisse (duplicate'ler kaldırıldı)")
                    
                    if len(unique_stocks) == 0:
                        print(f"[BOTTOM TEN] ⚠️ {group}: Unique hisse bulunamadı")
                        continue
                    
                    # En düşük %10'u seç (min 1, max 8) - unique_stocks üzerinden
                    total_unique_stocks = len(unique_stocks)
                    select_count = max(1, min(8, round(total_unique_stocks * 0.1)))
                    
                    # Final_SAS_skor'a göre sırala (düşükten yükseğe - en düşük skorlular)
                    unique_stocks.sort(key=lambda x: x['Final_SAS_skor'])
                    bottom_stocks = unique_stocks[:select_count]
                    
                    print(f"[BOTTOM TEN] ✅ {group}: {len(bottom_stocks)} hisse seçildi (bottom %10)")
            ...
            print(f"[BOTTOM TEN] ❌ Genel hata: {e}")
    ...
    def show_order_confirmation(self, title, symbols, scores, order_type, group):
        ...
        def save_to_trades_csv():
            ...
            try:
                ...
                csv_filename = get_csv_path('trades.csv')
        ...
        ttk.Button(button_frame, text="Seçili Emirleri Gönder", command=confirm_order, 
                  style='Accent.TButton').pack(side='left', padx=5)
        ttk.Button(button_frame, text="trades.csv'ye Kaydet", command=save_to_trades_csv, 
                  style='Accent.TButton').pack(side='left', padx=5)
        ttk.Button(button_frame, text="İptal Et", command=cancel_order).pack(side='left', padx=5)
    ...
    def reset_trades_csv(self):
        ...
        try:
            ...
            if os.path.exists(csv_filename):
                ...
                import time
                backup_filename = get_csv_path(f'trades_backup_{int(time.time())}.csv')
    ...
    def show_mini450_view(self):
        ...
        """
        try:
            print("🔍 MİNİ450 GÖRÜNÜMÜ AÇILIYOR...")
            print("=" * 60)
            
            # Mini450 aktif flag'ini set et (performans optimizasyonu için)
            ...
            janalldata_path = get_csv_path('janalldata.csv')
            if not os.path.exists(janalldata_path):
            ...
            # CSV'yi yükle
            df = pd.read_csv(janalldata_path)
            print(f"📊 JANALLDATA yüklendi: {len(df)} hisse")
            ...
            self.show_file_data(janalldata_path, is_main=True)
            ...
            print(f"❌ Mini450 görünümü hatası: {e}")
            messagebox.showerror("Hata", f"Mini450 görünümü hatası: {e}")
    ...
    def add_normal_view_button(self):
        ...
        if not hasattr(self, 'normal_view_button'):
            ...
            if hasattr(self, 'files_frame'):
                ...
                self.normal_view_button = ttk.Button(main_frame, text="🔙 Normal Görünüm", 
    ...
    def restore_normal_view(self):
        ...
        try:
            # Mini450 flag'ini sıfırla (performans optimizasyonu)
            self.is_mini450_active = False
            ...
            if hasattr(self, 'original_items_per_page'):
                ...
                # Başlığı güncelle
                self.title("JanAll - Normal Görünüm")
        ...
        while self.background_update_running:
            try:
                if self.is_mini450_active and hasattr(self, 'df') and not self.df.empty:
                    # Mini450 aktifken arka planda data cache'i güncelle
...
• Final_FB_skor, Final_SFS_skor gibi verileri görebilirsiniz"""
    ...
    def start_psfalgo_robot(self):
        ...
        try:
            ...
            self.psfalgo_window.title("Psfalgo Robot - Pozisyon Yönetimi")
            ...
            # Benchmark shift değeri (General BM Shifter için)
            # CSV'den yükle, yoksa 0.0 default
            ...
            # JFIN filtrelerini CSV'den yükle (UI oluşturulduktan sonra)
    ...
    def setup_psfalgo_ui(self):
        ...
        # Başlık
        title_label = ttk.Label(title_frame, text="Psfalgo Robot - Pozisyon Yönetimi", 
                               font=("Arial", 14, "bold"))
        title_label.pack(side='left')
        
        # Sağ üst frame (filtreler ve kontroller için)
        right_top_frame = ttk.Frame(title_frame)
        right_top_frame.pack(side='right')
        
        # JFIN Filtreleri çerçevesi (sağ üste, kompakt)
        filters_frame = ttk.LabelFrame(right_top_frame, text="🔍 JFIN Filtreleri", padding=5)
        filters_frame.pack(side='left', padx=5)
        
        # Long Filters (BB ve FB için) - kompakt tek satır
        ...
        # Short Filters (SAS ve SFS için) - kompakt tek satır
        ...
        save_filters_btn = ttk.Button(filters_frame, text="💾 Kaydet", width=8,
        ...
        ttk.Label(exposure_frame, text="Ortalama Hisse Fiyatı ($):").grid(row=0, column=2, padx=5, pady=5, sticky='w')
        ...
        # Butonlar için yeni frame (Ortalama Hisse Fiyatı'nın yanında)
        buttons_frame = ttk.Frame(exposure_frame)
        buttons_frame.grid(row=0, column=4, rowspan=2, padx=10, pady=5, sticky='nw')
        
        # Butonları grid olarak düzenle (2 satır x 3 kolon)
        # İlk satır
        self.controller_btn = ttk.Button(buttons_frame, text="🎛️ Controller: OFF", 
        ...
        self.threshold_label = ttk.Label(exposure_frame, text="Defansif Eşik: 52,545 lot | Ofansif Dönüş: 50,909 lot", 
        ...
        def calculate_exposure():
            try:
                ...
                self.threshold_label.config(text=f"Defansif Eşik: {defensive_threshold:,} lot | Ofansif Dönüş: {offensive_threshold:,} lot")
            except ValueError:
                pass
        ...
        self.start_btn = ttk.Button(control_frame, text="Robot Başlat", 
        ...
        # Log alanı
        log_frame = ttk.LabelFrame(self.psfalgo_window, text="Robot Logları")
    ...
    def check_jfin_daily_change_limit(self, symbol, current_qty, open_orders_qty, order_qty, order_side, gün_başı_pozisyon):
        """
        JFIN Hesaplanan Lot × 2 kontrolü - ADDNEWPOS için günlük değişim limiti
        
        Kural: Hesaplanan Lot × 2 > Bugünkü Değişim olmalı
        
        Args:
            symbol: Hisse sembolü
            current_qty: Mevcut pozisyon
            open_orders_qty: Açık emirler toplamı
            order_qty: İstenen emir miktarı
            order_side: BUY/SELL
            gün_başı_pozisyon: Gün başı pozisyon
        ...
        try:
            ...
            # ADDNEWPOS şu an sadece BB ile çalışıyor, bu yüzden 'BB' sekmesini kullan
            jfin_tab = 'BB'  # Varsayılan olarak BB (ADDNEWPOS şu an sadece BB kullanıyor)
            ...
            # Eğer JFIN bilgisi bulunamadıysa kontrol atla
            if jfin_calculated_lot <= 0:
                return order_qty, "JFIN hesaplanan lot bilgisi bulunamadı - kontrol atlandı"
            
            # Limit = Hesaplanan Lot × 2
            daily_change_limit = jfin_calculated_lot * 2
            
            # Mevcut potansiyel pozisyon (açık emirler dahil)
            current_potential = current_qty + open_orders_qty
            
            # Bugünkü değişim hesapla (mutlak değerle)
            if gün_başı_pozisyon is not None:
                bugünkü_değişim = abs(current_potential - gün_başı_pozisyon)
            else:
                # Gün başı pozisyon bilgisi yoksa, mevcut pozisyonu kullan
                bugünkü_değişim = abs(current_potential)
            
            # Yeni emir eklendikten sonra bugünkü değişim
            if order_side.upper() == 'BUY':
            ...
            # Kontrol: Potential Daily Change <= Limit olmalı
            if potential_daily_change > daily_change_limit:
                # Limit aşılıyor, ne kadar eklenebilir?
                if bugünkü_değişim >= daily_change_limit:
                    # Zaten limit dolu
                    allowed_qty = 0
                    reason = f"JFIN günlük değişim limiti: Zaten limit dolu (Bugünkü değişim: {bugünkü_değişim:.0f} >= Limit: {daily_change_limit:.0f}, Hesaplanan Lot: {jfin_calculated_lot:.0f})"
                else:
                    # Kalan kapasite
                    remaining_capacity = daily_change_limit - bugünkü_değişim
                    
                    # Eğer kalan kapasite 200'den azsa, emir gönderilmemeli
                    if remaining_capacity < 200:
                        allowed_qty = 0
                        reason = f"JFIN günlük değişim limiti: Kalan kapasite {remaining_capacity:.0f} < 200 lot - emir engellendi (Bugünkü değişim: {bugünkü_değişim:.0f}, Limit: {daily_change_limit:.0f}, Hesaplanan Lot: {jfin_calculated_lot:.0f})"
                    else:
                        # Emir miktarını kalan kapasiteye göre ayarla
                        allowed_qty = min(order_qty, remaining_capacity)
                        if allowed_qty < order_qty:
                            reason = f"JFIN günlük değişim limiti: Emir {order_qty} → {allowed_qty:.0f} lot'a düşürüldü (Bugünkü değişim: {bugünkü_değişim:.0f}, Limit: {daily_change_limit:.0f}, Hesaplanan Lot: {jfin_calculated_lot:.0f})"
                        else:
                            reason = f"JFIN günlük değişim limiti OK"
            else:
                # Limit içinde
                allowed_qty = order_qty
                reason = f"JFIN günlük değişim limiti OK (Bugünkü değişim: {bugünkü_değişim:.0f}, Limit: {daily_change_limit:.0f}, Hesaplanan Lot: {jfin_calculated_lot:.0f})"
            
            allowed_qty = max(0, allowed_qty)  # Negatif olamaz
            return allowed_qty, reason
            
        except Exception as e:
            print(f"[CONTROLLER] ❌ JFIN günlük değişim limit kontrol hatası ({symbol}): {e}")
            import traceback
            traceback.print_exc()
            return order_qty, f"Hata: {e}"
    ...
    def controller_check_order(self, symbol, order_side, requested_qty):
        ...
        try:
            ...
            # 3.5. JFIN HESAPLANAN LOT × 2 KONTROLÜ (ADDNEWPOS için - sadece pozisyon arttırma emirleri için)
            jfin_allowed_qty = pot_allowed_qty
            jfin_reason = ""
            if is_position_increase:
                # ADDNEWPOS durumunda mı kontrol et (RUNALL çalışıyorsa ve ADDNEWPOS başlatıldıysa)
                ...
                if is_addnewpos:
                    jfin_allowed_qty, jfin_reason = self.check_jfin_daily_change_limit(
                        symbol, current_qty, open_orders_qty, pot_allowed_qty, order_side, gün_başı_pozisyon
                    )
    ...
    def run_all_sequence(self, from_restart=False):
        ...
        try:
            ...
            # Eğer döngü çalışıyorsa durdur (ama restart'tan geliyorsa atla)
            ...
            def check_exposure_and_start_karbotu():
                ...
                try:
                    ...
                    # UI thread'ine geçiş yaparak buton durumlarını güncelle
                    def update_ui():
                        # Pot Toplam kontrolü
                        ...
                        if mode == "OFANSIF":
                            print("[RUNALL] ✅ Adım 4: Mode=OFANSIF → KARBOTU başlatılıyor...")
                            self.log_message("✅ Adım 4: Mode=OFANSIF → KARBOTU başlatılıyor...")
                            ...
                            # KARBOTU'yu non-blocking başlat
                            self.after(100, self.start_karbotu_automation)
                        else:
                            # DEFANSIF veya GECIS modunda REDUCEMORE kullan
                            print(f"[RUNALL] ✅ Adım 4: Mode={mode} → REDUCEMORE başlatılıyor...")
                            self.log_message(f"✅ Adım 4: Mode={mode} → REDUCEMORE başlatılıyor...")
                            
                            # GUI'yi güncelle (donmasını önle)
                            self.update_idletasks()
                            
                            # REDUCEMORE bitince otomatik ADDNEWPOS tetikleme için callback ekle
                    ...
                    self.log_message(f"❌ Exposure kontrolü hatası: {e}")
                    import traceback
                    traceback.print_exc()
                    # Hata olsa bile KARBOTU'yu başlat
                    self.after(0, lambda: self.start_karbotu_automation())
            
            # Exposure kontrolünü thread'de yap
            import threading
            ...
            print("[RUNALL] ✅ RUNALL sırası başlatıldı (Mode'a göre KARBOTU veya REDUCEMORE çalışıyor, bitince ADDNEWPOS kontrol edilecek)")
            self.log_message("✅ RUNALL sırası başlatıldı (Mode'a göre KARBOTU veya REDUCEMORE çalışıyor, bitince ADDNEWPOS kontrol edilecek)")
    ...
    def load_benchmark_shift_from_csv(self):
        """benchmark_shift.csv dosyasından benchmark shift değerini yükle"""
        try:
            import pandas as pd
            import os
            ...
            if os.path.exists(csv_file):
                ...
                if 'shift_value' in df.columns:
                    ...
                    shift_value = df.iloc[0, 0]  # İlk kolonun ilk değeri
                else:
                    shift_value = 0.0
                ...
                try:
                    ...
                    print(f"[BM SHIFTER] ✅ Benchmark shift değeri CSV'den yüklendi: {self.benchmark_shift:.4f}")
                except (ValueError, TypeError):
                    self.benchmark_shift = 0.0
                    print(f"[BM SHIFTER] ⚠️ CSV'deki değer geçersiz, default 0.0 kullanılıyor")
            else:
                # CSV yoksa default 0.0
                self.benchmark_shift = 0.0
                print(f"[BM SHIFTER] ℹ️ CSV dosyası bulunamadı, default 0.0 kullanılıyor")
                
        except Exception as e:
            print(f"[BM SHIFTER] ❌ CSV yükleme hatası: {e}")
            self.benchmark_shift = 0.0  # Hata durumunda default
    
    def save_benchmark_shift_to_csv(self, shift_value):
        """Benchmark shift değerini benchmark_shift.csv dosyasına kaydet"""
        try:
            import pandas as pd
            ...
            print(f"[BM SHIFTER] ✅ Benchmark shift değeri CSV'ye kaydedildi: {shift_value:.4f}")
                
        except Exception as e:
            print(f"[BM SHIFTER] ❌ CSV kaydetme hatası: {e}")
            raise
    
    def load_psfalgo_filters_from_csv(self):
        """psfalgofilters.csv dosyasından JFIN filtrelerini yükle"""
        try:
            import pandas as pd
            import os
            ...
            if os.path.exists(csv_file):
                ...
                print(f"[PSFALGO FILTERS] ✅ Filtreler CSV'den yüklendi: {csv_file}")
            else:
                print(f"[PSFALGO FILTERS] ℹ️ CSV dosyası bulunamadı, varsayılan değerler kullanılıyor")
                
        except Exception as e:
            print(f"[PSFALGO FILTERS] ❌ CSV yükleme hatası: {e}")
            import traceback
            traceback.print_exc()
    
    def save_psfalgo_filters_to_csv(self):
        """JFIN filtrelerini psfalgofilters.csv dosyasına kaydet"""
        try:
            import pandas as pd
            ...
            df.to_csv(csv_file, index=False)
            print(f"[PSFALGO FILTERS] ✅ Filtreler CSV'ye kaydedildi: {csv_file}")
            
        except Exception as e:
            print(f"[PSFALGO FILTERS] ❌ CSV kaydetme hatası: {e}")
            import traceback
            traceback.print_exc()
            raise
    
    def show_save_filters_dialog(self):
        """Filtreleri kaydetme dialog penceresi göster"""
        try:
            ...
            def save_and_close():
                try:
                    ...
                    messagebox.showinfo("Başarılı", "Filtreler başarıyla kaydedildi!", parent=dialog)
                    dialog.destroy()
                except Exception as e:
                    messagebox.showerror("Hata", f"Kaydetme hatası: {e}", parent=dialog)
            
            save_btn = ttk.Button(button_frame, text="💾 Kaydet ve Kapat", 
            ...
            print(f"[PSFALGO FILTERS] ❌ Dialog hatası: {e}")
            import traceback
            traceback.print_exc()
    ...
    def show_excluder_dialog(self):
        ...
        try:
            ...
            info_label = ttk.Label(main_frame, 
                                  text="Exclude edilecek ticker'ları yönetin",
                                  font=("Arial", 10, "bold"))
            ...
            ttk.Label(add_frame, text="Yeni Ticker Ekle (virgülle ayırın):", font=("Arial", 9)).pack(anchor='w')
            ...
            # Butonları oluştur
            ttk.Button(button_frame, text="✅ Kaydet ve Kapat", command=save_and_close).pack(side='left', padx=5)
            ttk.Button(button_frame, text="❌ İptal", command=cancel_dialog).pack(side='left', padx=5)
            ttk.Button(button_frame, text="🗑️ Seçileni Sil", command=delete_selected).pack(side='left', padx=5)
            ttk.Button(button_frame, text="📋 Tümünü Seç", command=select_all).pack(side='left', padx=5)
            ttk.Button(button_frame, text="🗑️ Tümünü Sil", command=clear_all).pack(side='left', padx=5)
            
        except Exception as e:
            messagebox.showerror("Hata", f"Dialog oluşturulamadı: {e}")
            self.log_message(f"❌ Excluder dialog hatası: {e}")
    
    def show_bm_shifter_dialog(self):
        """General BM Shifter dialog'unu göster - Benchmark shift değeri girmek için"""
        try:
            from tkinter import messagebox
            ...
            # Açıklama
            info_label = ttk.Label(main_frame, 
                                  text="Benchmark shift değerini girin\n(Negatif değer benchmark'ı düşürür, pozitif değer yükseltir)",
                                  font=("Arial", 10),
                                  justify='center')
            ...
            ttk.Label(input_frame, text="Shift Değeri:", font=("Arial", 9)).pack(side='left', padx=5)
            ...
            shift_entry.select_range(0, tk.END)  # Tüm metni seç
            ...
            def apply_shift():
                """Shift değerini uygula"""
                try:
                    shift_value = float(shift_var.get().strip())
                    
                    # Shift değerini kaydet
                    ...
                    # Log mesajı
                    self.log_message(f"📊 Benchmark shift değeri güncellendi: {shift_value:.4f}")
                    print(f"[BM SHIFTER] ✅ Benchmark shift değeri güncellendi: {shift_value:.4f}")
                    
                    # Bilgi mesajı
                    messagebox.showinfo("Başarılı", 
                                      f"Benchmark shift değeri güncellendi: {shift_value:.4f}\n\n"
                                      f"Tüm benchmark hesaplamaları bu değerle shift edilecek.")
                    
                    dialog.destroy()
                    
                    # Tüm skorları yeniden hesapla (shift değeri değiştiği için)
                    self.calculate_scores_for_all_stocks()
                    
                except ValueError:
                    messagebox.showerror("Hata", "Lütfen geçerli bir sayı girin (örn: -0.10 veya 0.05)")
                except Exception as e:
                    messagebox.showerror("Hata", f"Shift değeri uygulanamadı: {e}")
                    print(f"[BM SHIFTER] ❌ Hata: {e}")
                    import traceback
                    traceback.print_exc()
            
            def reset_shift():
                """Shift değerini sıfırla"""
                try:
                    self.benchmark_shift = 0.0
                    ...
                    # Log mesajı
                    self.log_message("📊 Benchmark shift değeri sıfırlandı")
                    print("[BM SHIFTER] ✅ Benchmark shift değeri sıfırlandı")
                    
                    # Bilgi mesajı
                    messagebox.showinfo("Başarılı", "Benchmark shift değeri sıfırlandı.\nBenchmark değerleri orijinal haline döndü.")
                    ...
                    messagebox.showerror("Hata", f"Shift değeri sıfırlanamadı: {e}")
                    print(f"[BM SHIFTER] ❌ Hata: {e}")
            ...
            ttk.Button(button_frame, text="Sıfırla", command=reset_shift).pack(side='left', padx=5)
            ttk.Button(button_frame, text="İptal", command=cancel_dialog).pack(side='left', padx=5)
            
        except Exception as e:
            messagebox.showerror("Hata", f"Dialog açılamadı: {e}")
            print(f"[BM SHIFTER] ❌ Dialog hatası: {e}")
            import traceback
            traceback.print_exc()
    ...
    def start_addnewpos_automation(self, from_runall=False):
        ...
        try:
            ...
            # Adım 1: Port Adjuster penceresini aç (non-blocking)
            ...
            # GUI'yi tekrar güncelle (pencere açıldıktan sonra)
            self.update_idletasks()
            
            # Port Adjuster penceresinin açılmasını bekle (non-blocking)
            ...
            # RUNALL'dan çağrıldıysa ve hata olduysa bile 2 dakika sonra restart yap
            if from_runall:
                ...
                if should_continue:
                    print("[RUNALL] ⚠️ ADDNEWPOS başlatılamadı, 2 dakika bekleniyor, sonra emirler iptal edilecek...")
                    self.log_message("⚠️ ADDNEWPOS başlatılamadı, 2 dakika bekleniyor, sonra emirler iptal edilecek...")
    ...
    def addnewpos_step_2_csv_yukle(self, port_adjuster_window):
        ...
        try:
            ...
            # Port Adjuster penceresindeki CSV'den Yükle butonunu bul ve tıkla (non-blocking)
            if hasattr(port_adjuster_window, 'load_settings_from_csv'):
                # CSV yükleme işlemini thread'de yap (bloklayıcı olabilir)
                def load_csv_thread():
                    try:
                        ...
                        print(f"[ADDNEWPOS] ❌ CSV yükleme hatası: {e}")
                        self.after(0, lambda: self.log_message(f"❌ CSV yükleme hatası: {e}"))
                
                import threading
                thread = threading.Thread(target=load_csv_thread, daemon=True)
                thread.start()
            else:
                print(f"[ADDNEWPOS] ❌ Port Adjuster'da load_settings_from_csv bulunamadı")
    ...
    def _addnewpos_csv_loaded(self, port_adjuster_window):
        """CSV yüklendikten sonra çağrılır"""
        try:
            ...
            def proceed_to_step_3():
                ...
                # Sonraki adıma geç (non-blocking)
            ...
            print(f"[ADDNEWPOS] ❌ CSV yükleme sonrası hatası: {e}")
            self.log_message(f"❌ CSV yükleme sonrası hatası: {e}")
            import traceback
            traceback.print_exc()
    ...
    def addnewpos_step_6_bb_filter(self):
        ...
        try:
            ...
            if final_thg:
                ...
                print(f"[ADDNEWPOS] 📊 Long Filtreler: SMA={long_sma_type} {long_sma_value}, FBtot={long_fbtot_type} {long_fbtot_value}, Gort={long_gort_type} {long_gort_value}")
                self.log_message(f"📊 Long Filtreler: SMA={long_sma_type} {long_sma_value}, FBtot={long_fbtot_type} {long_fbtot_value}, Gort={long_gort_type} {long_gort_value}")
                ...
                if hasattr(final_thg, 'bb_sma_filter_var'):
                    try:
                        ...
                        if hasattr(final_thg, 'apply_bb_filter'):
                            ...
                            print("[ADDNEWPOS] ✅ BB Long filtreleri uygulandı")
                            self.log_message("✅ Adım 6: BB Long filtreleri uygulandı")
                    except Exception as e:
                        print(f"[ADDNEWPOS] ❌ BB Long filtre ayarlama hatası: {e}")
                        self.log_message(f"❌ BB Long filtre ayarlama hatası: {e}")
                        import traceback
                        traceback.print_exc()
                ...
                if hasattr(final_thg, 'fb_sma_filter_var'):
                    try:
                        ...
                        if hasattr(final_thg, 'apply_fb_filter'):
                            ...
                            print("[ADDNEWPOS] ✅ FB Long filtreleri uygulandı")
                            self.log_message("✅ Adım 6: FB Long filtreleri uygulandı")
                    except Exception as e:
                        print(f"[ADDNEWPOS] ❌ FB Long filtre ayarlama hatası: {e}")
                        self.log_message(f"❌ FB Long filtre ayarlama hatası: {e}")
                        import traceback
                        traceback.print_exc()
    ...
    def addnewpos_step_7_jfin_50_bb(self):
        ...
        try:
            ...
            if final_thg and hasattr(final_thg, 'show_jfin_orders'):
                ...
                # ADDNEWPOS'un hangi sekmede çalıştığını sakla (controller için)
    ...
    def addnewpos_step_8_exclude_check(self):
        ...
        try:
            ...
            # Hata olsa bile sonraki adıma geç (RUNALL'dan çağrıldıysa)
            if hasattr(self, 'runall_addnewpos_started') and self.runall_addnewpos_started:
                print("[ADDNEWPOS] ⚠️ Adım 8 hatası nedeniyle Adım 9'a geçiliyor...")
                self.log_message("⚠️ Adım 8 hatası nedeniyle Adım 9'a geçiliyor...")
                # Sonraki adıma geç
    ...
    def addnewpos_step_9_auto_send_orders(self):
        ...
        try:
            # RUNALL Allowed modunda emirler zaten otomatik gönderiliyor, bu adımı atla
            if hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode:
                print("[ADDNEWPOS] ℹ️ Allowed modu aktif - Emirler zaten otomatik gönderildi, Adım 9 atlanıyor")
                self.log_message("ℹ️ Adım 9: Allowed modu aktif - Emirler zaten otomatik gönderildi")
                return
            
            self.log_message("📋 Adım 9: Emirleri Gönder butonuna otomatik tıklanıyor...")
            
            # JFIN penceresini bul - ÖNCE attribute'u kontrol et
            jfin_window = None
            
            # 1. Önce kaydedilmiş JFIN penceresini kontrol et (hala var mı?)
            if hasattr(self, 'addnewpos_jfin_window') and self.addnewpos_jfin_window:
                try:
                    if self.addnewpos_jfin_window.winfo_exists():
                        ...
                        print("[ADDNEWPOS] ✅ JFIN penceresi attribute'dan bulundu")
                except:
                    # Pencere kapanmış olabilir, attribute'u temizle
                    self.addnewpos_jfin_window = None
            
            # 2. Eğer bulunamadıysa, FinalThgLotDistributor penceresi içinde ara
            if not jfin_window:
                final_thg = None
                if hasattr(self, 'addnewpos_final_thg') and self.addnewpos_final_thg:
                    final_thg = self.addnewpos_final_thg
                        ...
                        for win in all_windows:
                            try:
                                if win.winfo_exists() and "JFIN" in win.title():
                                    ...
                                    print("[ADDNEWPOS] ✅ JFIN penceresi final_thg.win içinde bulundu")
                                    break
                            except:
                                continue
                    except Exception as e:
                        print(f"[ADDNEWPOS] ⚠️ final_thg.win içinde arama hatası: {e}")
            
            # 3. Hala bulunamadıysa, main_window altında tüm Toplevel pencereleri kontrol et
            if not jfin_window:
                try:
                    all_toplevels = []
                    ...
                    for win in all_toplevels:
                        try:
                            if win.winfo_exists():
                                jfin_window = win
                                self.addnewpos_jfin_window = win
                                print("[ADDNEWPOS] ✅ JFIN penceresi main_window altında bulundu")
                                break
                        except:
                            continue
                except Exception as e:
                    print(f"[ADDNEWPOS] ⚠️ main_window altında arama hatası: {e}")
            
            # 4. Son çare: addnewpos_find_jfin_window_recursive fonksiyonunu kullan
            if not jfin_window:
                try:
                    # Önce final_thg.win'den başla
                    final_thg = None
                    if hasattr(self, 'addnewpos_final_thg') and self.addnewpos_final_thg:
                        final_thg = self.addnewpos_final_thg
                    elif hasattr(self, 'addnewpos_port_adjuster') and hasattr(self.addnewpos_port_adjuster, 'final_thg_distributor'):
                    ...
                    if final_thg and hasattr(final_thg, 'win'):
                        ...
                        if hasattr(self, 'addnewpos_jfin_window') and self.addnewpos_jfin_window:
                            try:
                                if self.addnewpos_jfin_window.winfo_exists():
                                    jfin_window = self.addnewpos_jfin_window
                                    print("[ADDNEWPOS] ✅ JFIN penceresi recursive fonksiyon ile bulundu")
                            except:
                                pass
                    
                    # Eğer hala bulunamadıysa, main_window'dan başla
                    if not jfin_window:
                        ...
                        if hasattr(self, 'addnewpos_jfin_window') and self.addnewpos_jfin_window:
                            try:
                                if self.addnewpos_jfin_window.winfo_exists():
                                    jfin_window = self.addnewpos_jfin_window
                                    print("[ADDNEWPOS] ✅ JFIN penceresi main_window recursive ile bulundu")
                            except:
                                pass
                except Exception as e:
                    print(f"[ADDNEWPOS] ⚠️ Recursive arama hatası: {e}")
            ...
            if jfin_window:
                ...
                if send_button:
                    # Retry sayaçlarını sıfırla (başarılı oldu)
                ...
                # JFIN penceresi bulunamadı, retry yap
                if not hasattr(self, 'addnewpos_window_retry_count'):
                    self.addnewpos_window_retry_count = 0
                
                self.addnewpos_window_retry_count += 1
                if self.addnewpos_window_retry_count < 10:  # 10 deneme yap (pencere açılması zaman alabilir)
                    print(f"[ADDNEWPOS] ⚠️ JFIN onay penceresi bulunamadı, tekrar denenecek... ({self.addnewpos_window_retry_count}/10)")
                    self.log_message(f"⚠️ Adım 9: JFIN onay penceresi bulunamadı, tekrar denenecek... ({self.addnewpos_window_retry_count}/10)")
                    # 2 saniye sonra tekrar dene
                    self.after(2000, lambda: self.addnewpos_step_9_auto_send_orders())
                else:
                    print("[ADDNEWPOS] ❌ JFIN onay penceresi 10 denemede bulunamadı")
                    self.log_message("❌ Adım 9: JFIN onay penceresi bulunamadı, lütfen manuel kontrol edin")
    ...
    def karbotu_step_1_open_take_profit_longs(self):
        ...
        try:
            ...
            # Take Profit Longs penceresini aç (non-blocking)
            from .take_profit_panel import TakeProfitPanel
            ...
            # Gort kontrolü yap (Take Profit Longs için) - non-blocking
            self.after(200, self.karbotu_gort_check_take_profit_longs)
            
            # Adım 2'ye geç (Gort kontrolü bitince)
            self.karbotu_current_step = 2
            # Gort kontrolü bitince adım 2'ye geçilecek (karbotu_gort_check_take_profit_longs içinde)
        ...
        try:
            ...
            for item in self.take_profit_longs_panel.tree.get_children():
                ...
                try:
                    ...
                    # Fbtot tam 0.0 ise exclude et (N/A değeri)
                    if fbtot == 0.0:
                        continue
    ...
    def karbotu_step_8_open_take_profit_shorts(self):
        ...
        try:
            ...
            # Gort kontrolü yap (Take Profit Shorts için)
        ...
        try:
            ...
            for item in self.take_profit_shorts_panel.tree.get_children():
                ...
                try:
                    ...
                    # SFStot tam 0.0 ise exclude et (N/A değeri)
                    if sfstot == 0.0:
                        continue
    ...
    def karbotu_show_shorts_confirmation_window(self, positions, order_type, lot_percentage, step_name):
        ...
        try:
            ...
            ttk.Button(button_frame, text="Emirleri Gönder", command=on_confirm, style='Accent.TButton').pack(side='left', padx=5)
    ...
    def karbotu_gort_check_take_profit_longs(self):
        ...
        try:
            ...
            if not hasattr(self, 'take_profit_longs_panel') or not self.take_profit_longs_panel:
                print("[KARBOTU GORT] ⚠️ Take Profit Longs paneli bulunamadı")
                return
            
            # Lot yüzdesi: %10 (ama min 200 lot)
            lot_percentage = 10
            ...
            for item in self.take_profit_longs_panel.tree.get_children():
                ...
                ask_sell_pahalilik_str = values[9]  # Ask Sell Pahalılık kolonu (indeks 9)
                
                try:
                    # Gort'u güvenli şekilde parse et
                    gort = 0
                    ...
                    # Gort tam 0.0 ise exclude et (N/A değeri)
                    if gort == 0.0:
                        continue
                    ...
                    # Koşul: Gort > -1 ve Ask Sell pahalılık > -0.05
            ...
            if filtered_positions:
                print(f"[KARBOTU GORT] ✅ {len(filtered_positions)} pozisyon bulundu (Gort > -1 ve Ask Sell pahalılık > -0.05)")
                self.log_message(f"✅ KARBOTU Gort kontrolü: {len(filtered_positions)} pozisyon bulundu")
                
                # GUI'yi güncelle (donmasını önle)
                self.update_idletasks()
                
                # Pozisyonları seç ve özel lot hesaplama ile emir gönder (non-blocking)
                self.after(100, lambda: self.karbotu_gort_select_and_send_longs(filtered_positions, "Ask Sell", lot_percentage, "KARBOTU Gort Kontrolü (Longs)"))
            else:
                ...
                # Pozisyon bulunamadıysa direkt adım 2'ye geç (non-blocking)
    ...
    def karbotu_gort_check_take_profit_shorts(self):
        """KARBOTU: Take Profit Shorts için Gort kontrolü - Gort < 1 ve Bid Buy ucuzluk < +0.05"""
        try:
            print("[KARBOTU GORT] 📋 Take Profit Shorts için Gort kontrolü yapılıyor...")
            self.log_message("📋 KARBOTU Gort kontrolü (Shorts): Gort < 1 ve Bid Buy ucuzluk < +0.05")
            
            if not hasattr(self, 'take_profit_shorts_panel') or not self.take_profit_shorts_panel:
                print("[KARBOTU GORT] ⚠️ Take Profit Shorts paneli bulunamadı")
                return
            ...
            for item in self.take_profit_shorts_panel.tree.get_children():
                ...
                try:
                    ...
                    # 100 lot altı pozisyonları göz ardı et
                    qty = float(values[2])  # Quantity kolonu (negatif değer)
                    if abs(qty) < 100:
                        continue
                    
                    # Koşul: Gort < 1 ve Bid Buy ucuzluk < +0.05
            ...
            if filtered_positions:
                print(f"[KARBOTU GORT] ✅ {len(filtered_positions)} pozisyon bulundu (Gort < 1 ve Bid Buy ucuzluk < +0.05)")
                self.log_message(f"✅ KARBOTU Gort kontrolü: {len(filtered_positions)} pozisyon bulundu")
                
                # Pozisyonları seç ve özel lot hesaplama ile emir gönder
                self.karbotu_gort_select_and_send_shorts(filtered_positions, "Bid Buy", lot_percentage, "KARBOTU Gort Kontrolü (Shorts)")
            else:
                print("[KARBOTU GORT] ⚠️ Koşula uygun pozisyon bulunamadı")
                self.log_message("⚠️ KARBOTU Gort kontrolü: Koşula uygun pozisyon bulunamadı")
    ...
    def reduce_more_step_1_open_take_profit_longs(self):
        ...
        try:
            ...
            # Gort kontrolü bitince adım 2'ye geçilecek (reducemore_gort_check_take_profit_longs içinde)
    ...
    def reducemore_gort_check_take_profit_longs(self):
        """REDUCEMORE: Take Profit Longs için Gort kontrolü - Gort > -1 ve Ask Sell pahalılık > -0.08"""
        try:
            print("[REDUCEMORE GORT] 📋 Take Profit Longs için Gort kontrolü yapılıyor...")
            self.log_message("📋 REDUCEMORE Gort kontrolü (Longs): Gort > -1 ve Ask Sell pahalılık > -0.08")
            
            if not hasattr(self, 'take_profit_longs_panel_reducemore') or not self.take_profit_longs_panel_reducemore:
                print("[REDUCEMORE GORT] ⚠️ Take Profit Longs paneli bulunamadı")
                return
            
            # Lot yüzdesi: %20 (ama min 200 lot)
            lot_percentage = 20
            ...
            for item in self.take_profit_longs_panel_reducemore.tree.get_children():
                ...
                try:
                    ...
                    # Koşul: Gort > -1 ve Ask Sell pahalılık > -0.08
            ...
            if filtered_positions:
                print(f"[REDUCEMORE GORT] ✅ {len(filtered_positions)} pozisyon bulundu (Gort > -1 ve Ask Sell pahalılık > -0.08)")
                self.log_message(f"✅ REDUCEMORE Gort kontrolü: {len(filtered_positions)} pozisyon bulundu")
                
                # GUI'yi güncelle (donmasını önle)
                self.update_idletasks()
                
                # Pozisyonları seç ve özel lot hesaplama ile emir gönder (non-blocking)
                self.after(100, lambda: self.reducemore_gort_select_and_send_longs(filtered_positions, "Ask Sell", lot_percentage, "REDUCEMORE Gort Kontrolü (Longs)"))
            else:
                print("[REDUCEMORE GORT] ⚠️ Koşula uygun pozisyon bulunamadı")
                self.log_message("⚠️ REDUCEMORE Gort kontrolü: Koşula uygun pozisyon bulunamadı")
                
                # Pozisyon bulunamadıysa direkt adım 2'ye geç (non-blocking)
                self.after(200, self.reduce_more_step_2_fbtot_lt_110)
                
        except Exception as e:
            print(f"[REDUCEMORE GORT] ❌ Hata: {e}")
            self.log_message(f"❌ REDUCEMORE Gort kontrolü hatası: {e}")
            import traceback
            traceback.print_exc()
    
    def reducemore_gort_check_take_profit_shorts(self):
        """REDUCEMORE: Take Profit Shorts için Gort kontrolü - Gort < 1 ve Bid Buy ucuzluk < +0.08"""
        try:
            print("[REDUCEMORE GORT] 📋 Take Profit Shorts için Gort kontrolü yapılıyor...")
            self.log_message("📋 REDUCEMORE Gort kontrolü (Shorts): Gort < 1 ve Bid Buy ucuzluk < +0.08")
            
            if not hasattr(self, 'take_profit_shorts_panel_reducemore') or not self.take_profit_shorts_panel_reducemore:
                print("[REDUCEMORE GORT] ⚠️ Take Profit Shorts paneli bulunamadı")
                return
            ...
            for item in self.take_profit_shorts_panel_reducemore.tree.get_children():
                ...
                try:
                    ...
                    # Koşul: Gort < 1 ve Bid Buy ucuzluk < +0.08
            ...
            if filtered_positions:
                print(f"[REDUCEMORE GORT] ✅ {len(filtered_positions)} pozisyon bulundu (Gort < 1 ve Bid Buy ucuzluk < +0.08)")
                self.log_message(f"✅ REDUCEMORE Gort kontrolü: {len(filtered_positions)} pozisyon bulundu")
                
                # Pozisyonları seç ve özel lot hesaplama ile emir gönder
                self.reducemore_gort_select_and_send_shorts(filtered_positions, "Bid Buy", lot_percentage, "REDUCEMORE Gort Kontrolü (Shorts)")
            else:
                print("[REDUCEMORE GORT] ⚠️ Koşula uygun pozisyon bulunamadı")
                self.log_message("⚠️ REDUCEMORE Gort kontrolü: Koşula uygun pozisyon bulunamadı")
    ...
    def reducemore_send_orders_direct(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE emirlerini direkt gönder (Allowed modunda onay penceresi olmadan)"""
        try:
            print(f"[REDUCEMORE] 🔄 {step_name} emirleri direkt gönderiliyor (Allowed modu)...")
            self.log_message(f"🔄 {step_name} emirleri direkt gönderiliyor (Allowed modu)...")
            
            # Pozisyon verilerini hazırla
            order_data = {}
            
            # Panel seçimi (order_type'a göre)
            ...
            for pos in positions:
                ...
                # 200 lot altı pozisyonlar için tamamını gönder
                abs_qty = abs(qty)
                if abs_qty < 200:
                    # Eldeki lot < 200 ise tamamını gönder
                ...
                # MAXALW*3/4 limitini aşacaksa emir gönderme
                if potential_daily_change > max_change_limit:
                    print(f"[REDUCEMORE] ⚠️ {symbol}: MAXALW*3/4 limiti aşılacak ({potential_daily_change:.0f} > {max_change_limit:.0f}), emir atlandı")
                    self.log_message(f"⚠️ {symbol}: MAXALW*3/4 limiti aşılacak, emir atlandı")
                    continue
                ...
                if not market_data:
                    print(f"[REDUCEMORE] ❌ {symbol} market_data bulunamadı, atlandı")
                    continue
            ...
            print(f"[REDUCEMORE] ❌ Direkt emir gönderme hatası: {e}")
            self.log_message(f"❌ Direkt emir gönderme hatası: {e}")
            import traceback
            traceback.print_exc()
            # Hata olsa bile sonraki adıma geç (kısa bir bekleme ile)
    ...
    def reduce_more_show_confirmation_window(self, positions, order_type, lot_percentage, step_name):
        ...
        try:
            # RUNALL Allowed modunda otomatik onay kontrolü
            if hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode:
                print(f"[REDUCEMORE] ✅ Allowed modu aktif - Onay penceresi atlanıyor, emirler direkt gönderiliyor")
    ...
    def karbotu_gort_select_and_send_longs(self, positions, order_type, lot_percentage, step_name):
        ...
        try:
            ...
            # Özel lot hesaplama ile onay penceresi aç
            self.karbotu_gort_show_confirmation_window_longs(positions, order_type, lot_percentage, step_name)
            
        except Exception as e:
            print(f"[KARBOTU GORT] ❌ Pozisyon seçimi hatası: {e}")
            self.log_message(f"❌ KARBOTU Gort pozisyon seçimi hatası: {e}")
            import traceback
            traceback.print_exc()
    
    def karbotu_gort_select_and_send_shorts(self, positions, order_type, lot_percentage, step_name):
        """KARBOTU Gort: Shorts pozisyonları seç ve özel lot hesaplama ile emir gönder"""
        try:
            # Pozisyonları seç
            for pos in positions:
                self.take_profit_shorts_panel.tree.set(pos['item'], "select", "✓")
    ...
    def reducemore_gort_send_orders_direct_longs(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE Gort: Longs için direkt emir gönderme (Allowed modu için)"""
        try:
            order_data = {}
            ...
            for pos in positions:
                ...
                # Özel lot hesaplama: %20 ama min 200 lot, eğer eldeki lot < 200 ise eldeki lotun tamamı
                calculated_lot = qty * (lot_percentage / 100)  # %20 hesapla
                
                if qty < 200:
                    # Eldeki lot < 200 ise eldeki lotun tamamı
                    lot_qty = int(qty)
                else:
                    # Eldeki lot >= 200 ise
                    if calculated_lot < 200:
                        # %20 hesaplanan değer 200'den küçükse → 200 lot (min 200 kuralı)
                        lot_qty = 200
                    else:
                        # %20 hesaplanan değer 200'den büyük veya eşitse yuvarlama yap
                        if calculated_lot <= 200:
                            lot_qty = 200
                        elif calculated_lot <= 300:
                            lot_qty = 300
                            ...
                            lot_qty = int((calculated_lot + 99) // 100) * 100
                        # Min 200 kontrolü (yuvarlama sonrası)
                        if lot_qty < 200:
                            lot_qty = 200
            ...
            # Sonraki adıma geç (Gort kontrolü bitince sonraki adıma geçilecek)
            self.after(500, self.reduce_more_proceed_to_next_step)
            
        except Exception as e:
            print(f"[REDUCEMORE GORT] ❌ Direkt emir gönderme hatası: {e}")
            self.log_message(f"❌ REDUCEMORE Gort direkt emir gönderme hatası: {e}")
            import traceback
            traceback.print_exc()
            # Hata olsa bile sonraki adıma geç
            self.after(500, self.reduce_more_proceed_to_next_step)
    
    def reducemore_gort_select_and_send_longs(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE Gort: Longs pozisyonları seç ve özel lot hesaplama ile emir gönder"""
        try:
            # Pozisyonları seç
            for pos in positions:
                self.take_profit_longs_panel_reducemore.tree.set(pos['item'], "select", "✓")
            ...
            print(f"[REDUCEMORE GORT] ❌ Pozisyon seçimi hatası: {e}")
            self.log_message(f"❌ REDUCEMORE Gort pozisyon seçimi hatası: {e}")
            import traceback
            traceback.print_exc()
    
    def reducemore_gort_select_and_send_shorts(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE Gort: Shorts pozisyonları seç ve özel lot hesaplama ile emir gönder"""
        try:
            # Pozisyonları seç
            for pos in positions:
                self.take_profit_shorts_panel_reducemore.tree.set(pos['item'], "select", "✓")
        ...
        try:
            ...
            def on_confirm():
                ...
                try:
                    ...
                    # Pencereyi hemen kapat (işi bitti)
                    try:
    ...
    def karbotu_gort_send_orders_direct_longs(self, positions, order_type, lot_percentage, step_name):
        ...
        try:
            ...
            for pos in positions:
                ...
                # Özel lot hesaplama: %10 ama min 200 lot, eğer eldeki lot < 200 ise eldeki lotun tamamı
                ...
                if qty < 200:
                    ...
                    # Eldeki lot >= 200 ise
                    if calculated_lot < 200:
                        # %10 hesaplanan değer 200'den küçükse → 200 lot (min 200 kuralı)
                        lot_qty = 200
                    else:
                        # %10 hesaplanan değer 200'den büyük veya eşitse yuvarlama yap
                        if calculated_lot <= 200:
                            lot_qty = 200
                        elif calculated_lot <= 300:
                            lot_qty = 300
            ...
            print(f"[KARBOTU GORT] ❌ Direkt emir gönderme hatası: {e}")
            self.log_message(f"❌ KARBOTU Gort direkt emir gönderme hatası: {e}")
            import traceback
            traceback.print_exc()
            # Hata olsa bile sonraki adıma geç
            self.after(500, self.karbotu_step_2_fbtot_lt_110)
    ...
    def karbotu_gort_send_orders_direct_shorts(self, positions, order_type, lot_percentage, step_name):
        """KARBOTU Gort Shorts: Emirleri direkt gönder (Allowed modunda onay penceresi olmadan)"""
        try:
            print(f"[KARBOTU GORT] 🔄 {step_name} emirleri direkt gönderiliyor (Allowed modu)...")
            self.log_message(f"🔄 KARBOTU Gort {step_name} emirleri direkt gönderiliyor (Allowed modu)...")
            ...
            for pos in positions:
                item_values = self.take_profit_shorts_panel.tree.item(pos['item'])['values']
                symbol = pos['symbol']
                qty = float(item_values[2])  # Eldeki lot (negatif, örn: -276)
                abs_qty = abs(qty)  # Mutlak değer
    ...
    def karbotu_gort_show_confirmation_window_longs(self, positions, order_type, lot_percentage, step_name):
        """KARBOTU Gort: Longs için özel lot hesaplama ile onay penceresi"""
        try:
            # RUNALL Allowed modunda otomatik onay kontrolü
            if hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode:
                print(f"[KARBOTU GORT] ✅ Allowed modu aktif - Onay penceresi atlanıyor, emirler direkt gönderiliyor")
                self.log_message(f"✅ Allowed modu: {step_name} (GORT Longs) - Emirler otomatik gönderiliyor")
                # Emirleri direkt gönder (onay penceresi açmadan)
                self.karbotu_gort_send_orders_direct_longs(positions, order_type, lot_percentage, step_name)
                return
            
            print(f"[KARBOTU GORT] 🔄 Onay penceresi açılıyor: {step_name}")
            ...
            ttk.Label(title_frame, text=f"{order_type} - Özel Lot Hesaplama (Min 200 lot veya eldeki lotun tamamı)", font=('Arial', 12)).pack()
            ttk.Label(title_frame, text=f"{len(positions)} pozisyon seçildi", font=('Arial', 10)).pack()
            
            list_frame = ttk.Frame(confirm_win)
            list_frame.pack(fill='both', expand=True, padx=10, pady=5)
            
            columns = ('Symbol', 'Eldeki Lot', 'Hesaplanan Lot', 'Gort', 'Ask Sell Pahalılık', 'Emir Fiyat')
            pos_tree = ttk.Treeview(list_frame, columns=columns, show='headings', height=15)
            
            col_widths = {'Symbol': 80, 'Eldeki Lot': 80, 'Hesaplanan Lot': 100, 'Gort': 60, 'Ask Sell Pahalılık': 120, 'Emir Fiyat': 80}
            for col in columns:
                pos_tree.heading(col, text=col)
                pos_tree.column(col, width=col_widths[col])
            ...
            def on_confirm():
                try:
                    print(f"[KARBOTU GORT] 🔄 {step_name} emirleri gönderiliyor...")
                    self.log_message(f"🔄 KARBOTU Gort {step_name} emirleri gönderiliyor...")
                    ...
                    # Messagebox gösterme (Allowed modunda otomatik kapanacak)
                    if not (hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode):
                        # Allowed modu kapalıysa messagebox göster
                        self.after(100, lambda: messagebox.showinfo("Başarılı", f"{len(order_data)} emir gönderildi!"))
                    
                    # Sonraki adıma geç (Gort kontrolü bitince adım 2'ye geçilecek)
                    self.after(500, self.karbotu_step_2_fbtot_lt_110)
                    
                except Exception as e:
                    print(f"[KARBOTU GORT] ❌ Emir gönderme hatası: {e}")
                    self.log_message(f"❌ KARBOTU Gort emir gönderme hatası: {e}")
                    import traceback
                    traceback.print_exc()
                    # Hata olsa bile pencereyi kapat
                    try:
            ...
            print(f"[KARBOTU GORT] ❌ Onay penceresi hatası: {e}")
            self.log_message(f"❌ KARBOTU Gort onay penceresi hatası: {e}")
            import traceback
            traceback.print_exc()
    
    def karbotu_gort_show_confirmation_window_shorts(self, positions, order_type, lot_percentage, step_name):
        """KARBOTU Gort: Shorts için özel lot hesaplama ile onay penceresi"""
        try:
            # RUNALL Allowed modunda otomatik onay kontrolü
            if hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode:
                print(f"[KARBOTU GORT] ✅ Allowed modu aktif - Onay penceresi atlanıyor, emirler direkt gönderiliyor")
                self.log_message(f"✅ Allowed modu: {step_name} (GORT Shorts) - Emirler otomatik gönderiliyor")
                # Emirleri direkt gönder (onay penceresi açmadan)
    ...
    def reducemore_gort_show_confirmation_window_longs(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE Gort: Longs için özel lot hesaplama ile onay penceresi"""
        try:
            # RUNALL Allowed modunda otomatik onay kontrolü
            if hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode:
                print(f"[REDUCEMORE GORT] ✅ Allowed modu aktif - Onay penceresi atlanıyor, emirler direkt gönderiliyor")
                self.log_message(f"✅ Allowed modu: {step_name} (GORT Longs) - Emirler otomatik gönderiliyor")
                # Emirleri direkt gönder (onay penceresi açmadan)
                self.reducemore_gort_send_orders_direct_longs(positions, order_type, lot_percentage, step_name)
                return
            
            print(f"[REDUCEMORE GORT] 🔄 Onay penceresi açılıyor: {step_name}")
            ...
            def on_confirm():
                try:
                    print(f"[REDUCEMORE GORT] 🔄 {step_name} emirleri gönderiliyor...")
                    self.log_message(f"🔄 REDUCEMORE Gort {step_name} emirleri gönderiliyor...")
                    ...
                    confirm_win.destroy()
                    messagebox.showinfo("Başarılı", f"{len(order_data)} emir gönderildi!")
                    
                    # Sonraki adıma geç (Gort kontrolü bitince sonraki adıma geçilecek)
                    self.after(500, self.reduce_more_proceed_to_next_step)
                    
                except Exception as e:
                    print(f"[REDUCEMORE GORT] ❌ Emir gönderme hatası: {e}")
                    self.log_message(f"❌ REDUCEMORE Gort emir gönderme hatası: {e}")
                    import traceback
                    traceback.print_exc()
                    # Hata olsa bile sonraki adıma geç
                    self.after(500, self.reduce_more_proceed_to_next_step)
            ...
            def on_cancel():
                confirm_win.destroy()
                # İptal edildiğinde de sonraki adıma geç
                self.after(500, self.reduce_more_proceed_to_next_step)
            ...
            print(f"[REDUCEMORE GORT] ❌ Onay penceresi hatası: {e}")
            self.log_message(f"❌ REDUCEMORE Gort onay penceresi hatası: {e}")
            import traceback
            traceback.print_exc()
            # Hata olsa bile sonraki adıma geç
            self.after(500, self.reduce_more_proceed_to_next_step)
    ...
    def reducemore_gort_send_orders_direct_shorts(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE Gort: Shorts için direkt emir gönderme (Allowed modu için)"""
        try:
            order_data = {}
    ...
    def reducemore_gort_show_confirmation_window_shorts(self, positions, order_type, lot_percentage, step_name):
        """REDUCEMORE Gort: Shorts için özel lot hesaplama ile onay penceresi"""
        try:
            # RUNALL Allowed modunda otomatik onay kontrolü
            if hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode:
                print(f"[REDUCEMORE GORT] ✅ Allowed modu aktif - Onay penceresi atlanıyor, emirler direkt gönderiliyor")
    ...
    def karbotu_proceed_to_next_step(self):
        ...
        try:
            ...
            if next_step in step_methods:
                ...
                # Sonraki adımı non-blocking olarak çağır (GUI donmasını önle)
    ...
    def runall_check_karbotu_and_addnewpos(self):
        ...
        try:
                    ...
                    def process_addnewpos():
                        ...
                        if mode == "OFANSIF" and pot_total < pot_max_lot:
                            ...
                            def addnewpos_timeout():
                                # Eğer callback zaten tetiklendiyse (emirler gönderildiyse) timeout'u iptal et
                                if hasattr(self, 'runall_addnewpos_callback_set') and not self.runall_addnewpos_callback_set:
                                    print("[RUNALL] ⏰ ADDNEWPOS timeout: Callback zaten tetiklendi, timeout iptal edildi")
                                    return
                                ...
                                if should_continue:
                                    print("[RUNALL] ⏰ ADDNEWPOS timeout (5 dakika): Emirler gönderilmedi, 2 dakika bekleniyor, sonra restart yapılacak...")
                                    self.log_message("⏰ ADDNEWPOS timeout (5 dakika): Emirler gönderilmedi, 2 dakika bekleniyor, sonra restart yapılacak...")
                                    # 2 dakika bekle, sonra emirleri iptal et
                                    self.after(120000, lambda: self.runall_cancel_orders_and_restart())
                            
                            # 5 dakika sonra timeout kontrolü yap
            ...
            # Exposure kontrolünü thread'de başlat
            import threading
    ...
    def runall_cancel_orders_and_restart(self):
        ...
        print("[RUNALL] 🗑️ Emir iptal işlemi başlatılıyor...")
        self.log_message("🗑️ Emir iptal işlemi başlatılıyor...")
        
        # Restart flag'ini initialize et
        self._runall_restart_pending = False
        
        def cancel_orders_thread():
            """Emir iptal işlemini thread'de yap"""
            try:
                    ...
                    try:
                        ...
                        if ibkr_client and ibkr_client.is_connected():
                            ...
                            try:
                                ...
                                print(f"[RUNALL] ⚠️ Emir alma hatası: {e}")
                                open_orders = []
                            ...
                            if open_orders:
                                ...
                                for order in open_orders:
                                    # Döngü durdurulmuşsa çık (ama runall_allowed_mode aktifse devam et)
                                    runall_running = hasattr(self, 'runall_loop_running') and self.runall_loop_running
                                    runall_allowed = hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode
                                    if not runall_running and not runall_allowed:
                                        print("[RUNALL] ⏹️ Döngü durduruldu, emir iptal işlemi durduruluyor")
                                        break
                ...
                # Thread'den UI thread'ine geçiş yaparak restart'ı başlat (non-blocking)
                print("[RUNALL] ✅ Emir iptal işlemi tamamlandı, restart flag'i set edildi...")
                self.log_message("✅ Emir iptal işlemi tamamlandı, restart flag'i set edildi...")
                
                # Thread içinde widget metodlarını çağırmak thread-safe değil
                # Bunun yerine flag set edelim ve ana thread'de kontrol edelim
                # Flag'i set et (restart için)
                self._runall_restart_pending = True
                print("[RUNALL] ✅ _runall_restart_pending flag'i True yapıldı")
                ...
                import traceback
                traceback.print_exc()
                # Hata olsa bile devam et - UI thread'ine geçiş yap
                try:
                    ...
                    try:
                        ...
                        # Hiçbiri çalışmazsa direkt çağır
                        try:
                            self.runall_close_orders_window_and_restart(None)
                        except:
                            pass
        
        # Emir iptal işlemini thread'de başlat
        import threading
        thread = threading.Thread(target=cancel_orders_thread, daemon=True)
        thread.start()
        
        # Thread bitince restart yapmak için kontrol döngüsü başlat
        def check_thread_and_restart():
            ...
            print(f"[RUNALL] 🔍 check_thread_and_restart: restart_pending={restart_pending}, runall_allowed={runall_allowed}, runall_running={runall_running}, thread.is_alive()={thread.is_alive()}")
            
            # Eğer ne flag set edilmiş ne de runall_allowed_mode aktifse çık
            if not restart_pending and not runall_allowed:
                print(f"[RUNALL] ⏹️ Restart flag'i set edilmemiş ve runall_allowed_mode aktif değil, restart yapılmayacak")
                return
            
            # Thread hala çalışıyor mu kontrol et
            if thread.is_alive():
                # Thread hala çalışıyor, tekrar kontrol et
                self.after(500, check_thread_and_restart)
                return
            
            # Thread bitti, restart yap
            print("[RUNALL] ✅ Thread tamamlandı, restart başlatılıyor...")
            self.log_message("✅ Thread tamamlandı, restart başlatılıyor...")
            self._runall_restart_pending = False
            
            # runall_allowed_mode aktifse runall_loop_running'i True yap
            if runall_allowed and not runall_running:
                self.runall_loop_running = True
                print("[RUNALL] ✅ runall_loop_running True yapıldı (runall_allowed_mode aktif)")
            
            self.runall_close_orders_window_and_restart(None)
        
        # İlk kontrolü başlat (500ms sonra)
        self.after(500, check_thread_and_restart)
        ...
        try:
            ...
            for toplevel in all_toplevels:
                try:
                    ...
                    # Eğer sadece 1 buton varsa, ne olduğuna bakmaksızın tıkla (tek seçenekli mesaj kutusu)
                    if len(buttons) == 1:
                        btn = buttons[0]
                        try:
                            if not btn.winfo_exists():
                            ...
                            text = str(btn.cget('text')).lower().strip()
                            print(f"[RUNALL] ✅ Tek seçenekli mesaj kutusu bulundu: '{text}' (Pencere: '{title}'), tıklanıyor...")
                            self.log_message(f"✅ Otomatik onay (tek seçenek): '{text}' ({title})")
                    ...
                    # Birden fazla buton varsa, onay butonlarını ara
    ...
    def stop_runall_loop(self):
        ...
        try:
            ...
            # Döngü flag'ini hemen False yap
            self.runall_loop_running = False
            
            # Tüm pending after çağrılarını iptal etmek için bir mekanizma yok ama
            # en azından flag'i kontrol ederek yeni işlemlerin başlamasını engelleyebiliriz
    ...
    def runall_close_orders_window_and_restart(self, orders_window):
        ...
        try:
            # Döngü durdurulmuşsa devam etme (ama runall_allowed_mode aktifse devam et)
            runall_running = hasattr(self, 'runall_loop_running') and self.runall_loop_running
            runall_allowed = hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode
            if not runall_running and not runall_allowed:
            ...
            # Tamamlanan döngü sayısını göster (mevcut döngü sayacı - 1, çünkü yeni döngü henüz başlamadı)
            ...
            # Döngü sayacı label'ını güncelle
            if hasattr(self, 'runall_loop_label'):
                self.runall_loop_label.config(text=f"Döngü: {completed_loops} tamamlandı")
            ...
            # Controller'ın açık olduğundan emin ol (UI thread'inde)
            if not self.controller_enabled:
                self.controller_enabled = True
            ...
            # İptal işlemi tamamlandıktan sonra hemen yeni döngüye başla (2 dakika zaten ADDNEWPOS bitince beklendi)
            # after() kullanarak UI thread'ini bloklamadan restart yap
            def restart_loop():
                print("[RUNALL] 🔄 restart_loop() çağrıldı")
                # Döngü durumu kontrolü (her seferinde kontrol et)
                runall_running = hasattr(self, 'runall_loop_running') and self.runall_loop_running
                runall_allowed = hasattr(self, 'runall_allowed_mode') and self.runall_allowed_mode
                print(f"[RUNALL] 🔍 restart_loop kontrolü: runall_running={runall_running}, runall_allowed={runall_allowed}")
                if not runall_running and not runall_allowed:
                    print("[RUNALL] ⏹️ Döngü durdurulmuş, tekrar başlatılmayacak")
                    self.log_message("⏹️ Döngü durdurulmuş, tekrar başlatılmayacak")
                    return
                ...
                # Yeni döngü başlatılacak (sayacı run_all_sequence içinde artıracak)
                next_loop = (self.runall_loop_count + 1) if hasattr(self, 'runall_loop_count') else 1
                print(f"[RUNALL] 🔄 {next_loop}. döngü başlatılıyor (KARBOTU ile)...")
                self.log_message(f"🔄 {next_loop}. döngü başlatılıyor (KARBOTU ile)...")
                
                # run_all_sequence'ı çağır (KARBOTU ile başlayacak) - restart'tan geldiğini belirt
                # Direkt çağır (zaten UI thread'indeyiz)
                try:
                    print(f"[RUNALL] 🚀 run_all_sequence(from_restart=True) çağrılıyor...")
                    self.run_all_sequence(from_restart=True)
                    print(f"[RUNALL] ✅ run_all_sequence(from_restart=True) çağrıldı")
                except Exception as e:
                    ...
                    if hasattr(self, 'runall_loop_running') and self.runall_loop_running:
                        print("[RUNALL] 🔄 Hata sonrası tekrar denenecek (2 saniye sonra)...")
            ...
            print(f"[RUNALL] 🔍 Restart kontrolü: runall_running={runall_running}, runall_allowed={runall_allowed}, should_restart={should_restart}")
            ...
            if should_restart:
                ...
                # Emir iptalleri tamamlanır tamamlanmaz hemen yeni döngüye başla (non-blocking)
                # Kısa bir gecikme ile başlat (emir iptallerinin tamamlanması için)
                print("[RUNALL] ⏰ Restart fonksiyonu çağrılıyor (500ms sonra)...")
    ...
    def toggle_lot_divider(self):
        ...
        if self.lot_divider_enabled:
            ...
            self.btn_lot_divider.config(text="📦 Lot Bölücü: OFF")
