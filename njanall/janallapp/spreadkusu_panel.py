...
class SpreadkusuPanel:
    ...
    def get_group_from_dataframe(self, symbol):
        ...
        try:
            ...
            group_file_map = {
                'heldff': get_csv_path('ssfinekheldff.csv'),
                'helddeznff': get_csv_path('ssfinekhelddeznff.csv'), 
                'heldkuponlu': get_csv_path('ssfinekheldkuponlu.csv'),
                'heldnff': get_csv_path('ssfinekheldnff.csv'),
                'heldflr': get_csv_path('ssfinekheldflr.csv'),
                'heldgarabetaltiyedi': get_csv_path('ssfinekheldgarabetaltiyedi.csv'),
                'heldkuponlukreciliz': get_csv_path('ssfinekheldkuponlukreciliz.csv'),
                'heldkuponlukreorta': get_csv_path('ssfinekheldkuponlukreorta.csv'),
                'heldotelremorta': get_csv_path('ssfinekheldotelremorta.csv'),
                'heldsolidbig': get_csv_path('ssfinekheldsolidbig.csv'),
                'heldtitrekhc': get_csv_path('ssfinekheldtitrekhc.csv'),
                'highmatur': get_csv_path('ssfinekhighmatur.csv'),
                'notcefilliquid': get_csv_path('ssfineknotcefilliquid.csv'),
                'notbesmaturlu': get_csv_path('ssfineknotbesmaturlu.csv'),
                'nottitrekhc': get_csv_path('ssfineknottitrekhc.csv'),
                'salakilliquid': get_csv_path('ssfineksalakilliquid.csv'),
                'shitremhc': get_csv_path('ssfinekshitremhc.csv')
            }
    ...
    def show_qpcal_window(self):
        ...
        try:
            ...
            qpcal_win.geometry("900x700")
            qpcal_win.transient(self.win)
            ...
            info_label = ttk.Label(qpcal_win, 
                                  text="LRPAN fiyatına en uzak taraf (Bid/Ask) bazında profit hesaplama",
                                  font=("Arial", 10))
            info_label.pack(pady=5)
            
            # Qpcal sonuçları tablosu
            columns = ('Symbol', 'LRPAN', 'Bid', 'Ask', 'Qpcal', 'Direction', 'Both')
            qpcal_tree = ttk.Treeview(qpcal_win, columns=columns, show='headings', height=25)
            ...
            for col in columns:
                ...
                if col == 'Symbol':
                    ...
                    qpcal_tree.column(col, width=150, anchor='center')
                elif col == 'Direction':
                    qpcal_tree.column(col, width=80, anchor='center')
                elif col == 'Both':
                    qpcal_tree.column(col, width=60, anchor='center')
            
            # Scrollbar
            scrollbar = ttk.Scrollbar(qpcal_win, orient='vertical', command=qpcal_tree.yview)
            qpcal_tree.configure(yscrollcommand=scrollbar.set)
            
            qpcal_tree.pack(side='left', fill='both', expand=True, padx=10, pady=10)
            scrollbar.pack(side='right', fill='y', pady=10)
            ...
            for _, row in self.filtered_data.iterrows():
                ...
                # Qpcal hesapla
                qpcal_value, direction, both_flag = self.calculate_qpcal(symbol, lrpan_price, bid, ask)
                
                if qpcal_value is not None:
                    # Sonuçları kaydet
            ...
            # Qpcal değerine göre büyükten küçüğe sırala
            qpcal_results.sort(key=lambda x: x['qpcal'], reverse=True)
            
            # Tabloya ekle
            for result in qpcal_results:
                lrpan_str = f"${result['lrpan']:.2f}" if result['lrpan'] else "N/A"
            ...
            print(f"[QPcal] ✅ Analiz tamamlandı: {len(qpcal_results)} hisse")
