...
Bu modül CSV dosyalarını okur:
✅ DOĞRU: get_csv_path("etfs.csv") (njanall dizininde)
❌ YANLIŞ: "etfs.csv" (StockTracker dizininde)
=================================
"""
...
class ETFPanel(ttk.Frame):
    def __init__(self, parent, hammer_client):
        ...
        # Sadece L1 streaming kullanıyoruz - snapshot kaldırıldı
    ...
    def load_etf_prev_close_data(self):
        ...
        try:
            import os
            janeketfs_path = get_csv_path('janeketfs.csv')
            if os.path.exists(janeketfs_path):
                import pandas as pd
                etf_df = pd.read_csv(janeketfs_path)
                for _, row in etf_df.iterrows():
                    symbol = row['Symbol']
                    prev_close = row['prev_close']
                    if prev_close > 0:
                        self.etf_prev_close_data[symbol] = prev_close
                print(f"[ETF] OK ETF prev_close verileri yuklendi: {len(self.etf_prev_close_data)} ETF")
            else:
                print("[ETF] ⚠️ janeketfs.csv dosyası bulunamadı")
        except Exception as e:
            print(f"[ETF] ERROR ETF prev_close verileri yuklenirken hata: {e}")
    ...
    def get_etf_price(self, symbol):
        """Bir ETF için güncel fiyatı al (cache'den)"""
        if symbol in self.etf_data:
            return self.etf_data[symbol].get('last', 0)
        return 0
        
    def load_etfs_from_csv(self):
        """ETF'leri CSV'den yükle"""
        try:
            df = pd.read_csv(get_csv_path('etfs.csv'))
            self.etf_list = df['PREF IBKR'].tolist()
            self.etf_df = df
            print(f"[ETF] OK {len(self.etf_list)} ETF yuklendi: {self.etf_list}")
        except Exception as e:
            print(f"[ETF] ERROR ETF CSV yukleme hatasi: {e}")
            # Fallback: Sabit liste - IEI zaten mevcut
            self.etf_list = ["SHY", "IEF", "TLT", "IWM", "KRE", "SPY", "PFF", "PGF", "IEI"]
            self.etf_df = pd.DataFrame({'PREF IBKR': self.etf_list})
            print(f"[ETF] OK Fallback ETF listesi kullanildi: {self.etf_list}")
    ...
    def update_etf_data(self, symbol, market_data):
        ...
        try:
            ...
            # PrevClose yoksa streaming'den al (snapshot kaldırıldı!)
            if prev_close <= 0 and last > 0:
                # Streaming'den güncel prev_close al
    ...
    def update_etf_display(self):
        ...
        self.etf_table.tag_configure('negative', foreground='darkred', font=('Arial', 8, 'bold'))
            ...
            # SNAPSHOT FONKSİYONLARI KALDIRILDI - Sadece L1 streaming kullanıyoruz!