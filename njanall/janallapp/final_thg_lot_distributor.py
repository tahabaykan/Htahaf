...
class FinalThgLotDistributor:
    ...
    def apply_tumcsv_rules(self):
        ...
        try:
            ...
            for group in long_groups_only:
                ...
                # HELDKUPONLU grubu için özel CGRUP bazlı seçim
                if group.lower() == 'heldkuponlu':
                    print(f"   🎯 HELDKUPONLU grubu için özel CGRUP bazlı seçim uygulanıyor...")
                    
                    # BB Long ve SAS Short için kombinasyon seçimi (toplam 8 pair = 16 hisse, min 5 farklı CGRUP)
                    bb_long_stocks, sas_short_stocks = self.select_heldkuponlu_cgrup_combinations(
                        group_stocks, 'Final_BB_skor', 'Final_SAS_skor', min_cgrup_count=5, target_pair_count=8
                    )
                    
                    # FB Long ve SFS Short için kombinasyon seçimi (toplam 8 pair = 16 hisse, min 5 farklı CGRUP)
                    fb_long_stocks, sfs_short_stocks = self.select_heldkuponlu_cgrup_combinations(
                        group_stocks, 'Final_FB_skor', 'Final_SFS_skor', min_cgrup_count=5, target_pair_count=8
                    )
                    
                    # Eğer özel seçim başarısız olduysa (boş döndüyse), normal seçime dön
                    if len(bb_long_stocks) == 0 and len(sas_short_stocks) == 0:
                        print(f"   ⚠️ HELDKUPONLU BB-SAS özel seçim başarısız, normal seçime dönülüyor...")
                        selected_count = max(2, round(len(group_stocks) * 0.1))
                        bb_long_stocks = self.select_top_stocks_by_score(group_stocks, 'Final_BB_skor', selected_count, 'LONG')
                    
                    if len(fb_long_stocks) == 0 and len(sfs_short_stocks) == 0:
                        print(f"   ⚠️ HELDKUPONLU FB-SFS özel seçim başarısız, normal seçime dönülüyor...")
                        selected_count = max(2, round(len(group_stocks) * 0.1))
                        fb_long_stocks = self.select_top_stocks_by_score(group_stocks, 'Final_FB_skor', selected_count, 'LONG')
                ...
                if selected_orders:
                    ...
                    if success_count > 0:
                        ...
                        if should_schedule_callback:
                            self.main_window.log_message("✅ ADDNEWPOS emirleri gönderildi, 2 dakika sonra emirler iptal edilecek")
                            print(f"[RUNALL] ✅ ADDNEWPOS emirleri gönderildi ({success_count} başarılı), 2 dakika sonra emirler iptal edilecek")
                            ...
                            def schedule_cancel_and_restart():
                                print(f"[RUNALL] ⏰ schedule_cancel_and_restart() çağrıldı")
                                runall_still_running = hasattr(self.main_window, 'runall_loop_running') and self.main_window.runall_loop_running
                                runall_still_allowed = hasattr(self.main_window, 'runall_allowed_mode') and self.main_window.runall_allowed_mode
                                should_proceed = runall_still_running or runall_still_allowed
                                print(f"[RUNALL] 🔍 schedule_cancel_and_restart içinde: runall_loop_running={runall_still_running}, runall_allowed_mode={runall_still_allowed}, should_proceed={should_proceed}")
                                if should_proceed:
                                    # runall_loop_running'i True yap (eğer runall_allowed_mode aktifse)
                                    if runall_still_allowed and not runall_still_running:
                                        self.main_window.runall_loop_running = True
                                        print(f"[RUNALL] ✅ runall_loop_running True yapıldı (runall_allowed_mode aktif)")
                                    
                                    # Tamamlanan döngü sayısını göster
                                    completed_loops = self.main_window.runall_loop_count if hasattr(self.main_window, 'runall_loop_count') else 0
                                    self.main_window.log_message(f"⏰ 2 dakika geçti, {completed_loops}. döngü için emirler iptal ediliyor...")
                                    print(f"[RUNALL] ⏰ 2 dakika geçti, {completed_loops}. döngü için emirler iptal ediliyor...")
                                    self.main_window.runall_cancel_orders_and_restart()
                                else:
                                    print(f"[RUNALL] ⚠️ RUNALL döngüsü durdurulmuş, callback iptal edildi")
                            
                            print(f"[RUNALL] ⏰ 120 saniye (2 dakika) sonra schedule_cancel_and_restart() çağrılacak")
                            self.main_window.after(120000, schedule_cancel_and_restart)  # 2 dakika bekleme
    ...
    def select_heldkuponlu_cgrup_combinations(self, group_stocks, long_score_col, short_score_col, min_cgrup_count=5, target_pair_count=8):
        ...
        Toplam 8 pair seçer (16 hisse: 8 long + 8 short), minimum 5 farklı CGRUP garantisi ile
        BB-SAS ve FB-SFS için maksimum farklı aynı CGRUP pairlerini yakalar
        ...
        Args:
            ...
            min_cgrup_count: Minimum farklı CGRUP sayısı (varsayılan 5)
            target_pair_count: Toplam seçilecek pair sayısı (varsayılan 8 = 16 hisse)
        ...
        try:
            ...
            print(f"   🎯 Minimum {min_cgrup_count} farklı CGRUP garantisi")
            ...
            # Score difference'e göre sırala (en yüksek fark önce)
            cgrup_combinations.sort(key=lambda x: x['score_diff'], reverse=True)
            
            # Minimum CGRUP sayısını garanti et ve maksimum farklı pairleri seç
            selected_combinations = []
            selected_cgrups = set()
            cgrup_pair_counts = {}  # Her CGRUP'tan kaç pair seçildiğini takip et
            
            # Önce en yüksek farklı kombinasyonları seç (minimum 5 farklı CGRUP garantisi ile)
            for combo in cgrup_combinations:
                cgrup = combo['cgrup']
                
                # Eğer minimum CGRUP sayısına ulaşmadıysak, sadece yeni CGRUP'ları ekle
                if len(selected_cgrups) < min_cgrup_count:
                    if cgrup not in selected_cgrups:
                        selected_combinations.append(combo)
                        selected_cgrups.add(cgrup)
                        cgrup_pair_counts[cgrup] = 1
                        print(f"      ✅ CGRUP {cgrup} eklendi (min {min_cgrup_count} farklı CGRUP garantisi için) - Pair #{len(selected_combinations)}")
                else:
                    # Minimum CGRUP sayısına ulaştıktan sonra, maksimum farklı pairleri seç
                    if len(selected_combinations) < target_pair_count:
                        ...
                        print(f"      ✅ CGRUP {cgrup} eklendi (maksimum farklı pair için) - Pair #{len(selected_combinations)}")
                    else:
                        break  # Hedef pair sayısına ulaştık
            
            # Eğer minimum CGRUP sayısına ulaşamadıysak, kalanları da ekle
            if len(selected_cgrups) < min_cgrup_count:
                print(f"   ⚠️ Minimum {min_cgrup_count} CGRUP bulunamadı, mevcut {len(selected_cgrups)} CGRUP kullanılacak")
            
            # Şirket kısıtı uygula (heldkuponlu için her şirketten max 2 hisse)
            long_stocks = []
            short_stocks = []
            company_counts = {}
            max_per_company = 2  # HELDKUPONLU için özel: maksimum 2 hisse/şirket
            ...
            for combo in selected_combinations:
                ...
                # Long için şirket kısıtı kontrolü
                if long_company not in company_counts:
                    company_counts[long_company] = {'long': 0, 'short': 0}
                
                if company_counts[long_company]['long'] < max_per_company:
                    long_stocks.append(long_stock)
                    company_counts[long_company]['long'] += 1
                    print(f"      ✅ Long eklendi: {long_stock.get('PREF IBKR', 'N/A')} ({long_company}) "
                          f"[{company_counts[long_company]['long']}/{max_per_company}]")
                else:
                    print(f"      ❌ Long atlandı: {long_stock.get('PREF IBKR', 'N/A')} ({long_company}) - Şirket kısıtı (max {max_per_company})")
                
                # Short için şirket kısıtı kontrolü
                if short_company not in company_counts:
                    company_counts[short_company] = {'long': 0, 'short': 0}
                
                if company_counts[short_company]['short'] < max_per_company:
                    short_stocks.append(short_stock)
                    company_counts[short_company]['short'] += 1
                    print(f"      ✅ Short eklendi: {short_stock.get('PREF IBKR', 'N/A')} ({short_company}) "
                          f"[{company_counts[short_company]['short']}/{max_per_company}]")
                else:
                    print(f"      ❌ Short atlandı: {short_stock.get('PREF IBKR', 'N/A')} ({short_company}) - Şirket kısıtı (max {max_per_company})")
            ...
            for cgrup, count in sorted(cgrup_pair_counts.items()):
                print(f"      📊 CGRUP {cgrup}: {count} pair")
