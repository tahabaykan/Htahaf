...
# Market data imports
try:
    from ..data.hammer_market_data import HammerMarketDataManager
except ImportError:
    from data.hammer_market_data import HammerMarketDataManager

try:
    from .maltopla_window import MaltoplaWindow
except ImportError:
    from gui.maltopla_window import MaltoplaWindow
...
try:
    ...
    print("Warning: Could not import HammerIB components. Some features may not work.")
    ETFPanel = None
    create_opt_buttons = None
    BenchmarkPanel = None
...
try:
    from Ptahaf.gui.general_window import GeneralWindow
except ImportError:
    from gui.general_window import GeneralWindow

try:
    from Ptahaf.utils.order_management import OrderManager
except ImportError:
    from utils.order_management import OrderManager
...
try:
    ...
    from utils.bdata_storage import BDataStorage

try:
    from Ptahaf.gui.orderbook_window import OrderBookWindow
except ImportError:
    from gui.orderbook_window import OrderBookWindow

from datetime import datetime

try:
    from Ptahaf.utils.benchmark_calculator import BenchmarkCalculator
except ImportError:
    from utils.benchmark_calculator import BenchmarkCalculator

try:
    from Ptahaf.psfalgo import PsfAlgo as PsfAlgo1
except ImportError:
    from psfalgo import PsfAlgo as PsfAlgo1

try:
    from Ptahaf.psfalgo2 import PsfAlgo2
except ImportError:
    from psfalgo2 import PsfAlgo2
...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        # Market data manager
        self.market_data = HammerMarketDataManager()
        ...
        # Hammer Pro variables
        self.hammer_pro_client = None
        self.hammer_pro_connected = False
        ...
        # Map T-pref ticker to Group (benchmark group)
        self.tpref_cgrup_map = {}
        for _, row in self.historical_df.iterrows():
            ticker = str(row['PREF IBKR']).strip()
            cgrup = str(row['Group']).strip().lower()
        ...
        # --- PsfAlgo Exclude Listeleri ---
        self.komple_exclude = self.load_komple_exclude()
        self.half_sized_exclude = self.load_half_sized_exclude()
        
        # Geriye uyumluluk için eski listeyi birleştir
        self.psfalgo_exclude = self.komple_exclude.union(self.half_sized_exclude)

        # ✅ PSFAlgo1 ve PSFAlgo2 instance'larını oluştur
        self.psf_algo1 = PsfAlgo1(self.market_data, exclude_list=self.komple_exclude, half_sized_list=self.half_sized_exclude)
        self.psf_algo2 = PsfAlgo2(self.market_data, exclude_list=self.komple_exclude, half_sized_list=self.half_sized_exclude)
        ...
        # ✅ BEFDAY.csv güncellemesi kontrolü
        self.check_befday_update_and_warn()

    def setup_ui(self):
        # Birinci satır: Market data ve temel bağlantı butonları
        top1 = ttk.Frame(self)
        top1.pack(fill='x')
        
        # Market data bağlantı butonu
        self.btn_connect_market = ttk.Button(top1, text="Market Data'ya Bağlan", 
                                           command=self.connect_market_data, width=16)
        self.btn_connect_market.pack(side='left', padx=2)
        ...
        # İkinci satır: Maltopla ve diğer butonlar (10 buton)
        top2 = ttk.Frame(self)
        top2.pack(fill='x', pady=(2, 0))
        self.btn_opt50_maltopla = ttk.Button(top2, text="Opt50 maltopla", command=self.open_opt50_maltopla_window, width=14)
        self.btn_opt50_maltopla.pack(side='left', padx=2)
        self.btn_extlt35_maltopla = ttk.Button(top2, text="Extlt35 maltopla", command=self.open_extlt35_maltopla_window, width=14)
        self.btn_extlt35_maltopla.pack(side='left', padx=2)
        self.btn_exdiv = ttk.Button(top2, text="Paste Ex-Div List", command=self.paste_exdiv_list, width=16)
        self.btn_exdiv.pack(side='left', padx=2)
        self.btn_auto_data = ttk.Button(top2, text="AutoDataOFF", command=self.toggle_auto_data, width=14)
        self.btn_auto_data.pack(side='left', padx=2)
        self.btn_psf_algo1 = ttk.Button(top2, text="PsfAlgo1 OFF", command=self.activate_psf_algo1, width=14)
        self.btn_psf_algo1.pack(side='left', padx=2)
        self.btn_psf_algo2 = ttk.Button(top2, text="PsfAlgo2 OFF", command=self.activate_psf_algo2, width=14)
        self.btn_psf_algo2.pack(side='left', padx=2)
        self.btn_komple_exclude = ttk.Button(top2, text="Komple Exclude", command=self.open_komple_exclude_window, width=14)
        self.btn_komple_exclude.pack(side='left', padx=2)
        self.btn_half_sized = ttk.Button(top2, text="Half Sized", command=self.open_half_sized_window, width=14)
        self.btn_half_sized.pack(side='left', padx=2)
        self.btn_psf_reasoning = ttk.Button(top2, text="Psf Reasoning", command=self.open_psf_reasoning_window, width=16)
        self.btn_psf_reasoning.pack(side='left', padx=2)
        self.btn_befday = ttk.Button(top2, text="BEFDAY", command=self.befday_snapshot, width=10)
        self.btn_befday.pack(side='left', padx=2)
        
        # Üçüncü satır: BEFDAY Update ve PSF test butonları (10 buton)
        top3 = ttk.Frame(self)
        top3.pack(fill='x', pady=(2, 0))
        self.btn_befday_update = ttk.Button(top3, text="BEFDAY Update", command=self.update_befday_csv, width=12)
        self.btn_befday_update.pack(side='left', padx=2)
        self.btn_psf_test = ttk.Button(top3, text="PSF Test", command=self.test_psf_reverse, width=10)
        self.btn_psf_test.pack(side='left', padx=2)
        self.btn_psf_debug = ttk.Button(top3, text="PSF Debug", command=self.debug_psf_fills, width=10)
        ...
        # Dördüncü satır: Pozisyonlar ve emirler butonları (6 buton)
        top4 = ttk.Frame(self)
        top4.pack(fill='x', pady=(2, 0))
        self.btn_positions = ttk.Button(top4, text="Pozisyonlarım", command=self.open_positions_window, width=14)
        self.btn_positions.pack(side='left', padx=2)
        self.btn_orders = ttk.Button(top4, text="Emirlerim", command=self.open_orders_window, width=14)
        self.btn_orders.pack(side='left', padx=2)
        self.btn_t_top_losers_maltopla = ttk.Button(top4, text="T-top losers", command=self.open_t_top_losers_maltopla, width=14)
        self.btn_t_top_losers_maltopla.pack(side='left', padx=2)
        self.btn_t_top_gainers_maltopla = ttk.Button(top4, text="T-top gainers", command=self.open_t_top_gainers_maltopla, width=14)
        self.btn_t_top_gainers_maltopla.pack(side='left', padx=2)
        self.btn_long_take_profit = ttk.Button(top4, text='Long take profit', command=self.open_long_take_profit_window, width=14)
        self.btn_long_take_profit.pack(side='left', padx=2)
        self.btn_short_take_profit = ttk.Button(top4, text='Short take profit', command=self.open_short_take_profit_window, width=14)
        self.btn_short_take_profit.pack(side='left', padx=2)
        self.status_label = ttk.Label(top4, text="Durum: Bekleniyor")
        self.status_label.pack(side='left', padx=10)
        
        # Beşinci satır: SSFINEK CSV butonları (1. satır - 7 buton)
        ssfinek_frame1 = ttk.Frame(self)
        ssfinek_frame1.pack(fill='x', pady=(2, 0))
        self.btn_heldkuponlu = ttk.Button(ssfinek_frame1, text="heldkuponlu", command=lambda: self.open_ssfinek_window("../ssfinekheldkuponlu.csv"), width=12)
        self.btn_heldkuponlu.pack(side='left', padx=2)
        self.btn_heldbesmaturlu = ttk.Button(ssfinek_frame1, text="heldbesmaturlu", command=lambda: self.open_ssfinek_window("../ssfinekheldbesmaturlu.csv"), width=12)
        self.btn_heldbesmaturlu.pack(side='left', padx=2)
        self.btn_heldcilizyeniyedi = ttk.Button(ssfinek_frame1, text="heldcilizyeniyedi", command=lambda: self.open_ssfinek_window("../ssfinekheldcilizyeniyedi.csv"), width=12)
        self.btn_heldcilizyeniyedi.pack(side='left', padx=2)
        self.btn_heldcommonsuz = ttk.Button(ssfinek_frame1, text="heldcommonsuz", command=lambda: self.open_ssfinek_window("../ssfinekheldcommonsuz.csv"), width=12)
        self.btn_heldcommonsuz.pack(side='left', padx=2)
        self.btn_helddeznff = ttk.Button(ssfinek_frame1, text="helddeznff", command=lambda: self.open_ssfinek_window("../ssfinekhelddeznff.csv"), width=12)
        self.btn_helddeznff.pack(side='left', padx=2)
        self.btn_heldff = ttk.Button(ssfinek_frame1, text="heldff", command=lambda: self.open_ssfinek_window("../ssfinekheldff.csv"), width=12)
        self.btn_heldff.pack(side='left', padx=2)
        self.btn_heldflr = ttk.Button(ssfinek_frame1, text="heldflr", command=lambda: self.open_ssfinek_window("../ssfinekheldflr.csv"), width=12)
        self.btn_heldflr.pack(side='left', padx=2)
        
        # Altıncı satır: SSFINEK CSV butonları (2. satır - 7 buton)
        ssfinek_frame2 = ttk.Frame(self)
        ssfinek_frame2.pack(fill='x', pady=(2, 0))
        self.btn_heldgarabetaltiyedi = ttk.Button(ssfinek_frame2, text="heldgarabetaltiyedi", command=lambda: self.open_ssfinek_window("../ssfinekheldgarabetaltiyedi.csv"), width=12)
        self.btn_heldgarabetaltiyedi.pack(side='left', padx=2)
        self.btn_heldkuponlukreciliz = ttk.Button(ssfinek_frame2, text="heldkuponlukreciliz", command=lambda: self.open_ssfinek_window("../ssfinekheldkuponlukreciliz.csv"), width=12)
        self.btn_heldkuponlukreciliz.pack(side='left', padx=2)
        self.btn_heldkuponlukreorta = ttk.Button(ssfinek_frame2, text="heldkuponlukreorta", command=lambda: self.open_ssfinek_window("../ssfinekheldkuponlukreorta.csv"), width=12)
        self.btn_heldkuponlukreorta.pack(side='left', padx=2)
        self.btn_heldnff = ttk.Button(ssfinek_frame2, text="heldnff", command=lambda: self.open_ssfinek_window("../ssfinekheldnff.csv"), width=12)
        self.btn_heldnff.pack(side='left', padx=2)
        self.btn_heldotelremorta = ttk.Button(ssfinek_frame2, text="heldotelremorta", command=lambda: self.open_ssfinek_window("../ssfinekheldotelremorta.csv"), width=12)
        self.btn_heldotelremorta.pack(side='left', padx=2)
        self.btn_heldsolidbig = ttk.Button(ssfinek_frame2, text="heldsolidbig", command=lambda: self.open_ssfinek_window("../ssfinekheldsolidbig.csv"), width=12)
        self.btn_heldsolidbig.pack(side='left', padx=2)
        self.btn_heldtitrekhc = ttk.Button(ssfinek_frame2, text="heldtitrekhc", command=lambda: self.open_ssfinek_window("../ssfinekheldtitrekhc.csv"), width=12)
        self.btn_heldtitrekhc.pack(side='left', padx=2)
        
        # Yedinci satır: SSFINEK CSV butonları (3. satır - 7 buton)
        ssfinek_frame3 = ttk.Frame(self)
        ssfinek_frame3.pack(fill='x', pady=(2, 0))
        self.btn_highmatur = ttk.Button(ssfinek_frame3, text="highmatur", command=lambda: self.open_ssfinek_window("../ssfinekhighmatur.csv"), width=12)
        self.btn_highmatur.pack(side='left', padx=2)
        self.btn_notbesmaturlu = ttk.Button(ssfinek_frame3, text="notbesmaturlu", command=lambda: self.open_ssfinek_window("../ssfineknotbesmaturlu.csv"), width=12)
        self.btn_notbesmaturlu.pack(side='left', padx=2)
        self.btn_notcefilliquid = ttk.Button(ssfinek_frame3, text="notcefilliquid", command=lambda: self.open_ssfinek_window("../ssfineknotcefilliquid.csv"), width=12)
        self.btn_notcefilliquid.pack(side='left', padx=2)
        self.btn_nottitrekhc = ttk.Button(ssfinek_frame3, text="nottitrekhc", command=lambda: self.open_ssfinek_window("../ssfineknotcefilliquid.csv"), width=12)
        self.btn_nottitrekhc.pack(side='left', padx=2)
        self.btn_rumoreddanger = ttk.Button(ssfinek_frame3, text="rumoreddanger", command=lambda: self.open_ssfinek_window("../ssfinekrumoreddanger.csv"), width=12)
        self.btn_rumoreddanger.pack(side='left', padx=2)
        self.btn_salakilliquid = ttk.Button(ssfinek_frame3, text="salakilliquid", command=lambda: self.open_ssfinek_window("../ssfineksalakilliquid.csv"), width=12)
        self.btn_salakilliquid.pack(side='left', padx=2)
        self.btn_shitremhc = ttk.Button(ssfinek_frame3, text="shitremhc", command=lambda: self.open_ssfinek_window("../ssfinekshitremhc.csv"), width=12)
        self.btn_shitremhc.pack(side='left', padx=2)
    ...
    def connect_market_data(self):
        """Market data'ya bağlan"""
        try:
            print("[MAIN] 🔗 Market data bağlantısı başlatılıyor...")
            
            # ✅ Bağlantı denemesi
            if self.market_data.connect():
                self.status_label.config(text="Durum: Market data'ya bağlı")
                self.btn_connect_market.config(text="Bağlantıyı Kes", command=self.disconnect_market_data)
                ...
                if hasattr(self, 'psf_algo') and self.psf_algo is not None:
                    ...
                    print("[MAIN] PSFAlgo, market data event'lerine bağlandı")
                
                # ✅ Pozisyonları ve emirleri güncelle
                try:
                    print("[MAIN] 📊 BDATA başlatılıyor...")
                    self.init_bdata_from_market()
                    print("[MAIN] ✅ BDATA başlatıldı")
                except Exception as e:
                    print(f"[MAIN] ⚠️ BDATA hatası (normal): {e}")
                    print("[MAIN] 💡 Bu hata normal olabilir, uygulama çalışmaya devam edecek")
                
                try:
                    print("[MAIN] 📈 Tablolar güncelleniyor...")
                    self.update_tables()
                    print("[MAIN] ✅ Tablolar güncellendi")
                except Exception as e:
                    print(f"[MAIN] ⚠️ Tablo güncelleme hatası (normal): {e}")
                    print("[MAIN] 💡 Bu hata normal olabilir, uygulama çalışmaya devam edecek")
                
                print("[MAIN] ✅ Market data bağlantısı başarılı!")
                return  # ✅ Başarılı durumda return et
                
            else:
                self.status_label.config(text="Durum: Market data bağlantı hatası")
                print("[MAIN] ❌ Market data bağlantısı başarısız!")
                print("[MAIN] 💡 Çözüm önerileri:")
                print("   1. Hammer Pro açık mı?")
                print("   2. Hammer Pro ayarları doğru mu?")
                print("   3. Test scripti çalıştırın: python hammer_test.py")
                
        except Exception as e:
            print(f"[MAIN] ❌ Beklenmeyen bağlantı hatası: {e}")
            self.status_label.config(text="Durum: Market data bağlantı hatası")
            print("[MAIN] 💡 Detaylı hata analizi için test scripti çalıştırın")

    def disconnect_market_data(self):
        """Market data bağlantısını kapat"""
        self.market_data.disconnect()
        self.status_label.config(text="Durum: Market data bağlantısı kesildi")
        self.btn_connect_market.config(text="Market Data'ya Bağlan", command=self.connect_market_data)

    def toggle_live_data(self):
        """Canlı veriyi aç/kapat"""
        if not getattr(self, 'live_data_running', False):
            ...
            if self.market_data.subscribe_symbols(orig_tickers):
                ...
                print("[LIVE] ❌ Canlı veri başlatılamadı")
        else:
            # Sembollerden unsubscribe ol
            self.market_data.unsubscribe_symbols()
    ...
    def subscribe_visible(self):
        ...
        # Sembollere subscribe ol
        if getattr(self, 'live_data_running', False):
            self.market_data.subscribe_symbols(orig_tickers)
    ...
    def update_table(self, table, ticker_list, page):
        ...
        if hasattr(self.market_data, 'subscribe_symbol'):
            for orig in orig_tickers:
                # Orijinal sembolü direkt gönder (AMH PRG formatında)
                self.market_data.subscribe_symbol(orig)
        ...
        for orig, poly in zip(orig_tickers, poly_tickers):
            ...
            bid = d.get('bid', 'N/A')
            ask = d.get('ask', 'N/A')
            last = d.get('last', price)  # Use price if last not available
            ...
            try:
                last_f = float(last if last != 'N/A' else 0)
                prev_close_f = float(prev_close if prev_close != 'N/A' else 0)
                adj_prev_close = prev_close_f
                div = self.exdiv_data.get(orig, 0)
                if div:
                    adj_prev_close = prev_close_f - div
                daily_change = last_f - adj_prev_close if last_f and adj_prev_close else 'N/A'
                if daily_change != 'N/A':
            ...
            display_ticker = f"{orig} {'(LIVE)' if is_live else ''}"
            
            # Set row color based on live data
            ...
            table.insert('', 'end', values=(
                ...
                ask,
                last,
                adj_prev_close if 'adj_prev_close' in locals() else prev_close,
                volume,
                'N/A',  # SMA values not available in Hammer Pro
                'N/A',
                daily_change,
                'N/A',
                'N/A',
        ...
        table.tag_configure('live_data', background='lightgreen')
    ...
    def update_etf_panel(self):
        """ETF panelini canlı market data ile güncelle"""
        etf_data = {}
        
        # ETF'ler için market data al
        etf_symbols = ["PFF", "TLT", "SPY", "IWM", "KRE"]
        
        for sym in etf_symbols:
            # Market data'yı al
            data = self.market_data.get_market_data([sym])
            if data and sym in data:
                d = data[sym]
                
                # Fiyat bilgilerini al
                last = d.get('price', 'N/A')
                prev_close = d.get('prev_close', 'N/A')
                
                try:
                    if last != 'N/A' and prev_close != 'N/A':
                ...
                etf_data[sym] = {
                    'last': last if last != 'N/A' else 'N/A',
                    'change': change,
    ...
    def update_etf_data_once(self):
        ...
        try:
            print("[ETF UPDATE] 📊 ETF'ler güncelleniyor...")
            ...
            self.status_label.config(text="Durum: ETF'ler güncellendi")
            
            # Benchmark panelini de güncelle
            self.update_etf_panel()
            
        except Exception as e:
            print(f"[ETF UPDATE] ❌ ETF güncelleme hatası: {e}")
            self.status_label.config(text="Durum: ETF güncelleme hatası")
    ...
    def open_opt50_maltopla_window(self):
        import pandas as pd
        from Ptahaf.gui.maltopla_window import MaltoplaWindow
        def polygonize_ticker(symbol):
            symbol = str(symbol).strip()
            if ' PR' in symbol:
                base, pr = symbol.split(' PR', 1)
    ...
    def open_ssfinek_window(self, csv_filename):
        """
        SSFINEK CSV dosyaları için pencere açar (T-top losers/gainers ile aynı yapıda)
        """
        from Ptahaf.gui.maltopla_window import MaltoplaWindow
        import pandas as pd
        ...
        try:
            ...
            print(f"✅ {csv_filename} dosyası okundu: {len(df)} satır")
        except Exception as e:
            messagebox.showerror("Hata", f"{csv_filename} dosyası okunamadı: {e}")
            return
        
        # Pencere başlığını CSV adından çıkar
        window_title = csv_filename.replace("ssfinek", "").replace(".csv", "").upper()
        
        # Convert tickers to Polygon format
        tickers = df['PREF IBKR'].tolist()
        poly_tickers = [polygonize_ticker(t) for t in tickers]
        market_data_dict = self.market_data.get_market_data(poly_tickers)
        etf_data = self.market_data.get_etf_data()
        
        win = MaltoplaWindow(self, f"SSFINEK {window_title}", df, market_data_dict, etf_data, 
    ...
    def paste_exdiv_list(self):
        ...
        def save_to_csv():
            try:
                ...
                exdiv_dir = os.path.join('Ptahaf', 'exdivkayit')
        ...
        def load_todays_data():
            try:
                ...
                current_date = datetime.now().strftime('%d%m%y')
                filename = f"p{current_date}.csv"
                filepath = os.path.join('Ptahaf', 'exdivkayit', filename)
...
• Manuel fill eklemek için alt kısımdaki formu kullanabilirsiniz."""
        ...
        def load_from_ibkr():
            """IBKR'den mevcut pozisyonları yükle ve milat oluştur (batch snapshot ile)"""
            try:
                ...
                if not result:
                    return
                # Batch snapshot dictionary'leri hazırla
                market_data_dict = self.get_market_data_snapshot()
                benchmark_dict = self.get_benchmark_snapshot()
                positions = self.market_data.get_positions()
                bdata = BDataStorage()
                success = bdata.load_ibkr_positions(positions, market_data_dict, benchmark_dict)
                if success:
    ...
    def init_bdata_from_ibkr(self):
        ...
        try:
            ...
            # ✅ Timeout hatalarını normal kabul et
            if "timed out" in str(e).lower():
                print(f"[DEBUG] ⚠️ Pozisyon timeout hatası (normal): {e}")
                print("[DEBUG] ✅ Pozisyonlar zaten portfolio'dan geliyor")
                # Portfolio verileri zaten gelmiş, pozisyonları oradan al
                positions = []
                for pos in self.market_data.positions:
                    positions.append({
                ...
                print(f"[DEBUG] Portfolio'dan {len(positions)} pozisyon alındı")
            else:
                import tkinter.messagebox as mb
    ...
    def get_benchmarks(self):
        ...
        # ETF verilerini al
        etf_data = self.market_data.get_market_data(['PFF', 'TLT', 'SPY'])
        
        # PFF ve TLT fiyatlarını al
        pff_last = None
        tlt_last = None
        spy_last = None
        ...
        if etf_data:
            if 'PFF' in etf_data:
                pff_price = etf_data['PFF'].get('price', None)
                if pff_price != 'N/A':
                    try:
                        pff_last = float(pff_price)
                    except (ValueError, TypeError):
                        pass
                        
            if 'TLT' in etf_data:
                tlt_price = etf_data['TLT'].get('price', None)
                if tlt_price != 'N/A':
                    try:
                        tlt_last = float(tlt_price)
                    except (ValueError, TypeError):
                        pass
                        
            if 'SPY' in etf_data:
                spy_price = etf_data['SPY'].get('price', None)
                if spy_price != 'N/A':
                    try:
                        spy_last = float(spy_price)
                    except (ValueError, TypeError):
                        pass
        
        benchmarks = {}
        
        # T-pref: her ticker için
        if pff_last is not None and tlt_last is not None:
        ...
        # C-pref: SPY'ı benchmark olarak kullan
        if spy_last is not None:
            for ticker in self.extended_tickers:
                benchmarks[f'C-{ticker}'] = spy_last
    ...
    def load_psfalgo_exclude(self):
        """Geriye uyumluluk için eski exclude listesi"""
        path = "excpsfalgo.csv"
        if os.path.exists(path):
            try:
                df = pd.read_csv(path)
    ...
    def load_komple_exclude(self):
        """Komple exclude listesini yükle"""
        try:
            df = pd.read_csv('Ptahaf/kompleexclude.csv')
            return set(df['Symbol'].astype(str).str.strip().str.upper())
        except FileNotFoundError:
            print("kompleexclude.csv bulunamadı, boş set döndürülüyor")
            return set()
        except Exception as e:
            print(f"Komple exclude listesi yüklenemedi: {e}")
            return set()

    def save_komple_exclude(self):
        """Komple exclude listesini kaydet"""
        df = pd.DataFrame({'Symbol': list(self.komple_exclude)})
        df.to_csv('Ptahaf/kompleexclude.csv', index=False)

    def load_half_sized_exclude(self):
        """Half sized exclude listesini yükle"""
        try:
            df = pd.read_csv('Ptahaf/halfsized.csv')
            return set(df['Symbol'].astype(str).str.strip().str.upper())
        except FileNotFoundError:
            print("halfsized.csv bulunamadı, boş set döndürülüyor")
            return set()
        except Exception as e:
            print(f"Half sized exclude listesi yüklenemedi: {e}")
            return set()

    def save_half_sized_exclude(self):
        """Half sized exclude listesini kaydet"""
        df = pd.DataFrame({'Symbol': list(self.half_sized_exclude)})
        df.to_csv('Ptahaf/halfsized.csv', index=False)
    ...
    def open_komple_exclude_window(self):
        """Komple exclude penceresi"""
        win = tk.Toplevel(self)
        win.title("Komple Exclude - Psfalgo'dan tamamen hariç")
        win.geometry("400x500")
        
        # Açıklama metni
        info_label = tk.Label(win, text="Bu listedeki hisseler PSFAlgo tarafından hiçbir zaman işlem görmez", 
        ...
        def refresh_table():
            for i in table.get_children():
                table.delete(i)
            for sym in sorted(self.komple_exclude):
                table.insert('', 'end', values=(sym,))
        ...
        def add_symbol():
            sym = entry.get().strip().upper()
            if sym and sym not in self.komple_exclude:
                self.komple_exclude.add(sym)
        ...
        def remove_selected():
            for item in table.selection():
                sym = table.item(item, 'values')[0]
                self.komple_exclude.discard(sym)
            refresh_table()
        
        ttk.Button(win, text="Seçiliyi Sil", command=remove_selected).pack(pady=5)
        
        def save_and_close():
            self.save_komple_exclude()
            # PSFAlgo instance'larına güncellemeyi bildir
            if hasattr(self, 'psf_algo1'):
                self.psf_algo1.exclude_list = self.komple_exclude
            if hasattr(self, 'psf_algo2'):
                self.psf_algo2.exclude_list = self.komple_exclude
            win.destroy()
        
        ttk.Button(win, text="Kaydet ve Kapat", command=save_and_close).pack(pady=10)

    def open_half_sized_window(self):
        """Half sized exclude penceresi"""
        win = tk.Toplevel(self)
        win.title("Half Sized - Yarı boyutlu emir")
        win.geometry("400x500")
        
        # Açıklama metni
        info_label = tk.Label(win, text="Bu listedeki hisseler için ADV yarıya düşer ve emir boyutları yarıya iner", 
        ...
        def refresh_table():
            for i in table.get_children():
                table.delete(i)
            for sym in sorted(self.half_sized_exclude):
                table.insert('', 'end', values=(sym,))
        ...
        def add_symbol():
            sym = entry.get().strip().upper()
            if sym and sym not in self.half_sized_exclude:
                self.half_sized_exclude.add(sym)
                entry.delete(0, tk.END)
                refresh_table()
        ...
        def remove_selected():
            for item in table.selection():
                sym = table.item(item, 'values')[0]
                self.half_sized_exclude.discard(sym)
            refresh_table()
        
        ttk.Button(win, text="Seçiliyi Sil", command=remove_selected).pack(pady=5)
        
        def save_and_close():
            self.save_half_sized_exclude()
            # PSFAlgo instance'larına güncellemeyi bildir
            if hasattr(self, 'psf_algo1'):
                self.psf_algo1.half_sized_list = self.half_sized_exclude
            if hasattr(self, 'psf_algo2'):
                self.psf_algo2.half_sized_list = self.half_sized_exclude
            win.destroy()
...
Tarih/Saat: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
    ...
    def check_befday_update_and_warn(self):
        """BEFDAY.csv güncellemesi kontrolü ve uyarı sistemi"""
        import os
        ...
        try:
            ...
            now = datetime.now()
            today = date.today()
            cutoff_time = time(16, 30)  # 16:30
            
            # BEFDAY.csv dosyası var mı?
            if not os.path.exists('BEFDAY.csv'):
                messagebox.showwarning(
                    "BEFDAY.csv Eksik!", 
                    "❌ BEFDAY.csv dosyası bulunamadı!\n\n"
                    "PSFAlgo'yu kullanmadan önce BEFDAY.csv dosyasını oluşturun.\n\n"
                    "Bu dosya günlük pozisyon limitlerini kontrol eder."
                )
                return
            ...
            print(f"[BEFDAY CHECK] Dosya son güncelleme: {last_modified_datetime}")
            print(f"[BEFDAY CHECK] Bugün: {today}, Cutoff: {cutoff_time}")
            
            # Bugün güncellenmiş mi?
            if last_modified_date == today:
                print(f"[BEFDAY CHECK] ✅ BEFDAY.csv bugün güncellendi - OK")
                return
            
            # Bugünden farklı bir gün güncellenmişse - uyarı ver
            message = (
                f"⚠️ BEFDAY.csv GÜNCELLEMESİ GEREKLİ!\n\n"
                f"Son güncelleme: {last_modified_date.strftime('%d.%m.%Y')}\n"
                f"Bugün: {today.strftime('%d.%m.%Y')}\n\n"
                f"PSFAlgo'yu kullanmadan önce BEFDAY.csv'yi güncelleyin.\n\n"
                f"🕐 Güncelleme saatleri: Her gün 16:30'dan önce bir kez\n"
                f"📋 Bu dosya günlük 600 lot limitini kontrol eder\n\n"
                f"Güncellemeden devam ederseniz PSFAlgo emirleri reddedecektir."
            )
            
            # Eğer 16:30'dan önce ise daha urgent uyarı
            if now.time() < cutoff_time:
                message += f"\n\n⏰ ŞU ANDA SAAT: {now.strftime('%H:%M')} - Güncelleme için hala vakit var!"
            else:
                message += f"\n\n⏰ ŞU ANDA SAAT: {now.strftime('%H:%M')} - Cutoff 16:30 geçti!"
            
            messagebox.showwarning("BEFDAY.csv Güncellemesi Gerekli", message)
            
        except Exception as e:
            print(f"[BEFDAY CHECK] ❌ Kontrol hatası: {e}")
            messagebox.showerror(
                "BEFDAY.csv Kontrol Hatası", 
                f"BEFDAY.csv kontrol edilirken hata oluştu:\n{e}\n\n"
                f"PSFAlgo'yu kullanmadan önce dosyayı kontrol edin."
            )

    def update_befday_csv(self):
        """BEFDAY.csv'yi bugünkü tarihle güncelle (manuel tetikleme)"""
        import os
        from datetime import datetime
        
        try:
            if not os.path.exists('BEFDAY.csv'):
                messagebox.showerror("Hata", "BEFDAY.csv dosyası bulunamadı!")
                return
            
            # Dosyayı 'touch' et (son değişiklik tarihini bugüne güncelle)
            # Aslında dosya içeriğini değiştirmek daha güvenli
            import pandas as pd
            
            # Dosyayı oku
            df = pd.read_csv('BEFDAY.csv')
            
            # Yeni bir kolona bugünün tarihini ekle (güncelleme kanıtı için)
            today_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            df['Last_Update'] = today_str
            
            # Dosyayı kaydet
            df.to_csv('BEFDAY.csv', index=False)
            
            print(f"[BEFDAY UPDATE] ✅ BEFDAY.csv güncellendi: {today_str}")
            
            # PSFAlgo1'deki status'u da güncelle
            if hasattr(self, 'psf_algo1') and self.psf_algo1:
                self.psf_algo1.befday_update_status = self.psf_algo1.check_befday_update_status()
                print(f"[BEFDAY UPDATE] PSFAlgo1 status güncellendi: {self.psf_algo1.befday_update_status}")
            
            messagebox.showinfo("Başarılı", f"✅ BEFDAY.csv başarıyla güncellendi!\n\nGüncelleme zamanı: {today_str}")
            
        except Exception as e:
            print(f"[BEFDAY UPDATE] ❌ Güncelleme hatası: {e}")
            messagebox.showerror("Hata", f"BEFDAY.csv güncelleme hatası:\n{e}")

    def open_long_take_profit_window(self, on_close_callback=None):
        """Long take profit penceresi aç"""
        def polygonize_ticker(symbol):
            # T-pref tickerları için polygonize et
        ...
        # CSV dosyasını yükle
        try:
            df = pd.read_csv('long_take_profit.csv')
            df['Symbol'] = df['Symbol'].apply(polygonize_ticker)
            
            # Pencere oluştur
            win = tk.Toplevel(self)
            win.title("Long Take Profit")
            win.geometry("1200x800")
            
            # Maltopla penceresi olarak aç
            from Ptahaf.gui.maltopla_window import MaltoplaWindow
            maltopla = MaltoplaWindow(win, df, "Long Take Profit")
            
            # PSFAlgo'ya pencere referansını ver
            if hasattr(self, 'psf_algo1') and self.psf_algo1:
                self.psf_algo1.on_window_opened(maltopla)
                maltopla.set_data_ready_callback(lambda: self.psf_algo1.on_data_ready(maltopla))
            
            if on_close_callback:
                win.protocol("WM_DELETE_WINDOW", lambda: [win.destroy(), on_close_callback()])
                
        except Exception as e:
            print(f"[GUI] ❌ Long Take Profit penceresi açılamadı: {e}")

    def open_heldkuponlu_window(self, on_close_callback=None):
        """HELDKUPONLU penceresini aç"""
        print("[MAIN] HELDKUPONLU penceresi açılıyor")
        ...
        # ✅ PISDoNGU durumuna göre başlık belirle
        base_title = "HELDKUPONLU - Maltopla"
        
        # PSFAlgo1 kontrolü (yeni 4 adımlı HELDKUP sistemi)
        if hasattr(self, 'psf_algo1') and self.psf_algo1 and self.psf_algo1.is_active:
            chain_title = self.psf_algo1.get_chain_state_title()
            if chain_title:
                title = f"{chain_title} | {base_title}"
        ...
        # ✅ ssfinekheldkuponlu.csv'den ticker listesi al
        try:
            df = pd.read_csv('../ssfinekheldkuponlu.csv')
            print("[HELDKUPONLU] ssfinekheldkuponlu.csv yüklendi")
        except FileNotFoundError:
            try:
                df = pd.read_csv('../finekheldkuponlu.csv')
                print("[HELDKUPONLU] finekheldkuponlu.csv yüklendi")
            except FileNotFoundError:
                print("[HELDKUPONLU] ❌ CSV dosyası bulunamadı!")
                return
        
        # ✅ CSV'den ticker listesi al
        if 'PREF IBKR' in df.columns:
            tickers = [polygonize_ticker(str(row['PREF IBKR']).strip()) for _, row in df.iterrows()]
        else:
            print("[HELDKUPONLU] ❌ PREF IBKR sütunu bulunamadı!")
            return
        ...
        # ✅ ssfinekheldkuponlu.csv kullan
        win = MaltoplaWindow(self, title, '../ssfinekheldkuponlu.csv', market_data_dict, etf_data, 'T', self.market_data.ib, exdiv_data=self.exdiv_data)
        win.protocol("WM_DELETE_WINDOW", lambda w=win: (win.destroy(), on_close_callback() if on_close_callback else None))
        
        # PSFAlgo referansını ayarla
        if hasattr(self, 'psf_algo1'):
            win.psf_algo = self.psf_algo1
            print("[HELDKUPONLU] PSFAlgo referansı ayarlandı")
        
        print("[HELDKUPONLU] ✅ HELDKUPONLU penceresi açıldı")
        return win

    def get_market_data_snapshot(self):
        # Tüm tickers için market data dictionary'si döndür
        all_tickers = self.historical_tickers + self.extended_tickers
        poly_tickers = [self.ticker_map[orig] for orig in all_tickers]
        data = self.market_data.get_market_data(poly_tickers)
        return {orig: data.get(self.ticker_map[orig], {}) for orig in all_tickers}

    def get_benchmark_snapshot(self):
        # Tüm tickers için benchmark dictionary'si döndür
        etf_data = self.market_data.get_etf_data()
        pff_last = etf_data.get('PFF', {}).get('last', None)
        tlt_last = etf_data.get('TLT', {}).get('last', None)
        benchmarks = {}
        for ticker in self.historical_tickers:
            bench = self.get_tpref_benchmark(ticker, pff_last, tlt_last)
            if bench is not None:
                benchmarks[ticker] = bench
        # C-pref ve diğerleri için de ekle (örnek, C-Benchmark)
        # ...
        return benchmarks

    def open_order_confirmation_window(self, order_details, step_number):
        """HELDKUP emir onay penceresi açma"""
        print(f"[MAIN WINDOW] 📋 HELDKUP {step_number}. adım için onay penceresi açılıyor...")
        
        # Onay penceresi oluştur
        confirm_window = tk.Toplevel(self)
        confirm_window.title(f"HELDKUP {step_number}. Adım - Emir Onayı")
        confirm_window.geometry("600x400")
        confirm_window.transient(self)
        confirm_window.grab_set()
        confirm_window.focus_set()  # Pencereyi öne getir
        confirm_window.lift()       # Pencereyi en üste getir
        confirm_window.attributes('-topmost', True)  # Pencereyi en üstte tut
        
        # Başlık
        title_label = tk.Label(confirm_window, text=f"HELDKUP {step_number}. Adım - {len(order_details)} Emir", 
                              font=("Arial", 14, "bold"))
        ...
        # Treeview oluştur
        columns = ("Ticker", "Side", "Size", "Current Price", "Order Type", "Order Price")
        ...
        def on_approve():
            print(f"[MAIN WINDOW] ✅ HELDKUP {step_number}. adım emirleri onaylandı")
            # Gerçek emir gönderimi
            for order in order_details:
                final_price = order.get('final_price', order['price'])
                print(f"[MAIN WINDOW] 📤 {order['ticker']} {order['side']} {order['size']} lot @ {final_price} ({order['order_type']})")
                # Gerçek emir gönderimi
                if hasattr(self, 'psf_algo1'):
                    self.psf_algo1.send_order(order['ticker'], final_price, 0, order['side'], order['size'])
            
            confirm_window.destroy()
            # Sonraki adıma geç
            if hasattr(self, 'psf_algo1'):
                self.psf_algo1.advance_chain()
        
        def on_reject():
            print(f"[MAIN WINDOW] ❌ HELDKUP {step_number}. adım emirleri reddedildi")
            confirm_window.destroy()
            # Sonraki adıma geç
            if hasattr(self, 'psf_algo1'):
                self.psf_algo1.advance_chain()
        
        approve_btn = ttk.Button(button_frame, text="✅ Onayla", command=on_approve)
        approve_btn.pack(side="left", padx=5)
        
        reject_btn = ttk.Button(button_frame, text="❌ Reddet", command=on_reject)
        ...
        print(f"[MAIN WINDOW] ✅ HELDKUP {step_number}. adım onay penceresi açıldı")

    def show_hammer_pro_settings(self):
        """Show Hammer Pro connection settings dialog"""
        if HammerProDialog:
            dialog = HammerProDialog(self, self.on_hammer_pro_settings_connect)
            self.wait_window(dialog)
    
    def on_hammer_pro_settings_connect(self, config):
        """Callback when Hammer Pro settings are confirmed"""
        try:
            # Store configuration
            self.hammer_pro_config = config
            
            # Try to connect
            self.connect_hammer_pro()
            
        except Exception as e:
            messagebox.showerror("Hata", f"Hammer Pro bağlantı hatası: {str(e)}")
    
    def connect_hammer_pro(self):
        """Connect to Hammer Pro and switch to it as market data provider"""
        try:
            if not hasattr(self, 'hammer_pro_config'):
                messagebox.showwarning("Uyarı", "Önce Hammer Pro ayarlarını yapın!")
                return
            
            if self.hammer_pro_connected:
                messagebox.showinfo("Bilgi", "Zaten Hammer Pro'ya bağlısınız!")
                return
            
            # Create Hammer Pro client
            config = self.hammer_pro_config
            self.hammer_pro_client = HammerProClient(
                host=config['host'],
                port=config['port'],
            ...
            self.hammer_pro_client.on_connect = self._on_hammer_pro_connect
            self.hammer_pro_client.on_disconnect = self._on_hammer_pro_disconnect
            self.hammer_pro_client.on_market_data = self._on_hammer_pro_market_data
            
            # Connect
            if self.hammer_pro_client.connect():
                self.hammer_pro_connected = True
                self.status_label.config(text="Hammer Pro: Bağlandı")
                
                # Start data streamer
                self.hammer_pro_client.start_data_streamer(config['streamer_id'])
                
                # Switch market data provider
                self.market_data = self.hammer_pro_client
                
                # Subscribe visible symbols
                self.subscribe_visible()
                
                messagebox.showinfo("Başarılı", "Hammer Pro'ya başarıyla bağlandı!")
            else:
                messagebox.showerror("Hata", "Hammer Pro'ya bağlanılamadı!")
                
        except Exception as e:
            messagebox.showerror("Hata", f"Hammer Pro bağlantı hatası: {str(e)}")
    
    def disconnect_hammer_pro(self):
        """Disconnect from Hammer Pro and switch back to Polygon"""
        try:
            if self.hammer_pro_client:
                # Disconnect from Hammer Pro
                self.hammer_pro_client.disconnect()
                self.hammer_pro_client = None
                self.hammer_pro_connected = False
                
                # Switch back to Polygon
                self.market_data = self.polygon_data
                
                # Update status
                self.status_label.config(text="Hammer Pro: Bağlantı Kesildi")
                messagebox.showinfo("Bilgi", "Hammer Pro bağlantısı kesildi!")
                
                # Refresh data
                self.update_data_once()
                
        except Exception as e:
            messagebox.showerror("Hata", f"Hammer Pro bağlantı kesme hatası: {str(e)}")
    
    def _on_hammer_pro_connect(self):
        """Called when Hammer Pro connects"""
        self.hammer_pro_connected = True
        self.status_label.config(text="Hammer Pro: Bağlandı")
    
    def _on_hammer_pro_disconnect(self):
        """Called when Hammer Pro disconnects"""
        self.hammer_pro_connected = False
        self.status_label.config(text="Hammer Pro: Bağlantı Kesildi")
    
    def _on_hammer_pro_market_data(self, symbol, data):
        """Called when Hammer Pro sends market data"""
        try:
            # Update UI if needed
            if data.get('is_live', False):
                self.after(100, self.update_tables)
            
        except Exception as e:
            print(f"Error handling Hammer Pro market data: {e}")
    
    def get_hammer_pro_market_data(self, symbol):
        """Get market data from Hammer Pro"""
        if self.hammer_pro_client and self.hammer_pro_connected:
            return self.hammer_pro_client.get_market_data(symbol)
        return {}
