...
class PsfAlgo:
    ...
    def start_pisdongu_cycle(self):
        ...
        # 4. Chain'i başlat
        print(f"[🔄 OTOMATİK RESTART] 4️⃣ PSFAlgo1 chain başlatılıyor (1. adım: HELDKUP_BB)...")
    ...
    def start_chain(self):
        """PSFAlgo chain'ini başlat - HELDKUP 4 ADIMLI SİSTEM"""
        if not self.is_active or self.chain_state == 'IDLE':
            return
            
        print(f"[PSFAlgo CHAIN] Başlatılıyor - Durum: {self.chain_state}")
        
        # HELDKUP 4 ADIMLI SİSTEM
        if self.chain_state == 'HELDKUP_BB':
            self.run_heldkup_bb()  # 1. FINAL BB en yüksek 5 → bid buy
        elif self.chain_state == 'HELDKUP_AS':
            self.run_heldkup_as()  # 2. FINAL AS en düşük 5 → ask sell
        elif self.chain_state == 'HELDKUP_FB':
            self.run_heldkup_fb()  # 3. FINAL FB en yüksek 3 → front buy (T-Losers 2. adım)
        elif self.chain_state == 'HELDKUP_FS':
            self.run_heldkup_fs()  # 4. FINAL FS en düşük 3 → front sell (T-Gainers 2. adım)
    ...
    def advance_chain(self):
        """Chain'de bir sonraki aşamaya geç - HELDKUP 4 ADIMLI SİSTEM"""
        ...
        if self.chain_state == 'HELDKUP_BB':
            ...
            print(f"[PSFAlgo CHAIN] 1→2: HELDKUP_BB → HELDKUP_AS")
        elif self.chain_state == 'HELDKUP_AS':
            self.chain_state = 'HELDKUP_FB'
            print(f"[PSFAlgo CHAIN] 2→3: HELDKUP_AS → HELDKUP_FB")
        elif self.chain_state == 'HELDKUP_FB':
            self.chain_state = 'HELDKUP_FS'
            print(f"[PSFAlgo CHAIN] 3→4: HELDKUP_FB → HELDKUP_FS")
        elif self.chain_state == 'HELDKUP_FS':
            self.chain_state = 'FINISHED'
            print(f"[PSFAlgo CHAIN] 4→FINISHED: HELDKUP_FS → FINISHED")
        elif self.chain_state == 'FINISHED':
            self.finish_chain()
    ...
    def on_data_ready(self, window):
        """Veri hazır olduğunda çağrılır - HELDKUP 4 ADIMLI SİSTEM"""
        ...
        # Pencere tipine ve chain state'e göre işlem yap - HELDKUP SİSTEMİ
        window_title = window.title().lower()
        
        # HELDKUPONLU penceresi için (tüm adımlar)
        if "heldkuponlu" in window_title:
            if self.chain_state == 'HELDKUP_BB':
                # 1. ADIM: FINAL BB en yüksek 5 → Bid Buy
                self.run_heldkup_bb_data_ready()
            elif self.chain_state == 'HELDKUP_AS':
                # 2. ADIM: FINAL AS en düşük 5 → Ask Sell
                self.run_heldkup_as()
            elif self.chain_state == 'HELDKUP_FB':
                # 3. ADIM: FINAL FB en yüksek 3 → Front Buy (T-Losers 2. adım)
                self.run_heldkup_fb()
            elif self.chain_state == 'HELDKUP_FS':
                # 4. ADIM: FINAL FS en düşük 3 → Front Sell (T-Gainers 2. adım)
                self.run_heldkup_fs()
        else:
    ...
    def get_top_stocks_by_score(self, window, score_column, count=5, ascending=True, score_range=(0, 1500)):
        ...
        try:
            ...
            if not valid_stocks:
                print(f"[SCORE SELECTION] ❌ {score_column} için geçerli hisse bulunamadı")
                return []
    ...
    def run_heldkup_fb(self):
        """HELDKUP FB - FINAL FB en yüksek 3 hisse → Front Buy (T-Losers 2. adım)"""
        print("[HELDKUP CHAIN 3/4] 🎯 HELDKUP FB → FINAL FB en yüksek 3 hisse → Front Buy (T-Losers 2. adım)")
    ...
    def run_heldkup_fs(self):
        """HELDKUP FS - FINAL FS en düşük 3 hisse → Front Sell (T-Gainers 2. adım)"""
        print("[HELDKUP CHAIN 4/4] 🎯 HELDKUP FS → FINAL FS en düşük 3 hisse → Front Sell (T-Gainers 2. adım)")
    ...
    def prepare_orders_for_tickers(self, tickers, side, order_type, step_number):
        ...
        for ticker in tickers:
            ...
            # Emir tipine göre fiyat hesapla
            final_price = self.calculate_order_price(ticker, price, order_type, side)
            ...
            order_details.append({
                ...
                'order_type': f"HIDDEN {order_type}"
            })
            print(f"[HELDKUP CHAIN {step_number}] 📤 {ticker} {side} 200 lot @ {price} → HIDDEN {order_type} @ {final_price}")
    ...
    def calculate_order_price(self, ticker, current_price, order_type, side):
        """Emir tipine göre fiyat hesapla - DOĞRU KURALLAR"""
        try:
            # Bid/Ask fiyatlarını al
            bid_ask_result = self.get_bid_ask_prices(ticker)
            if not bid_ask_result or bid_ask_result[0] is None or bid_ask_result[1] is None:
                print(f"[HELDKUP] ⚠️ {ticker} bid/ask alınamadı, current price kullanılıyor")
                return current_price
            
            bid, ask = bid_ask_result[0], bid_ask_result[1]
            spread = ask - bid
            print(f"[HELDKUP] ✅ {ticker} bid/ask alındı: Bid={bid:.3f}, Ask={ask:.3f}, Spread={spread:.3f}")
            
            if order_type == 'BID_BUY':
                # Bid + (Spread × 0.15) fiyatından HIDDEN BUY
                price = bid + (spread * 0.15)
                print(f"[HELDKUP] 🎯 {ticker} BID_BUY: {bid:.3f} + ({spread:.3f} × 0.15) = {price:.3f}")
                return price
                
            elif order_type == 'ASK_SELL':
                # Ask - (Spread × 0.15) fiyatından HIDDEN SELL
                price = ask - (spread * 0.15)
                print(f"[HELDKUP] 🎯 {ticker} ASK_SELL: {ask:.3f} - ({spread:.3f} × 0.15) = {price:.3f}")
                return price
                
            elif order_type == 'FRONT_BUY':
                # Last Price + 0.01 fiyatından HIDDEN BUY
                price = current_price + 0.01
                print(f"[HELDKUP] 🎯 {ticker} FRONT_BUY: {current_price:.3f} + 0.01 = {price:.3f}")
                return price
                
            elif order_type == 'FRONT_SELL':
                # Last Price - 0.01 fiyatından HIDDEN SELL
                price = current_price - 0.01
                print(f"[HELDKUP] 🎯 {ticker} FRONT_SELL: {current_price:.3f} - 0.01 = {price:.3f}")
                return price
                
            elif order_type == 'ASK_BUY':
                # Ask + 0.01 fiyatından HIDDEN BUY
                price = ask + 0.01
                print(f"[HELDKUP] 🎯 {ticker} ASK_BUY: {ask:.3f} + 0.01 = {price:.3f}")
                return price
                
            elif order_type == 'BID_SELL':
                # Bid - 0.01 fiyatından HIDDEN SELL
                price = bid - 0.01
                print(f"[HELDKUP] 🎯 {ticker} BID_SELL: {bid:.3f} - 0.01 = {price:.3f}")
                return price
            ...
            print(f"[HELDKUP] ❌ {ticker} fiyat hesaplama hatası: {e}")
            return current_price
    ...
    def open_confirmation_window(self, order_details, step_number):
        ...
        # Debug: main_window kontrolü
        print(f"[HELDKUP CHAIN {step_number}] DEBUG: self.main_window var mı? {self.main_window is not None}")
        if self.main_window:
            print(f"[HELDKUP CHAIN {step_number}] DEBUG: main_window.open_order_confirmation_window var mı? {hasattr(self.main_window, 'open_order_confirmation_window')}")
        
        # Gerçek onay penceresi açma işlemi
        if self.main_window and hasattr(self.main_window, 'open_order_confirmation_window'):
            print(f"[HELDKUP CHAIN {step_number}] ✅ Onay penceresi açılıyor...")
            try:
                self.main_window.open_order_confirmation_window(order_details, step_number)
                print(f"[HELDKUP CHAIN {step_number}] ✅ Onay penceresi başarıyla açıldı")
            except Exception as e:
                print(f"[HELDKUP CHAIN {step_number}] ❌ Onay penceresi açılırken hata: {e}")
                # Hata durumunda manuel emir gönderimi
                print(f"[HELDKUP CHAIN {step_number}] ✅ Manuel emir gönderimi yapılıyor...")
                for order in order_details:
                    print(f"[HELDKUP CHAIN {step_number}] 📤 {order['ticker']} {order['side']} {order['size']} lot @ {order['price']}")
                self.advance_chain()
        else:
            print(f"[HELDKUP CHAIN {step_number}] ❌ Onay penceresi açılamadı - manuel emir gönderimi")
            # Manuel emir gönderimi
            for order in order_details:
                print(f"[HELDKUP CHAIN {step_number}] 📤 {order['ticker']} {order['side']} {order['size']} lot @ {order['price']}")
            # Sonraki adıma geç
            self.advance_chain()
