...
class BDataStorage:
    ...
    def load_ibkr_positions(self, positions, market_data_dict, benchmark_dict):
        """IBKR'den mevcut pozisyonları yükle ve BDATA'ya milat olarak kaydet (batch snapshot ile)"""
        ...
        for pos in positions:
            ...
            # Batch snapshot dictionary'den veri çek
            if ticker not in market_data_dict or ticker not in benchmark_dict:
                print(f"[BDATA] {ticker} için market data veya benchmark eksik! Atlanıyor.")
                continue
            current_price = market_data_dict[ticker].get('last')
            current_benchmark = benchmark_dict[ticker]
            if current_price is None or current_benchmark is None:
                print(f"[BDATA] {ticker} için current_price veya current_benchmark eksik! Atlanıyor.")
                continue
            ...
            self.add_fill(
                ...
                benchmark_at_fill=avg_benchmark,
        ...
        print(f"[IBKR LOAD] ✅ {loaded_count} pozisyon BDATA'ya milat olarak eklendi (batch snapshot)")
        self.update_csv()
        return True
    ...
    def add_fill(self, ticker, direction, fill_price, fill_size, fill_time, benchmark_at_fill, is_position_increase=True):
        ...
        # Pozisyon arttırma ise Avg benchmark'ı güncelle
        if is_position_increase:
            self.update_avg_benchmark_on_position_increase(ticker, direction, fill_size, benchmark_at_fill)
    ...
    def get_position_summary_with_snapshot(self):
        ...
        for key, val in summary.items():
            ...
            if val['total_size'] > 0:
                ...
                # Snapshot kontrolü - milat durumunda snapshot'tan al
                snapshot = self.get_latest_snapshot(ticker)
                if snapshot:
                    # Snapshot varsa snapshot'tan avg_benchmark al
                    val['avg_benchmark'] = snapshot['avg_benchmark']
                else:
                    # Snapshot yoksa fill'lerden hesapla (milat durumu)
    ...
    def update_csv(self):
        ...
        try:
            ...
            for (ticker, direction), data in summary.items():
                ...
                if not snapshot:
                    # MİLAT durumu - Avg benchmark'ı yeniden hesapla
                    avg_cost = data['avg_cost']
                    calculated_avg_benchmark = current_benchmark - (current_price - avg_cost)
                    data['avg_benchmark'] = calculated_avg_benchmark
                    avg_outperformance = 0.0  # Milat için 0
                    
                    print(f"[BDATA CSV MILAT] {ticker}: avg_cost={avg_cost:.4f}, "
                          f"current_price={current_price:.4f}, current_benchmark={current_benchmark:.4f}, "
                          f"calculated_avg_benchmark={calculated_avg_benchmark:.4f}")
                else:
                    # Snapshot varsa normal hesaplama
                ...
                print(f"[BDATA CSV] {ticker}: size={total_size}, avg_cost={avg_cost:.4f}, "
                      ...
                      f"avg_outperf={avg_outperformance:.4f}")
    ...
    def update_avg_benchmark_on_position_increase(self, ticker, direction, new_fill_size, new_fill_benchmark):
        """Pozisyon arttırma sırasında Avg benchmark'ı ağırlıklı ortalama ile güncelle"""
        try:
            # Mevcut pozisyon özetini al
            summary = self.get_position_summary_with_snapshot()
            
            # Ticker için pozisyon var mı?
            key = (ticker, direction)
            if key not in summary:
                print(f"[BDATA AVG UPDATE] {ticker} için pozisyon bulunamadı")
                return False
            
            current_data = summary[key]
            current_total_size = current_data['total_size']
            current_avg_benchmark = current_data['avg_benchmark']
            
            # Yeni toplam size
            new_total_size = current_total_size + new_fill_size
            
            # Ağırlıklı ortalama hesapla
            # Avg benchmark = (current_size * current_avg_benchmark + new_size * new_benchmark) / total_size
            new_avg_benchmark = (current_total_size * current_avg_benchmark + new_fill_size * new_fill_benchmark) / new_total_size
            
            print(f"[BDATA AVG UPDATE] {ticker}: current_size={current_total_size}, current_avg_bench={current_avg_benchmark:.4f}, "
                  f"new_size={new_fill_size}, new_bench={new_fill_benchmark:.4f}, "
                  f"new_total_size={new_total_size}, new_avg_bench={new_avg_benchmark:.4f}")
            
            # Snapshot varsa güncelle
            snapshot = self.get_latest_snapshot(ticker)
            if snapshot:
                snapshot['avg_benchmark'] = new_avg_benchmark
                self._save_snapshots()
                print(f"[BDATA AVG UPDATE] {ticker} snapshot avg_benchmark güncellendi: {new_avg_benchmark:.4f}")
            
            return True
            
        except Exception as e:
            print(f"[BDATA AVG UPDATE] {ticker} avg benchmark güncelleme hatası: {e}")
            return False 
