...
def merge_comek_files():
    ...
    for file in comek_files:
        try:
            ...
            df = pd.read_csv(get_simulation_filename(file), encoding='utf-8-sig')
            df['SOURCE_FILE'] = file
    ...
    filtered_df.to_csv(get_simulation_filename(output_file), index=False, encoding='utf-8-sig')
...
def merge_ek_files_with_crdt_scores():
    ...
    try:
        allcomek_df = pd.read_csv(get_simulation_filename('allcomek.csv'), encoding='utf-8-sig')
    ...
    allcomek_df.to_csv(get_simulation_filename('allcomek.csv'), index=False, encoding='utf-8-sig')
...
def main():
    ...
    merged_df.to_csv(get_simulation_filename(output_file), index=False, encoding='utf-8-sig')
