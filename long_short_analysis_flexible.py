...
def analyze_long_short_positions_flexible():
    """Her CSV dosyası için long ve short pozisyonları analiz et - Esnek threshold'lar"""
    ...
    print("=" * 100)
    print("LONG & SHORT POZİSYON ANALİZİ - ESNEK THRESHOLD'LAR")
    print("=" * 100)
    print("📊 LONG LIST: Top %35 (Ortalama * 1.5'ten büyük)")
    print("📉 SHORT LIST: Bottom %30 (Ortalama * 0.5'ten küçük)")
    print("=" * 100)
    ...
    for i, filename in enumerate(files_to_analyze, 1):
        ...
        try:
            ...
            # Calculate flexible thresholds
            long_threshold = avg_final_thg * 1.5  # 1.75'ten 1.5'e düşürüldü
            short_threshold = avg_final_thg * 0.5  # 0.6'dan 0.5'e düşürüldü
            
            print(f"🎯 LONG threshold (Ortalama * 1.5): {long_threshold:.2f}")
            print(f"🎯 SHORT threshold (Ortalama * 0.5): {short_threshold:.2f}")
            ...
            print(f"\n🏆 LONG LIST (Top %35 + Ortalama * 1.5'ten büyük):")
            print("-" * 70)
            if len(long_list) > 0:
            ...
            print(f"\n📉 SHORT LIST (Bottom %30 + Ortalama * 0.5'ten küçük):")
            print("-" * 70)
            if len(short_list) > 0:
                print(f"{'Sıra':<4} {'Hisse Kodu':<15} {'FINAL THG':<12} {'Sektör':<15} {'Kredi Skoru':<12}")
                print("-" * 70)
    ...
    # Overall summary
    print(f"\n{'='*100}")
    print("GENEL ÖZET RAPOR - ESNEK THRESHOLD'LAR")
    print(f"{'='*100}")
    ...
    print(f"   • LONG/SHORT oranı: {total_long/total_short:.2f}:1" if total_short > 0 else "   • LONG/SHORT oranı: Sadece LONG pozisyonlar")
    ...
    print(f"\n🔄 THRESHOLD DEĞİŞİKLİĞİ ETKİSİ:")
    print("-" * 60)
    print(f"   • Önceki LONG threshold: Ortalama × 1.75")
    print(f"   • Yeni LONG threshold: Ortalama × 1.5 (daha esnek)")
    print(f"   • Önceki SHORT threshold: Ortalama × 0.6")
    print(f"   • Yeni SHORT threshold: Ortalama × 0.5 (daha esnek)")
    print(f"   • Beklenen sonuç: Daha fazla LONG, daha az SHORT pozisyon")
    
    return all_results

if __name__ == "__main__":
    results = analyze_long_short_positions_flexible() 