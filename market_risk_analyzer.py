...
def analyze_market_conditions(price_changes, market_data=None):
    ...
    base_solidity = 2.5
    base_yield = 1000  # 1500'den 1000'e çekildi - optimum denge için
    base_adv = 0.00025

    # --- Daha da daha agresif lineer momentum ağırlıklandırması ---
    yield_weight = base_yield * (1 + (market_momentum / 20) * 7.0)
    solidity_weight = base_solidity * (1 - (market_momentum / 20) * 3.5)
    yield_weight = max(yield_weight, base_yield * 0.05)
    solidity_weight = max(solidity_weight, base_solidity * 0.05)
...
def generate_market_report(price_changes, market_weights):
    ...
    print(f"Yield Ağırlık: {market_weights['yield_weight']:.2f} (Baz: 1000.00)")
    ...
    print(f"Yield Değişim: %{((market_weights['yield_weight']/1000 - 1) * 100):.1f}")
            ...
            if len(df) > 0:
                ...
                if 'date' in df.columns and df['date'].iloc[0] == today:
                    weights = {
                        ...
                        'market_momentum': df['market_momentum'].iloc[0] if 'market_momentum' in df.columns else 0
                    }
...
def main():
    ...
    if ib is None:
        ...
        return {'solidity_weight': 2.5, 'yield_weight': 1000, 'adv_weight': 0.00025}
    try:
        all_symbols = []
        for symbols in RISK_INDICATORS.values():
