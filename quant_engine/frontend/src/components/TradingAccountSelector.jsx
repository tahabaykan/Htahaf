  ...
  const handleModeChange = async (newMode) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Auto BEFDAY capture for IBKR accounts (first time today)
        if (result.new_mode === 'IBKR_GUN' || result.new_mode === 'IBKR_PED') {
          const befAccount = result.new_mode === 'IBKR_GUN' ? 'ibgun' : 'ibped'
          try {
            const befResponse = await fetch(`/api/befday/capture/${befAccount}`, { method: 'POST' })
            const befResult = await befResponse.json()
            if (befResult.success) {
              if (befResult.already_captured) {
                console.log(`📋 [BEFDAY] ${befAccount} already captured today`)
              } else {
                console.log(`📋 [BEFDAY] ${befAccount} captured: ${befResult.position_count} positions`)
              }
            }
          } catch (befErr) {
            console.warn(`[BEFDAY] ${befAccount} capture failed:`, befErr)
          }
        }
