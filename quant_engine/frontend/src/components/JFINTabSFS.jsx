  ...
  if (!stocks || stocks.length === 0) {
    return (
      <div className="jfin-tab-empty">
        No SFS (Soft Front Sell) stocks selected
      </div>
    )
  }
  ...
  return (
    <div className="jfin-tab">
      <div className="jfin-tab-header">
        <h3>SFS (Soft Front Sell) - Short Positions</h3>
        ...
        <table className="jfin-table">
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort('final_sfs_skor')} className="sortable">
                Final SFS Skor {sortConfig.key === 'final_sfs_skor' && (sortConfig.direction === 'asc' ? ' ↑' : ' ↓')}
              </th>
              <th onClick={() => handleSort('sfstot')} className="sortable">
                SFStot {sortConfig.key === 'sfstot' && (sortConfig.direction === 'asc' ? ' ↑' : ' ↓')}
              </th>
          ...
          <tbody>
            {sortedStocks.map((stock, idx) => (
              <tr key={stock.symbol || idx}>
                ...
                <td>{stock.group || '-'}</td>
                <td>{stock.cgrup || '-'}</td>
                <td className="jfin-score">{stock.final_sfs_skor?.toFixed(2) || '-'}</td>
