...
function formatValue(value, columnKey, row) {
  ...
  if (!isNaN(num) && isFinite(num)) {
    ...
    if (columnKey === 'overlay_benchmark_type') {
      ...
      // NEW: Show bench_source if available (cleaned up), or fallback to DOS GRUP
      // bench_source format: "Group: heldsolidbig (n=12)" -> "heldsolidbig"
      const benchSource = row?.bench_source
      if (benchSource && benchSource.startsWith('Group: ')) {
        return benchSource.split('Group: ')[1].split(' (')[0]
      }

      // Fallback to formatted GROUP (DOS GRUP)
      const groupValue = formatValue(row?.GROUP, 'GROUP', row)
      return groupValue !== '-' ? groupValue : (row?.overlay_benchmark_type || '-')
    }
    ...
    if (columnKey === 'overlay_benchmark_chg') {
      ...
      // NEW: Try bench_chg first (cents)
      const benchChg = row?.bench_chg !== undefined ? row.bench_chg : row?.overlay_benchmark_chg
      ...
      const sign = num >= 0 ? '+' : ''
      return sign + num.toFixed(2) // Cent display, 2 decimals
    }
