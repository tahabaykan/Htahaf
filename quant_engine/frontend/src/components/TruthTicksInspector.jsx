...
function TruthTicksInspector({ symbol, onClose }) {
  ...
  const { summary, filtering_report, volav_details, time_segmentation, path_dataset, overlay_bands, markers, volav_timeline, temporal_analysis } = data
  ...
  return (
    <div className="truth-ticks-inspector">
      ...
      <div className="inspector-tabs">
        ...
        <button
          className={activeTab === 'gem_analysis' ? 'active' : ''}
          onClick={() => setActiveTab('gem_analysis')}
          style={{ borderBottom: '2px solid #e83e8c', color: '#e83e8c' }}
        >
          💎 Gem Analysis
        </button>
      </div>
      ...
      <div className="inspector-content">
        {activeTab === 'gem_analysis' && (
          <div className="tab-content">
            <h3>💎 Gem Temporal Analysis</h3>
            <p className="tab-description">
              Analysis of price changes based on <strong>Trading Time</strong> (Volume-based lookback).
              Unlike wall-clock time, this finds the valid Volav level for the active periods relative to the last Truth Tick.
            </p>

            <div className="gem-analysis-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '20px' }}>
              {temporal_analysis && Object.entries(temporal_analysis).map(([tf, data]) => (
                <div key={tf} className="gem-card" style={{ background: '#1e1e1e', padding: '15px', borderRadius: '8px', border: '1px solid #333' }}>
                  <h4 style={{ margin: '0 0 10px 0', color: '#e83e8c', textTransform: 'uppercase' }}>{tf} Change</h4>
                  {data && data.change !== null ? (
                    <>
                      <div style={{ fontSize: '24px', fontWeight: 'bold', color: data.change > 0 ? '#28a745' : data.change < 0 ? '#dc3545' : '#fff' }}>
                        {data.change > 0 ? '+' : ''}{formatNumber(data.change)}
                      </div>
                      <div style={{ fontSize: '12px', color: '#888', marginTop: '5px' }}>
                        Hist Volav: ${formatNumber(data.hist_volav)}
                      </div>
                      <div style={{ fontSize: '11px', color: '#666', marginTop: '2px' }}>
                        T: {formatTimestampShort(data.found_ts)}
                      </div>
                    </>
                  ) : (
            ...
            {/* Integration Note */}
            <div className="integration-status" style={{ padding: '10px', background: 'rgba(232, 62, 140, 0.1)', borderLeft: '4px solid #e83e8c', borderRadius: '4px' }}>
              <strong>Integrated Logic:</strong> This data is calculated directly by the Truth Ticks Engine using the <code>volav_timeline</code>.
            </div>
          </div>
        )}
