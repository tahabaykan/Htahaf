...
class InternalLedgerStore:
    ...
    def has_symbol(self, account_id: str, symbol: str) -> bool:
        """Check if symbol exists in ledger (to distinguish explicit 0 from missing)"""
        if account_id not in self.ledgers:
            self.load_ledger(account_id)
        return symbol in self.ledgers.get(account_id, {})
