...
class JFINEngine:
    ...
    def _select_heldkuponlu_stocks(
        ...
        """
        HELDKUPONLU special logic: CGRUP-based selection (JANALL BİREBİR MANTIK)
        
        Janall logic (from final_thg_lot_distributor.py lines 5040-5181):
        - C600, C625, C650: Free (can be 0 or more)
        - Other CGRUPs: At least 1 each
        - Fill remaining by score with 40% company limit
        - Total: heldkuponlu_pair_count pairs per pool
        """
        target_count = self.config.heldkuponlu_pair_count
        special_cgrups = ['C600', 'C625', 'C650']
        
        logger.info(f"[JFIN] HELDKUPONLU CGRUP selection: target={target_count} per pool")
        
        def select_by_score_with_cgrup_rules(
            candidates: List[Dict[str, Any]], 
            score_column: str, 
            direction: str,
            excluded_symbols: set = None
        ) -> List[Dict[str, Any]]:
            """CGRUP-based selection with Janall rules"""
            if not candidates:
                return []
            ...
            filtered = [c for c in candidates 
                       if c.get('symbol', c.get('PREF_IBKR', c.get('PREF IBKR', ''))) not in excluded_symbols]
            ...
            # Sort by score
            def get_score(c):
                score = c.get(score_column) or c.get(score_column.lower(), 0) or 0
                try:
                    return float(score) if not math.isnan(float(score)) else 0
                except:
                    return 0
            
            if direction == 'LONG':
                sorted_candidates = sorted(filtered, key=get_score, reverse=True)
            else:
                sorted_candidates = sorted(filtered, key=get_score, reverse=False)
            
            # Group by CGRUP
            cgrup_groups = {}
            for c in sorted_candidates:
                cgrup = str(c.get('cgrup', c.get('CGRUP', 'N/A')))
                if cgrup not in cgrup_groups:
                    cgrup_groups[cgrup] = []
                cgrup_groups[cgrup].append(c)
            
            # Calculate company limits (40% rule)
            company_totals = {}
            for c in filtered:
                company = c.get('CMON', c.get('cmon', 'N/A'))
                company_totals[company] = company_totals.get(company, 0) + 1
            ...
            for company, total in company_totals.items():
                max_per = max(1, round(total * 0.4))
                company_limits[company] = min(max_per, total)
            ...
            for c in sorted_candidates:
                ...
                cgrup = str(c.get('cgrup', c.get('CGRUP', 'N/A')))
                company = c.get('CMON', c.get('cmon', 'N/A'))
                symbol = c.get('symbol', c.get('PREF_IBKR', c.get('PREF IBKR', '')))
                
                # Skip special CGRUPs in step 1
            ...
            for c in sorted_candidates:
                ...
                if any(s.get('symbol', s.get('PREF_IBKR', s.get('PREF IBKR', ''))) == symbol for s in selected):
                    continue
            ...
            logger.info(f"[JFIN] HELDKUPONLU {direction} {score_column}: selected {len(selected)}/{target_count}")
            return selected
        
        # BB Long selection
        bb_selected = select_by_score_with_cgrup_rules(
            group_candidates, 'Final_BB_skor', 'LONG'
        )
        
        # FB Long selection (can overlap with BB)
        fb_selected = select_by_score_with_cgrup_rules(
            group_candidates, 'Final_FB_skor', 'LONG'
        )
        
        # Exclude LONG stocks from SHORT selection
        long_symbols = set()
        for c in bb_selected + fb_selected:
            sym = c.get('symbol', c.get('PREF_IBKR', c.get('PREF IBKR', '')))
            long_symbols.add(sym)
        
        # SAS Short selection (exclude LONG symbols)
        sas_selected = select_by_score_with_cgrup_rules(
            group_candidates, 'Final_SAS_skor', 'SHORT', long_symbols
        )
        
        # SFS Short selection (exclude LONG symbols, can overlap with SAS)
        sfs_selected = select_by_score_with_cgrup_rules(
            group_candidates, 'Final_SFS_skor', 'SHORT', long_symbols
        )
        ...
        logger.info(f"[JFIN] HELDKUPONLU final: BB={len(bb_stocks)}, FB={len(fb_stocks)}, SAS={len(sas_stocks)}, SFS={len(sfs_stocks)}")
