...
class PositionBook(Enum):
    """Position Book Type (LT vs MM)"""
    LT = "LT"   # Long Term (Karbotu, Reducemore, AddNewPos, Trim)
    MM = "MM"   # Market Making (Greatest, Sidehit, Aura)
    
    
class OrderSubtype(Enum):
    """
    Detailed Order Subtype (8 Types)
    Used for strict tracking and BEFDAY limits.
    """
    # LT INCREASES (via ADDNEWPOS)
    LT_LONG_INCREASE = "LT_LONG_INCREASE"
    LT_SHORT_INCREASE = "LT_SHORT_INCREASE"
    
    # LT DECREASES (via KARBOTU, REDUCEMORE, LT TRIM)
    LT_LONG_DECREASE = "LT_LONG_DECREASE"
    LT_SHORT_DECREASE = "LT_SHORT_DECREASE"
    
    # MM INCREASES (via Greatest MM, etc.)
...
class OrderProposal:
    ...
    # PHASE 9: Strict Tagging (LT/MM, Increase/Decrease)
    book: str = "LT"  # Default to LT
    order_subtype: str = "UNKNOWN"  # Detailed subtype (e.g. LT_LONG_INCREASE)
