    ...
    def add_intent(self, intent: Intent) -> bool:
        ...
        try:
            ...
            if intent.id is None:
                import uuid
                intent.id = f"INT-{uuid.uuid4().hex[:8].upper()}"
                logger.debug(f"[INTENT_STORE] Generated ID {intent.id} for {intent.symbol}")
            ...
            # Update status index (only if intent has status attribute)
            intent_status = getattr(intent, 'status', 'PENDING')
            if intent_status not in self._status_index:
                self._status_index[intent_status] = []
            self._status_index[intent_status].append(intent.id)
            ...
            # Handle action - could be string or enum
            action_str = intent.action.value if hasattr(intent.action, 'value') else str(intent.action)
            logger.debug(f"[INTENT_STORE] Added intent {intent.id} for {intent.symbol} ({action_str})")
            return True
