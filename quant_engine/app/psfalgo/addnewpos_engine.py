...
class AddnewposEngine:
    ...
    def is_eligible(self, exposure: Optional[ExposureSnapshot], exposure_mode: Optional[str] = None) -> Tuple[bool, str]:
        ...
        JANALL LOGIC (simplified):
        - mode == OFANSIF
        - pot_total < pot_max_lot
        ...
        # JANALL UYUMLU: exposure_ratio_threshold konfigürasyonda yoksa veya 1.0 ise kontrol yapma
        exposure_ratio_threshold = self.eligibility.get('exposure_ratio_threshold')
        ...
        # JANALL UYUMLU: pot_total < pot_max kontrolü (exposure_ratio kontrolü devre dışı)
        if exposure.pot_max > 0:
            # Janall mantığı: pot_total < pot_max_lot ise ADDNEWPOS çalışır
            if exposure.pot_total >= exposure.pot_max:
                return False, f"pot_total {exposure.pot_total:.0f} >= pot_max {exposure.pot_max:.0f} - no room to add"
        
        # exposure_ratio_threshold kontrolü - sadece konfigüre edilmişse ve < 1.0 ise
        if exposure_ratio_threshold is not None and exposure_ratio_threshold < 1.0:
            if exposure.pot_max > 0:
                exposure_ratio = exposure.pot_total / exposure.pot_max
        ...
        return True, f"mode={exposure_mode}, pot_total={exposure.pot_total:.0f} < pot_max={exposure.pot_max:.0f}"
    ...
    async def _process_addlong(
        ...
        for symbol in available_symbols:
            ...
            decision = Decision(
                ...
                step_number=1,
                reason=f"Pos:{int(existing_qty)} | AddLong: Bid Buy Ucuzluk={bid_buy:.4f}, Fbtot={fbtot:.2f}",
    ...
    async def _process_addshort(
        ...
        for symbol in available_symbols:
            ...
            decision = Decision(
                ...
                step_number=2,
                reason=f"Pos:{int(existing_qty)} | AddShort: Ask Sell Pahalılık={ask_sell:.4f}, SFStot={sfstot:.2f}",
                confidence=confidence,
                metrics_used={
    ...
    def _calculate_lot_mental_model(
        ...
        if intent_info and goodness is not None:
            ...
            # Pass existing_qty=0 and action="BUY" for fresh position logic
            final_lot = calculate_rounded_lot(raw_lot, self.lot_policy_config, existing_qty=0, action="BUY")
            ...
            # Pass existing_qty=0 and action="BUY" for simple calculation
            return calculate_rounded_lot(base_lot, self.lot_policy_config, existing_qty=0, action="BUY")
