...
from app.psfalgo.intent_math import calculate_rounded_lot
...
class PSFALGOActionPlanner:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Determine final book (Default to what snapshot says or LT)
            # IMPORTANT: For new positions (ADD), this is critical.
            # Usually handled by Proposal Engine, but Planner might need it for limits?
            # Actually, calculate_limits uses Portfolio total, which is generic.
    ...
    def plan_action(
        ...
        account_id: str = "IBKR_GUN" # Default if not provided
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            account_id: Account ID for open orders routing (Added in Phase 11)
        ...
        try:
            ...
            # Additional context for limits
            daily_net_change = snapshot.get('daily_net_change', 0.0) # Need to ensure this is passed/calc'd
            if daily_net_change == 0.0 and 'net_change' in snapshot:
                 daily_net_change = snapshot['net_change']
                 
            # If snapshot doesn't have portfolio total, use a default or fetch?
            # Usually MAXALW is enough proxy for 'Size Magnitude'. 
            # But Increase Rules need Portfolio Total Qty.
            # Snapshot usually has it? Or pass it in `guards`?
            # For now, if missing, assume a standard portfolio size?
            # Better: Use MAXALW * 50 as proxy? No.
            # Let's check if snapshot has 'portfolio_total_qty'.
            portfolio_total_qty = snapshot.get('portfolio_total_qty', 100000.0) # Safe default
            ...
            if action in ['ADD_LONG', 'ADD_SHORT', 'REDUCE_LONG', 'REDUCE_SHORT']:
                # Initial Estimate (Raw)
                raw_estimate = 0
                if maxalw is not None:
                    raw_estimate = (maxalw * size_percent) / 100.0
                elif abs(current_qty) > 0:
                    raw_estimate = (abs(current_qty) * size_percent) / 100.0
                else:
                    raw_estimate = size_percent
                ...
                size_lot_estimate = calculate_rounded_lot(
                    ...
                    action=rounding_action
                )
                    
                # -------------------------------------------------------------------------
                # PHASE 11: DAILY LIMITS & OPEN ORDER CHECK
                # -------------------------------------------------------------------------
                from app.psfalgo.daily_limit_service import get_daily_limit_service
                limit_service = get_daily_limit_service()
                
                # Calculate Limits for this stock
                limits = limit_service.calculate_limits(
                    symbol=symbol,
                    befday_qty=befday_qty,
                    current_qty=current_qty,
                    maxalw=maxalw or 5000.0, # Safe fallback
                ...
                if not is_allowed:
                    if trimmed_qty < self.min_lot:
                        # Veto
                        action = 'BLOCKED'
                        size_lot_estimate = 0
                        reason_parts.append(f"Blocked by Daily Limit: {limit_reason}")
                    else:
                        # Trim
                ...
                # Apply min_lot constraint (Final check) - Now handled by calculate_rounded_lot mostly
                # But kept as safety if daily limit trimmed it weirdly
                if size_lot_estimate > 0 and size_lot_estimate < self.min_lot and "ADD" in action:
                     # Only enforce min_lot for ADDS (Buys)
                     # For REDUCE, small lots are allowed (exact close)
            ...
            # Resolve Strategy Tag (Phase 10: Smart Auto-Tagging)
            from app.psfalgo.tag_utils import resolve_strategy_tag
            
            # Determine Intended Book based on Rule Source
            intended_book = "LT" # Default
            if applied_rule_key and "mm" in applied_rule_key.lower():
                 intended_book = "MM"
            elif applied_rule_key and ("karbotu" in applied_rule_key or "reducemore" in applied_rule_key or "addnewpos" in applied_rule_key):
                 intended_book = "LT"
                 
            strategy_tag = None
            if action not in ['HOLD', 'BLOCKED'] and action != 'NONE':
                strategy_tag = resolve_strategy_tag(
                    action=action,
                    current_qty=current_qty,
                    intended_book=intended_book
                )
