...
class RulesStore:
    ...
    def update_rules(self, updates: Dict[str, Any], validate: bool = True) -> Dict[str, Any]:
        ...
        try:
            with self.rules_lock:
                # 1. Create a deep copy of current rules to test the update
                current_copy = json.loads(json.dumps(self.rules))  # Simple deep copy
                
                # 2. Apply updates to the copy
                self._deep_merge(current_copy, updates)
                
                # 3. Validate the transformed copy (COMPLETE rules)
                if validate:
                    validation_result = self.validate_rules(current_copy)
                ...
                # 4. If valid, commit to actual rules
                self.rules = current_copy
                self.last_updated = datetime.now()
