    ...
    def _compute_positions_from_ledger(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for symbol, entries_list in symbol_entries.items():
                ...
                last_book = "LT"  # Default
                
                for entry in entries_list:
                    action = entry['psfalgo_action']
                    size_lot = entry['size_lot_estimate']
                    
                    # Update Book from entry if present
                    if entry.get('book'):
                        last_book = entry.get('book')
                ...
                # If position is closed, reset book to default or keep last history?
                # User likely wants to know what it WAS. match BEFDAY logic.
                # But for active implementation:
                if current_qty == 0:
                     last_book = "LT" # Reset if flat? Or keep last known?
                     # Let's keep it as is, but practically it matters for active positions.
