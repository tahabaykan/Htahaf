...
class AuraMMEngine:
    ...
    TARGET_CYCLES_2D = 5  # Target cycles for 2D timeframe (per day: 2.5)
    ...
    def compute_mm_score(
        ...
        try:
            ...
            if mode == 'MARKET_LIVE':
                ...
                primary_timeframes = ['TF_1D', 'TF_2D']  # Character/bias
            
            for timeframe_name in ['TF_4H', 'TF_1D', 'TF_2D']:
            ...
            if mode == 'MARKET_LIVE':
                ...
                # MARKET_CLOSED: Use TF_2D to project next session
                tf_2d_data = timeframe_pingpong.get('TF_2D', primary_data)
                fast_cycles_2d = tf_2d_data.get('fast_cycles', 0)
                # Project: avg cycles per day * 1 trading day
                expected_cycles = (fast_cycles_2d / 2.0) * 1.0
