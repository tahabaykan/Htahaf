...
- Live market data (bid/ask/last/spread/prev_close) from Hammer
- Static data (AVG_ADV, SMI, SMA63chg, SMA246chg) from CSV

IMPORTANT: prev_close comes from live Hammer market data, NOT from CSV
"""
...
class DerivedMetricsEngine:
    ...
    NOTE: prev_close must come from live Hammer market data.
    """
    ...
    def compute_scores(
        ...
        Args:
            ...
            market_data: Live market data with keys: bid, ask, last, spread, volume, prev_close
        ...
        """
        try:
            # Extract market data (prev_close comes from Hammer, not CSV)
            ...
            prev_close = self._safe_float(market_data.get('prev_close') or market_data.get('prevClose'))
            ...
            prev_close = prev_close or 0.0  # From Hammer, not CSV
            ...
            inputs = {
                ...
                'last': last,
                'spread': spread,
                'prev_close': prev_close,  # From Hammer
                'AVG_ADV': avg_adv,
                'SMI': smi,
                'SMA63_chg': sma63_chg,
            ...
            logger.error(f"Error computing scores for {symbol}: {e}", exc_info=True)
            return {
                'symbol': symbol,
                'inputs': None,
                'scores': {},
    ...
    def compute_batch(
        ...
        Args:
            ...
            market_data_dict: {symbol: market_data} (must include prev_close from Hammer)
