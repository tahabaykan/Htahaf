                ...
                if dirty_snapshot:
                    ...
                    for symbol in dirty_snapshot:
                        try:
                            ...
                            # =====================================================
                            # SECURITY CONTEXT TRUTH PUSH (cold-path)
                            # Push computed GRPAN to SecurityContext.truth
                            # =====================================================
                            try:
                                from app.core.security_registry import get_security_registry
                                registry = get_security_registry()
                                if registry:
                                    ctx = registry.get(symbol)
                                    if ctx:
                                        # Extract latest_pan metrics
                                        latest_pan = all_results.get('latest_pan', {})
                                        grpan_price = latest_pan.get('grpan_price')
                                        concentration = latest_pan.get('concentration_percent')
                                        print_count = latest_pan.get('print_count', 0)
                                        
                                        if grpan_price is not None:
                                            ctx.update_truth(
                                                dominant_price=grpan_price,
                                                concentration_percent=concentration,
                                                turn_count=print_count,
                                                source="GRPAN"
                                            )
                            except Exception:
                                pass  # Non-critical
