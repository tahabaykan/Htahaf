...
class JanallMetricsEngine:
    ...
    def compute_symbol_metrics(
        ...
        Args:
            ...
            benchmark_chg: Benchmark change (Group Average Daily Change) in cents
        
        Args:
            symbol: Symbol (PREF_IBKR)
            static_row: Static CSV data
            live_row: Live Hammer market data (must include prev_close)
            benchmark_chg: Benchmark change value (float)
        ...
        try:
            ...
            benchmark_chg_percent = 0.0 # Not used for Final FB, placeholder
            benchmark_symbol = "GroupAvg"
            ...
            # A2) Daily Change calculation (for group benchmark) - IN CENTS, NOT PERCENTAGE
            # daily_chg = last - prev_close (e.g., 0.13 means stock went up 13 cents)
            daily_chg = None
            ...
            result = {
                ...
                'daily_chg': round(daily_chg, 4) if daily_chg is not None else None,
                ...
                # GENOBS EXPLICIT FIELDS
                'final_thg': final_thg,
                'short_final': short_final,
                'smi': self._safe_float(static_row.get('SMI'))  # Pass-through from CSV
            }
        ...
        try:
            ...
            for group_key, group_metrics in groups.items():
                ...
                for metrics in group_metrics:
                    ...
                    # NEW: Collect daily_chg for group benchmark (IN CENTS)
                    daily_chg = metrics.get('daily_chg')
                    if daily_chg is not None and isinstance(daily_chg, (int, float)):
    ...
    def apply_group_overlays(
        ...
        try:
            ...
            # bench_chg = average daily_chg of all stocks in this group
            group_avg_daily_chg = stats.get('group_avg_daily_chg')
            symbol_metrics['bench_chg'] = group_avg_daily_chg  # NEW: Group benchmark
            symbol_metrics['bench_source'] = f"Group: {group_key} (n={stats.get('daily_chg_count', 0)})"
            
            # DEBUG: Log first 3 symbols to verify bench_chg calculation
            symbol = symbol_metrics.get('symbol') or symbol_metrics.get('_symbol')
            if not hasattr(self, '_bench_chg_debug_count'):
                self._bench_chg_debug_count = 0
            if self._bench_chg_debug_count < 3:
                self._bench_chg_debug_count += 1
                logger.info(
                    f"[GROUP_BENCH_DEBUG] {symbol}: "
                    f"group={group_key}, daily_chg={symbol_metrics.get('daily_chg')}, "
    ...
    def compute_batch_metrics(
        ...
        """
        try:
            # Step 0: Pre-calculate Group Averages (Benchmarks)
            group_changes = defaultdict(list)
            for symbol in symbols:
                static_data = static_store.get_static_data(symbol)
                live_data = market_data_cache.get(symbol)
                if static_data and live_data:
                    grp = resolve_group_key(static_data)
                    last = self._safe_float(live_data.get('last') or live_data.get('price'))
                    prev = self._safe_float(live_data.get('prev_close'))
                    if last > 0 and prev > 0:
                        chg = last - prev
                        # Exclude outliers > $5
                        if abs(chg) < 5.0:
                            group_changes[grp].append(chg)
            
            group_benchmarks = {}
            for grp, changes in group_changes.items():
                if changes:
                    group_benchmarks[grp] = sum(changes) / len(changes)
            ...
            for symbol in symbols:
                ...
                # Get group benchmark
                grp = resolve_group_key(static_data)
                bench_chg = group_benchmarks.get(grp, 0.0)
                
                # Compute symbol metrics with Group Benchmark
                metrics = self.compute_symbol_metrics(symbol, static_data, live_data, bench_chg)
            ...
            # Step 6: Persist to Redis (for GemEngine/API access)
            try:
