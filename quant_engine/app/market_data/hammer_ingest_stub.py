...
class HammerIngest:
    ...
    def _normalize_tick(self, tick: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract exchange/venue (User confirmed 'e' field for exchange in getTicks)
            exch = tick.get("e") or tick.get("exch") or tick.get("exchange") or tick.get("venue") or tick.get("MMID")
            if exch:
                normalized["exch"] = str(exch).upper()
