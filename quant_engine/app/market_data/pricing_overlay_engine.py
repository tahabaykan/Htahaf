...
class PricingOverlayEngine:
    ...
    def compute_overlay_scores(
        ...
        try:
            ...
            # Resolve group key for BENCHMARK
            # DOS Group Logic: benchmark = avg daily change of the group
            from app.market_data.grouping import resolve_group_key
            group_key = resolve_group_key(static_row)
            
            benchmark_chg = None
            benchmark_type = group_key
            
            # Fetch Group Stats from Redis (calculated by MarketContextWorker)
            # This ensures we use the exact "Avg Daily Chg" of the DOS Group
            try:
                # Need Redis client here. Ideally injected or lazy loaded.
                from app.core.redis_client import get_redis_client
                redis_client = get_redis_client().sync
                
                group_stats_json = redis_client.get(f"group_index:{group_key}")
                if group_stats_json:
                    group_stats = json.loads(group_stats_json)
                    # 'avg_chg' is computed by MarketContextWorker as sum(daily_chgs) / count
                    # It is in CENTS (e.g., 0.15), not percentage.
                    benchmark_chg = group_stats.get('avg_chg')
            except Exception as e:
                logger.debug(f"Failed to fetch group stats for {group_key}: {e}")
            
            # If Redis data missing, defaults to None (Absolute Pricing Mode)
