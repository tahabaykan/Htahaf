...
class TruthTicksEngine:
    ...
    BUCKET_SIZE = 0.01
    MIN_SIZE = 15 # Updated to 15 per new rule
    ...
    def __init__(self):
        ...
        # Deduplication using unique tick signatures
        # Format: "{ts}_{price}_{size}_{exch}"
        ...
        self.MIN_SIZE = self.config.get('print_realism', {}).get('min_lot_ignore', 15)
        
        # Saliency & Thresholds
        self.MIN_TICKS_FOR_SUFFICIENT_DATA = 15
        self.REQUIRED_TRUTH_TICKS = 100
        self.MIN_TRUTH_TICKS = 15
        # FNRA_ALLOWED_SIZES removed - replaced by strict 100/200 rule in code
    ...
    def add_tick(self, symbol: str, tick: Dict[str, Any]):
        ...
        try:
            with self._tick_lock:
                ...
                if normalized_tick:
                    # FILTER CRITICAL STEP:
                    # Check if this tick is a valid Truth Tick BEFORE storing it
                    # This prevents garbage (size 1, random FNRA/ADFN) from polluting the store
                    weight = self.calculate_print_weight(
                        normalized_tick['size'], 
                        normalized_tick['exch']
                    )
                    
                    if weight <= 0.0:
                        # Ignore invalid tick (noise)
                        return

                    # Deduplication Check
                    # Deduplication Check
                    # Create a unique signature for this tick
                    # Precision handling: Format timestamp to 3 decimal places to avoid float drift issues
                    ts_str = f"{normalized_tick['ts']:.3f}"
                    tick_hash = f"{symbol}_{ts_str}_{normalized_tick['price']}_{normalized_tick['size']}_{normalized_tick.get('exch','UNK')}"
                    
                    if tick_hash in self.seen_hashes:
                        # Duplicate tick (likely from polling overlap) - Ignore
                        return
                    
                    self.seen_hashes.add(tick_hash)
                    
                    # Memory Management: Prevent seen_hashes from growing indefinitely
                    if len(self.seen_hashes) > 200000:
                        # Crude flush mechanism - accepted trade-off for simplicity
                        # In a perfect world, use LRU or time-based expiry
                        self.seen_hashes.clear()
                        # Re-add current to avoid immediate potential duplicate? No, safe enough.
                        self.seen_hashes.add(tick_hash)
    ...
    def find_volav_at_time_window(self, truth_ticks: List[Dict], target_ts: float, avg_adv: float) -> Optional[float]:
        """
        Find Volav price at a specific historical timestamp with tolerance.
        
        Strategy:
        1. Look for ticks in [target - 30m, target + 30m]
        2. Fallback: Look for ticks in [target - 2h, target + 2h]
        3. If found, compute Volav of those ticks.
        """
        if not truth_ticks:
            return None
            
        # 1. Primary Window: +/- 30 minutes
        TOLERANCE_PRIMARY = 1800  # 30 mins
        window_ticks = [
            t for t in truth_ticks 
            if abs(t.get('ts', 0) - target_ts) <= TOLERANCE_PRIMARY
        ]
        
        if window_ticks:
            volav_levels, _ = self.compute_volav_levels(window_ticks, top_n=1, avg_adv=avg_adv)
            if volav_levels:
                return volav_levels[0]['price']
                
        # 2. Fallback Window: +/- 2 hours
        TOLERANCE_SECONDARY = 7200 # 2 hours
        window_ticks = [
            t for t in truth_ticks 
            if abs(t.get('ts', 0) - target_ts) <= TOLERANCE_SECONDARY
        ]
        ...
        # 3. Last Resort: Find LATEST tick BEFORE target_ts (Carried State)
        # Scan backwards up to 24 hours (86400s) to find the "state" at that time
        # This handles illiquid stocks where the last trade was yesterday morning
        PAST_SEARCH_LIMIT = 86400 # 24 hours back from target
        
        # Filter ticks that are BEFORE target_ts and within limit
        past_ticks = [
            t for t in truth_ticks
            if 0 < (target_ts - t.get('ts', 0)) <= PAST_SEARCH_LIMIT
        ]
        
        if past_ticks:
            # Sort by timestamp descending (newest first)
            past_ticks.sort(key=lambda x: x.get('ts', 0), reverse=True)
            # Take the most recent one (closest to target_ts from the left)
            # Use a small window around that tick to get "Volav" context (e.g. 5 ticks)
            latest_past_tick = past_ticks[0]
            
            # Find closest neighbors to this tick to form a Volav
            # (Just taking one price is risky, better to take the cluster it belongs to)
            cluster_ts = latest_past_tick.get('ts', 0)
            cluster_ticks = [
                t for t in truth_ticks
                if abs(t.get('ts', 0) - cluster_ts) <= 300 # 5 min window around that past tick
            ]
            
            if cluster_ticks:
                volav_levels, _ = self.compute_volav_levels(cluster_ticks, top_n=1, avg_adv=avg_adv)
                if volav_levels:
                    return volav_levels[0]['price']
            
            # If clustering fails, just return the single tick price
            return latest_past_tick.get('price')
    ...
    def calculate_print_weight(self, size: float, venue: str = 'UNKNOWN') -> float:
        """
        Calculate realism weight based on STRICT Venue Policy (Binary 1.0 or 0.0):
        
        1. Base Rule: Size < 15 -> ALWAYS REJECT (0.0)
        2. FNRA Rule: If venue is FNRA, size MUST be 100 or 200 -> (1.0)
           (All other FNRA sizes, e.g. 98, 300, 500 are REJECTED)
        3. Non-FNRA Rule: Any size >= 15 -> ACCEPT (1.0)
        ...
        Returns:
            1.0 if Truth Tick, 0.0 otherwise.
        """
        # Global Min Size Rule
        if size < 15:
            return 0.0

        venue_upper = str(venue).upper()
        
        # FNRA / ADFN Specific Rule
        # Added ADFN, FINRA, OTC to restricted list per user request
        if venue_upper in ['FNRA', 'ADFN', 'FINRA', 'OTC', 'DARK']:
            # Strictly only 100 or 200 allowed for FNRA/ADFN
            # The user explicitly asked for "FNRA elemesi" - we treat this as "Strict Mode"
            # Strictly only 100 or 200 allowed for FNRA
            if size == 100 or size == 200:
                return 1.0
            else:
                return 0.0
        
        # Non-FNRA Rule (NYSE, ARCA, UNKNOWN, etc.)
        # Already checked size >= 15 above
        return 1.0

    def is_truth_tick(self, tick: Dict[str, Any]) -> bool:
        """
        Check if a tick is a Truth Tick based on strict rules.
        """
        try:
            size = tick.get('size', 0)
            exch = tick.get('exch', 'UNKNOWN')
            return self.calculate_print_weight(size, exch) > 0.0
        except Exception:
            return False
    ...
    def compute_metrics(
        ...
        try:
            ...
            # Get last 10000 ticks (renamed from 200, kept var name for minimal diff but logic changed)
            ...
            # CRITICAL FIX: Use 'truth_ticks_200_filtered' (full history) instead of 'truth_ticks_100'
            # to ensure timeline covers days, not just last 100 ticks.
            volav_timeline = self.compute_volav_timeline(truth_ticks_200_filtered, num_windows=5, avg_adv=avg_adv)
            ...
            # --- GEM INTEGRATION: Temporal Analysis (Robust Search) ---
            # Calculate changes relative to the LAST TRUTH TICK (Trading Time)
            temporal_analysis = {
                '1h': {'change': None, 'hist_volav': None, 'ref_ts': None},
                '4h': {'change': None, 'hist_volav': None, 'ref_ts': None},
                '1d': {'change': None, 'hist_volav': None, 'ref_ts': None},
                '3d': {'change': None, 'hist_volav': None, 'ref_ts': None}
            }
            
            # Find latest strict truth tick (100/200 lot) for Reference Point
            ref_tick = None
            for i in range(len(truth_ticks_100) - 1, -1, -1):
                t = truth_ticks_100[i]
                if t.get('size') in [100, 200]:
                    ref_tick = t
                    break
            
            if ref_tick and volav_timeline:
                ref_ts = ref_tick.get('ts', 0)
                ref_price = ref_tick.get('price', 0)
                
                # Helper to calculate Business Days back
                def get_target_time(base_ts, mode):
                    dt = datetime.fromtimestamp(base_ts)
                    if mode == '1h':
                        return base_ts - 3600
                    elif mode == '4h':
                        return base_ts - 14400
                    elif mode == '1d':
                        # Go back 1 business day
                        days_back = 0
                        while days_back < 1:
                            dt -= timedelta(days=1)
                        ...
                        # Go back 2 business days (changed from 3d to 2d per user request)
                        days_back = 0
                        while days_back < 2:
                            dt -= timedelta(days=1)
                            if dt.weekday() < 5: # Mon-Fri
                ...
                search_stages = [1800, 3000, 10800] 
                
                print(f"[TruthCheck-Metrics] [{symbol}] Ref Price: {ref_price:.2f} @ {datetime.fromtimestamp(ref_ts)}")

                for tf in ['1h', '4h', '1d', '2d']:
                    target_ts = get_target_time(ref_ts, tf)
                    best_window = None
                    
                    target_dt = datetime.fromtimestamp(target_ts)
                    
                    for stage_limit in search_stages:
                        min_diff = float('inf')
                        found_in_stage = None
                        
                        for w in volav_timeline:
                            w_mid = (w['start_timestamp'] + w['end_timestamp']) / 2
                            if w['start_timestamp'] <= target_ts <= w['end_timestamp']:
                                found_in_stage = w
                                break 
                            diff = abs(w_mid - target_ts)
                            if diff < min_diff and diff <= stage_limit:
                                min_diff = diff
                                found_in_stage = w
                        
                        if found_in_stage:
                            best_window = found_in_stage
                            break
                    
                    if best_window:
                        hist_volav = best_window.get('volav1')
                        if hist_volav:
                            change = ref_price - hist_volav
                            
                            found_dt = datetime.fromtimestamp(best_window['start_timestamp'])
                            print(f"[TruthCheck-Metrics] [{symbol}] {tf} FOUND: Target={target_dt} Found={found_dt} Price={hist_volav:.2f}")

                            temporal_analysis[tf] = {
                                'change': change,
                                'hist_volav': hist_volav,
                                'ref_ts': target_ts,
                                'found_ts': best_window['start_timestamp'],
                                'search_window_used': stage_limit if 'stage_limit' in locals() else 'exact'
                            }
                    else:
                        # Fallback: Find "Last Known" Volav before target
                        # For illiquid stocks, if we land in a gap, the prev Volav is the truth.
                        latest_prior_window = None
                        max_ts = -1
                        
                        for w in volav_timeline:
                             if w['end_timestamp'] <= target_ts:
                                 if w['end_timestamp'] > max_ts:
                                     max_ts = w['end_timestamp']
                                     latest_prior_window = w
                        
                        if latest_prior_window:
                            hist_volav = latest_prior_window.get('volav1')
                            if hist_volav:
                                change = ref_price - hist_volav
                                found_dt = datetime.fromtimestamp(latest_prior_window['start_timestamp'])
                                print(f"[TruthCheck-Metrics] [{symbol}] {tf} [Fallback] FOUND: Target={target_dt} Found={found_dt} Price={hist_volav:.2f}")
                                
                                temporal_analysis[tf] = {
                                    'change': change,
                                    'hist_volav': hist_volav,
                                    'ref_ts': target_ts,
                                    'found_ts': latest_prior_window['start_timestamp'],
                                    'search_window_used': 'fallback_last_known'
                                }
                        else:
                            print(f"[TruthCheck-Metrics] [{symbol}] {tf} NOT FOUND: Target={target_dt} (Max search {search_stages[-1]}s)")
            ...
            result = {
                ...
                'temporal_analysis': temporal_analysis, # <--- NEW: Gem Integration
                'flags': flags,
    ...
    def get_inspect_data(self, symbol: str, avg_adv: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for tick in all_ticks:
                ...
                # New Logic: Strict Binary
                # Global Min Size 15
                if size < self.MIN_SIZE:
                    excluded_small_size += 1
                # FNRA Rule: Only 100 or 200 allowed
                elif exch == 'FNRA':
                    if size == 100 or size == 200:
                        included_fnra_100_200 += 1
                        # Note: 'included_fnra_100_200' name preserved for simplicity, 
                        # essentially means "Trusted FNRA ticks"
                    else:
                        excluded_fnra_non_100_200 += 1
                # Non-FNRA Rule: Any size >= 15 allowed
                else:
                    included_non_fnra += 1
            
            # Filter to TruthTicks (uses self.is_truth_tick which matches above logic)
            truth_ticks_200 = self.filter_truth_ticks(all_ticks)
            ...
            # Volav Timeline - How Volav levels evolve over time (5 windows)
            # CRITICAL FIX: Use 'truth_ticks_200_filtered' (full 10-day history) instead of 'truth_ticks_100'
            volav_timeline = self.compute_volav_timeline(truth_ticks_200_filtered, num_windows=5, avg_adv=avg_adv)
            ...
            # --- GEM INTEGRATION: Temporal Analysis (Robust Search & Business Days) ---
            temporal_analysis = {
                '1h': {'change': None, 'hist_volav': None, 'ref_ts': None},
                '4h': {'change': None, 'hist_volav': None, 'ref_ts': None},
                '1d': {'change': None, 'hist_volav': None, 'ref_ts': None},
                '2d': {'change': None, 'hist_volav': None, 'ref_ts': None}
            }

            # Find latest strict truth tick for Reference Point
            # CRITICAL FIX: truth_ticks_100 is ALREADY filtered by is_truth_tick (Strict Venue/Size Rules)
            # So the last tick IS the latest valid Truth Tick. 
            # We don't need to re-filter for 100/200 size (which incorrectly skipped valid NYSE trades like 59 lots).
            ref_tick = None
            if truth_ticks_100:
                ref_tick = truth_ticks_100[-1]
            ...
            if ref_tick and volav_timeline:
                ...
                # Expanding Search Windows
                # Legacy Search Stages replaced by centralized find_volav_at_time_window
                
                print(f"[TruthCheck] [{symbol}] Ref Price: {ref_price:.2f} @ {datetime.fromtimestamp(ref_ts)}")

                for tf in ['1h', '4h', '1d', '2d']:
                    target_ts = get_target_time(ref_ts, tf)
                    
                    # Use centralized logic (includes Primary, Secondary, and Backwards Fallback)
                    # Pass 'truth_ticks_200_filtered' (or even full 'all_ticks' if needed, but filtered is safer for outliers)
                    # Actually, better to use 'truth_ticks_200' (all reliable ticks) to find historical data
                    found_price = self.find_volav_at_time_window(truth_ticks_200, target_ts, avg_adv)
                    
                    if found_price is not None:
                        # Calculate Change (Cents)
                        # GemEngine expects value in CENTS (price diff), not percentage
                        chg = ref_price - found_price
                             
                        temporal_analysis[tf]['change'] = chg
                        temporal_analysis[tf]['hist_volav'] = found_price
                        temporal_analysis[tf]['ref_ts'] = target_ts
                         
                        print(f"[TruthCheck] [{symbol}] {tf} FOUND: Target={datetime.fromtimestamp(target_ts)} Price={found_price:.2f}")
                    else:
                         print(f"[TruthCheck] [{symbol}] {tf} NOT FOUND (New Logic Fallback Failed): Target={datetime.fromtimestamp(target_ts)}") 


            # (Redundant legacy logic removed)
    ...
    def compute_metrics_for_timeframe(
        ...
        try:
            ...
            # TIMEFRAME-FIRST: Volav1 Start/End Logic (Time-Specific)
            volav1_start = None
            volav1_end = None
            volav1_displacement = None
            volav_shift_abs = None
            ...
            # --- START CALCULATE VOLAV AT TARGET TIME ---
            # Calculates "Where was the price exactly X time ago?"
            target_hist_ts = anchor_timestamp - timeframe_seconds
            
            # Use ALL Truth Ticks (truth_ticks_all) not just timeframe ticks
            # to support finding data even if it's slightly outside the rigid window
            volav1_start = self.find_volav_at_time_window(
                truth_ticks_all, 
                target_hist_ts, 
                avg_adv
            )
            
            # Volav End is always the Current/Anchor State (Last 20% of visible window or just recent ticks)
            # Use standard logic for "Current" price (last 20% of timeframe ticks)
            if truth_tick_count >= 1:
                # Use last 20% of AVAILABLE timeframe ticks for stability
                last_20_percent_count = max(1, int(truth_tick_count * 0.2))
                last_20_percent_ticks = truth_ticks_timeframe[-last_20_percent_count:]
            ...
            # --- END CALCULATE VOLAV AT TARGET TIME ---
            ...
            result = {
                ...
                # Compatibility Aliases for GemEngine
                'change': volav1_displacement, 
                'hist_volav': volav1_start,
