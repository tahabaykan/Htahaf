...
class PortAdjusterConfig(BaseModel):
    ...
    # LT vs MM Split (NEW)
    lt_ratio_pct: float = Field(70.0, ge=0.0, le=100.0, description="Long Term allocation percentage (0-100)")
    mm_ratio_pct: float = Field(30.0, ge=0.0, le=100.0, description="Market Making allocation percentage (0-100)")
    
    # Potential fill multipliers (NEW)
    lt_potential_multiplier: float = Field(1.5, ge=1.0, le=5.0, description="Max LT potential orders / LT base (default 1.5x)")
    mm_potential_multiplier: float = Field(2.0, ge=1.0, le=5.0, description="Max MM potential orders / MM base (default 2.0x)")
    
    # LT Long/Short ratio (within LT allocation)
    long_ratio_pct: float = Field(70.0, ge=0.0, le=100.0, description="LT Long ratio percentage (0-100)")
    short_ratio_pct: float = Field(30.0, ge=0.0, le=100.0, description="LT Short ratio percentage (0-100)")
...
class PortAdjusterSnapshot(BaseModel):
    ...
    # LT/MM Base Exposures (NEW)
    lt_exposure_usd: float = Field(0.0, ge=0.0, description="LT base exposure in USD")
    mm_exposure_usd: float = Field(0.0, ge=0.0, description="MM base exposure in USD")
    lt_max_potential_usd: float = Field(0.0, ge=0.0, description="Max LT potential orders in USD")
    mm_max_potential_usd: float = Field(0.0, ge=0.0, description="Max MM potential orders in USD")
    
    # Core calculations (now LT-specific)
    total_lot: float = Field(..., ge=0.0, description="Total LT lot (lt_exposure_usd / avg_pref_price)")
    long_lot: float = Field(..., ge=0.0, description="LT Long lot (total_lot * long_ratio_pct / 100)")
    short_lot: float = Field(..., ge=0.0, description="LT Short lot (total_lot * short_ratio_pct / 100)")
