...
def initialize_market_data_services():
    ...
    # Initialize DataFabric (Central Singleton) - Critical for internal API readiness
    # This prevents "DataFabric static data not ready" warnings
    if settings.AUTO_LOAD_CSV:
        try:
            from app.core.data_fabric import DataFabric
            data_fabric = DataFabric() # Singleton
            # Only load if not already loaded (DataFabric tracks its own state)
            if not data_fabric.is_ready():
                logger.info("🏗️ DataFabric: Loading static data on startup (syncing with StaticDataStore)...")
                data_fabric.load_static_data()
        except Exception as e:
            logger.error(f"❌ DataFabric initialization failed: {e}")
...
async def get_market_data_snapshot():
    """
    Get raw market data cache snapshot (for internal workers).
    Returns the current dictionary of {symbol: {bid, ask, last, ...}}.
    """
    global market_data_cache
    return {
        "success": True,
        "count": len(market_data_cache),
        "data": market_data_cache
    }


@router.get("/snapshots/{symbol}/history")
async def get_snapshot_history(symbol: str):
    """
    Get last 4 snapshots from Redis history list.
    
    Args:
        symbol: Symbol (e.g. CIM PRB)
        
    Returns:
        List of snapshot objects (max 4)
    """
    try:
        from app.core.redis_client import get_redis_client
        redis_client = get_redis_client()
        
        # FIX: Access the underlying sync client
        client = redis_client.sync
        if not client:
             raise Exception("Redis client sync client not available")
             
        history_key = f"market_data:history:{symbol}"
        # Fetch all items (limit controlled by LTRIM in scheduler)
        raw_items = client.lrange(history_key, 0, -1)
        ...
        logger.error(f"Error fetching snapshot history for {symbol}: {e}", exc_info=True)
        return {
            "success": False,
            "error": str(e),
            "data": []
...
async def get_merged_market_data():
    ...
    try:
                    ...
                    if metric_engine and snapshot_store:
                        ...
                        for symbol in symbols:
                            try:
                                ...
                                # 💀 LIFELESS MODE INTERCEPTION (Batch Loop)
                                try:
                                    from app.core.data_fabric import get_data_fabric
                                    fabric = get_data_fabric()
        ...
        for symbol in symbols:
            ...
            # 💀 LIFELESS MODE INTERCEPTION
            # If DataFabric is in Lifeless Mode, we MUST read from it (Synthetic/Snapshot data)
            # because market_data_cache is disconnected from Hammer blocking.
            try:
                from app.core.data_fabric import get_data_fabric
                fabric = get_data_fabric()
                if fabric and fabric.is_lifeless_mode():
                    # Get fast snapshot which includes synthetic data + offsets
                    fast_snap = fabric.get_fast_snapshot(symbol)
                    if fast_snap:
                        # Map FastSnapshot fields to what this endpoint expects
                        # FastSnapshot keys: bid, ask, last, volume, timestamp, ...
                        market_data = {
                            'bid': fast_snap.get('bid'),
                            'ask': fast_snap.get('ask'),
                            'last': fast_snap.get('last'),
                            'volume': fast_snap.get('volume'),
                            'timestamp': fast_snap.get('timestamp'),
                            'spread': None, # recalculated below
                            'price': fast_snap.get('last'), # 'price' alias used in some views
                            'prev_close': fast_snap.get('prev_close') # Critical for Genobs/Gem
                        }
            except Exception as e:
                pass
