...
class IBKRNativeClient(EWrapper, EClient):
    ...
    def place_order(self, symbol, side, quantity, price, order_type="LIMIT", hidden=True, account_key=None):
        ...
        try:
            ...
            print(f"[IBKR-NATIVE] 🔄 Emir gönderiliyor: {symbol} {side} {quantity} @ ${price:.2f}")
            ...
            print(f"[IBKR-NATIVE] ✅ Emir gönderildi: {symbol} {side} {quantity} @ ${rounded_price:.2f}")
            print(f"[IBKR-NATIVE] 📋 Order ID: {order.orderId}")
            print(f"[IBKR-NATIVE] 📋 Contract: {symbol} @ {contract.exchange}")
