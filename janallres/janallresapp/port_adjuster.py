...
❌ YANLIŞ: "janallresres/exposureadjuster.csv"
=================================
"""
...
# Grup isimleri çeviri sözlüğü (main_window.py ile aynı)
GROUP_NAME_MAPPING = {
...
class PortAdjusterWindow:
    ...
    def update_long_groups_table(self, total_long_lot):
        ...
        for group, percentage in self.long_groups.items():
            ...
            display_name = get_display_name(group)  # Görüntüleme ismini al
            
            self.long_tree.insert('', 'end', values=[
                display_name,  # Görüntüleme ismini kullan
                f"{lot_amount:,.0f}",
                f"${total_value:,.0f}"
            ])
            ...
            # Küme ismi etiketi - SOL TARAFTA (görüntüleme ismini kullan)
            group_label = ttk.Label(entry_frame, text=f"{display_name}:", width=25, anchor='w')
