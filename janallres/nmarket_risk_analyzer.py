...
def analyze_market_conditions(price_changes, market_data=None):
    ...
    for symbol in ["SPY", "IWM", "KRE"]:
        if market_data and symbol in market_data:
            try:
                ...
                diff20, diff100, diff200 = calculate_sma_diffs(df)
                for d, label in zip([diff20, diff100, diff200], ["SMA20", "SMA100", "SMA200"]):
                    if not np.isnan(d):
                        momentum_scores.append(d)
                        sma_table.append((symbol, label, d))
                # --- 90 günlük high/low yakınlığı ---
                if len(df) >= 90:
                    last_90 = df.iloc[-90:]
                    high_90 = last_90['close'].max()
                    low_90 = last_90['close'].min()
                    close = last_90['close'].iloc[-1]
                    if high_90 > low_90:
                        proximity = (close - low_90) / (high_90 - low_90)
                        highlow_scores.append(proximity)
                        print(f"{symbol} 90g High: {high_90:.2f}, Low: {low_90:.2f}, Close: {close:.2f}, Proximity: {proximity:.2f}")
            except Exception as e:
                print(f"SMA/highlow hesaplama hatası ({symbol}): {e}")
    market_momentum = np.mean(momentum_scores) if momentum_scores else 0
    # High/low katkısı: ortalama proximity * 10 (0-10 arası ek bullishlik)
    if highlow_scores:
        highlow_contrib = np.mean(highlow_scores) * 10
        print(f"High/Low katkısı: {highlow_contrib:.2f}")
        market_momentum += highlow_contrib
    ...
    # Daha agresif ağırlıklar
    yield_weight = 8 + (40 - 8) * momentum_normalized
    solidity_weight = 4 - (4 - 0.8) * momentum_normalized
    
    base_adv = 0.00025

    # YENİ AĞIRLIK HESAPLAMALARI
    
    # 1. Adj Risk Premium Weight: 500-1700 arası, baz 1100
    # Risk-on'da yüksek (1700), risk-off'da düşük (500)
    adj_risk_premium_weight = 500 + (1700 - 500) * momentum_normalized
    
    # 2. SOLCALL Score Weight: 1-7 arası, baz 4
    # Risk-on'da yüksek (7), risk-off'da düşük (1)
    solcall_score_weight = 1 + (7 - 1) * momentum_normalized
    
    # 3. Credit Score Norm Weight: 0.5-3.5 arası, baz 2
    # Risk-on'da düşük (0.5), risk-off'da yüksek (3.5) - TERSİ
...
def generate_market_report(price_changes, market_weights):
    ...
    if price_changes is None:
        ...
        print(f"Adj Risk Premium Ağırlık: {market_weights['adj_risk_premium_weight']:.0f} (Aralık: 500-1700)")
        ...
        print(f"Adj Risk Premium Değişim: %{((market_weights['adj_risk_premium_weight']/1100 - 1) * 100):.1f}")
    ...
    try:
        ...
        if len(df) > 0:
            weights = {
                ...
                'adj_risk_premium_weight': df.get('adj_risk_premium_weight', 1100).iloc[0],
            ...
            print(f"✓ Kaydedilmiş ağırlıklar yüklendi:")
        ...
        print(f"⚠️ Kaydedilmiş ağırlıklar yüklenemedi: {e}")
