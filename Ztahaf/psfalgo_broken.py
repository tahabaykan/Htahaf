import pandas as pd
from Ntahaf.utils.order_management import OrderManager, log_reasoning
import time
import logging
import os
...
class PsfAlgo:
    def __init__(self, market_data, exclude_list=None, half_sized_list=None, order_manager=None):
        ...
        # BDATA entegrasyonu
        self.bdata_storage = BDataStorage('Ntahaf/data/bdata_fills.json')
    ...
    def get_current_step_number(self):
        """Mevcut adım numarasını döndürür (1-4 HELDKUP sistemi)"""
        step_mapping = {
            'HELDKUP_BB': 1,      # HELDKUP BB - FINAL BB en yüksek 5 hisse
            'HELDKUP_AS': 2,      # HELDKUP AS - FINAL AS en düşük 5 hisse (SMI < 0.28)
            'HELDKUP_FB': 3,      # HELDKUP FB - FINAL FB en yüksek 3 hisse (çakışma kontrolü)
            'HELDKUP_FS': 4,      # HELDKUP FS - FINAL FS en düşük 3 hisse (SMI < 0.28)
    ...
    def start_pisdongu_cycle(self):
        ...
        # 4. Chain'i başlat
        print(f"[🔄 OTOMATİK RESTART] 4️⃣ HELDKUP chain başlatılıyor (1. adım: HELDKUP_BB)...")
        self.chain_state = 'HELDKUP_BB'
        self.start_chain()
        
        print(f"[🔄 OTOMATİK RESTART] ✅ Restart tamamlandı - HELDKUP döngü #{self.pisdongu_cycle_count} aktif!")
    ...
    def start_chain(self):
        """PSFAlgo chain'ini başlat - YENİ 4 ADIMLI HELDKUP SİSTEMİ"""
        if not self.is_active or self.chain_state == 'IDLE':
            return
            
        print(f"[HELDKUP CHAIN] Başlatılıyor - Durum: {self.chain_state}")
        
        # YENİ 4 ADIMLI HELDKUP SİSTEMİ (1-4)
        if self.chain_state == 'HELDKUP_BB':
            self.run_heldkup_bb()  # 1. HELDKUP BB - FINAL BB en yüksek 5 hisse → Bid Buy
        elif self.chain_state == 'HELDKUP_AS':
            # 2. HELDKUP AS - mevcut HELDKUPONLU penceresinde işlem yap
            print("[HELDKUP CHAIN 2] HELDKUP AS - mevcut pencerede FINAL AS işlemi tetikleniyor...")
            if self.current_window and hasattr(self.current_window, 'rows'):
                self.run_heldkup_as()
            else:
                print("[HELDKUP CHAIN 2] ❌ HELDKUPONLU penceresi bulunamadı, bir sonraki adıma geç")
                self.advance_chain()
        elif self.chain_state == 'HELDKUP_FB':
            # 3. HELDKUP FB - mevcut HELDKUPONLU penceresinde işlem yap
            print("[HELDKUP CHAIN 3] HELDKUP FB - mevcut pencerede FINAL FB işlemi tetikleniyor...")
            if self.current_window and hasattr(self.current_window, 'rows'):
                self.run_heldkup_fb()
            else:
                print("[HELDKUP CHAIN 3] ❌ HELDKUPONLU penceresi bulunamadı, bir sonraki adıma geç")
                self.advance_chain()
        elif self.chain_state == 'HELDKUP_FS':
            # 4. HELDKUP FS - mevcut HELDKUPONLU penceresinde işlem yap
            print("[HELDKUP CHAIN 4] HELDKUP FS - mevcut pencerede FINAL FS işlemi tetikleniyor...")
            if self.current_window and hasattr(self.current_window, 'rows'):
                self.run_heldkup_fs()
            else:
                print("[HELDKUP CHAIN 4] ❌ HELDKUPONLU penceresi bulunamadı, bir sonraki adıma geç")
                self.advance_chain()
    ...
    def advance_chain(self):
        """Chain'de bir sonraki aşamaya geç - YENİ 4 ADIMLI HELDKUP SİSTEMİ"""
        ...
        # YENİ 4 ADIMLI HELDKUP SİSTEMİ (1-4)
        if self.chain_state == 'HELDKUP_BB':
            self.chain_state = 'HELDKUP_AS'
            print(f"[HELDKUP CHAIN] 1→2: HELDKUP_BB → HELDKUP_AS")
        elif self.chain_state == 'HELDKUP_AS':
            self.chain_state = 'HELDKUP_FB'
            print(f"[HELDKUP CHAIN] 2→3: HELDKUP_AS → HELDKUP_FB")
        elif self.chain_state == 'HELDKUP_FB':
            self.chain_state = 'HELDKUP_FS'
            print(f"[HELDKUP CHAIN] 3→4: HELDKUP_FB → HELDKUP_FS")
        elif self.chain_state == 'HELDKUP_FS':
            self.chain_state = 'FINISHED'
            print(f"[HELDKUP CHAIN] 4→FINISHED: HELDKUP_FS → FINISHED")
        elif self.chain_state == 'FINISHED':
            self.finish_chain()
            return  # finish_chain çağrıldığında start_chain çağrılmamalı
        
        # Sonraki aşamayı başlat (sadece onay beklenmiyorsa)
        print(f"[HELDKUP CHAIN] Yeni state: {self.chain_state}, pencere açılıyor...")
        self.start_chain()
    ...
    def on_data_ready(self, window):
        ...
        # Pencere tipine ve chain state'e göre işlem yap - YENİ HELDKUP SİSTEMİ
        window_title = window.title().lower()
        
        # HELDKUPONLU penceresi için (1-4. adım)
        if "heldkuponlu" in window_title:
            if self.chain_state == 'HELDKUP_BB':
                # 1. ADIM: FINAL BB en yüksek 5 → Bid Buy
                self.run_heldkup_bb_data_ready()
            elif self.chain_state == 'HELDKUP_AS':
                # 2. ADIM: FINAL AS en düşük 5 → Ask Sell (SMI < 0.28)
                self.run_heldkup_as()
            elif self.chain_state == 'HELDKUP_FB':
                # 3. ADIM: FINAL FB en yüksek 3 → Front Buy (çakışma kontrolü)
                self.run_heldkup_fb()
            elif self.chain_state == 'HELDKUP_FS':
                # 4. ADIM: FINAL FS en düşük 3 → Front Sell (SMI < 0.28)
                self.run_heldkup_fs()
            else:
                print(f"[HELDKUP CHAIN] Pencere '{window.title()}' için otomatik işlem yapılmıyor (chain_state: {self.chain_state})")
    ...
    def get_chain_state_title(self):
        """PISDoNGU chain state'ine göre işlem başlığını döndür - YENİ 4 ADIMLI HELDKUP SİSTEMİ"""
        if not self.pisdongu_active:
            return ""
        ...
        state_titles = {
            'IDLE': "",
            # YENİ 4 ADIMLI HELDKUP SİSTEMİ (1-4)
            'HELDKUP_BB': "🔄 HELDKUP (1/4) - FINAL BB en yüksek 5 hisse → Bid Buy",
            'HELDKUP_AS': "🔄 HELDKUP (2/4) - FINAL AS en düşük 5 hisse → Ask Sell (SMI < 0.28)",
            'HELDKUP_FB': "🔄 HELDKUP (3/4) - FINAL FB en yüksek 3 hisse → Front Buy (çakışma kontrolü)",
            'HELDKUP_FS': "🔄 HELDKUP (4/4) - FINAL FS en düşük 3 hisse → Front Sell (SMI < 0.28)",
            'FINISHED': "✅ HELDKUP Tamamlandı"
        }
    ...
    # ===== YENİ 4 ADIMLI HELDKUPONLU SİSTEMİ =====
    
    def run_heldkup_bb(self):
        """HELDKUP BB - FINAL BB en yüksek 5 hisse → Bid Buy"""
        print("[HELDKUP CHAIN 1/4] 🎯 HELDKUP BB → FINAL BB en yüksek 5 hisse → Bid Buy")
        
        if not self.is_active:
            print("[PSFAlgo] ⏸️ PSFAlgo pasif - HELDKUP BB işlenmedi")
            return
            
        # HELDKUPONLU penceresini aç - ssfinekheldkuponlu.csv kullan
        if self.main_window and hasattr(self.main_window, 'open_heldkuponlu_window'):
            # Eğer zaten açık bir pencere varsa, onu kullan
            if hasattr(self, 'current_window') and self.current_window:
                print("[HELDKUP CHAIN 1] ✅ Mevcut HELDKUPONLU penceresi kullanılıyor...")
                return
            else:
                # Yeni pencere aç
                self.current_window = self.main_window.open_heldkuponlu_window()
                print("[HELDKUP CHAIN 1] HELDKUPONLU penceresi açılıyor...")
        else:
            print("[HELDKUP CHAIN 1] ❌ HELDKUPONLU penceresi açılamadı")
            self.advance_chain()

    def run_heldkup_bb_data_ready(self):
        """HELDKUP BB - FINAL BB en yüksek 5 hisse → Bid Buy (veri hazır)"""
        print("[HELDKUP CHAIN 1/4] 🎯 HELDKUP BB → FINAL BB en yüksek 5 hisse → Bid Buy (veri hazır)")
        
        if not self.is_active or not self.current_window:
            print("[HELDKUP CHAIN 1] ❌ Pencere bulunamadı veya PSFAlgo pasif")
            self.advance_chain()
            return
        
        # FINAL BB en yüksek 15 hisse seç
        ...
        if not selected_stocks:
            print("[HELDKUP CHAIN 1] ❌ FINAL BB kriterleri sağlayan hisse bulunamadı")
            self.advance_chain()
            return
        
        # İlk 5 hisseyi al
        valid_tickers = []
        for ticker, score in selected_stocks[:5]:
            valid_tickers.append(ticker)
            print(f"[HELDKUP CHAIN 1] ✅ {ticker} (BB:{score:.2f})")
        
        # Sonuçları bildir
        print(f"[HELDKUP CHAIN 1] ✅ {len(valid_tickers)} hisse seçildi:")
        for ticker in valid_tickers:
            print(f"[HELDKUP CHAIN 1]   ✅ {ticker}")
        
        # Emir gönderimi için hazırla
        if valid_tickers:
            self.prepare_orders_for_tickers(valid_tickers, 'BUY', 'BID_BUY', step_number=1)
        else:
            print("[HELDKUP CHAIN 1] ❌ Geçerli hisse bulunamadı")
            self.advance_chain()

    def run_heldkup_as(self):
        """HELDKUP AS - FINAL AS en düşük 5 hisse → Ask Sell (SMI < 0.28)"""
        print("[HELDKUP CHAIN 2/4] 🎯 HELDKUP AS → FINAL AS en düşük 5 hisse → Ask Sell (SMI < 0.28)")
        
        if not self.is_active or not self.current_window:
            print("[HELDKUP CHAIN 2] ❌ Pencere bulunamadı veya PSFAlgo pasif")
            self.advance_chain()
            return
        
        # FINAL AS en düşük 15 hisse seç
        selected_stocks = self.get_top_stocks_by_score(
            self.current_window, 
            'Final AS skor', 
            count=15, 
        ...
        if not selected_stocks:
            print("[HELDKUP CHAIN 2] ❌ FINAL AS kriterleri sağlayan hisse bulunamadı")
            self.advance_chain()
            return
        
        # SMI kontrolü ile hisse filtrele
        valid_tickers = []
        
        for ticker, score in selected_stocks[:5]:  # İlk 5'i al
            # SMI kontrolü (short arttırma durumu için)
            smi_rate = self.get_smi_rate(ticker)
            if smi_rate > 0.28:
                print(f"[HELDKUP CHAIN 2] ❌ {ticker} (AS:{score:.2f}) - SMI {smi_rate:.4f} > 0.28")
                continue
            else:
                valid_tickers.append(ticker)
                print(f"[HELDKUP CHAIN 2] ✅ {ticker} (AS:{score:.2f}) - SMI {smi_rate:.4f} < 0.28")
        ...
        # Sonuçları bildir
        print(f"[HELDKUP CHAIN 2] ✅ {len(valid_tickers)} hisse seçildi:")
        for ticker in valid_tickers:
            print(f"[HELDKUP CHAIN 2]   ✅ {ticker}")
        
        # Emir gönderimi için hazırla
        if valid_tickers:
            self.prepare_orders_for_tickers(valid_tickers, 'SELL', 'ASK_SELL', step_number=2)
        else:
            print("[HELDKUP CHAIN 2] ❌ Geçerli hisse bulunamadı")
            self.advance_chain()

    def run_heldkup_fb(self):
        """HELDKUP FB - FINAL FB en yüksek 3 hisse → Front Buy (çakışma kontrolü)"""
        print("[HELDKUP CHAIN 3/4] 🎯 HELDKUP FB → FINAL FB en yüksek 3 hisse → Front Buy (çakışma kontrolü)")
        
        if not self.is_active or not self.current_window:
            print("[HELDKUP CHAIN 3] ❌ Pencere bulunamadı veya PSFAlgo pasif")
            self.advance_chain()
            return
        
        # FINAL FB en yüksek 10 hisse seç
        selected_stocks = self.get_top_stocks_by_score(
            self.current_window, 
            'Final FB skor', 
            count=10, 
        ...
        if not selected_stocks:
            print("[HELDKUP CHAIN 3] ❌ FINAL FB kriterleri sağlayan hisse bulunamadı")
            self.advance_chain()
            return
        
        # İlk 3 hisseyi al
        valid_tickers = []
        for ticker, score in selected_stocks[:3]:
            valid_tickers.append(ticker)
            print(f"[HELDKUP CHAIN 3] ✅ {ticker} (FB:{score:.2f})")
        
        # Sonuçları bildir
        print(f"[HELDKUP CHAIN 3] ✅ {len(valid_tickers)} hisse seçildi:")
        for ticker in valid_tickers:
            print(f"[HELDKUP CHAIN 3]   ✅ {ticker}")
        
        # Emir gönderimi için hazırla
        if valid_tickers:
            self.prepare_orders_for_tickers(valid_tickers, 'BUY', 'FRONT_BUY', step_number=3)
        else:
            print("[HELDKUP CHAIN 3] ❌ Geçerli hisse bulunamadı")
            self.advance_chain()

    def run_heldkup_fs(self):
        """HELDKUP FS - FINAL FS en düşük 3 hisse → Front Sell (SMI < 0.28)"""
        print("[HELDKUP CHAIN 4/4] 🎯 HELDKUP FS → FINAL FS en düşük 3 hisse → Front Sell (SMI < 0.28)")
        
        if not self.is_active or not self.current_window:
            print("[HELDKUP CHAIN 4] ❌ Pencere bulunamadı veya PSFAlgo pasif")
            self.advance_chain()
            return
        
        # FINAL FS en düşük 10 hisse seç
        selected_stocks = self.get_top_stocks_by_score(
            self.current_window, 
            'Final FS skor', 
            count=10, 
        ...
        if not selected_stocks:
            print("[HELDKUP CHAIN 4] ❌ FINAL FS kriterleri sağlayan hisse bulunamadı")
            self.advance_chain()
            return
        ...
        for ticker, score in selected_stocks[:3]:  # İlk 3'ü al
            # SMI kontrolü (short arttırma durumu için)
            smi_rate = self.get_smi_rate(ticker)
            if smi_rate > 0.28:
                print(f"[HELDKUP CHAIN 4] ❌ {ticker} (FS:{score:.2f}) - SMI {smi_rate:.4f} > 0.28")
                continue
            else:
                valid_tickers.append(ticker)
                print(f"[HELDKUP CHAIN 4] ✅ {ticker} (FS:{score:.2f}) - SMI {smi_rate:.4f} < 0.28")
        ...
        # Sonuçları bildir
        print(f"[HELDKUP CHAIN 4] ✅ {len(valid_tickers)} hisse seçildi:")
        for ticker in valid_tickers:
            print(f"[HELDKUP CHAIN 4]   ✅ {ticker}")
        
        # Emir gönderimi için hazırla
        if valid_tickers:
            self.prepare_orders_for_tickers(valid_tickers, 'SELL', 'FRONT_SELL', step_number=4)
        else:
            print("[HELDKUP CHAIN 4] ❌ Geçerli hisse bulunamadı")
            self.advance_chain()

    def prepare_orders_for_tickers(self, tickers, side, order_type, step_number):
        """Seçilen hisseler için emir hazırla"""
        print(f"[HELDKUP CHAIN {step_number}] 📋 {len(tickers)} hisse için emir hazırlanıyor...")
        ...
        for ticker in tickers:
            ...
            if not price:
                print(f"[HELDKUP CHAIN {step_number}] ❌ {ticker} için fiyat alınamadı")
                continue
            ...
            print(f"[HELDKUP CHAIN {step_number}] 📤 {ticker} {side} 200 lot @ {price}")
        
        if order_details:
            self.open_confirmation_window(order_details, step_number)
        else:
            print(f"[HELDKUP CHAIN {step_number}] ❌ Emir hazırlanamadı")
            self.advance_chain()

    def open_confirmation_window(self, order_details, step_number):
        """Emir onay penceresi açma"""
        print(f"[HELDKUP CHAIN {step_number}] 📋 Onay penceresi açılıyor...")
        
        # Onay penceresi açma işlemi burada yapılacak
        # Bu kısım GUI entegrasyonu gerektiriyor
        print(f"[HELDKUP CHAIN {step_number}] ✅ Onay penceresi açıldı - {len(order_details)} emir bekliyor")
        
        # Geçici olarak emirleri gönder
        for order in order_details:
            print(f"[HELDKUP CHAIN {step_number}] 📤 {order['ticker']} {order['side']} {order['size']} lot @ {order['price']}")
        
        # Sonraki adıma geç
        self.advance_chain()

    def manual_fill_check(self):
        """Manuel fill kontrolü - pozisyon değişikliklerini kontrol et"""
        try:
            if not self.is_active:
                return
            
            # IBKR'den güncel pozisyonları al
            current_positions = self.market_data.get_positions() if self.market_data else {}
            
            # Her hisse için pozisyon değişikliğini kontrol et
            for ticker, position in current_positions.items():
                if ticker in self.befday_positions:
                    befday_pos = self.befday_positions[ticker]
                    current_pos = position
                    
                    # Pozisyon değişikliği var mı?
                    if current_pos != befday_pos:
                        change = current_pos - befday_pos
                        print(f"[FILL CHECK] {ticker}: {befday_pos} → {current_pos} (değişim: {change:+d})")
                        
                        # Pozisyon değişikliği varsa benchmark hesapla
                        if change != 0:
                            self.calculate_benchmark_at_fill(ticker)
                            
        except Exception as e:
            print(f"[MANUAL FILL CHECK] ❌ Hata: {e}")
