...
class PolygonMarketData:
    ...
    def connect_ibkr(self, host='127.0.0.1', port=4001, client_id=1):
        ...
        try:
            ...
            # ✅ Test scriptindeki gibi yeni IB instance oluştur
            from ib_insync import IB, util
            import logging
            
            # Logging ayarla (test scriptindeki gibi)
            util.logToConsole(logging.INFO)
            
            # Yeni IB instance oluştur
            self.ib = IB()
            
            # ✅ Önce IB nesnesini temizle
            try:
                self.ib.disconnect()
            except:
                pass
            
            # ✅ Gateway için özel ayarlar
            print(f"[IBKR Manager] 🔄 Gateway bağlantısı deneniyor...")
            
            # Gateway için daha uzun timeout ve farklı client ID'ler dene
            client_ids_to_try = [client_id, 999, 888, 777]  # Farklı Client ID'ler
            
            for try_client_id in client_ids_to_try:
                try:
                    print(f"[IBKR Manager] 🔄 Client ID {try_client_id} deneniyor...")
                    self.ib.connect(host, port, clientId=try_client_id, timeout=30)  # 30 saniye timeout
            ...
            if self.ib.isConnected():
                ...
                return True
                        ...
                        print(f"[IBKR Manager] ❌ Client ID {try_client_id} başarısız")
                        self.ib.disconnect()
                        
                except Exception as e:
                    print(f"[IBKR Manager] ⚠️ Client ID {try_client_id} hatası: {e}")
                    try:
                        self.ib.disconnect()
                    except:
                        pass
            ...
            print("[IBKR Manager] 💡 Gateway Kontrol Edilecekler:")
            ...
            print("[IBKR Manager] ⚠️ Gateway olmadan da uygulama çalışabilir (sadece market data)")
                self.ib_connected = False
                return False
                
        except Exception as e:
            print(f"[IBKR Manager] ❌ Beklenmeyen bağlantı hatası: {e}")
            print("[IBKR Manager] ⚠️ Gateway olmadan da uygulama çalışabilir (sadece market data)")
                self.ib_connected = False
                return False
