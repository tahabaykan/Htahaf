...
class OrderManager:
    ...
    def place_order_for_selected(self, order_type):
        ...
        print(f"📤 {len(selected_tickers)} ticker için {order_type} emri gönderiliyor...")
        ...
        for ticker in selected_tickers:
            try:
                ...
                if not market_data:
                    print(f"⚠️ {ticker} için market data bulunamadı")
                    continue
                ...
                print(f"📤 {ticker} {action} {lot_size} lot @ {price:.4f} ({order_type})")
                
                # Gerçek emir gönderimi buraya gelecek
                # self.hammer.place_order(ticker, action, lot_size, price, hidden=True)
                
            except Exception as e:
                print(f"❌ {ticker} için emir gönderilirken hata: {e}")
    ...
    def update_orderbook(self):
        ...
        try:
            ...
            if l2_data:
                ...
                for print_data in l2_data.get('last_prints', [])[-10:]:  # Son 10 print
