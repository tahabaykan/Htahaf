...
tickers = [
    'AGNCL','ATH PRA','BANC PRF','CIM PRC','CNOBP','EFC PRB','EFC PRC','FLG PRA','KMPB','KEY PRL','KEY PRI','MTB PRH','NYMTN','NYMTL','RF PRC','RZB','RITM PRD','TWO PRA','TWO PRB','VOYA PRB','WAL PRA'
]
...
pd.DataFrame(rows, columns=all_columns).to_csv('ekheldnff.csv', index=False)
print('ekheldnff.csv oluşturuldu (veriler öncelikli olarak nalltogether, nnotheldpff, alltogether; bulunamayanlar boş).') 