...
def get_market_weights():
    ...
    try:
        ...
        return {'solidity_weight': 2.5, 'yield_weight': 1000}  # 1500'den 1000'e çekildi
...
def prepare_data_for_calculation(normalized_df, solidity_df, ibkr_df, market_weights=None):
    ...
    try:
        ...
        try:
            ...
            # COUPON ve Div adj.price kolonlarını sayısala çevir
            ...
            # CUR_YIELD hesapla: (25*COUPON)/Div adj.price (varsa), yoksa Last Price
            def calc_cur_yield(row):
                ...
                if pd.notnull(row['COUPON']) and price and price != 0:
                    return (25 * row['COUPON']) / price / 100
                else:
                    return np.nan
            ...
            print("Formül: (25 * COUPON) / Div adj.price (varsa), yoksa Last Price")
            print(df[['PREF IBKR', 'COUPON', 'Div adj.price', 'Last Price', 'CUR_YIELD']].head().to_string())
...
def calculate_final_thg(df, market_weights=None):
    ...
    try:
        ...
        if market_weights is None:
            ...
            yield_weight = 1000       # 1500'den 1000'e çekildi - optimum denge için
            adv_weight = 0.00025     # Varsayılan AVG_ADV ağırlığı
            print("\nVarsayılan ağırlıklar kullanılıyor: Solidity=2.5, Yield=1000, ADV=0.00025")
        else:
...
def main():
    try:
        ...
        # CSV dosyasını oluştur - filtrelenmiş veri ile
        filtered_df.to_csv('final_thg_results.csv', 
                      index=False,
                      float_format='%.4f',  # Tüm float değerler için 4 ondalık basamak
        ...
        for _, row in top_20.iterrows():
            print(f"{row['PREF IBKR']:<10} {row['SOLIDITY_SCORE']:>8.2f} {row['CUR_YIELD']:>10.4f} {row['FINAL_THG']:>10.2f} "
