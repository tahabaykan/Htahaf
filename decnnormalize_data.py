...
def process_csv_file(input_file, output_file):
    ...
    try:
        ...
        print(f"OK {input_file} yüklendi: {len(df)} satır")
        ...
        if 'Div adj.price' not in df.columns:
            ...
            print(f"OK Hesaplamalarda '{price_column}' kullanılıyor.")
        ...
        print(f"OK {output_file} dosyasına kaydedildi ({len(df)} satır)")
...
def main():
    ...
    for input_file in input_files:
        if os.path.exists(input_file):
            ...
            print(f"\nOK İşlem tamamlandı! {processed_count} dosya normalize edildi.")
