...
class PSFAlgo1Orders:
    ...
    def on_fill(self, ticker, side, price, size, **kwargs):
        ...
        if ticker in self.exclude_list:
            print(f"[PSFAlgo1 KOMPLE EXCLUDE] ❌ {ticker} komple exclude listesinde - fill işlenmedi")
            return
        
        # ✅ HALF SIZED - fill işleme devam et ama uyarı ver
        if ticker in self.half_sized_list:
            print(f"[PSFAlgo1 HALF SIZED] 📉 {ticker} half sized listesinde - fill işleniyor: {side} {size} @ {price}")
    ...
    def send_order(self, ticker, price, final_thg, side, size=200):
        ...
        if ticker in self.exclude_list:
            print(f"[ORDER KOMPLE EXCLUDE] ❌ {ticker} komple exclude listesinde - emir gönderilmedi")
            return False
        ...
        if ticker in self.half_sized_list:
            ...
            print(f"[ORDER HALF SIZED] 📉 {ticker} half sized listesinde - emir boyutu: {original_size} → {size}")
            ...
            if size < minimum_lot_threshold:
                print(f"[ORDER HALF SIZED] ❌ {ticker} yarıya düşürülen lot ({size}) minimum threshold'dan ({minimum_lot_threshold}) küçük - emir gönderilmedi")
