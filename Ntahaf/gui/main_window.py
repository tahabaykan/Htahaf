...
from ..ib_api.manager import PolygonMarketData
...
from Ntahaf.gui.general_window import GeneralWindow
from Ntahaf.utils.order_management import OrderManager
from ib_insync import LimitOrder
import os
import csv
from Ntahaf.utils.bdata_storage import BDataStorage
from Ntahaf.gui.orderbook_window import OrderBookWindow
from datetime import datetime
from Ntahaf.utils.benchmark_calculator import BenchmarkCalculator
from Ntahaf.psfalgo import PsfAlgo as PsfAlgo1
from Ntahaf.psfalgo2 import PsfAlgo2
...
class MainWindow(tk.Tk):
    ...
    def setup_ui(self):
        ...
        self.btn_komple_exclude = ttk.Button(top1, text="Komple Exclude", command=self.open_komple_exclude_window, width=14)
        self.btn_komple_exclude.pack(side='left', padx=2)
        self.btn_half_sized = ttk.Button(top1, text="Half Sized", command=self.open_half_sized_window, width=14)
        ...
        self.btn_befday_update = ttk.Button(top1, text="BEFDAY Update", command=self.update_befday_csv, width=12)
        ...
        # Üçüncü satır: SSFINEK CSV butonları (1. satır - 7 buton)
        ...
        # Dördüncü satır: SSFINEK CSV butonları (2. satır - 7 buton)
        ...
        # Beşinci satır: SSFINEK CSV butonları (3. satır - 7 buton)
        ...
        self.btn_nottitrekhc = ttk.Button(ssfinek_frame3, text="nottitrekhc", command=lambda: self.open_ssfinek_window("../ssfineknottitrekhc.csv"), width=12)
    ...
    def connect_ibkr(self):
        ...
        try:
            print("[MAIN] 🔗 IBKR bağlantısı başlatılıyor...")
            ...
            if self.market_data.connect_ibkr():
                ...
                # ✅ Pozisyonları ve emirleri güncelle (timeout'u önlemek için try-catch)
                try:
                    print("[MAIN] 📊 BDATA başlatılıyor...")
                    self.init_bdata_from_ibkr()
                ...
                print("[MAIN] ✅ IBKR bağlantısı başarılı!")
                return  # ✅ Başarılı durumda return et
                
            else:
                self.status_label.config(text="Durum: IBKR bağlantı hatası")
                print("[MAIN] ❌ IBKR bağlantısı başarısız!")
                print("[MAIN] 💡 Çözüm önerileri:")
                print("   1. TWS/Gateway açık mı?")
                print("   2. TWS'de API ayarları doğru mu?")
                print("   3. Test scripti çalıştırın: python ibkr_test.py")
    ...
    def open_opt50_maltopla_window(self):
        import pandas as pd
        from Ntahaf.gui.maltopla_window import MaltoplaWindow
        def polygonize_ticker(symbol):
            symbol = str(symbol).strip()
            if ' PR' in symbol:
                base, pr = symbol.split(' PR', 1)
    ...
    def paste_exdiv_list(self):
        ...
        def save_to_csv():
            try:
                ...
                exdiv_dir = os.path.join('Ntahaf', 'exdivkayit')
        ...
        def load_todays_data():
            try:
                ...
                current_date = datetime.now().strftime('%d%m%y')
                filename = f"p{current_date}.csv"
                filepath = os.path.join('Ntahaf', 'exdivkayit', filename)
...
• Manuel fill eklemek için alt kısımdaki formu kullanabilirsiniz."""
    ...
    def load_komple_exclude(self):
        ...
        try:
            df = pd.read_csv('Ntahaf/kompleexclude.csv')
            return set(df['Symbol'].astype(str).str.strip().str.upper())
    ...
    def save_komple_exclude(self):
        ...
        df.to_csv('Ntahaf/kompleexclude.csv', index=False)

    def load_half_sized_exclude(self):
        """Half sized exclude listesini yükle"""
        try:
            df = pd.read_csv('Ntahaf/halfsized.csv')
            return set(df['Symbol'].astype(str).str.strip().str.upper())
        except FileNotFoundError:
    ...
    def save_half_sized_exclude(self):
        ...
        df.to_csv('Ntahaf/halfsized.csv', index=False)
