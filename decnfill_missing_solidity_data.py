...
def process_csv_file(csv_file):
    ...
    try:
        ...
        scored_stocks = pd.read_csv(get_simulation_filename(csv_file), encoding='utf-8-sig')
        ...
        # Sonra eksik verileri doldurduğumuz hisselerin skorlarını 20 puan düşür
        ...
        # SOLIDITY_SCORE_NORM kolonunu 2 ondalık hane ile yuvarla
        if 'SOLIDITY_SCORE_NORM' in scored_stocks.columns:
            scored_stocks['SOLIDITY_SCORE_NORM'] = scored_stocks['SOLIDITY_SCORE_NORM'].round(2)
        ...
        if filled_stocks.any():
            print(f"\nEksik verileri doldurulan ve SOLIDITY_SCORE değeri 20 puan düşürülen {filled_stocks.sum()} hisse:")
        ...
        # Çıktı dosya adını oluştur (sld yerine sldf ekle)
        output_filename = get_simulation_filename(csv_file.replace('sld', 'sldf'))
        
        # Sonuçları yeni bir CSV dosyasına kaydet - 2 ondalık basamak formatıyla
        scored_stocks.to_csv(output_filename, index=False, float_format='%.2f')
        print(f"\nTamamlanmış veriler '{output_filename}' dosyasına kaydedildi.")
        
        # SOLIDITY bileşenlerini analiz et
        print(f"\n\n=============================================")
        print(f"{output_filename} - SOLIDITY SCORE BİLEŞENLERİ ANALİZİ")
        print(f"=============================================")
        ...
        for component in solidity_components:
            if component in scored_stocks.columns:
...
# İşlenecek CSV dosyaları - sld ile başlayan dosyalar
csv_files = [
...
for csv_file in csv_files:
    ...
    if os.path.exists(get_simulation_filename(csv_file)):
        process_csv_file(csv_file)
    else:
        print(f"! {csv_file} dosyası bulunamadı, atlanıyor.")
