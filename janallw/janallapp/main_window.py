...
class MainWindow(tk.Tk):
    ...
    def update_scores_with_market_data(self):
        ...
        try:
            ...
            for idx, row in self.df.iterrows():
                ...
                print(f"[DEBUG] {ticker}: DataFrame'den df_prev_close={df_prev_close}")
                
                if df_prev_close != 'N/A' and df_prev_close > 0:
                    prev_close = float(df_prev_close)
                    print(f"[DEBUG] {ticker}: DataFrame'den prev_close kullanılıyor: {prev_close}")
                else:
                    # Cache'den al (fallback)
                    prev_close = self.get_prev_close_for_symbol(ticker)
                    print(f"[DEBUG] {ticker}: Cache'den prev_close alındı: {prev_close}")
    ...
    def calculate_scores(self, ticker, row, bid, ask, last_price, prev_close, benchmark_chg=0):
        """Ntahaf formüllerine göre skorları hesapla - Parametre olarak gelen prev_close kullan"""
        try:
            ...
            if ticker in ['AHL PRE', 'AHL PRD', 'ATH PRD']:
                ...
                print(f"  final_bb hesaplama: {final_thg:.2f} - 1000 * {bid_buy_ucuzluk:.4f} = {final_thg - 1000 * bid_buy_ucuzluk:.2f}")
            ...
            final_sas = short_final - 1000 * ask_sell_pahali if short_final > 0 else 0
            final_sfs = short_final - 1000 * front_sell_pahali if short_final > 0 else 0
            final_sbs = short_final - 1000 * bid_sell_pahali if short_final > 0 else 0
        ...
        if not hasattr(self, '_last_visible_tickers') or self._last_visible_tickers != visible_tickers:
            ...
            for ticker in visible_tickers:
                try:
                    ...
                    available_columns = [col for col in ['PREF IBKR', 'prev_close', 'CMON', 'CGRUP', 'FINAL_THG', 'AVG_ADV', 'SMI', 'SHORT_FINAL'] if col in self.df.columns]
    ...
    def show_file_data(self, filename, is_main=False):
        ...
        try:
            ...
            # CSV'den sadece belirli kolonları al - prev_close'u PREF IBKR yanına taşı
            csv_columns_to_show = ['PREF IBKR', 'prev_close', 'CMON', 'CGRUP', 'FINAL_THG', 'AVG_ADV', 'SMI', 'SHORT_FINAL']
            ...
            # GRUP PENCERELERİNDE SKORLARI HESAPLA!
            if not is_main:
                print(f"[CSV] 🔄 {filename}: Grup penceresinde skorlar hesaplanıyor...")
    ...
    def get_prev_close_for_symbol(self, symbol: str) -> float:
        ...
        try:
            ...
            if hasattr(self, 'prev_close_cache') and symbol in self.prev_close_cache:
                ...
                print(f"[PREV CLOSE] ✓ {symbol}: cache'den prev_close={value}")
                return value
            ...
            if hasattr(self, 'etf_panel') and self.etf_panel:
                if symbol in self.etf_panel.etf_prev_close_data:
                    ...
                    print(f"[PREV CLOSE] ✓ {symbol}: ETF panel'den prev_close={value}")
                    return value
            
            print(f"[PREV CLOSE] ❌ {symbol}: prev_close cache'de bulunamadı")
            if hasattr(self, 'prev_close_cache'):
                print(f"[PREV CLOSE] 📋 Cache'deki ticker'lar: {list(self.prev_close_cache.keys())[:10]}...")
            else:
                print(f"[PREV CLOSE] 📋 Cache boş!")
            return 0.0
            
        except Exception as e:
            print(f"[PREV CLOSE] ❌ {symbol} genel hata: {e}")
            return 0.0
