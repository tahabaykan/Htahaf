...
def get_file_specific_rules(file_name):
    ...
    rules = {
        'ssfinekheldsolidbig.csv': {
            'long_percent': 30, 'long_multiplier': 1.45,  # 40→30, 1.3→1.45
            'short_percent': 20, 'short_multiplier': 0.6,
            'max_short': 2
        },
        'ssfinekheldbesmaturlu.csv': {
            'long_percent': 15, 'long_multiplier': 1.5,  # 25→15, 1.35→1.5
        ...
        'ssfinekheldkuponlukreorta.csv': {
            'long_percent': 20, 'long_multiplier': 1.45,  # 30→20, 1.3→1.45
            'short_percent': 30, 'short_multiplier': 0.7,
            'max_short': 3
        },
        'ssfinekheldflr.csv': {
            'long_percent': 30, 'long_multiplier': 1.4,  # 40→30, 1.25→1.4
            'short_percent': 20, 'short_multiplier': 0.6,
            'max_short': 2
        },
        'ssfinekheldkuponlukreciliz.csv': {
            'long_percent': 20, 'long_multiplier': 1.5,  # 30→20, 1.35→1.5
            'short_percent': 30, 'short_multiplier': 0.7,
            'max_short': 3
        },
        'ssfinekheldcommonsuz.csv': {
            'long_percent': 10, 'long_multiplier': 1.6,  # 20→10, 1.45→1.6
        ...
        'ssfinekrumoreddanger.csv': {
            'long_percent': 5, 'long_multiplier': 1.75,  # 15→5, 1.6→1.75
            'short_percent': 10, 'short_multiplier': 0.3,
            'max_short': 2
        },
        ...
        'ssfinekheldnff.csv': {
            'long_percent': 25, 'long_multiplier': 1.45,  # 35→25, 1.3→1.45
            'short_percent': 20, 'short_multiplier': 0.5,
            'max_short': 2
        },
        'ssfinekheldotelremorta.csv': {
            'long_percent': 15, 'long_multiplier': 1.55,  # 25→15, 1.4→1.55
            'short_percent': 20, 'short_multiplier': 0.5,
            'max_short': 3
        },
        'ssfineksalakilliquid.csv': {
            'long_percent': 10, 'long_multiplier': 1.55,  # 20→10, 1.4→1.55
        ...
        'ssfinekheldkuponlu.csv': {
            'long_percent': 30, 'long_multiplier': 1.35,  # 40→30, 1.20→1.35
...
def process_ssfinek_files():
    ...
    # SSFINEK dosyalarını bul (ana dizinden - orada oluşturuluyor)
        ...
        try:
            ...
            for _, row in long_stocks_limited.iterrows():
                stock_info = {
                    ...
                    'KUME_ORT': avg_final_thg,
                    'KUME_PREM': row['FINAL_THG'] - avg_final_thg
                }
            ...
            for _, row in short_stocks_limited.iterrows():
                stock_info = {
                    ...
                    'LONG_KURAL': f"Top {rules['long_percent']}% + {rules['long_multiplier']}x",
                    'SHORT_KURAL': f"Bottom {rules['short_percent']}% + {rules['short_multiplier']}x",
                    'KUME_ORT': avg_short_final,
                    'KUME_PREM': avg_short_final - row['SHORT_FINAL']
                }
